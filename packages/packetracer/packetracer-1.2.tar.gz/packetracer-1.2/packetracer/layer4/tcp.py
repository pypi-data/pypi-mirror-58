...
from packetracer.layer4 import ssl
from packetracer.layer567 import bgp, http, mqtt, rtp, sip, telnet, tpkt, pmap
...
class TCPOptSingle(packetracer.Packet):
    __hdr__ = (
        ("type", "B", 0),
    )

    type_t = packetracer.get_property_translator("type", "TCP_OPT_")


class TCPOptMulti(packetracer.Packet):
    """
...
class TCP(packetracer.Packet):
    ...
    flags_t = packetracer.get_property_translator("flags", "TH_", cb_get_description=cb_get_flag_description)
    ...
    ra_segments = packetracer.get_ondemand_property("ra_segments", lambda: {})
    ...
    def ra_collect(self, pkt_list):
        ...
        for segment in pkt_list:
            if self.direction(segment) != packetracer.Packet.DIR_SAME or len(segment.body_bytes) == 0:
                continue
