...
from packetracer.layer3.ip_shared import IP_PROTO_HOPOPTS, IP_PROTO_ROUTING, IP_PROTO_FRAGMENT,\
...
# handler
from packetracer.layer3 import esp, icmp6, igmp, ipx, ospf, pim
...
class IP6(packetracer.Packet):
	...
	src_s = packetracer.get_property_ip6("src")
	dst_s = packetracer.get_property_ip6("dst")
	nxt_t = packetracer.get_property_translator("nxt", "IP_PROTO_")
	...
	def _update_fields(self):
		...
		idval = packetracer.Packet.get_id_for_handlerclass(self.__class__, self.higher_layer.__class__)
	...
	def direction(self, other):
		...
		if self.src == other.src and self.dst == other.dst:
			...
			return packetracer.Packet.DIR_SAME | packetracer.Packet.DIR_REV
		if self.src == other.dst and self.dst == other.src:
			return packetracer.Packet.DIR_REV
...
#
class IP6OptsHeader(packetracer.Packet):
	__hdr__ = (
...
class IP6Option(packetracer.Packet):
	__hdr__ = (
		("type", "B", 0),
		("len", "B", 0)
	)
...
class IP6OptionPad(packetracer.Packet):
	__hdr__ = (
		("type", "B", 0),
	)
...
class IP6RoutingHeader(packetracer.Packet):
	__hdr__ = (
		("nxt", "B", 0),  # next extension header protocol
...
class IP6FragmentHeader(packetracer.Packet):
	__hdr__ = (
		("nxt", "B", 0),			# next extension header protocol
...
class IP6AHHeader(packetracer.Packet):
	__hdr__ = (
		("nxt", "B", 0),			 # next extension header protocol
...
class IP6ESPHeader(packetracer.Packet):
	def _dissect(self, buf):
