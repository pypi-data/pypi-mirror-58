...
from packetracer.checksum import crc_btle_check
from packetracer.structcbs import unpack_I
...
class AdvData(packetracer.Packet):
    __hdr__ = (
        ("len", "B", 0),
        ("type", "B", 0)
    )
...
class AdvInd(packetracer.Packet):
    __hdr__ = (
        ("adv_addr", "6s", b"\xff" * 6),
...
class AdvNonconnInd(packetracer.Packet):
    __hdr__ = (
        ("adv_addr", "6s", b"\xff" * 6),
        ("adv_data", None, triggerlist.TriggerList)
    )
...
class ScanRequest(packetracer.Packet):
    __hdr__ = (
...
class ScanResponse(packetracer.Packet):
    __hdr__ = (
        ("adv_addr", "6s", b"\xff" * 6),
        ("adv_data", None, triggerlist.TriggerList)
    )
...
class ConnRequest(packetracer.Packet):
    __hdr__ = (
...
class DataLLID0(packetracer.Packet):
    pass


class DataLLID1(packetracer.Packet):
    pass


class DataLLID2(packetracer.Packet):
    pass


# LLX-packets
class LLTerminateInd(packetracer.Packet):
    pass


class LLEncReq(packetracer.Packet):
    __hdr__ = (
        ("rand", "8s", b"\x00" * 8),
        ("encrdiv", "H", 0),
        ("masterdiv", "Q", 0),
...
class LLEncResp(packetracer.Packet):
    __hdr__ = (
        ("slavediv", "8s", b"\x00" * 8),
        ("slaveinit", "I", 0)
    )
...
class LLStartEnc(packetracer.Packet):
    pass


class LLVersionInd(packetracer.Packet):
    __hdr__ = (
        ("version", "B", 0),
        ("company", "H", 0),
        ("subcompany", "H", 0)
...
class LLFeatureReq(packetracer.Packet):
    pass


class LLRejectInd(packetracer.Packet):
    pass
...
class DataLLID3(packetracer.Packet):
    __hdr__ = (
        ("opcode", "B", 0),
    )
...
class BTLE(packetracer.Packet):
    ...
    def bin(self, update_auto_fields=True):
        ...
        return packetracer.Packet.bin(self, update_auto_fields=update_auto_fields) + self.crc
...
class BTLEHdr(packetracer.Packet):
    __hdr__ = (
        ("channel", "B", 0),
        ("signal", "B", 0),
        ("noise", "B", 0),
