...
def nessus_parser(args, root, data_nessus, workbook):
    ...
    autoclassify_file = pkg_resources.resource_string(
        __name__, '../data/autoclassify.yaml')
    autoclassify = yaml.load(autoclassify_file, Loader=yaml.FullLoader)

    tls_file = pkg_resources.resource_string(__name__, '../data/tls.yaml')
    tls_obs = yaml.load(tls_file, Loader=yaml.FullLoader)

    x509_file = pkg_resources.resource_string(__name__, '../data/x509.yaml')
    x509_obs = yaml.load(x509_file, Loader=yaml.FullLoader)

    http_file = pkg_resources.resource_string(__name__, '../data/http.yaml')
    ...
    if args.output_xlsx:
        ...
        worksheet_portscan = workbook.add_worksheet('Nessus SYN')
        ...
        worksheet_httpscan = workbook.add_worksheet('Nessus HTTP')
