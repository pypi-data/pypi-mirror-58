...
IPAddr = "192.168.78.95"  # socket.gethostbyname(hostname)
port = '6000'
app = Flask(__name__)

__author__ = 'Umesha HP'

ALLOWED_EXTENSIONS = set(['tar.gz', 'tar', 'gz','rar'])
home = expanduser("~")
...
@app.route('/api/v1/netjson-config/jsontoopenwrt/', methods=['POST'])
...
@app.route('/api/v1/netjson-config/openwrttojson/', methods=['POST'])
def upload_file():
    ...
    print("File received  is {}".format(file.filename))
    if file.filename == '':
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            logging.error("Exception in OPENWrt conversion ", str(err))
            return jsonify({"Exception in OPENWrt conversion check the file size": str(err)})

    else:
        resp = jsonify({'message': 'Allowed file type is only tar.gz'})
        resp.status_code = 400
        return resp
...
if __name__ == '__main__':
    ...
    ensure_dir(path2log)
    logging.basicConfig(filename=os.path.join(path2log, 'apps.log'), filemode='w+',
                        format='%(name)s - %(levelname)s - %(message)s')
    logging.warning('This will get logged to a file')
    print("Server running on {} with port {}".format(IPAddr, port))
