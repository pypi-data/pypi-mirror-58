...
class TaskProcessDispatcher(object):
    ...
    def __init__(self, proc_monitor):
        ...
        self.cur_idx = 0
        self.process_list = proc_monitor.my_processes
        self.process_len = len(self.process_list)
        self.process_call_match_dict = {}
        self.scheduler = BackgroundScheduler()
        self.scheduler.start()
        self.once_timers = {}

    def get_dispatcher_process(self):
        the_process = self.process_list[self.cur_idx % self.process_len]
        self.cur_idx += 1
        return the_process
    ...
    def send_task_get_result(self, *args, **kwargs):
        ...
        if task_id is None:
            return self.get_dispatcher_process().send_task_get_result(*args, **kwargs)
        else:
    ...
    def send_one_task(self, *args, **kwargs):
        task_id = kwargs.get('task_id')
        if task_id is None:
            the_process = self.get_dispatcher_process()
        else:
            the_process = self.process_call_match_dict.get(task_id)
    ...
    def broadcast_one_task(self, *args, **kwargs):
        wk_ids = []
        for the_process in self.process_list:
            wk_id = the_process.send_one_task(*args, **kwargs)
            self.process_call_match_dict[wk_id] = the_process
            wk_ids.append(wk_id)
        return wk_ids
