...
#  bxmodels
#
#  Created by Oliver Borchert on 08/28/19.
...
from .utils import log_normal, log_responsibilities, max_likeli_means, \
    max_likeli_covars
...
class GMM(xnn.Configurable, xnn.Estimator, nn.Module):
    """
    The GMM represents mixture of a fixed number of multivariate gaussian
    distributions. This class may be used to find clusters whenever you expect
    data to be generated from a (fixed-size) set of gaussian distributions.
    """
    ...
    def reset_parameters(self, data=None, max_iter=100):
        """
        Initializes the parameters of the GMM, optionally based on some data. If
        no data is given, means are initialized randomly from a gaussian 
        distribution, unit covariances are used and prior probabilities are 
        assigned randomly using a uniform distribution.

        Parameters:
        -----------
        - data: torch.Tensor [N, D], default: None
            An optional set of datapoints to initialize the means and
            covariances of the gaussian distributions from. K-Means will be run
            to find the means and the datapoints belonging to a respective
            cluster are used to estimate the covariance. Note that the given
            data may be a (small) subset on the actual data that the GMM should
            be fitted on.
        - max_iter: int, default: 100
            If data is given and K-Means is run, this defines the maximum
    ...
    def fit(self, data, max_iter=100, batch_size=None, eps=1e-4):
        """
        Optimizes the GMM's parameters according to the given data. We use
        (mini-batch) expectation-maximization for optimizing the parameters.
        Consider running `reset_parameters` on (a subset of) the data first to
        ...
        - batch_size: int, default: None
            The batch size to use for training. If `None` is given, we do not
            uses batches.
        - eps: float, default: 1e-4
            The difference in the log-likelihood (per datapoint) after which to
            stop the EM-algorithm before running `max_iter` iterations.

        Returns:
        --------
        - bxtorch.nn.History
            A history object containing the negative log-likelihoods over the
            course of the EM-training.
        """
        ...
        weights = torch.ones(num_batches, device=device)
    ...
    def evaluate(self, data, return_responsibilities=False):
        """
        Computes the negative log-likelihood of the given data (normalized by
        ...
        - return_responsibilities: bool, default: False
            Whether to return the responsibilities for the datapoints and each
            component.
    ...
    def predict(self, data):
        ...
        -----------
        - data: torch.Tensor [..., D]
            The data for which to compute the most likely components that they
            were generated from. The size is arbitrary, only the last dimension
            must be the same as the dimensionality of the gaussian
            distributions.

        Returns:
        --------
        - torch.Tensor [...]
            The indices of the most likely components. The shape is the same as
            of the given data, except for the last dimension.
        """
        size = data.size()
    ...
    def sample(self, n, return_components=False):
        ...
        - return_components: bool, default: False
            Whether to return the indices of the components from which the
        ...
        - torch.Tensor [N]
            Optionally, the indices of the components corresponding to the
            returned samples.
        """
        # 1) Sample components
    ...
    def _estimate(self, data, batch=False):
        ...
        if batch: 
            # In case of batching, we need to compute the covariance at a later
            # point.
            return {
    ...
    def _em_full(self, data):
        device = self.component_weights.device
        estimates = self._estimate(data.to(device))
    ...
    def _em_batch(self, data, batch_size, weights):
        ...
        # 2) Initialize containers to aggregate information from different
        #    batches.
        ...
        for i in range(num_batches):
            ...
            component_weights += \
                estimates['component_weights'].cpu() * weights[i]
            component_sums += \
                estimates['component_sums'].cpu() * weights[i]
            means += \
                estimates['means'].cpu() * weights[i]
            neg_log_likeli += \
                (estimates['neg_log_likelihood'] / batch.size(0)) * weights[i]

        # 3.4) Update means
        means = means / component_sums.unsqueeze(1)

        # 4) Now, given the overall means, we compute the covariances.
        #    For this, we naturally need the responsibilities. Although we
        #    computed them earlier, we choose to recompute them as, otherwise,
        #    memory consumption would be too high and there wouldn't be a
        #    reason to perform batching in the first place.

        # 4.1) Initialize containers - here, we can reuse `component_sums` from
        #      above as the responsibilities do not change.
        K = self.num_components
        D = self.num_features
        ...
        for i in range(num_batches):
            ...
            # 4.3) Now we actually compute the covariance. Due to the odd
            #      division by the component_sums, we cannot use the functions
            #      from `.utils` and use an adjusted version from the code
            #      there.
            if self.covariance in ('diag', 'spherical'):
                covars_x_sq += \
                    torch.matmul(resp.t(), batch * batch) * weights[i]
                covars_xm_wo_means += \
                    torch.matmul(resp.t(), batch) * weights[i]
            else:
                pass

        # 4.4) After having processed all batches, we can aggregate the results
        #      to compute the actual covariance
        if self.covariance in ('diag', 'spherical'):
