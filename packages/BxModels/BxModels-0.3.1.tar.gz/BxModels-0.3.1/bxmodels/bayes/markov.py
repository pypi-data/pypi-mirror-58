...
#  bxmodels
#
#  Created by Oliver Borchert on 09/01/19.
...
class MarkovModel(xnn.Configurable, xnn.Estimator, nn.Module):
    """
    The MarkovModel models a simple MarkovChain with a fixed set of states. You
    may use this class whenever states are known and transition probabilities
    are the only quantity of interest. In case of any additional output from
    the states, consider using the `HMM` model.
    """
    ...
    def reset_parameters(self):
        """
        Resets the parameter of the model by sampling initial probabilities as
        well as transition probabilities from a uniform distribution.
        """
    ...
    def fit(self, sequences, teleport_alpha=0):
        ...
        - sequences: torch.Tensor [N, S]
            N sequences to train the Markov model on. The sequences are
            expected to be of uniform length S. Each value of the tensor must
            be the index of a state.
        - teleport_alpha: float, default: 0
            The probability of "invalid" transitions in the given sequences.
            This setting is motivated by a random walker which teleports after
            every step with probability alpha.

        Returns:
        --------
        - bxtorch.nn.History
            For completeness, it returns a history object. However, apart from
            the duration of the training, the object does not carry any
            information.
        """
        tic = time.time()
    ...
    def evaluate(self, sequences):
        """
        Computes the negative log-likelihood of observing the given sequences
        under this stochastic model.
    ...
    def sample(self, num_sequences, sequence_length):
        """
        Samples the given number of sequences with the given length from the
        model's underlying probability distribution.
    ...
    def stationary_distribution(self):
        """
        Computes the stationary distribution of the Markov chain. This equals
        ...
        --------
        - torch.Tensor [N]
            The probability of a random walker visiting each of the states after
            infinitely many steps.
        """
        return power_iteration(self.transition_probs.t())
