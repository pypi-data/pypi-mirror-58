...
#  bxmodels
#
#  Created by Oliver Borchert on 08/22/19.
...
def log_normal(x, means, covars):
    """
    Computes the log-probability of the given data for multiple multivariate
    normal distributions defined by their means and covariances.
    ...
    - covars: torch.Tensor ([K, D] or [D] or [K])
        ...
        In the first case, each distribution is assumed to have its own diagonal
        covariance matrix, in the second case, the covariance matrix is shared
        among all distributions, and in the last one, the covariance matrix is
        spherical. The type of covariance is therefore inferred by the size of
        the input. If the dimension does not fit any of the documented sizes,
        no error will be thrown but the result is undefined.
...
def log_responsibilities(log_probs, comp_priors, return_log_likelihood=False):
    ...
    - return_log_likelihood: bool, default: False
        Whether to return the log-likelihood of observing the data given the
        log-probabilities and the component priors.
    ...
    - torch.Tensor [N]
        If `return_log_likelihood` is `True`, the log-likelihood of observing
        each of the datapoints ("evidence"). To compute the log-likelihood of
        the entire data, sum these log-likelihoods up.
    """
...
def max_likeli_means(data, responsibilities, comp_sums=None):
    """
    Maximizes the likelihood of the data with respect to the means for a
    ...
    - comp_sums: torch.Tensor [K], default: None
        The cumulative probabilities for all components. If not given, this
        method can be used for e.g. implementing mini-batch GMM. The means
        are not at all normalized.
...
def max_likeli_covars(data, responsibilities, comp_sums, means, covar_type, 
                      reg=1e-6):
    """
    Maximizes the likelihood of the data with respect to the covariances for a
    gaussian mixture model.
    ...
    - covar_type: str
        The type of the covariance to maximize the likelihood for. Must be one
        of ('diag', 'diag-shared', 'spherical').
    - reg: float, default: 1e-6
        Regularization term added to the covariance to ensure that it is
        positive.

    Returns:
    --------
    - torch.Tensor ([K, D] or [D] or [K])
        The likelihood-maximizing covariances where the shape depends on the 
    ...
    """
    if covar_type == 'diag':
        return _max_likeli_diag_covars(
            data, responsibilities, comp_sums, means, reg
        )
    elif covar_type == 'diag-shared':
        return _max_likeli_shared_diag_covars(
            data, responsibilities, comp_sums, means
        )
    else:
        return _max_likeli_diag_covars(
            data, responsibilities, comp_sums, means, reg
...
def _max_likeli_shared_diag_covars(data, responsibilities, comp_sums, means,
                                   reg):
    K = responsibilities.size(1)
...
def power_iteration(A, eps=1e-7, max_iterations=100):
    """
    Computes the eigenvector corresponding to the largest eigenvalue of the
    given square matrix.

    Parameters:
    -----------
    - A: torch.Tensor [N, N]
        The square matrix for which to compute the eigenvector corresponding to
        the largest eigenvalue.
    - eps: float, default: 1e-7
        Convergence criterion. When the change in the vector norm is less than
        the given value, iteration is stopped.
    - max_iterations: int, default: 100
        The maximum number of iterations to do when the epsilon-based
        convergence criterion does not kick in.

    Returns:
    --------
    - torch.Tensor [N]
        The eigenvector corresponding to the largest eigenvalue of the given 
        square matrix.
    """
