...
def fix_led_permissions():
    for led in ['red', 'green', 'blue']:
        subprocess.check_output(['sudo', 'chmod', '-R', '777', '/sys/class/leds/pinephone:{}:user'.format(led)])


def write_led(color, on=False):
    with open('/sys/class/leds/pinephone:{}:user/brightness'.format(color)) as handle:
        if on:
            handle.write('1\n')
        else:
            handle.write('0\n')
