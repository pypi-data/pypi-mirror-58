...
class ProxyPool:
    ...
    def state(self):
        ...
        print("[PPR] Total: {len_pool} | ".format(len_pool = len(self.pool))
              "Dead: {len_pooldead} | ".format(len_pooldead = len(self.pool_dead))
              "Active: {active_proxies} | ".format(active_proxies=len(active_proxies))
              "Cooldown: {cooldown_proxies} | ".format(cooldown_proxies=len(cooldown_proxies))
              "Unknown: {unchecked_proxies}".format(unchecked_proxies=len(unchecked_proxies))
