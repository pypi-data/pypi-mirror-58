...
@adapters.register_for('rethinkdb')
class RethinkDB(ConnectionConfigurationMixin, NoSQLAdapter):
    dbengine = 'rethinkdb'
    drivers = ('rethinkdb',)

    def find_driver(self):
        super(RethinkDB, self).find_driver()
        ...
        if int(version.split('.')[0]) < 2:
            raise RuntimeError(
                "pydal requires rethink version >= 2.0, found '%s'" % version)

    def _initialize_(self, do_connect):
        super(RethinkDB, self)._initialize_(do_connect)
        #: uri parse
