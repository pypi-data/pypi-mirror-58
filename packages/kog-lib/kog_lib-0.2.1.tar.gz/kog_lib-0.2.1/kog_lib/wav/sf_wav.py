...
class SFWav:
    ...
    def help():
        print("""
        ----------------------SFWav类 使用方法--------------------------------------
        from kog_lib.wav import sf_wav\n
        实例化：
            my_wav = sf_wav.SFWav(wav_path)
            或，
            my_wav = sf_wav.SFWav()
        ...
        得到区间内峰值：
            my_wav.get_peak(selected_channel,start_time,end_time)
                ...
                start_time: 起始时间，默认0
                end_time: 终止时间，默认总时长\n
        切割音频: 从start_time开始且duration时长的音频，保存到output
            my_wav.cut_wav(output,start_time,duration)\n
        其他方法：
    ...
    def __get_unpack_filter(self,samplewidth,nframes=1):
        if samplewidth == 1:
            return 'B'
        if samplewidth == 2:
            return 'h'
...
# path = "./20190000P40002a0051-6channels.wav"
# mywav = SFWav()
# mywav.open(path)
# mywav.wav_head.print_head()
# max_value,min_value,cut_duration = mywav.get_peak(3)
# print(f"""
# 波峰：{max_value}
# 波谷：{min_value}
# 切片时长：{cut_duration}
