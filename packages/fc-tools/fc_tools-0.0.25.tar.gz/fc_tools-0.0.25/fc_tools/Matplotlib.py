...
def set_axes_equal(**kwargs):
  ...
  #    StrictVersion('3.0.0') < StrictVersion('3.0.0')
  
  #if (matplotlib.__version__ != '3.1.1'):
    ## print('Cannot use set_aspect(''equal''): bug in Matplotlib 3.1.1') 
  ##else:
    #ax.set_aspect('equal','box') # old version
    #ax.set_aspect('equal') # old version <=3.0.0
    
    #ax.axis('equal') # new version >= 3.1.0
...
def SaveFigAsFile(fignum,filename,**kwargs):
  ...
  from distutils.version import StrictVersion 
  import matplotlib
  if StrictVersion(matplotlib.__version__) <= StrictVersion('3.0.3'):
...
def SaveAllFigsAsFiles(filename,**kwargs):
  ...
  for nfig in Fignums:
    File=filename+'_fig'+str(nfig)  
    SaveFigAsFile(nfig,File,**kwargs)
