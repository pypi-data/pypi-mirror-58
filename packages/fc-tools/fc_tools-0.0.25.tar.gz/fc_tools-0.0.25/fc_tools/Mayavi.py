...
def vtk_SaveAllFigsAsFiles(filename,**kwargs):
  ...
  figext=kwargs.pop('format', 'png' )
  savedir=kwargs.pop('dir', '.' )
  figs=kwargs.pop('figs',None) # list of figures number
  ...
  for i in range(len(scenes)):
    ...
    mlab.savefig(File,**kwargs)#,magnification=scale)  Trouble with magnification fc_mayavi4mesh.plot_node_indices
    if crop:
      if verbose:
        print('  Crop file %s'%File)
      from PIL import Image  # needs pillow package
      bg = Image.open(File) # The image to be cropped
      new_im = trim(bg)
      new_im.save(File)
    fig.scene.background=old_bg
...
def fc_legend(names,colors,d,txt=None):
  ...
  legendBox.position[0]=0
  legendBox.position[1]=0
  legendBox.position2[0]=0
  legendBox.position2[1]=0
