...
class AccessReference:
    ...
    def __eq__(self, other: 'AccessReference'):
...
def _access_ref_from_payload(json: dict) -> Union[DtlError, AccessReference]:
    ref_id = json.get("id")
    if not isinstance(ref_id, str):
        return DtlError("An access reference needs an 'id' field")
    else:
        ref_id = UUID(ref_id)

    ref_type = json.get("type")
    if not isinstance(ref_type, str):
        return DtlError("An access reference needs a '_type' field")
    else:
        ref_type = store_type_from_str(ref_type)

    name = json.get("name")
    return AccessReference(ref_id, name, ref_type)
...
class Access:
    ...
    def _base_payload(self) -> dict:
        return dict([(Access.type_field, self.type.value)])
...
class S3Access(Access):
    ...
    def __init__(self, region: str, client_id: str, client_secret: str):
        Access.__init__(self, S3Access.type_str)
        self.region = region
...
class AzureStorageAccess(Access):
    ...
    def __init__(self, account_name: str, account_key: str, endpoint_protocol: AzureEndpointProtocol,
        ...
        Access.__init__(self, AzureStorageAccess.type_str)
...
class SocrataAccess(Access):
    type_str = StoreType.Socrata

    def __init__(self, domain: str, token: str):
        Access.__init__(self, SocrataAccess.type_str)
        self.domain = domain
        self.token = token
