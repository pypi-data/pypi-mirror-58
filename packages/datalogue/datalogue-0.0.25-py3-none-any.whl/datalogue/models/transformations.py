...
from datalogue.models.datastore import DataStore, _data_store_from_payload, StoreDefinition
import abc
...
def _replace_label_from_payload(json: dict) -> Union[DtlError, ReplaceLabel]:
    array = _array_from_dict(json, MapFilterNotByClass.type_str, "path")
    if isinstance(array, DtlError):
        return array
...
def _join_from_payload(json: dict, join_type: str) -> Union[DtlError, OuterJoin, InnerJoin]:
    ...
    source = _data_store_from_payload(source_field)
    ...
    on = _array_from_dict(json, InnerJoin.type_str, "on")
    if isinstance(on, DtlError):
        return on

    equals = _array_from_dict(json, InnerJoin.type_str, "equals")
...
def _transformation_from_payload(json: dict) -> Union[DtlError, Transformation]:
    ...
    if parsing_function is None:
        return DtlError("Looks like '%s' transformation is not handled by the SDK" % Transformation.type_field)
