...
def getPackageListFromIndex(baseurl):
    ...
    try:
        ...
        logging.warn(f"Connection error while getting package list: {err}")
    except requests.HTTPError as err:
        logging.warn("HTTP unsuccessful response"
                     f" while getting package list: {err}")
    except requests.Timeout as err:
        logging.warn(f"Timeout error while getting package list: {err}")
    except requests.TooManyRedirects as err:
        logging.warn("TooManyRedirects error "
                     f"while getting package list: {err}")
    except Exception as err:
        logging.warn(f"Unknown Error: {err}")
    else:
        retpkgs = newpkgs
...
def parseCommandLine():
    ...
    group.add_argument('-c', dest='config_file', type=argparse.FileType('r'),
    ...
    group.add_argument('-i', dest='index', action='store_true',
                       help='pypi index site to get packages from')
    ...
    if parsed_dir_match:
        ...
        if download_file:
            ...
            try:
                ...
                logging.warn("Connection error while getting package file "
                             + file_name + ": {0}".format(err))
                download_error_counter.update_counter()
            except requests.HTTPError as err:
                logging.warn("HTTP unsuccessful response while "
                             "getting package file "
                             + file_name + ": {0}".format(err))
                download_error_counter.update_counter()
            except requests.Timeout as err:
                logging.warn("Timeout error while getting package file "
                             + file_name + ": {0}".format(err))
                download_error_counter.update_counter()
            except requests.TooManyRedirects as err:
                logging.warn("TooManyRedirects error "
...
def shouldDownload(pkg, base_url, base_file_loc):
    ...
    try:
        ...
        logging.warn("Connection error while getting index for package "
                     + pkg + ": {0}".format(err))
    except requests.HTTPError as err:
        logging.warn("HTTP unsuccessful response "
        ...
        logging.warn("Timeout error while getting index for package "
                     + pkg + ": {0}".format(err))
    except requests.TooManyRedirects as err:
        logging.warn("TooManyRedirects error while getting index for package "
                     + pkg + ": {0}".format(err))
    except Exception as err:
        logging.warn("Unknown Error: {}".format(err))
...
def processPackageFilesSingle(pkg_name, base_url, base_save_loc):  # noqa: C901
    ...
    try:
                ...
                for release in json_page['releases']:
                    if len(json_page['releases'][release]) > 0:
                        for file in json_page['releases'][release]:
                            ...
                            file_url_time = file['upload_time']
                            # time format returned: 2019-04-16T20:36:54
                            # Epoch time version of file_url_time
                            file_url_time_epoch = int(
                            ...
                            # Here we need to parse out the directory structure
                            # for locally storing the file
                            parsed_dir_match = re.search(
                                r"http[s]{0,1}://[^/]+/(.*)/",
                                file_url, re.IGNORECASE)
                            if parsed_dir_match:
                                parsed_dir = parsed_dir_match.group(1)
                                file_loc = (web_loc + "/"
                                            + parsed_dir
                                            + "/" + file_name)
                                file_dir = web_loc + "/" + parsed_dir
                                # Here we first get the stats of a
                                # possible already existing file
                                download_file = False
                                if os.path.exists(file_loc):
                                    file_info = os.stat(file_loc)
                                    file_size = file_info.st_size
                                ...
                                if download_file:
                                    ...
                                    try:
                                        ...
                                        os.makedirs(file_dir, exist_ok=True)
                                        package_file_req = requests.get(
                                            file_url,
                                            stream=True)
                                        package_file_req.raise_for_status()
                                        with open(file_loc, 'wb') as outfile:
                                            shutil.copyfileobj(
                                                package_file_req.raw,
                                                outfile)
                                        os.utime(file_loc,
                                                 (file_url_time_epoch,
                                                  file_url_time_epoch))
                                ...
                                logging.warn("No package file url matched,"
                                             " skipping...")
                                continue
    except requests.ConnectionError as err:
        logging.warn("Connection error while getting json info for package "
        ...
        logging.warn("Timeout error while getting json info for package "
                     + pkg_name + ": {0}".format(err))
    except requests.TooManyRedirects as err:
        logging.warn("TooManyRedirects error "
                     "while getting json info for package "
...
def processPackageFiles(pkg_name, base_url, base_save_loc, thread_count):
    ...
    try:
        ...
        logging.warn("TooManyRedirects error while "
                     "getting json info for package "
                     + pkg_name + ": {0}".format(err))
        error_count += 1
    except Exception as err:
...
def main():
    ...
    if args.config_file:
        logging.info("Grabbing list of packages from file: "
                     + args.config_file.name)
        pkgs = args.config_file.read().split()
        if len(pkgs) == 0:
            logging.info("Config file empty, grabbing list of "
                         "packages from pypi index: " + repo_url)
            pkgs = getPackageListFromIndex(repo_url)
    elif args.index:
        logging.info("Grabbing list of packages from pypi index: "
                     + repo_url)
        pkgs = getPackageListFromIndex(repo_url)
    elif args.package_name:
        logging.info("Grabbing package name from command line: "
                     + args.package_name)
        pkgs = []
    ...
    for p in pkgs:
        ...
        if shouldDownload(p, repo_url, mirror_repo_loc):
            if thread_count == 1:
                err = processPackageFilesSingle(p, repo_url, mirror_repo_loc)
            else:
