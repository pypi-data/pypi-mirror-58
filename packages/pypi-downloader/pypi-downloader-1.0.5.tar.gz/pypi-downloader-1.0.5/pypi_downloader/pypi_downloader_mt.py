...
def downloadReleaseFile(file_download_info,
    ...
    if parsed_dir_match:
        ...
        if download_file:
            ...
            logger.info(file_name + " exists, skipping...")

    else:
        logger.warning("No package file url matched, skipping...")
        download_error_counter.update_counter()
                    ...
                    if len(json_page['releases'][release]) > 0:
                        for file in json_page['releases'][release]:
                            ...
                            if parsed_dir_match:
                                ...
                                logger.warning("No package file url matched,"
                                               " skipping...")
                                continue
    except requests.ConnectionError as err:
...
def main():
    ...
    if args.config_file:
        logger.info("Grabbing list of packages from file: "
                    + args.config_file.name)
        pkgs = args.config_file.read().split()
        if len(pkgs) == 0:
            logger.info("Config file empty, grabbing list of "
    ...
    for p in pkgs:
        ...
        if shouldDownload(p, repo_url, mirror_repo_loc):
            ...
            if err:
                ...
                if err2:
                    logger.warning("Error while updating package  "
                                   + p + " index file")
                else:
                    logger.info("Successful processing of package {}".format(p))
