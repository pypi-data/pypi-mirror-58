...
class ApiAsana:
    ...
    def patch_task(self, task_gid: int, data: dict) -> tuple:
        ...
        return response.ok, json.loads(response.text).get('data', {})

    def get_webhooks(self, workspace: int) -> list:
        url = f'{self.base_url}/webhooks'
        response = requests.get(url=url, params={'workspace': workspace}, headers=self.headers)
        return json.loads(response.text).get('data', [])

    def post_webhook(self, resource: int, target: str) -> bool:
        url = f'{self.base_url}/webhooks'
