...
class LockObjHandler:
    ...
    # def process(self, stack_tree, is_top_main_thread, process_data, is_target_lock_tree=False):
    #     ref_cause = False
    #
    #     for stack_line in stack_tree:
    #         if is_wait_lock_line(stack_line):
    ...
    #             obj_path = finders[1]
    #             hold_tid = finders[2]
    #             if obj not in self.wait_lock_obj_list:
    #                 self.wait_lock_obj_list.append(obj)
    #                 self.wait_lock_obj_map[obj] = [obj_path, hold_tid]
    #
    #             if obj not in self.wait_process_data_map:
    #                 self.wait_process_data_map[obj] = process_data
    #
    #             # if is_top_main_thread:
    #             #     # 主线程被阻塞
    #             #     self.top_main_thread_lock_obj = obj
    #             #     if obj not in self.stack_obj_for_main_lock:
    #             #         self.stack_obj_for_main_lock.append(obj)
    #             #     ref_cause = True
    #             # elif obj in self.stack_obj_for_main_lock:
    #             #     # 其他线程被阻塞
    #             #     ref_cause = True
    #             # elif is_target_lock_tree:
    #             #     # 该阻塞其它的线程，被其他线程阻塞
    #             #     if obj not in self.stack_obj_for_main_lock:
    #             #         self.stack_obj_for_main_lock.append(obj)
    #             #     ref_cause = True
    #
    #         elif is_lock_stack_line(stack_line):
    #             finders = LOCK_OBJ_INFO.findall(stack_line)[0]
    #             obj = finders[0]
    #             obj_path = finders[1]
    #             if obj not in self.lock_obj_list:
    #                 self.lock_obj_list.append(obj)
    #                 self.lock_obj_map[obj] = obj_path
    #
    #             if obj not in self.lock_process_data_map:
    #                 self.lock_process_data_map[obj] = process_data
    #
    #             if obj in self.stack_obj_for_main_lock:
    #                 # 找到阻塞的地方
    #                 ref_cause = True
    #
    #     return ref_cause
