...
if __name__ == "__main__":  
    ...
    s = Sprite()                         # 角色本身是隐藏的
    item = s.stamp()                     # 图章是虫子造型
    s.shape('res/cat1.png')              # 变身为小猫的造型
    s.screen.cv.tag_lower(s.turtle._item)# 下移一层
    
    while True:                          # 当真
      s.stampgoto(item,*mouse_pos())     # 图章移到鼠标指针
      if s.stampcollide(item,s):         # 如果图章碰到s，则在标题栏显示
         s.screen.title("图章碰到我了")  # 打印
      else:
