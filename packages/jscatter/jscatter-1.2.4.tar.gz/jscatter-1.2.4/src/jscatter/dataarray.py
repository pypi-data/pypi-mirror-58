...
class dataListBase(list):
    ...
    def getfromcomment(self, attrname, ignorecase=False):
        """
        ...
        """
        for element in self:
            element.getfromcomment(attrname=attrname, ignorecase=ignorecase)
...
class dataList(dataListBase):
    ...
    def makeErrPlot(self, title=None, showfixpar=True, **kwargs):
        ...
        if not (hasattr(self, '_errplot') and self._errplot.is_open()):
            # we need to make a new one with some attributes describing type and legend
            ...
            if isinstance(headless, str) and headless in ['agr', 'png', 'jpg', 'svg', 'pnm', 'pdf']:
                self._errplot._headlessformat = headless
            else:
                # None results in agr for grace and backend default in mpl
                self._errplot._headlessformat = None
    ...
    def showlastErrPlot(self, title=None, modelValues=None, **kwargs):
        ...
        for name in sorted(self._freepar):
            ...
            values = np.atleast_1d(modelValues.__getdatalistattr__(name))
...
class dataArrayBase(np.ndarray):
    ...
    def __getattr__(self, attribute):
        ...
        if attribute not in protectedNames + protectedIndicesNames + ['_isdataArray']:
            if attribute[0] == '_' and hasattr(self, 'columnname'):
        ...
        if ignorecase:
            for i, line in enumerate(self.comment):
                ...
                if len(words) > 0 and words[0].lower() == attrname.lower():
                    setattr(self, attrname.lower(), ' '.join(words[1:]))
                    del self.comment[i]
                    return
        else:
            for i, line in enumerate(self.comment):
