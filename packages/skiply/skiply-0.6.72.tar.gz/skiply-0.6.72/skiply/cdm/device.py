    ...
    try:
        if default_monitoring:
            ...
            if res_notin is not None and len(res_notin)>0:
                ...
                results = session.query(Device, skiply.cdm.powerSupply.PowerSupply).outerjoin(skiply.cdm.powerSupply.PowerSupply, skiply.cdm.powerSupply.PowerSupply.device_skiply_id == Device.device_skiply_id).filter(func.max(skiply.cdm.powerSupply.PowerSupply.power_time)).filter(Device.entity_id.notin_(ids)).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_start).filter(or_(Device.device_last_transmission <= deltatime_end, func.concat("0x", func.substr(Device.device_battery_level, 5, 4)) <= power)).all()
            else:
