...
def get_devices_to_monitor(hours_start, hours_end, power_threshold, power_threshold_percent, default_monitoring):
    ...
    try:
        if default_monitoring:
            print("get_devices_to_monitor(hours_start, hours_end, power_threshold, default_monitoring) : Start request contract/service")
            ...
            if res_notin is not None and len(res_notin)>0:
                ...
                print("get_devices_to_monitor(hours_start, hours_end, power_threshold, default_monitoring) : Start request devices for all entities excluded {}".format(res_notin))
                #results = session.query(Device).filter(Device.entity_id.notin_(ids)).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_start).filter(or_(Device.device_last_transmission <= deltatime_end, func.concat("0x", func.substr(Device.device_battery_level, 5, 4)) <= power)).all()
                results = session.query(Device, func.max(skiply.cdm.powerSupply.PowerSupply.power_time), skiply.cdm.powerSupply.PowerSupply.power_voltage, skiply.cdm.powerSupply.PowerSupply.power_voltage_idle, skiply.cdm.powerSupply.PowerSupply.power_voltage_percentage).outerjoin(skiply.cdm.powerSupply.PowerSupply, skiply.cdm.powerSupply.PowerSupply.device_skiply_id == Device.device_skiply_id).filter(Device.device_status == 'IN-SERVICE', Device.entity_id.notin_(ids)).filter(Device.device_last_transmission != None, Device.device_last_transmission > deltatime_start).filter(or_(Device.device_last_transmission <= deltatime_end, and_(skiply.cdm.powerSupply.PowerSupply.power_voltage <= power_threshold, skiply.cdm.powerSupply.PowerSupply.power_voltage > 0), and_(skiply.cdm.powerSupply.PowerSupply.power_voltage_percentage <= power_threshold_percent, skiply.cdm.powerSupply.PowerSupply.power_voltage_percentage > 0))).group_by(Device.device_skiply_id).all()
            else:
                print("get_devices_to_monitor(hours_start, hours_end, power_threshold, default_monitoring) : Start request devices")
        ...
        print("DB Request get_devices_to_monitor(hours_start, hours_end, power_threshold, default_monitoring) Failed with error : {}".format(e))
        results=None
    finally:
        session.close()
