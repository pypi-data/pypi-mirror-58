...
def get_contract_with_service_code(service_code):
    ...
    try:
        ...
        if service_results != None:
            ...
            results = session.query(Contract).join(Contract.service).joint(AssociationContractService.contract).filter(AssociationContractService.service_id._in(str_service_ids)).all()
