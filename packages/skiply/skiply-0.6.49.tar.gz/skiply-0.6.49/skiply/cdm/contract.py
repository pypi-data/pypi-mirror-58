...
def get_contract_with_service_code(service_code):
    ...
    try:
        ...
        if service_results != None and len(service_results)==1:
            ...
            results = session.query(Contract).join(Contract.services).join(AssociationContractService.contract).filter(AssociationContractService.service_id = str_service_ids).all()
