...
def get_contracts_for_services(service_ids):
    session = db_session()
    try:
        results = session.query(AssociationContractService).join(Contract).join(Service).filter(Service.id._in(service_ids)).all()
    except Exception as e:
...
def get_contract_with_service_code(service_code):
    ...
    try:
        ...
        if service_results != None and len(service_results)==1:
            service_ids = []
            service_ids.append(service_results[0]);
            print(service_ids)

            # Get contract_id 
            assoc_results = skiply.cdm.associationContractService.get_contracts_for_services(service_ids);
            print(assoc_results)

            if assoc_results != None and len(assoc_results)>0:
                contract_ids = []
                for assoc in assoc_results:
                    contract_ids.append(assoc.contract_id)

                print(contract_ids)

                results = session.query(Contract).filter(Contract.id._in(contract_ids))
            else:
