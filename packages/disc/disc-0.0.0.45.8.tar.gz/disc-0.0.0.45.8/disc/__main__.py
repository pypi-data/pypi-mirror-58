...
def inference(dataset, model, sess, output_dir, batch_size, workers, manager, log_fn=print):
    ...
    imputation_matrix = imputation_file.create_dataset("matrix",
                                                       ...
                                                       fletcher32=False,
                                                       dtype=np.float32)
    gene_feature_file = h5py.File("{}/gene_feature.hdf5".format(output_dir), "w")
    gene_feature_file["CellID"] = dataset.cell_id.astype(np.string_)
    gene_feature_file["Gene"] = dataset.gene_name.astype(np.string_)
    gene_feature = gene_feature_file.create_dataset("gene_feature",
                                                    shape=(model.gene_number, dataset.cell_number, model.depth[-2]),
                                                    chunks=(1, dataset.cell_number, model.depth[-2]),
                                                    fletcher32=False,
                                                    dtype=np.float32)
    ...
    for _ in range(generator.steps_per_epoch):
        ...
        impute_data[:, dataset.target_gene_mask], output_feature, this_gene_feature = sess.run(model.output_element, feed_dict=feed_dict)
        feature_matrix[:, batch_index] = output_feature.transpose()
        gene_feature[:, batch_index, :] = this_gene_feature
