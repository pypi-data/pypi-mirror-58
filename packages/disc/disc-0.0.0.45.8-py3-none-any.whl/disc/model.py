...
class DISC:
    ...
    def __init__(self, gene_name, norm_max, depth="16_8_1", repeats=3, dimension_number=512, compress_dimensions=50,
        ...
        with tf.compat.v1.variable_scope("imputer", reuse=False):
            self._imputer()
            self.merged_features = tf.add_n([feature_ * tf.expand_dims(tf.transpose(a_t), 2) for feature_, a_t in zip(self.features, self.attention_coefficients_list)])
        ...
        with tf.compat.v1.variable_scope("compression", reuse=False):
            self._compression()
        self.output = self.merge_impute_denorm[0]
        self.output_feature = self.compressed_feature[0]
        self.output_element = [self.output, self.output_feature, self.gene_feature]
    ...
    def _expression_predictor(self):
        ...
        #  predictor
        self.hidden_target.append(tf.matmul(self.input_norm * tf.cast(tf.logical_not(self.known_expressed), tf.float32), self.weights_encoder))
        for _ in range(self.repeats):
            #  feature refers "latent representation" of our paper
            features_input = [tf.stop_gradient(tf.matmul(current_input[0], self.weights_encoder)),
                              tf.matmul(current_input[1], self.weights_encoder)]
            hidden_layer_1_feature = [tf.expand_dims(tf.transpose(self.phi + 1), 2) * tf.transpose(tf.reshape(tf.matmul(self.en_de_act_fn(features_stop_), self.predictor_weights_list[0]) + self.predictor_bias_list[0], [tf.shape(features_stop_)[0], self.gene_number, self.depth[0]]), [1, 0, 2]) for features_stop_ in features_input]
            self.features.append(hidden_layer_1_feature[0])
            ...
            self.impute_latents.append(hidden_layer_feature)
            ...
            this_psi = self._psi(hidden_layer_feature[0], self.weights_psi)
            output_layer_feature = [tf.transpose(tf.squeeze(self.output_scale_factor * latent_1 * this_psi, 2) + tf.squeeze(self.output_scale_factor * latent_2 * (1 - this_psi), 2)) for latent_1, latent_2 in zip(output_1, output_2)]
            self.predictions.append([self.output_activation_function(latent_) for latent_ in output_layer_feature])
            self.zero_predictions.append(self.output_activation_function(tf.transpose(tf.squeeze(self.output_scale_factor * output_1[0] * tf.stop_gradient(this_psi), 2) + tf.squeeze(self.output_scale_factor * output_2[0] * tf.stop_gradient(1 - this_psi), 2))))
            ...
            self.hidden_target.append(tf.matmul(tf.stop_gradient(self.predictions[-1][0]) * tf.cast(tf.logical_not(self.known_expressed), tf.float32), self.weights_encoder))
    ...
    def _attention(self):
        logits = [tf.stop_gradient(tf.nn.selu(feature_)) for feature_ in self.features]
        ...
        self.attention_coefficients_merged = tf.reshape(tf.transpose(tf.nn.softmax(tf.concat([tf.transpose(tf.matmul(feature_, self.weights_attention),[1, 0, 2]) for feature_ in logits], 2), 2), [2, 0, 1]), [tf.shape(logits[0])[1] * len(logits), self.gene_number])
        self.attention_coefficients_list = tf.split(self.attention_coefficients_merged, self.repeats, 0)

    def _imputer(self):
        weighted_latents = [[l_ * tf.expand_dims(tf.transpose(a_), 2) for l_ in latent_] for latent_, a_ in zip(self.impute_latents, self.attention_coefficients_list)]
        merge_gene_feature = [tf.add_n([latent_[i] for latent_ in weighted_latents]) for i in range(len(weighted_latents[0]))]
        this_psi = self._psi(merge_gene_feature[0], self.weights_psi)
        output_1 = [tf.expand_dims(tf.transpose(self.phi + 1), 2) * (tf.matmul(self.en_de_act_fn(latent_), self.predictor_weights_list[-2]) + self.predictor_bias_list[-2]) for latent_ in merge_gene_feature]
        output_2 = [tf.expand_dims(tf.transpose(self.phi + 1), 2) * (tf.matmul(self.en_de_act_fn(latent_), self.predictor_weights_list[-1]) + self.predictor_bias_list[-1]) for latent_ in merge_gene_feature]
        self.gene_feature = self.en_de_act_fn(merge_gene_feature[0])
        self.merge_impute = [self.output_activation_function(tf.transpose(tf.squeeze(self.output_scale_factor * latent_1 * this_psi, 2) + tf.squeeze(self.output_scale_factor * latent_2 * (1 - this_psi), 2))) for latent_1, latent_2 in zip(output_1, output_2)]
    ...
    def training(self, learning_rate, var_list=None):
        ...
        expression_mask_1 = [tf.where(tf.less(self.input_raw, 1), 3. * tf.ones_like(self.input_norm), tf.zeros_like(self.input_norm)) for _ in self.predictions]
        ...
        feature_targets = self.hidden_target[:-1]
        ...
        self.latent_representation_loss = tf.add_n([tf.reduce_mean(tf.reduce_sum(tf.square(feature_), 1)) for feature_ in self.hidden_layer]) / self.repeats + \
                                          tf.add_n([tf.reduce_mean(tf.reduce_sum(tf.square(feature_ - tf.stop_gradient(tf.matmul(self.merge_impute[0] * tf.cast(tf.logical_not(self.known_expressed), tf.float32), self.weights_encoder))), 1)) for feature_ in self.hidden_layer]) / self.repeats
