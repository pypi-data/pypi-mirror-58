...
,allow_status=[200],timeout=(10,10),method='get',data=None,is_value=None,retry=3,timesleep=0,show_result=False,allow_redirects=False):
    request_count = 0
    ...
    while True:
        ...
        if method.lower() == 'get':
            ...
            if response:
                ...
                elif response.status_code not in allow_status:
                    status_code_not_allow += 1
                elif proxy and response.status_code == 403:
                    proxy = get_ip()
                if is_value in response.text and is_value:
                    return (response,conn,proxy)
                elif not is_value in response.text and proxy:
                    proxy = get_ip()
                if show_result:
                    print(response.text)
            else:
                raise ValueError('无response响应对象')
