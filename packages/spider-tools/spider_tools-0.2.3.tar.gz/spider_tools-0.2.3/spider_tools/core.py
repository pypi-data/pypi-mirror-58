...
# i.insert_data(item)
class Auto_sinsert():
    def __init__(self,host='192.168.4.205',username='root',password='mysql',db='storm',drop_column=["id","updated"]):
        red_print("--------------------------------------------------------------------------------------------")
...
class query_db():
    ...
    def __init__(self):
        self.conn = pymysql.connect(host='192.168.4.205', port=3306, user='root',passwd='mysql', db='storm', charset='utf8')
        self.cursor = self.conn.cursor()
        print("连接数据库成功")
    ...
    def insert_max_data(self,b2b_name):
        sql = 'select url from {}'.format(self.table_name) + ' where b2b=%r' % b2b_name
        print("正在查询{}表{}url数据。。。".format(self.table_name,b2b_name))
        self.cursor.execute(sql)
        urls = self.cursor.fetchall()
        id_list = []
        for url in urls:
        ...
        if id_list:
            ...
            id_list.remove(key)
            max_id = max(id_list)
            print("查询的最大ID为 {}".format(max_id))
            for url in urls:
                print(url[0])
                if str(max_id) in url[0]:
                    print("存入{}网站最大url".format(b2b_name))
                    insert_into = '''insert into max_url(url,b2b) values ('%s','%s') '''% (url[0],b2b_name)
...
def my_request(conn,url,proxy=None,headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36'}
,allow_status=[200],timeout=(5,8),method='get',data=None,is_value='',no_value='',retry=10,timesleep=0,show_result=False,allow_redirects=False,verify=False):
    request_count = 0
    status_code_not_allow = 0

    while True:
        if proxy and status_code_not_allow>10:
            proxy = get_ip()
        try:
            if method.lower() == 'get':
                response = conn.get(url=url, headers=headers, params=data, timeout=timeout, allow_redirects=allow_redirects,
                                    proxies=proxy,verify=verify)
                print("响应状态：{} 访问url：{}".format(response.status_code, url))
                if response.status_code in allow_status:
                    if is_value and is_value in response.text:
                        return (response, conn, proxy)
                    if no_value and no_value not in response.text:
                        return (response, conn, proxy)
                    elif proxy:
                        proxy = get_ip()
                    return (response,conn,proxy)
                ...
                response = conn.post(url=url, headers=headers, params=data, timeout=timeout, allow_redirects=allow_redirects,
                                    proxies=proxy,verify=verify)
