...
import sys
import json
from PySide2.QtCore import QMimeData, Signal
...
class Strategy(metaclass=ABCMeta):
    notifyMsgSignal = Signal(float, str)
    ...
    def notifyMsg(self, process: float = 0, msg: str = ""):
        self.notifyMsgSignal.emit(process, msg)
    ...
    def saveImage(self, imageSrcList: list = None) -> rx.Observable:
        ...
        def _createObserver(observable, scheduler):
            ...
            try:
                ...
                if self.strategy:
                    self.strategy.notifyMsgSignal.connect(notifyMsg)
                for img in _imageSrcList:
                    if self.strategy:
                        ...
                        if result["code"] == 'success':
                            observable.on_next(
                                HandlerStatusInfo(StatusFlag.Process, msg="upload image: %s success" % img, value=100))
        ...
        def _createObserver(observable, scheduler):
            ...
            try:
                ...
                if self.strategy:
                    ...
                    if result["code"] == 'success':
                        observable.on_next(
                            HandlerStatusInfo(StatusFlag.Process, msg="upload image: %s success" % tempFileName,
                                              value=100))
                        observable.on_next(result['data'])
                    else:
                        observable.on_next(
