...
if sys.version_info[0] < 3:  # python2
	...
	def get_argspec(f):
		...
		# (name of all args, having or not default values), name of *args, name of **kwargs, (name of args without default values), (Ordered dict containing name:default values)
...
class Signature(object):
	...
	def construct_arguments(self, args, kwargs, options, bound=False):
		...
		# Args
			- args : arguments that are explicitely called
			- kwargs : kwargs that are explicitely called
			- options : dictionnary of sacred options		
	...
	def __fill_in_options(self, args, kwargs, options, bound):
		# modified version to correct the object sharing bug
		free_params = self.get_free_parameters(args, kwargs, bound)
		new_kwargs = dict(kwargs) if free_params else kwargs
		for param in free_params:
			if param in options:				
				new_kwargs[param] = copy.deepcopy(options[param])
		return args, new_kwargs
