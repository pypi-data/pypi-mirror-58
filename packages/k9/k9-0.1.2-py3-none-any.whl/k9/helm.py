...
def helm_repo_add(repo_name: str, repo_url: str):
    ...
    """
    the_call = ['helm', 'repo', 'add', repo_name, repo_url]
    return subprocess.check_output(the_call)
...
def helm_repo_ls():
    ...
    """
    the_call = ['helm', 'repo', 'list', '-o', 'json']
...
def helm_repo_remove(repo_name: str):
    ...
    :return: output string on success, exception if failure.
    """
    the_call = ['helm', 'repo', 'remove', repo_name]
    return subprocess.check_output(the_call)
...
def helm_uninstall(release_name: str):
    ...
    """
    return subprocess.check_output(['helm', 'uninstall', '-n', get_default_namespace(), release_name])
