...
def set_storage_output(output_flag=True):
    """
    :param output_flag: Set to true if storage API calls should be output to console.
    :return: None
    """
    global output
    output = output_flag
...
def create_storage_class(fn: str):
    ...
    :return: `V1StorageClass <https://github.com/kubernetes-client/python/blob/master/kubernetes/docs/V1StorageClass.md>`_
    ...
    result = subprocess.check_output(['kubectl', 'create', '-f', fn])
...
def delete_storage_class(name: str):
    ...
    """
    result = subprocess.check_output(['kubectl', 'delete', 'storageclass', name])
    result = str(result, 'utf-8')
    if output:
        print(result)
...
def list_storage_classes():
    ...
    result = subprocess.check_output(['kubectl', 'get', 'storageclass', '-o', 'json'])
