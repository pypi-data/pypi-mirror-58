...
from k9.core import last_word, namespace_exists, get_default_namespace, create_namespace

# Print output if true.
output = True

def set_helm_output(output_flag=True):
    """
    :param output_flag: Set to true if helm API calls should be output to console.
    :return: None
    """
    global output
    output = output_flag
...
def helm_repo_add(repo_name: str, repo_url: str):
    ...
    """
    result = subprocess.check_output(['helm', 'repo', 'add', repo_name, repo_url])
...
def helm_repo_update():
    ...
    :return: output string on success, exception if failure.
    """
    result = subprocess.check_output(['helm', 'repo', 'update'])
    result = str(result, 'utf-8')
...
def helm_repo_ls():
    ...
    """
    result = subprocess.check_output(['helm', 'repo', 'list', '-o', 'json'])
...
def helm_repo_remove(repo_name: str):
    ...
    :return: output string on success, exception if failure.
    """
    result = subprocess.check_output(['helm', 'repo', 'remove', repo_name])
    result = str(result, 'utf-8')
...
def helm_install(chart_name: str, params, release_name: str = None, debug = False):
    ...
    try:
        ...
        result = subprocess.check_output(the_call)
        result = str(result, 'utf-8')
...
def helm_ls():
    ...
    values = subprocess.check_output(['helm', '-n', get_default_namespace(), 'ls', '-o', 'json'])
...
def helm_uninstall(release_name: str):
    ...
    """
    result = subprocess.check_output(['helm', 'uninstall', '-n', get_default_namespace(), release_name])
    result = str(result, 'utf-8')
