...
class PublicationBuilder:
    ...
    def _has_unique_metadata(self, key: str) -> bool:
        meta = self._metadata if self._metadata is not None else []
        return len([m for m in meta if m.key == key]) == 1
    ...
    def build(self) -> Publication:
        ...
        if not self._title and not self._has_unique_metadata(SemanticMetadata.TITLE):
            pattern = 'Missing or multiple title when building publication {}'
            raise InvalidPublication(pattern.format(self._id))
