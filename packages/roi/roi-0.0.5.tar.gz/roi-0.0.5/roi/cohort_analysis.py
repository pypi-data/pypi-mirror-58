...
class cohort_analysis():
    ...
    def analysis(self,input_df):
        ...
        if(1):
            ...
            self.retention_table =x['CustomerID'].divide(x['CustomerID'][1],axis='rows')
    ...
    def plot_retention(self):
      ...
      """
      sns.set(style='white')
      fig,ax = plt.subplots(figsize=c.retention_table.shape)
      sns.heatmap(c.retention_table,annot=True,fmt='.0%')
      ax.xaxis.tick_top()
