...
"""
    ...
    with open(os.path.join(out, "log.md"), "w") as f:
        f.write(log_md)

    with open(os.path.join(out, "build.sh"), "w") as f:
        f.write(build_sh)

    os.chmod(os.path.join(out, "build.sh"), 0o755)

    with open(os.path.join(out, "pandoc.css"), "w") as f:
        f.write(css)
    
    with open(os.path.join(out, "links-to-html.lua"), "w") as f:
        f.write(links_to_html)
