...
@LastEditTime : 2020-01-06 02:31:38
...
class SparkCreator(object):
    ...
    def __viewparse2code(self, view):
        ...
        tt = self.spark.sql(hive_sql).where(col('col_name') == "View Text").select('data_type').collect()
    ...
    def __sqlparse2table(self, query):
        ...
        for plan_item in plan_items:
            if plan_item['class'] == 'org.apache.spark.sql.catalyst.analysis.UnresolvedRelation':
                ...
                table_name = "{}.{}".format(
                    database, table) if database else table
                if table_name not in cte:
                    tables.update([table_name.lower()])
                    schema.update([database.lower()])
        ...
        def one_round(query):
            ...
            if temp_sch:
                ...
                for sc in temp_sch:
                    ...
                    views = [sc + '.' + x.name for x in temp if x.tableType == 'VIEW']
                    for view in views:
    ...
    def batch_kudu2view(self, tables, ifprint=True):
        ...
        for table in tables:
            ...
            if table not in self.__customkudus:
                hive_cmd = "hive -e \"show tblproperties {0} ('kudu.table_name');\"".format(table)
                cmdreturn = os.popen(hive_cmd).read()
                if 'kudu.table_name' not in cmdreturn:
    ...
    def batch_excutesql(self, sql, sqlsel=[], ifview=False, ifkudu=True):
        '''
        @description: batch excute
        @return: excutesql
        '''
        querys = self.__sqlparse2sqls(sql)
        ...
        if ifkudu:
            ...
            for query in querys:
                for tb in tables_set:
                    query = re.sub(tb, self.__customkudus[tb]['view_name'], query, flags=re.I)
        ...
        for query in querys:
            i += 1
            print('\rExcute Progress: {0}/{1}'.format(str(i), cnt_query), end='')
