...
from django.db.models import BigIntegerField, BooleanField, Max
...
class Lotsman_documents_hierarcy_view(AuditModel):
    ...
    def SPC_CLM_MATERIAL_id(self):
        ...
        position_in_document_dict = Lotsman_document_attr_cross.objects.filter(document_id=self.id).aggregate(Max('position_in_document'))
        ...
        if position_in_document == None:
            ...
            position_in_document += 1
        Lotsman_document_attr_cross.objects.get_or_create(document_id=self.id, attribute=attribute, position_in_document=position_in_document)
        return attribute.id
