...
def Pathes_view_Add(request):
    return JsonResponse(DSResponseAdd(data=Pathes_view.objects.createFromRequest(request=request), status=RPCResponseConstant.statusSuccess).response)


@JsonResponseWithException()
def Pathes_view_Update(request):
    return JsonResponse(DSResponseUpdate(data=Pathes_view.objects.updateFromRequest(request), status=RPCResponseConstant.statusSuccess).response)
