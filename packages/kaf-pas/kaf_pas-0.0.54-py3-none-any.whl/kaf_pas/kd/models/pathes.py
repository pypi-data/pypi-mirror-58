        ...
        with transaction.atomic():
            for i in range(0, len(ids), 2):
                if ids[i + 1] == 'hide':
                    ...
                    Lotsman_documents_hierarcy.objects.filter(document=ids[i]).delete()

                    Documents_thumb.objects.filter(document=ids[i]).delete()
                    Documents_thumb10.objects.filter(document=ids[i]).delete()
                    
                    res, count = Documents.objects.filter(path=ids[i]).delete()
                    res += super().filter(id=ids[i]).delete()[0]
                    if len(count) > 0:
                        refresh_mat_view('kd_documents_mview', loger_func=lambda str: self.logging(str))
                        refresh_mat_view('kd_lotsman_documents_hierarcy_mview', loger_func=lambda str: self.logging(str))
        return res
