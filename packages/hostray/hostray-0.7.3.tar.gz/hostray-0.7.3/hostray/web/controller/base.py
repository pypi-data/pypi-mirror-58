...
class ControllerAddon():
    ...
    async def invoke_service_async(self, service_name: str, method: str = 'get', streaming_callback: Callable = None, **kwargs) -> Response:
        if self.services is None:
            self.services = self.component_manager.get_component(
                OptionalComponentTypes.Service)
        return await self.services.invoke_async(service_name, method=method, streaming_callback=streaming_callback, **kwargs)
