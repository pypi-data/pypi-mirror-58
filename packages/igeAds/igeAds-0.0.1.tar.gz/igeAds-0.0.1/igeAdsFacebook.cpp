...
static PyObject *onAdsFacebookCB = nullptr;

void adsFacebook_ProcessEventCallbackHandler(const char* adsType, int number, const char* reward)
{
	if(onAdsFacebookCB) {
		...
		PyObject *result = PyEval_CallObject(onAdsFacebookCB, arglist);
		Py_XDECREF(result);
	}
}

PyObject* adsFacebook_RegisterEventListener(adsApplovin_obj* self, PyObject* args)
{
	if (!PyArg_ParseTuple(args, "O", &onAdsFacebookCB))
		return NULL;

	if (!PyCallable_Check(onAdsFacebookCB)) {
		PyErr_SetString(PyExc_TypeError, "Callback function must be a callable object!");
		return NULL;
	}
	Py_XINCREF(onAdsFacebookCB);
...
PyMethodDef adsFacebook_methods[] = {
	...
	{ "registerEventListener", (PyCFunction)adsFacebook_RegisterEventListener, METH_VARARGS, adsAdmobRegisterEventListener_doc },
	{ NULL,	NULL }
};
