...
import refinitiv.dataplatform.function.json_requests
...
import pandas as pd
from refinitiv.dataplatform.core.session.elektronError import ElektronError
...
def get_timeseries(rics, fields='*', start_date=get_date_from_today(100), end_date=get_date_from_today(0),
    ...
    response = refinitiv.dataplatform.function.json_requests.send_json_request(TimeSeries_UDF_endpoint, payload, debug=debug)
    ...
    if len(ts_status_errors) == len(ts_timeserie_data):
        ...
        raise ElektronError('Error', message=ts_error_messages)
