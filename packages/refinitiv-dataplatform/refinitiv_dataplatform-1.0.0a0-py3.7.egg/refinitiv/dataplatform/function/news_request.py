...
import refinitiv.dataplatform.function.json_requests, refinitiv.dataplatform.core.session
import json
...
def get_news_headlines(query='Topic:TOPALL and Language:LEN', count=10, date_from=None,
    ...
    response = refinitiv.dataplatform.function.json_requests.send_json_request(News_Headlines_UDF_endpoint, payload, debug=debug)
...
def get_data_frame(json_data):
    ...
    headlines_dataframe = pd.DataFrame(headlines, pd.np.array(first_created, dtype='datetime64'),
...
def get_news_story(story_id, raw_output=False, debug=False):
    ...
    response = refinitiv.dataplatform.function.json_requests.send_json_request(News_Story_UDF_endpoint, payload, debug=debug)
