...
class Stream(object):
    ...
    async def _wait_for_response(self):
        ...
        try:
            ...
            self._session.log(logging.ERROR, f'Error occurred on stream {self._stream_id}: {e!r}')
    ...
    async def open_async(self):
        ...
        from refinitiv.dataplatform.core import ElektronError
        if self._session is None:
            raise AttributeError("Session is mandatory")

        if self._session.get_open_state() is Session.State.Closed:
            raise ElektronError(-1, "Session must be opened")
        ...
        if result:
            ...
            if self._service is None:
                message = "Subscribe to {default service}" + f".{self._name}"
            else:
                message = f"Subscribe to {self._service}.{self._name}"
            if self._fields:
