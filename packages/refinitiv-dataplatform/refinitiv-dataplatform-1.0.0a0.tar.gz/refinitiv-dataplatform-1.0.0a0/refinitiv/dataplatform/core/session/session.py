...
from refinitiv.dataplatform.delivery.stream import StreamConnection
...
class DacsParams(object):
    ...
    def __init__(self, *args, **kwargs):
        self.dacs_username = kwargs.get("dacs_username", "user")
...
class Session(object):
    ...
    class Params(object):
        ...
        def with_dacs_username(self, user):
            if user:
    ...
    def register_session(cls, session):
        with cls.__register_session_lock:
            if not session:
                raise ElektronError('Error', 'Try to register unavailable session')
    ...
    def unregister_session(cls, session):
        with cls.__register_session_lock:
            if not session:
                raise ElektronError('Error', 'Try to unregister unavailable session')
            session_id = session.session_id
            if session_id is None:
                raise ElektronError('Error', 'Try to unregister unavailable session')
            if session_id not in cls.__all_sessions:
                raise ElektronError('Error',
    ...
    def get_session(cls, session_id):
        ...
        """
        if session_id not in cls.__all_sessions:
            raise ElektronError('Error', 'Try to get unknown session id {}'.format(session_id))
        return cls.__all_sessions.get(session_id)

    def __init__(self, app_key, on_state=None, on_event=None,
                 token=None, dacs_username=None, dacs_position=None, dacs_application_id=None):
        ...
        if dacs_username:
            self._dacs_params.dacs_username = dacs_username
        if dacs_position:
    ...
    def app_key(self, app_key):
        ...
        """
        from refinitiv.dataplatform.function.tools import is_string_type
    ...
    def _stop_streaming(self):
        ...
        if self._streaming_session is not None:
            ...
            self.log(logging.INFO, 'Streaming session {} was closed'.format(self._streaming_session.streaming_session_id))
    ...
    def _register_stream(self, stream):
        with self._stream_register_lock:
            if stream is None:
                raise ElektronError('Error', 'Try to register None subscription')

            if stream._stream_id in self._all_stream_subscriptions:
                raise ElektronError('Error', f'Subscription {stream._stream_id} is already registered')
            if stream._stream_id is not None:
                raise ElektronError('Error', f'Try to register again subscription {stream._stream_id}')
    ...
    def _unregister_stream(self, stream):
        with self._stream_register_lock:
            if not stream or not stream._stream_id:
                raise ElektronError(-1, 'Try to unregister unavailable stream')
    ...
    def _get_stream(self, stream_id):
        with self._stream_register_lock:
            if stream_id is None:
                raise ElektronError('Error', 'Try to retrieve undefined stream')
