...
from refinitiv.dataplatform.function.tools import to_utc_datetime, to_utc_date, to_utc_datetime_isofmt
...
def _convert_interval_to_iso8601(interval):
    ...
    elif interval in _ISO8601_INTERVALS:
        return interval
    else:
...
class HistoricalPricing(object):
    """
    """
    _URL_HISTORICALPRICING = u'data/historical-pricing/v1'
    ...
    def __init__(self, session=None, on_response=None):
        from refinitiv.dataplatform.function.tools import get_default_session
        ...
        self._data = None
        self._endpoint = Endpoint(session,
                                  HistoricalPricing._URL_HISTORICALPRICING,
    ...
    async def _get_events_async(self,
        ...
        _url = '/'.join([HistoricalPricing._URL_HISTORICALPRICING, u'views/events/{universe}'])
    ...
    async def _get_summaries_async(self,
        ...
        _url = '/'.join([HistoricalPricing._URL_HISTORICALPRICING, u'views/interday-summaries/{universe}'])

        function_format_datetime = to_utc_date
        if interval:
            interval = _convert_interval_to_iso8601(interval)
            if interval in _INTRADAY:
                _url = '/'.join([HistoricalPricing._URL_HISTORICALPRICING, u'views/intraday-summaries/{universe}'])
    ...
    def _convert_historical_json_to_pandas(json_historical_data):
        _ric = json_historical_data['universe']['ric']
        ...
        if _data:
            ...
            _numpy_array = pd.np.array(_data)
            # build timestamp as index for dataframe
            _timestamp_array = pd.np.array(_numpy_array[:, _timestamp_index], dtype='datetime64')
            # remove timestamp column from numpy array
            _numpy_array = pd.np.delete(_numpy_array, pd.np.s_[_timestamp_index], 1)

            historical_dataframe = pd.DataFrame(_numpy_array, columns=_fields, index=_timestamp_array)
    ...
    async def _get_historicalpricing(self,
        ...
        if fn_format_datetime is None:
            raise ValueError('Format datetime function must defined.')
        ...
        if count < 1:
            raise AttributeError('count mininum value is 1')
