...
def send_json_request(entity, payload, ID='123', debug=False, raw_response=False):
    ...
    if _session:
        ...
        if network_error:
            ...
            logger.error(error_msg)
            raise ElektronError('401', error_msg)
...
def check_server_error(server_response, session=None):
    ...
    if isinstance(server_response, dict):
        if 'ErrorCode' in server_response and 'ErrorMessage' in server_response:
            ...
            raise ElektronError(server_response['ErrorCode'], error_message)
    ...
    if 'error' in server_response:
        ...
        logger.error(error_message)
        raise ElektronError(400, error_message)
...
def raise_for_status(response):
    ...
    if error_msg:
        ...
        raise ElektronError(response.status_code, error_msg)
