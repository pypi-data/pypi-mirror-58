...
class AttachmentLevel(Enum):
    """Represents the values of the `attachmentLevel` attribute of the <Attribute>
    SDMX element.
    """
...
def build_dimension_mask(structure: DatasetStructure, dimensions: Dimensions) -> str:
    """Return a string representing a filter to search series by dimension,
    often used in SDMX APIs.
...
ObsValue = Union[float, Literal["NaN"]]
...
def load_dataset_structure(structure_element: Element) -> DatasetStructure:
    ...
    return DatasetStructure(
        ...
        names=load_lang_dict(keyfamily_element, tag=NAME_TAG),
        codelists=list(iter_codelists(structure_element.find(f"./{CODELISTS_TAG}"))),
        concepts=list(iter_concepts(structure_element.find(f"./{CONCEPTS_TAG}"))),
        dimensions=list(
            iter_dimensions(keyfamily_element.iterfind(f".//{DIMENSION_TAG}"))
        ),
        attributes=list(
            iter_attributes(keyfamily_element.iterfind(f".//{ATTRIBUTE_TAG}"))
        ),
    )
...
def iter_codelists(codelists_element: Element) -> Iterator[CodeList]:
    for codelist_element in codelists_element.iterfind(f"./{CODELIST_TAG}"):
        yield CodeList(
            id=codelist_element.attrib["id"],
            names=load_lang_dict(codelist_element, tag=NAME_TAG),
            codes=list(map(load_code, codelist_element.iterfind(f"./{CODE_TAG}"))),
        )


def iter_concepts(concepts_element: Element) -> Iterator[Concept]:
    for concept_element in concepts_element.iterfind(f"./{CONCEPT_TAG}"):
        yield Concept(
            id=concept_element.attrib["id"],
            names=load_lang_dict(concept_element, tag=NAME_TAG),
        )


def iter_dimensions(dimension_elements: Sequence[Element]) -> Iterator[Dimension]:
...
def iter_attributes(attribute_elements: Sequence[Element]) -> Iterator[Attribute]:
...
def load_lang_dict(element: Element, tag: str) -> Dict[Lang, str]:
    return {
...
def load_code(code_element: Element) -> Code:
    return Code(
        value=code_element.attrib["value"],
        descriptions=load_lang_dict(code_element, tag=DESCRIPTION_TAG),
        parent_code=code_element.attrib.get("parentCode"),
    )


def load_dataset(dataset_element: Element) -> Dataset:
    return Dataset(series=list(iter_series(dataset_element)))


def iter_series(dataset_element: Element) -> Iterator[Series]:
    for series_element in dataset_element.iterfind(f"./{SERIES_TAG}"):
        attribute_element = series_element.find(f"./{ATTRIBUTES_TAG}")
        attributes = (
            list(iter_values(attribute_element))
            if attribute_element is not None
            else []
        )
        yield Series(
            key=list(iter_values(series_element.find(f"./{SERIES_KEY_TAG}"))),
            attributes=attributes,
            observations=list(iter_obs(series_element)),
        )


def iter_values(element: Element) -> Iterator[Value]:
...
def iter_obs(series_element: Element) -> Iterator[Obs]:
    for obs_element in series_element.iterfind(f"./{OBS_TAG}"):
        ...
        attributes = (
            list(iter_values(attributes_element))
...
def parse_observation_value(value: str) -> ObsValue:
    ...
    try:
        ...
        raise ValueError("Invalid value {value!r} for a SDMX observation") from exc


def remove_prepared_date(element: Element) -> Element:
    """Remove prepared date to avoid triggering a false commit in source data.
    Mutate `element` and return it to ease using that function as a callback,
    for example with `loaders.fetch_or_read_xml`.
    """
...
def get_one_name(
    ...
    Default value is `None`, which means that the first available name will be returned.
    """
...
def keyfamily_to_dataset_references(
    ...
    """Return a `DatasetReference` build from `keyfamily`.

    Use `lang_candidates` to choose a preferred language.
    It is forwarded to `get_one_name`.
    """
    ...
    def iter_value_labels(
        ...
        for item in items:
            ...
            if not used_value_codes:
                continue
            yield item.concept_id, dict(iter_values(item, used_value_codes))
...
def series_to_series_json(series: Series) -> dict:
    ...
    def get_attribute_headers(observations: List[Obs]) -> List[str]:
        """Iterate `observations` and return a list of all attribute concept IDs,
        in the order they are encountered.

        Raise `NoTimeDimensionError` if series has no time dimension.
        """
...
def build_used_codes(
    all_series: Sequence[Series], get_values: Callable[[Series], Sequence[Value]],
) -> Dict[str, Set[str]]:
