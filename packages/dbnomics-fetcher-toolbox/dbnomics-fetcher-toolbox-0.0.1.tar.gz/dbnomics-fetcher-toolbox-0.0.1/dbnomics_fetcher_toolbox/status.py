...
class BaseEvent(BaseModel):
    ...
    def to_json_data(self):
        return orjson.loads(self.json(exclude_none=True))
...
@contextmanager
def open_writer(args: argparse.Namespace) -> Iterator[Callable[[BaseEvent], None]]:
    ...
    """
    with jsonlines.open(
        args.target_dir / STATUS_JSONL, mode="a", flush=args.flush_status,
    ) as writer:
...
def load_events(target_dir: Path, dedupe: bool = True) -> Optional[List[BaseEvent]]:
    """Load events from `status.jsonl` expected to be found in `target_dir`.

    If `dedupe` is `True` (default), the events are deduped by `id`, keeping only the
...
def load_events_from_file(file: Path, dedupe: bool = True) -> List[BaseEvent]:
...
def iter_events(file: Path) -> Iterator[BaseEvent]:
