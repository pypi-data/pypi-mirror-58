...
def iter_dataset_references(category_tree: CategoryTree) -> Iterator[DatasetReference]:
    """Yield `DatasetReference` objects by iterating `category_tree` recursively."""
...
def write_series_jsonl(directory: Path, series: Iterable[dict]):
    """Encode `series` to JSON Lines and write it to "series.jsonl" under `directory`.
