...
def jyfm_data_soybean_meal(indicator="豆粕月度产量", headers=""):
    """
    交易法门-数据-农产品-豆粕
    :param indicator: ["大豆月度进口", "大豆原料库存", "压榨开工率", "企业压榨利润", "豆粕月度产量", "豆粕每日成交", "豆粕周度库存", "豆粕平衡表"]
    :type indicator: str
    :param headers: headers with cookies
    :type headers: dict
    :return: result
...
def jyfm_data_soybean_oil(indicator="豆油年度产能", headers=""):
    """
    交易法门-数据-农产品-豆油
    :param indicator: ["豆油年度产能", "压榨装置开工率", "油厂周度产量", "豆油现货成交", "豆油商业库存", "豆油毛利润", "豆油月度产量", "豆油月度进口", "豆油月度消费", "豆油月度出口", "豆油月度库存"]
    :type indicator: str
    :param headers: headers with cookies
    :type headers: dict
    :return: result
    ...
    """
    res = requests.get(
        jyfm_data_soybean_oil_name_url_dict[indicator], headers=headers,
    )
    return pd.DataFrame(res.json())
...
def jyfm_data_sugar(indicator="白糖产区库存", headers=""):
    """
    交易法门-数据-农产品-白糖
    :param indicator: ["国内种植面积", "年度产糖率", "白糖年度产销", "白糖进出口量", "食糖产需缺口", "白糖月度产量", "白糖月度销量", "白糖月度进口", "食糖工业库存", "白糖产区库存"]
    :type indicator: str
    :param headers: headers with cookies
    :type headers: dict
    :return: result
...
def jyfm_data_cocking_coal(indicator="焦煤总库存", headers=""):
    """
    交易法门-数据-黑色系-焦煤
    :param indicator: ["焦煤总库存", "焦煤焦企库存100", "焦煤焦企库存230", "焦煤钢厂库存", "焦煤港口库存"]
    :type indicator: str
    :param headers: headers with cookies
    :type headers: dict
    :return: result
    ...
    """
    res = requests.get(
        jyfm_data_cocking_coal_url_dict[indicator], headers=headers,
    )
    # 由于返回的数据长度不一致，先补齐再转置
...
if __name__ == "__main__":
    ...
    print(df)
    df = jyfm_data_soybean_meal(indicator="豆粕月度产量", headers=headers)
    print(df)
    df = jyfm_data_soybean_oil(indicator="豆油年度产能", headers=headers)
    print(df)
    ...
    print(df)
    # 黑色系
    df = jyfm_data_cocking_coal(indicator="焦煤总库存", headers=headers)
    print(df)
