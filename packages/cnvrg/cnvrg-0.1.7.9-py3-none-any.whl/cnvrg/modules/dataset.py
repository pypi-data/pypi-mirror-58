...
class Dataset(CnvrgFiles):
    ...
    def get_query(self, query_slug: AnyStr, data_dir: AnyStr=None):
        return self._fetch_query(query_slug,data_dir)
    ...
    def _fetch_query(self, query_slug: AnyStr, data_dir: str=None):
        response = apis_helper.get(apis_helper.url_join(self.get_base_url(), "queries", query_slug), data={"working_dir": data_dir})
        return response.get("files")
    ...
    def download_file(self, file):
        ...
        if not os.path.exists(file_path):
            ...
            else: self.storage.download_single_file(file)
        return file_path
