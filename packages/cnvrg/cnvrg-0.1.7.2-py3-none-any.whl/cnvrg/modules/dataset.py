...
class Dataset(CnvrgFiles):
    def __init__(self, dataset=None, dataset_url=None, working_dir=None):
        ...
        c = data_working_dir_helper.init_data_working_dir(working_dir)
        if dataset_url:
    ...
    def get_query(self, query_slug: AnyStr, working_dir=None):
        return self._fetch_query(query_slug, working_dir)

    def search(self, search: AnyStr):
        return self._search(search)

    def _search(self, search: AnyStr):
        response = apis_helper.post(apis_helper.url_join(self.get_base_url(), "search"), data={"search": search})
        return response.get("files")

    def _fetch_query(self, query_slug: AnyStr, working_dir=None):
        response = apis_helper.get(apis_helper.url_join(self.get_base_url(), "queries", query_slug), data={"working_dir": working_dir})
