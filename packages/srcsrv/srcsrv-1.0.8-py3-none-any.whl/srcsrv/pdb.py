...
class PDB(base.Base):
    ...
    def sources(self):
        ...
        try:
            cp = subprocess.run([srctool, '-r', '-z', '-h', f'-l:{self.path_match}' + 'cpp', self.args.pdb], stdout=subprocess.PIPE, check=True)
            files = cp.stdout.splitlines()[:-1]
            cp = subprocess.run([srctool, '-r', '-z', '-h', f'-l:{self.path_match}' + 'h', self.args.pdb], stdout=subprocess.PIPE, check=True)
            files = files + cp.stdout.splitlines()
