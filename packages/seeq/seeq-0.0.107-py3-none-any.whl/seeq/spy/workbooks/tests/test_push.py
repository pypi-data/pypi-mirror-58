...
def _compare_inventory(old, new):
    for old_id, old_item in old.items():
        new_items = [i for i in new.values() if i.name == old_item.name]
        assert len(new_items) != 0, '"%s" not found in new inventory' % old_item
...
def test_push_repeatedly():
    ...
    old_inventory = workbook.scope_inventory.copy()
    push1_df = spy.workbooks.push(workbooks, path='test_push_repeatedly')
    new_inventory = workbook.scope_inventory.copy()
    _compare_inventory(old_inventory, new_inventory)
    ...
    push3_df = spy.workbooks.push(workbooks, path='test_push_repeatedly', label='repeat_with_label', use_full_path=True)
    assert push3_df.iloc[0]['ID'] != push3_df.iloc[0]['Pushed Workbook ID'] == workbooks[0].id

    push4_df = spy.workbooks.push(workbooks, path='test_push_repeatedly', label='repeat_with_label', use_full_path=True,
                                  refresh=False)
...
def test_workbook_push_and_refresh():
    ...
    item_output = items_api.get_item_and_all_properties(id=workbook.id)  # type: ItemOutputV1
    item_properties = {p.name: p.value for p in item_output.properties}

    # Since we originally constructed this without pulling/loading, it should have a name-based Data ID that reflects
    # the FIRST time we pushed
    assert item_properties['Data ID'] == '[agent_api_key] {Workbook} My First From-Scratch Workbook'
    ...
    item_output = items_api.get_item_and_all_properties(id=worksheet.id)  # type: ItemOutputV1
    item_properties = {p.name: p.value for p in item_output.properties}

    data_id = '[agent_api_key] {Worksheet} My First From-Scratch Workbook'
    data_id = data_id + ' >> My First From-Scratch Worksheet'
    assert item_properties['Data ID'] == data_id
    ...
    data_id = '[agent_api_key] {CalculatedSignal} My First From-Scratch Workbook'
    data_id = data_id + ' >> My First Sinusoid'
    assert item_properties['Data ID'] == data_id
