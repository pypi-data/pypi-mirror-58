...
def test_push_repeatedly():
    ...
    push3_df = spy.workbooks.push(workbooks, path='test_push_repeatedly', use_full_path=True)
    assert push3_df.iloc[0]['ID'] == push3_df.iloc[0]['Pushed Workbook ID'] == workbooks[0].id

    push4_df = spy.workbooks.push(workbooks, path='test_push_repeatedly', use_full_path=True, refresh=False)
