...
class Item:
    ...
    def get_item_path(self):
        raise TypeError('Item._get_item_path() not overridden in derived class: %s' % self)

    def _construct_data_id(self, label):
        if self.provenance == Item.CONSTRUCTOR:
            # If the user has constructed objects in memory, then choose a Data ID that does not change each time the
            # object is constructed. (Constructed objects receive a generated 'ID' property that is different every
            # time.) This way a user can write a script and execute it over and over without having to spend a lot of
            # time managing IDs and worrying about unwanted duplication.
            return '[%s] {%s} %s' % (label, self.type, self.get_item_path())
        elif self.provenance == Item.PULL or self.provenance == Item.LOAD:
            # If the user has pulled or loaded the object, then use the ID as part of the Data ID. This is desirable
            # because (unlike the case above), the ID values will not be changing and therefore we'll avoid creating
            # duplicates.
            return '[%s] %s' % (label, self.id)

        raise RuntimeError('Item._construct_data_id() called with unknown provenance: %s' % self)

    def find_me(self, label):
        ...
        #
        # If that method doesn't work, then we try to look up the item using a canonical Data ID format. The Data ID
        # format has two flavors:
        #
        # - If the item was created by either pulling from a server or loading from a disk, the we deem the ID field
        #   to be a good identifier for idempotency.
        #
        # - If the item was created by just constructing it in memory, then its ID field will be generated. A
        #   popular use case is to create items in a Jupyter Notebook and execute the notebook over and over. Each time
        #   the user re-executes the cell that constructs the item, it will have a new ID. Yet the user does not want to
        #   keep pushing duplicates. So instead of using the ID field for idempotency, we construct a path from the
        #   names of the things involved: workbook, worksheet, item. Intuitively the user would expect that if something
        #   is named the same, it would keep being overwritten by their script.
        #
