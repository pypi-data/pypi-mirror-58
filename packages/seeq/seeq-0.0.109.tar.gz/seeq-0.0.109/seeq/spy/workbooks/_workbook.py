...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def push(self, *, owner=None, folder_id=None, item_map=None, label=None,
             access_control=None, override_max_interp=False, include_inventory=True, status=None):
        self.status = Status.validate(status)
        ...
        if not workbook_item:
            ...
            if folder_id == _common.PATH_ROOT:
                if workbook_output.parent_folder_id:
        ...
        if include_inventory:
            self._push_inventory(item_map, label, override_max_interp, workbook_output)
    ...
    def _push_inventory(self, item_map, label, override_max_interp, workbook_output):
