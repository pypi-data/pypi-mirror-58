...
class Item:
    ...
    @staticmethod
    def _data_id_from_item_id(item_id, label):
        return '[%s] %s' % (item_id, label)
    ...
    def adopt_new_ids(self, item_map):
        self.definition = json.loads(_replace_items(json.dumps(self.definition), item_map))
...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def adopt_new_ids(self, item_map):
        super().adopt_new_ids(item_map)

        for worksheet in self.worksheets:  # type: Worksheet
            worksheet.adopt_new_ids(item_map)

        new_inventory = dict()
        for item_id, item in self.item_inventory.items():
            item.adopt_new_ids(item_map)
            new_inventory[item_map[item_id]] = item
    ...
    def find_item(item_id, label):
        ...
        data_id = Item._data_id_from_item_id(item_id, label)
        _filters = [
...
class Worksheet(Item):
    ...
    def display_items(self):
        if self.workbook['Workbook Type'] == 'Topic':
            self.workbook.status.update('Displayed Items are only valid for Analyses, not Topics', Status.CANCELED)
            return
    ...
    def view(self):
        if self.workbook['Workbook Type'] == 'Topic':
            self.workbook.status.update('Worksheet View is only valid for Analyses, not Topics', Status.CANCELED)
            return
    ...
    def display_range(self):
        if self.workbook['Workbook Type'] == 'Topic':
            self.workbook.status.update('Display Range is only valid for Analyses, not Topics', Status.CANCELED)
            return
    ...
    def investigate_range(self):
        if self.workbook['Workbook Type'] == 'Topic':
            self.workbook.status.update('Investigate Range is only valid for Analyses, not Topics', Status.CANCELED)
            return
    ...
    def date_ranges(self):
        if self.workbook['Workbook Type'] != 'Topic':
            self.workbook.status.update('Date Ranges are only valid for Topics, not Analyses', Status.CANCELED)
            return
    ...
    def adopt_new_ids(self, item_map):
        ...
        for workstep in self.worksteps:  # type: Workstep
            workstep.adopt_new_ids(item_map)
...
class Annotation:
    ...
    def adopt_new_ids(self, item_map):
        self.html = _replace_items(self.html, item_map)
...
class CalculatedSignal(CalculatedItem):
    ...
    def push(self, datasource_maps, *, pushed_workbook_id=None, item_map=None, label=None, override_max_interp=False):
        ...
        data_id = Item._data_id_from_item_id(self.id, label)
        item = Workbook.find_item(self.id, label)
