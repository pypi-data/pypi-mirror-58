...
def test_reaching_up_and_down():
    ...
    class Grandchild(Asset):
        ...
        def My_Children(self, metadata):
            return sum([GreatGrandchild({
                'Name': 'GreatGrandchild 1'
            }, parent=self).build(metadata), GreatGrandchild({
                'Name': 'GreatGrandchild 2'
            }, parent=self).build(metadata)], list())
    ...
    class Child(Asset):
        ...
        @Asset.Component()
        def My_Children(self, metadata):
            return sum([Grandchild({
                'Name': 'Grandchild 1'
            }, parent=self).build(metadata), Grandchild({
                'Name': 'Grandchild 2'
            }, parent=self).build(metadata)], list())
    ...
    class Reaching_Up_and_Down(Asset):
        ...
        def My_Children(self, metadata):
            return sum([Child({
                'Name': 'Child 1'
            }, parent=self).build(metadata), Child({
                'Name': 'Child 2'
            }, parent=self).build(metadata)], list())
...
def test_workbook_build():
    class Tree_With_Displays(Asset):
        ...
        @Asset.Display()
        def My_Display(self, workstep):
            workstep.display_items = [{
                'Item': self.Wet_Bulb()
            }]

        @Asset.DateRange()
        def My_Date_Range(self):
            return {}

        @Asset.Document()
        def My_Document(self, document):
            document.render_template(filename=os.path.join(os.path.dirname(__file__), 'example_document.html'),
                                     asset=self)
    ...
    push_df = spy.push(metadata=build_df, workbook='test_workbook_build >> Built Workbook', worksheet='only_worksheet')
    ...
    workbook = [w for w in workbooks if isinstance(w, spy.workbooks.Analysis)][0]
    worksheet = workbook.get_worksheet('only_worksheet')
