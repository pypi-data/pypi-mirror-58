...
from ._worksheet import Worksheet, AnalysisWorksheet, TopicWorksheet
...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def item_errors_str(self):
        return '\n'.join(['%s: %s' % (_it, _str) for _it, _str in self.item_errors])
    ...
    def refresh_from(self, new_item, item_map):
        ...
        new_inventory = new_item.scope_inventory.copy()
        for inventory_item_id, inventory_item in self.scope_inventory.copy().items():
            ...
            del self.scope_inventory[inventory_item_id]
            self.scope_inventory[new_inventory_item_id] = inventory_item
        ...
        for new_inventory_item_id, new_inventory_item in new_inventory.items():
            if new_inventory_item_id not in self.scope_inventory:
                self.scope_inventory[new_inventory_item_id] = new_inventory_item
    ...
    def _find_by_name(self, folder_id):
        ...
        if self.name.lower() in content_dict and content_dict[self.name.lower()].type == 'Workbook':
    ...
    def push(self, *, owner=None, folder_id=None, item_map=None, label=None,
        ...
        remaining_inventory = dict(self.scope_inventory)
        while len(remaining_inventory) > 0:
            ...
            if not at_least_one_thing_pushed:
                for dependency_not_found in dependencies_not_found:
                    self.item_errors.add((dependency_not_found, 'Not found (dependency)'))
        ...
        for worksheet in self.worksheets:
            ...
            worksheet_output = worksheet.push(workbook_output.id, item_map, existing_worksheet_identifiers, label=label)
        ...
        if len(dependencies_not_found) > 0:
            raise _common.DependencyNotFound(None, '\n'.join(dependencies_not_found))
    ...
    def push_containing_folders(self, item_map, use_full_path, parent_folder_id, owner, label, access_control):
        ...
        for ancestor_id in self['Ancestors']:
            ...
            if create_folders_now:
                folder = self.scope_inventory[ancestor_id]  # type: Folder
    ...
    def save(self, workbook_folder=None, overwrite=False):
        ...
        with open(items_json_file, 'w', encoding='utf-8') as f:
            json.dump(self.scope_inventory, f, indent=4, sort_keys=True, cls=ItemJSONEncoder)
    ...
    def _load(self, workbook_folder):
        ...
        self.scope_inventory = Workbook._load_inventory(Workbook._get_items_json_file(workbook_folder))
        for scope_item in self.scope_inventory.values():
    ...
    def _scrape_datasource_inventory(self):
        ...
        for item in self.scope_inventory.values():  # type: Item
    ...
    def _scrape_scope_inventory(self):
    ...
    def _scrape_inventory_from_item(self, item_id):
        if item_id in self.scope_inventory:
            return
        ...
        for dependency in dependencies:
            if dependency.id in self.scope_inventory:
                continue
    ...
    def _scrape_inventory_from_ancillaries(self):
        ...
        current_inventory = list(self.scope_inventory.values())
    ...
    def add_to_scope(self, item):
        ...
        self.scope_inventory[item.id] = item

        if not isinstance(item, Folder):
            item.scoped_to = self

    def create_worksheet(self, definition):
        raise RuntimeError('Workbook.create_worksheet() not overridden in derived class')


class Analysis(Workbook):
    def create_worksheet(self, definition):
        return AnalysisWorksheet(self, definition)


class Topic(Workbook):
    def create_worksheet(self, definition):
        return TopicWorksheet(self, definition)
