...
def test_workbook_build():
    ...
    assert pd.Timestamp(static_date_range['Start']) == pd.Timestamp('2019-03-01', tz='America/Los_Angeles')
    assert pd.Timestamp(static_date_range['End']) == pd.Timestamp('2019-04-01', tz=_login.get_user_timezone())
...
def test_topic_with_images():
    class Area(Asset):
        ...
        @Asset.Attribute()
        def Wet_Bulb(self, metadata):
            return metadata[metadata['Name'].str.endswith('Wet Bulb')]

        @Asset.Image(image_format='png')
        def Scattermatrix(self, metadata):
            pull_df = self.pull([self.Temperature(), self.Humidity(), self.Wet_Bulb()], header='Name')
        ...
        def My_Document(self, metadata, topic):
            ...
            document.render_template(filename=os.path.join(os.path.dirname(__file__), 'test_topic_with_images.html'),
                                     asset=self)
    ...
    build_df = spy.assets.build(All_Areas, metadata_df, start='2019-01-01', end='2019-02-01')

    push_df = spy.push(metadata=build_df, workbook='test_topic_with_images >> Image Workbook',
