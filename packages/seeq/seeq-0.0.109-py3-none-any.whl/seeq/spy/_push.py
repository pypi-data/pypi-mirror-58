...
def push(data=None, *, metadata=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
    ...
    if workbook is not None:
        ...
        if _common.is_guid(workbook):
            ...
            search_query = create_workbook_search_query(workbook)
            search_df = spy.workbooks.search(search_query, quiet=True)
            if len(search_df) == 0:
                workbook_object = spy.workbooks.Analysis({'Name': search_query['Name']})
                workbook_object.create_worksheet({'Name': worksheet})
                spy.workbooks.push(workbook_object, path=search_query['Path'], include_inventory=False,
                                   quiet=True)
            else:
    ...
    if workbook_object:
        display_items = push_result_df[push_result_df['Type'].isin(['StoredSignal', 'CalculatedSignal',
                                                                    ...
                                                                    'CalculatedScalar', 'Chart', 'ThresholdMetric'])]
        ...
        spy.workbooks.push(workbook_object, include_inventory=False, quiet=True, refresh=False)
...
def create_workbook_search_query(workbook):
    ...
    if len(workbook_spec_parts) > 1:
        ...
        search_query['Name'] = '/^%s$/' % workbook_spec_parts[-1]
    else:
        search_query['Name'] = '/^%s$/' % workbook_spec_parts[0]
    return search_query
