...
def push(data=None, *, metadata=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
    ...
    if workbook is not None:
        ...
        if _common.is_guid(workbook):
            workbook_object = spy.workbooks.pull(pd.DataFrame([{
                'ID': _common.sanitize_guid(workbook),
                'Type': 'Workbook',
                'Workbook Type': 'Analysis'
            }]), include_inventory=False, quiet=True)[0]
        else:
            search_query, workbook_name = create_workbook_search_query(workbook)
            search_df = spy.workbooks.search(search_query, quiet=True)
            if len(search_df) == 0:
                workbook_object = spy.workbooks.Analysis({'Name': workbook_name})
                workbook_object.create_worksheet({'Name': worksheet})
                spy.workbooks.push(workbook_object, path=_common.get(search_query, 'Path'), include_inventory=False,
                                   quiet=True)
            else:
                workbook_object = spy.workbooks.pull(search_df, include_inventory=False, quiet=True)[0]

        worksheet_object = workbook_object.get_worksheet(worksheet)
        if worksheet_object is None:
            worksheet_object = workbook_object.create_worksheet({'Name': worksheet})

        workbook_id = workbook_object.id
        ...
        if item_type == 'Signal':
            for column in data:
                try:
                    ...
                    earliest_sample_in_ms, latest_sample_in_ms, signal_id = _push_signal(column, signal_metadata, data,
                                                                                         earliest_sample_in_ms,
                                                                                         latest_sample_in_ms,
                                                                                         signals_api, type_mismatches,
                                                                                         index,
                                                                                         status,
                                                                                         _update_push_data_status)
            ...
            try:
                ...
                earliest_sample_in_ms, latest_sample_in_ms, condition_id = _push_condition(condition_metadata,
                                                                                           conditions_api, data,
                                                                                           earliest_sample_in_ms,
                                                                                           latest_sample_in_ms,
    ...
    if metadata is not None:
        workbooks_to_push = _get_workbooks_to_push(metadata, workbook_object, worksheet_object)

    scope_string = 'and globally scoped.'
    if workbook_object:
        workbook_found = False
        for workbook_to_push in workbooks_to_push:  # type: spy.workbooks.Workbook
            if workbook_to_push.name == workbook_object.name:
                workbook_found = True
                workbook_object = workbook_to_push
                if workbook_object.get_worksheet(worksheet):
                    # We've already defined the worksheet as part of spy.assets.build().
                    worksheet_object = None
                else:
                    worksheet_object = workbook_object.create_worksheet(worksheet)
                break
        ...
        if worksheet_object:
            scope_string = 'and scoped to workbook ID <strong>%s</strong><br>Click the following link to see what ' \
                           'you pushed in Seeq:<br><a href="%s" target="_new">%s</a>' % (workbook_object.id,
                                                                                         worksheet_object['URL'],
                                                                                         worksheet_object['URL'])
    ...
    _push_workbooks(push_result_df, workbooks_to_push)

    status.update(
        'Pushed successfully to datasource <strong>%s [%s]</strong> %s' % (
...
def create_workbook_search_query(workbook):
    workbook_spec_parts = re.split(r'\s+>>\s+', workbook)
    search_query = dict()
    if len(workbook_spec_parts) > 1:
        search_query['Path'] = ' >> '.join(workbook_spec_parts[0:-1])
...
def _push_signal(column, signal_metadata, data, earliest_sample_in_ms, latest_sample_in_ms, signals_api,
...
def _push_condition(condition_metadata, conditions_api, data, earliest_sample_in_ms, latest_sample_in_ms, status_index,
                    status,
                    update_status):
...
def _get_workbooks_to_push(metadata, default_workbook, default_worksheet):
    workbook_rows = metadata[metadata['Type'] == 'Workbook']
    workbooks = list()
    for _, workbook_row in workbook_rows.iterrows():
        workbook = workbook_row['Object']  # type: spy.workbooks.Analysis
        if workbook.name is None:
            workbook.name = default_workbook.name if default_workbook else _common.DEFAULT_WORKBOOK_NAME
        for worksheet in workbook.worksheets:
            if worksheet.name is None:
                worksheet.name = default_worksheet.name if default_worksheet else _common.DEFAULT_WORKSHEET_NAME

        workbooks.append(workbook)  # type: spy.workbooks.Analysis

    return workbooks


def _push_workbooks(push_result_df, workbooks):
            ...
            for workstep in worksheet.worksteps.values():  # type: spy.workbooks.AnalysisWorkstep
                ...
                for index, display_item in display_items.iterrows():
                    if not _common.present(display_item, 'ID') or _common.get(display_item, 'Reference', False):
                        ...
                        display_items.at[index, 'ID'] = pushed_item.iloc[0]['ID']

                workstep.display_items = display_items
                workstep.set_as_current()

    spy.workbooks.push(workbooks, refresh=False, include_inventory=False)
