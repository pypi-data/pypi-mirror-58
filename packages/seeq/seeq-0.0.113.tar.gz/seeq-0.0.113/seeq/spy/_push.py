...
def push(data=None, *, metadata=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
    ...
    if workbook is not None:
        ...
        if _common.is_guid(workbook):
            ...
            workbook_path = workbook_object.path
        else:
            ...
            if len(search_df) == 0:
                ...
                spy.workbooks.push(workbook_object, path=workbook_path, include_inventory=False,
                                   quiet=True)
            else:
    ...
    if workbook_object:
        ...
        for workbook_to_push in workbooks_to_push:
            if isinstance(workbook_to_push, spy.workbooks.Analysis) and workbook_to_push.name == workbook_object.name:
                workbook_found = True
    ...
    _push_workbooks(push_result_df, workbook_path, workbooks_to_push)
...
def _get_workbooks_to_push(metadata, default_workbook, default_worksheet):
    ...
    for _, workbook_row in workbook_rows.iterrows():
        workbook = workbook_row['Object']  # type: spy.workbooks.Workbook
...
def _push_workbooks(push_result_df, workbook_path, workbooks):
    ...
    spy.workbooks.push(workbooks, path=workbook_path, refresh=False, include_inventory=False)
