...
from seeq.spy.workbooks import Analysis, AnalysisWorksheet, Topic, TopicDocument
...
class _AssetBase:
            ...
            if not at_least_one_built:
                if len(workbooks_not_built) > 0:
                    for not_built in workbooks_not_built:
                        if (not_built.workbook_type, not_built.workbook) not in self.workbooks:
                            if not_built.workbook_type == 'analysis':
                                workbook = Analysis({'Name': not_built.workbook})
                            else:
                                workbook = Topic({'Name': not_built.workbook})
                            self.workbooks[(not_built.workbook_type, not_built.workbook)] = workbook
                        else:
                            workbook = self.workbooks[(not_built.workbook_type, not_built.workbook)]

                        if (not_built.workbook_type, not_built.workbook, not_built.worksheet) not in self.workbooks:
                            if not_built.workbook_type == 'analysis':
                                worksheet = workbook.get_worksheet(not_built.worksheet)
                                if worksheet is None:
                                    worksheet = workbook.create_worksheet({'Name': not_built.worksheet})
                            else:
                                worksheet = workbook.get_document(not_built.worksheet)
                                if worksheet is None:
                                    worksheet = workbook.create_document({'Name': not_built.worksheet})
                            self.workbooks[
                                (not_built.workbook_type, not_built.workbook, not_built.worksheet)] = worksheet
    ...
    def embed(self, display, date_range):
        pass
...
class Asset(_AssetBase):
    ...
    # noinspection PyPep8Naming
    @classmethod
    def Display(cls, analysis=None, worksheet=None):
        """
        < NEED INFO >
        """
        ...
        def display_decorator(func):
            ...
            def display_wrapper(self, metadata):
                ...
                if ('analysis', analysis, worksheet) not in self.workbooks:
                    raise WorkbookNotBuilt('analysis', analysis, worksheet)

                worksheet_object = self.workbooks[('analysis', analysis, worksheet)]  # type: AnalysisWorksheet
                ...
                self.workbooks[(analysis, worksheet, func.__name__)] = workstep_object
    ...
    def DateRange(cls):
        ...
        def date_range_decorator(func):
            # noinspection PyUnusedLocal
            def date_range_wrapper(self, metadata):
                return
    ...
    # noinspection PyPep8Naming
    @classmethod
    def Document(cls, topic=None, name=None):
        """
        < NEED INFO >
        """
        ...
        def document_decorator(func):
            # noinspection PyUnusedLocal
            def document_wrapper(self, metadata):
                # type: (_AssetBase, pd.DataFrame) -> None

                if func.__name__ in self._build_cache:
                    # We've already built this
                    return

                if ('analysis', topic, name) not in self.workbooks:
                    raise WorkbookNotBuilt('topic', topic, name)

                document_object = self.workbooks[('topic', topic, name)]  # type: TopicDocument

                func(self, document_object)

                self._build_cache[func.__name__] = document_object
