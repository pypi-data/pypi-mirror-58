...
class StoredOrCalculatedItem(Item):
    ...
    def get_item_path(self):
        if self.scoped_to is None:
            raise RuntimeError('StoredOrCalculatedItem._get_item_path called when self.scoped_to is None: %s' % self)

        scoped_item_path = self.scoped_to.get_item_path()

        return '%s >> %s' % (scoped_item_path, self.name)
