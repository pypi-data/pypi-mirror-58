...
class Annotation:
    ...
    def push(self, pushed_workbook_id, pushed_worksheet_id, item_map, push_images):
        ...
        doc = _item.replace_items(self.html, item_map)
    ...
    def add_image(self, image_path, placement='end'):
        ...
        image_html = f'<img src="/api/annotations/{self.id}/images/{image_name}">'
    ...
    def add_image_to_render(self, image_render_info):
        image_id = _common.new_placeholder_guid()
        filename = f'{image_id}.{image_render_info.image_format}'
        self.images[(self.id, filename)] = image_render_info
        return f'<img class="fr-fic fr-fin fr-dii" src="/api/annotations/{self.id}/images/{filename}"/>'

    def render_images(self):
        for k, v in self.images.copy().items():
            if isinstance(v, spy.assets.ImageRenderInfo):
                self.images[k] = v.render_function()
