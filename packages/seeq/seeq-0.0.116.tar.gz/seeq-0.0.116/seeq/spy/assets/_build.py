...
def build(model, metadata, *, start=None, end=None):
    """
    ...
    return pd.DataFrame(_build(model, metadata, start, end))


def _build(model, metadata, start, end):
    results = list()
    ...
    context = BuildContext(start=start, end=end)
