...
class BuildContext:
    def __init__(self, start=None, end=None):
        ...
        self.cache = dict()
        self.push_df = None
        self.start, self.end = _pull.validate_start_and_end(start, end)
...
class _AssetBase:
    ...
    def pull(self, items, *, start_offset=None, end_offset=None, grid='15min', header='__auto__', group_by=None,
             shape='auto', capsule_properties=None, tz_convert=None, calculation=None, bounding_values=False,
             errors='raise'):
        # type: ((list, pd.DataFrame), pd.Timedelta, pd.Timedelta) -> pd.DataFrame
        ...
        start = self.context.start + start_offset if start_offset else self.context.start
        end = self.context.end + end_offset if end_offset else self.context.end
        ...
        return _pull.pull(pd.DataFrame(items), start=start, end=end, grid=grid, header=header, group_by=group_by,
                          ...
                          calculation=calculation, bounding_values=bounding_values, errors=errors, quiet=True)
...
class Asset(_AssetBase):
    ...
    def DateRange(cls):
        def date_range_decorator(func):
            ...
            def date_range_wrapper(self, metadata=None):
                ...
                self.context.cache[(self, func.__name__)] = date_range_spec
    ...
    def Image(cls, image_format):
        def image_decorator(func):
            # noinspection PyUnusedLocal
            def image_wrapper(self, metadata=None):
                if (self, func.__name__) in self.context.cache:
                    # We've already built this
                    return self.context.cache[(self, func.__name__)]
                ...
                image_render_info = ImageRenderInfo(image_format, _image_function)
                self.context.cache[(self, func.__name__)] = image_render_info

                return image_render_info

            # Setting this attribute on the function itself makes it discoverable during build()
            setattr(image_wrapper, 'spy_model', 'image')
