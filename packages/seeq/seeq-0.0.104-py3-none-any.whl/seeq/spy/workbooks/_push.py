...
def push(workbooks, *, path=None, owner=None, label=None, datasource_map_folder=None,
         use_full_path=False, access_control=None, override_max_interp=False, refresh=True, errors='raise',
    ...
    while len(remaining_workbooks) > 0:
        ...
        for index, workbook in remaining_workbooks.copy():  # type: (int, Workbook)
            ...
            try:
                if label is None and _common.get(workbook.definition, 'Is Template', default=False):
                    # If a label is not supplied but the workbook is marked as a template, we want to isolate users
                    # so they're not stepping on each other when they do spy.workbook.push(). All of the Example
                    # Exports are marked with "Is Template" equal to True.
    ...
    if refresh:
        ...
        new_workbooks = _pull.pull(pull_df)
