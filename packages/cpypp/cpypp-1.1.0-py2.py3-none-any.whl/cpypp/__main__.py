...
class cpypp_command_line(object):
    ...
    def process_all(self):
        ...
        # if file_list is provided then load it

        try:
            if self._file_list:
                with (sys.stdin if self._file_list == '-' else open(self._file_list)) as f:
                    for line in f:
                        self._files.append(line.strip())
    ...
    def parse_args(self):
        ...
        parser = argparse.ArgumentParser(
                      prog=_PROG, 
                      description="c-style python preprocessor by Wellington Rats <wellrats@gmail.com>",
                      formatter_class=formatter_class
                 )

        parser.add_argument("-v", "--version", dest="version", action="store_true", 
                            help=("print cpypp version"))
        ...
        parser.add_argument('-i', metavar='FILE', dest='file_list',
                            help=('add all the files and directories listed in '
                                  'FILE to the list considered for compilation; '
                                  "Use '-i -' to read from stdin"))

        parser.add_argument("-o", dest="output_file", metavar="FILE",
                            help=("write output to FILE when you are preprocessing just one file at once. "
