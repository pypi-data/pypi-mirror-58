...
from ..utils import ALL_STRING_CLASS, CustomJSONEncoder, to_unicode_safe
...
class JSCB(RuleCallback):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for callback_name in LIFECYCLE_METHODS.values():
            if callback_name in rule_callbacks:
                method = functools.update_wrapper(functools.partial(
                    self.execute, callback_name), self.execute)
                setattr(self, callback_name, method)
    ...
    def execute(self, name, original, *args, **kwargs):
        """ Fetches a context before executing _execute
        """
        if name == LIFECYCLE_METHODS["PRE"]:
            return_value = None
        else:
            return_value = args[0]
            args = args[1:]
        ...
        budget_in_ms = self.get_remaining_budget(kwargs)
