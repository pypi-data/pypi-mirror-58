...
from ..utils import Iterable, Mapping, update_wrapper
...
def execute_callback(
    ...
    storage=runtime, override_budget=None
):
    ...
    if override_budget is not None and callback.collaborative:
        kwargs["__sqreen_override_budget"] = override_budget
    ...
    try:
        ...
        if override_budget is not None and callback.collaborative:
            kwargs.pop("__sqreen_override_budget", None)
    ...
    for callback in callbacks:
        ...
        with storage.trace() as callback_trace:
            result = execute_callback(
                queue, callback, method, original, result_action, args, kwargs,
                storage=storage, override_budget=budget
            )
...
def execute_pre_callbacks(
    key, strategy, original, args=None, kwargs=None, storage=runtime,
    override_budget=None
):
