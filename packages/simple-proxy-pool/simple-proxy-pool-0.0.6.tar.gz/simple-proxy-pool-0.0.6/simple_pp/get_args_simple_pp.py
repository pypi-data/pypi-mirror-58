...
def get_args_simple_pp(lst=''):
    ...
    parser.add_argument(
        ...
        metavar='proxy',
        type=str,
        help='proxies (0 or more), if no input is given, the clipboard content will be used.',  # noqa
    )
    parser.add_argument(
        '-p',
        '--proxies',
    ...
    parser.add_argument(
        ...
        dest='tot_anonymous',
        action='store_true',
        help='Attempt to scrape this many anonymous proxies.',
    )
    parser.add_argument(
        '-d',
        '--debug',
...
def test_proxies_input():
    ...
    assert _ == Namespace(debug=True, proxies=['a', 'b', 'c', 'x', 'y', 'z'])  # noqa
