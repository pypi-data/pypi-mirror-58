...
#     group.add_argument( "--maxdeg", action = "store", type = "int", metavar = "NRNT", default = 2, help = "maximum number of missing nt for incomplete stop" )
    ...
    for i in range(len(sequences)):
        ...
        fastawriter( featurelist, sequences[0]["sequence"], None, acc, "fas", outfile = "%s/result.fas" % odir)
        fastawriter( featurelist, sequences[0]["sequence"], args.code, acc, "faa", outfile = "%s/result.faa" % odir)
