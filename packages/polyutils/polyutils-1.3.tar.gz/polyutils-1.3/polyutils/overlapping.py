...
def two_triangles_overlap(pol1, pol2):
    ...
    points = []
    for p in pairs:
        r = triangles_intersection(p[0], p[1], get_points=False)
        points.append(r)
        print('{}-{}:  {}'.format(p[0], p[1], r))
...
def triangular_overlap_list(polygons):
    """
    Return those polygons whose are overlapping.
    :param polygons: list of polygons
    :return: True and the pairs of overlapping polygons, or False.
    ...
    for pol in pairs:
        if two_triangles_overlap(pol[0], pol[1]) is True:
            bad_polygons.append(pol[0])
            bad_polygons.append(pol[1])
...
def main():
    ...
    b.change_vertex([1, 1, 2])

    polygons = [a, b]

    x, bad = triangular_overlap_list(polygons)
    print(x)
