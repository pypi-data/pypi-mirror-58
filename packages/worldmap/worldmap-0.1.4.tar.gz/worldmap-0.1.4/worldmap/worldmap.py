""" This function colors maps, either countries in the worldmap or counties for a specific country
 ...
 EXAMPLE
   ...
   names = worldmap.getmapnames(loadmap='world')
   names = worldmap.getmapnames(loadmap='netherlands')
   names = worldmap.getmapnames(loadmap='belgium')
   names = worldmap.getmapnames(loadmap='new zealand')

   #### Retrieve citynames for abbrevations
   A=worldmap.code2city('NL')
   A=worldmap.code2city('GB')

   #### Retrieve abbrevations for city names
   A=worldmap.city2code('Netherlands')
   A=worldmap.city2code('Germany')


   #### Coloring of maps   
   names   = ['Norway','Nederland','Dominican Republic','Salvador']
   opacity = [10, 25, 5, 100]
   A       = worldmap.colormap(names, opacity=opacity, loadmap='world')

   names = ['Nederland', 'belgium']
   cmap  = ['#0f0f0f','#0f0f0f']
   A     = worldmap.colormap(names, loadmap='world', cmap=cmap)

   names   = ['zeeland','Overijssel', 'flevoland']
   opacity = [0.1, 0.2, 0.6]
   cmap    = ['#ff0000'] # Different red accents
   A = worldmap.colormap(names,opacity=opacity, cmap='Set1', loadmap='netherlands', filename='c://temp/nederlandMap.svg')
