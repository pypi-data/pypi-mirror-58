...
def get_model(name: str):
    ma = re.sub('[-_ ]', '', name).lower()
    if ma in LOADED_MODEL:
        return LOADED_MODEL[ma]
    elif ma == 'asuka':
        model = torch.hub.load('GalAster/StyleGAN-Zoo', 'style_asuka', pretrained=True)
        LOADED_MODEL[ma] = model
        return model
    elif ma == 'horo':
...
def generate(
    ...
    if save is None:
        ...
        o = [StyleGAN.new(method, i, j) for i, j in zip(gene, data)]
        return o[:num]
    else:
        pass
...
if __name__ == "__main__":
    ...
    s.show()
    # s.save('.')
    m = generate('asuka', 5, batch_size=2)
