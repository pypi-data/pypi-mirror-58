...
class BasicGUI:
    ...
    def load_canvas(self):
        ...
        # create input
        self.entry = Text(self.frame, height=45, width=600, font=("Arial", 25))
        self.entry.pack()

        return width, height

    @staticmethod
    def filter_and_map(files, d):
        allowed_ext = ['.jpg', '.jpeg', '.png']

        files = list(filter(lambda f: os.path.splitext(f)[1] in allowed_ext, files))
        files.sort(key=BasicGUI.sort_custom)

        files = list(map(lambda f: os.path.join(d, f), files))
        return files

    @staticmethod
    def sort_custom(item):
        items = []

        item = os.path.splitext(os.path.basename(item))[0]
        items_list = item.split('_')

        items.append(len(items_list))
    ...
    def get_sorted_files(input_dir):
        ...
        all_files = []
        virgin = False
        for r, dirs, fl in os.walk(input_dir):
            fl.sort(key=BasicGUI.sort_custom)

            if not virgin:
                all_files = BasicGUI.filter_and_map(fl, r)
                virgin = True

            dirs.sort(key=BasicGUI.sort_custom)
            for d in dirs:
                d = os.path.join(r, d)
                files = BasicGUI.filter_and_map(os.listdir(d), d)
    ...
    def load_files(self):
        ...
        self.files = BasicGUI.get_sorted_files(self.input_dir)
    ...
    def key_up(self, event):
        print(event.keycode)
        if event.keycode == 116 or event.keycode == 111:
            if event.keycode == 111:  # arrow down
                self.prev_image()

            if event.keycode == 116:  # arrow up
                self.next_image()

        if event.keycode == 67:  # F1
            labels = list(map(lambda r: r.file_path, self.records))

            empty_labels = list(filter(lambda r: r.label != '', self.records))
            empty_labels = list(map(lambda r: r.file_path, empty_labels))

            not_labels = list(filter(lambda p: not(p not in labels and p in empty_labels), self.files))

            if len(not_labels) != 0:
                first_path = not_labels[0]
                self.index = self.files.index(first_path)
