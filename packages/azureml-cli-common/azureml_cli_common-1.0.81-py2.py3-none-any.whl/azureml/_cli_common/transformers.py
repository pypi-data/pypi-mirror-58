...
def transform_service_show(result_tuple):
    ...
    if result['computeType'] == ACI_WEBSERVICE_TYPE or result['computeType'] == AKS_WEBSERVICE_TYPE:
        ...
        if 'imageDetails' in result and result['imageDetails'] and result['imageDetails']['modelDetails']:
            model_names_versions = ['{} [Ver. {}]'.format(model['name'], model['version']) for model in
                                    result['imageDetails']['modelDetails']]
        elif 'modelDetails' in result and result['modelDetails']:
            model_names_versions = ['{} [Ver. {}]'.format(model['name'], model['version']) for model in
                                    result['modelDetails']]
        if model_names_versions:
            model_names_versions = ', '.join(model_names_versions)

        runtime_type = None
        if result.get('imageDetails') and result['imageDetails'].get('targetRuntime'):
            runtime_type = result['imageDetails']['targetRuntime'].get('runtimeType')
