...
def get_cache(path, maxsize=100):
    import numpy as np
    import pickle
    if not path.exists():
        cache = {}
...
class CoverByRect:
    ...
    path = Path(__file__).parent / 'cover_by_rect.pkl'
    cache = get_cache(path)

    @staticmethod
    def save():
        import pickle
        with open(CoverByRect.path, 'wb') as fp:
            pickle.dump(CoverByRect.cache, fp)
    ...
    def get(x, y):
        ...
        if (x != y and x != y - 1) or x < 256 or y < 256:
            return CoverByRect._calc(x, y)[1]
        x1 = x // 2
        x2 = x - x1
        y1 = y // 2
        y2 = y - y1
