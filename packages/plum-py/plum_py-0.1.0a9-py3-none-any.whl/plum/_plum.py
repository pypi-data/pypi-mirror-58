    ...
    try:
        if isinstance(fmt, PlumType):
            ...
            for i, (subfmt, value) in enumerate(zip(fmt, args)):
                if dump:
                    dump = dump.add_row(cls=subfmt, access=f'[{i}]', indent=indent)
...
def _unpack(fmt, buffer, offset, dump, prohibit_excess, add_row=True):
    ...
    try:
        if isinstance(fmt, PlumType):
            ...
            for i, cls in enumerate(fmt):
                ...
                item, offset = _unpack(cls, buffer, offset, dump, False, False)
                items.append(item)
            items = tuple(items)
