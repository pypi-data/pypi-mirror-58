...
def _add_str_rows_to_dump(dump, zipped_char_bytes, buffer=None, string=None):
    ...
    for index, chars, bytes_ in _iter_str_rows(zipped_char_bytes):
        dump.add_level(access=f'[{index}:{index + len(chars)}]', value=repr(chars), memory=bytes_)
        nbytes += len(bytes_)
        nchar += len(chars)
...
class Str(str, Plum, metaclass=StrType):
    ...
    def __unpack__(cls, buffer, offset, limit, dump, parent):
        ...
        chunk, offset, limit = getbytes(buffer, offset, nbytes, limit, dump, cls)
        ...
        if cls._zero_termination:
            ...
            if dump:
                ...
                if termination:
                    dump.add_level(access='--termination--', memory=bytes(termination))
                if leftover_bytes:
    ...
    def __pack__(cls, buffer, offset, value, dump):
        ...
        if dump:
            ...
            if cls._zero_termination:
                dump.add_level('--termination--', memory=b'\x00')
