...
def __pack_defined__(cls, buffer, offset, item, dump):
    ...
    if isinstance(item, PlumView):
        ...
        if (has_touchups and not isinstance(item, cls)) or (len_item != len(types)):
    ...
    def __new__(mcs, name, bases, namespace):
        ...
        if member_info.members:
            ...
            if all([isinstance(member.__nbytes__, int) for member in types]):
                ...
                namespace.update({"__offsets__": member_offsets})
