...
class Array(list, Plum, metaclass=ArrayType):
    ...
    @classmethod
    def __unpack__(cls, buffer, offset, limit, dump, parent, dims=None, outer_level=True):
        ...
        if dims == GREEDY_DIMS:
            ...
            raise TypeError('greedy multidimensional array types do not support unpack operation')
        else:
            itemdims = dims[1:]
            if dump:
                if itemdims:
                    for i in range(dims[0]):
                        ...
                        item, offset, limit = cls.__unpack__(
                            buffer, offset, limit, subdump, self, itemdims, False)
                        self.append(item)
                else:
                    item_cls = cls.__item_cls__
                    for i in range(dims[0]):
                        ...
                        item, offset, limit = item_cls.__unpack__(
                            buffer, offset, limit, subdump, self)
                        self.append(item)
            else:
                if itemdims:
                    for i in range(dims[0]):
                        item, offset, limit = cls.__unpack__(
                            buffer, offset, limit, None, self, itemdims, False)
                        self.append(item)
                else:
                    item_cls = cls.__item_cls__
                    for i in range(dims[0]):
                        item, offset, limit = item_cls.__unpack__(
                            buffer, offset, limit, None, self)
                        self.append(item)
