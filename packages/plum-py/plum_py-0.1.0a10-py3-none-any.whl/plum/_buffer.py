...
class Buffer:
    ...
    def unpack(self, fmt):
        ...
        try:
            ...
            items, self.offset = _unpack(fmt, self.buffer, self.offset, None, False)
        except Exception:
            # do it over to include dump in exception message
            _unpack(fmt, self.buffer, self.offset, Dump(), False)
    ...
    def unpack_and_dump(self, fmt):
        ...
        """
        dump = Dump()
        items, self.offset = _unpack(fmt, self.buffer, self.offset, dump, False)
        return items, dump
