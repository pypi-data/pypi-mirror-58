...
class Ppos(Module):
    ...
    def createStaking(self, typ, benifit_address, node_id, external_id, node_name, website, details, amount,
        ...
        data = HexBytes(rlp.encode([rlp.encode(int(1000)), rlp.encode(typ), rlp.encode(bytes.fromhex(benifit_address)),
                                    ...
                                    rlp.encode(website), rlp.encode(details), rlp.encode(reward_per),
                                    rlp.encode(amount), rlp.encode(program_version),
    ...
    def getDelegateReward(self, from_address, node_ids=None):
        data = [rlp.encode(int(5100))]
        if node_ids is not None:
            data = data.append(rlp.encode(node_ids))
        raw_data = call_obj(self, from_address, self.web3.delegateRewardAddress, data)
        receive = json.loads(str(raw_data, encoding="ISO-8859-1"))
        try:
            raw_data_dict = receive["Ret"]
            decimal_10 = []
            if raw_data_dict["reward"]:
                for i in raw_data_dict["reward"]:
                    decimal_10.append(int(i, 16))
                raw_data_dict["reward"] = decimal_10
            receive["Ret"] = raw_data_dict
        except:...
        return receive
