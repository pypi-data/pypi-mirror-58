...
class GanClass:
    ...
    def getSSName(self, tgid):
        return ShishenName[BZBaseTools.ShishenList[tgid][self.ganid]]

    def getGanName(self):
        return Gan[self.ganid]

    def getWuxingName(self):
        return Wuxing[self.ganid]

    def getWuxingshengwang(self, zhiid):
        id = BZBaseTools.WuxingShengwang[zhiid][BZBaseTools.ganwuxing[
            self.ganid]]
        return WuxingShengwangName[id]
...
class ZhuClass:
    ...
    def getShierchangshengName(self, tg, dz):
        ...
        return ShierchangshengName[id]

    # 得到本柱的纳音
    def getNayin(self):
        return Nayin[self.tiangan.getGanName() +
...
class Eight2(object):
    ...
    def compile(self, name=None, gender=1, birthday=None, solar=True):
        """
        八字起盘
        ...
        if solar is False:
            lunar = sxtwl.Lunar()
            birth = lunar.getDayBySolar(birthday.year, birthday.month, birthday.day)
            hours = lunar.getShiGz(birth.Lday2.tg, 0)
