    ...
    for t in range(n, T, s):
        ...
        if weight_vect.sum() == 0.:
            if previousValidWeights is None:
                weight_mat.iloc[t: t_s] = np.ones([t_s - t+1, N]) / N
            else :
                weight_mat.iloc[t: t_s] = previousValidWeights
        elif weight_vect.sum() != 1.:
            weight_mat.iloc[t: t_s] = np.ones([t_s - t+1, 1]) @ weight_vect.T / weight_vect.sum()
            previousValidWeights =  weight_mat.iloc[t: t_s]
        else:
            weight_mat.iloc[t: t_s] = np.ones([t_s - t+1, 1]) @ weight_vect.T
            previousValidWeights =  weight_mat.iloc[t: t_s]
