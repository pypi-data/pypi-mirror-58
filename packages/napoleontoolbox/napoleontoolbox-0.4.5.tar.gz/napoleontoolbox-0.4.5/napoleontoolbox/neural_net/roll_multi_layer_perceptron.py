    ...
    def unroll_features(self,dates, feature_names):
        ...
        for eval_slice, test_slice in self:
            ...
            if np.random.rand()>=0.8:
                print(txt)
        import pickle
        with open('/home/ubuntu/data/rows_list.pickle', 'wb') as handle:
            pickle.dump(rows_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open('/home/ubuntu/data/dates_list.pickle', 'wb') as handle:
            pickle.dump(dates_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
