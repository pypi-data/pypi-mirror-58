...
class FeaturesAssembler(AbstractAssembler):
    ...
    def preprocessFeature(self):
        ...
        mini = 1
        # features['NYSE New High'].where(features['NYSE New High'] >= mini, mini)
        # features['NYSE New Low'].where(features['NYSE New Low'] >= mini, mini)
        # features['US2/10']=features['US10YR']-features['US2YR']
        # features['EU2/10']=features['EUR10YR']-features['EUR2YR']
        # features['IGHYSPREAD']=features['HY spread']-features['IG spread']
        features['Date'] = pd.to_datetime(features['Date'])
        col = list(features.columns.values)
        del col[0]
        # MM 3 mois
        for i in col:
            features["MM63_{}".format(i)] = features[i] / features[i].shift(1).rolling(window=63).mean() - 1
        # Z score 21 jours
        for i in col:
            features["Z_{}".format(i)] = (features[i] - features[i].rolling(window=21).mean()) / features[i].rolling(
                21).std()
        # Z score 63 jours
        for i in col:
            features["Z63_{}".format(i)] = (features[i].rolling(window=5).mean() - features[i].rolling(
                window=63).mean()) \
                                           / features[i].rolling(63).std()
        # MM 126 jours
        for i in col:
            features["MM126_{}".format(i)] = features[i] / features[i].shift(1).rolling(window=126).mean() - 1
        # MM 252 jours
        for i in col:
            features["MM252_{}".format(i)] = features[i] / features[i].shift(1).rolling(window=252).mean() - 1
        # MM 20 jours
        for i in col:
            features["MM20_{}".format(i)] = features[i] / features[i].shift(1).rolling(window=20).mean() - 1
        # ecart type:
        for i in col:
            features["vol20_{}".format(i)] = features[i].rolling(window=20).std() / features[i].rolling(
                window=20).mean()
