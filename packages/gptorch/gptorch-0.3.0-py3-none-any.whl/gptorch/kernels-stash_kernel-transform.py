...
from .util import as_variable, squared_distance, torch_dtype
...
class Kernel(Model, abc.ABC):
    ...
    @abc.abstractmethod
    def Kdiag(self, X: torch.Tensor) -> torch.Tensor:
        """
        Compute the diagonal of the kernel matrix.

        :param X: [n x dim]
        :return: 1D Tensor, shape=(n,)
        """
        raise NotImplementedError()

    @abc.abstractmethod
    def K(self, X: torch.Tensor, X2: torch.Tensor=None) -> torch.Tensor:
        """
        Compute [n1 x n2] kernel matrix from X [n1 x dim] and X2 [n2 x dim].
        If X2 si not provided, compute K(X, X) [n1 x n1]
        """
        raise NotImplementedError()
...
class White(Static):
    ...
    def K(self, X, X2=None, presliced=False):
        if X2 is None:
            ...
            return torch.zeros(*_k_shape(X, X2))
...
class Matern32(Stationary):
    def K(self, X, X2=None):
        r3 = torch.Tensor([np.sqrt(3.0)]) * self.dist(X, X2)
...
def transform_inputs(func):
    """
    Warpper for the Transform kernel's ops
    """

    def wrapped(obj, *args, **kwargs):
        t_args = [obj.transform(a) for a in args]
        t_kwargs = {}
        t_kwargs.update(kwargs)
        for x_key in ["X", "X2"]:
            if x_key in t_kwargs:
                t_kwargs[x_key] = obj.transform(t_kwargs[x_key])
        return func(obj, *t_args, **t_kwargs)
...
class Transform(Kernel):
    """
    Kernel where a torch.nn.Module instance transforms the inputs to a "normal"
    kernel, i.e.

    X, X2 -> transform (datum-wise) -> ordinary kernel operation

    When the transform is a deep neural network, this is "deep kernel learning"
    (Wilson et al., JMLR 2016)
    """
    def __init__(self, dim: int, transform: torch.nn.Module, base_kernel: Kernel):
        """
        :param dim: The transform module's input dimension
        :param transform: The module that transforms the inputs
        :param base_kernel: The "normal" kernel that the outputs of the 
            transform enter into.
        """
        super().__init__(dim)
