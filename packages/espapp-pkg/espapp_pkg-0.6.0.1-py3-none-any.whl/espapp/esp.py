...
def send_data():
    ...
    try:
        ...
        name = input("Enter email that is registered with IoT Dashboard: ")
        # while len(name) != 12:
        #     print("Must be of length 12")
        #     name = input("Enter Device ID  : ")

        userpass = getpass("Enter IoT Dashboard password (Hidden Field): ")
        while checker(userpass, length=64):
            userpass = getpass("Enter IoT Dashboard password (Hidden Field): ")

        user_repass = getpass("Re-Enter IoT Dashboard password (Hidden Field): ")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                # send = send_per
                send = "http://18.202.17.224:5003/api/network/"
