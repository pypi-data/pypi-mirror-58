...
class MVCDesal(SinkComponent):
    ...
    def rec_pow(self, pow_rec, n):
        ...
        self.ld_module.pow = np.maximum(pow_req, self.size*(1-self.td))

        # update water level
        pow_tot = np.minimum(pow_req+pow_rec, self.size)  # additional water
        pow_use = np.minimum(
            (1-self.st_module.soc)*self.enr_int*self.st_module.size/self.dt,
            pow_tot
        )
        pow_use[self.st_module.size == 0] = wat_req*self.enr_int
        self.st_module.soc += pow_tot*self.dt/(
            self.enr_int*self.st_module.size
        )
