...
class HyperDriveConfig(object):
    ...
    def __init__(self,
        ...
        if resume_from is not None:
            ...
            if len(resume_from) > MAX_PARENT_RUNS:
                raise UserErrorException(("The number of parent runs should not be greater than {}. "
                                          "The number of parent runs was {}.").format(
                                              MAX_PARENT_RUNS, len(resume_from)))
    ...
    def _validate_resume_child_runs(self, resume_child_runs, is_bayesian):
        ...
        if len(resume_child_runs) > self._max_total_runs:
            raise UserErrorException(("The number of resume_child_runs cannot exceed max_total_runs of "
                                      "current run. max_total_runs is {}, and {} child runs were "
                                      "found.").format(self._max_total_runs, len(resume_child_runs)))
        ...
        for run in resume_child_runs:
            if run.id in child_set:
                raise UserErrorException(("resume_child_runs cannot contain duplicate runs. More than "
                                          "one run with id '{}' was found").format(run.id))
    ...
    def _validate_warm_start(self, resume_list, is_bayesian, is_resume_from=False):
        ...
        for hdrun in resume_list:
            ...
            if hdrun._runtype != "hyperdrive":
                raise TypeError(("All parent runs should be HyperDriveRun objects. An object of runtype "
                                 "'{}' was found.").format(hdrun._runtype))
            ...
            if hdrun.hyperdrive_config._platform_config["ServiceAddress"] \
                ...
                raise UserErrorException(("The host should be the same for all runs. A run was found "
                                          "with ServiceAddress '{}', while another was found with ServiceAddress "
                                          "'{}'.").format(
                                              resume_list[0].hyperdrive_config._platform_config["ServiceAddress"],
                                              hdrun.hyperdrive_config._platform_config["ServiceAddress"]))
            if is_bayesian:
        ...
        if is_resume_from and _total_child_runs > RESUME_FROM_TOTAL_RUNS:
            raise UserErrorException(("The number of total runs should not be greater than {}. "
                                      "The number of total runs was {}.").format(
                                          RESUME_FROM_TOTAL_RUNS,
                                          _total_child_runs))
