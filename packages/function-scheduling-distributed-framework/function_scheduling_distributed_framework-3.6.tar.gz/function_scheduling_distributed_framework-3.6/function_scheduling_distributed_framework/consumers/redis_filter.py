...
if __name__ == '__main__':
    ...
    filter = RedisImpermanencyFilterUsingRedisKey('filter_dir', 300)
    for i in range(1000000):
        filter.add_a_value({'x': i, 'y': i * 2})

    filter.manual_delete_a_value({'a': 1, 'b': 2})
    print(filter.check_value_exists({'a': 1, 'b': 2}))
    filter.delete_expire_filter_task_cycle()
    filter.add_a_value({'a': 1, 'b': 5})
    print(filter.check_value_exists({'a': 1, 'b': 2}))
    time.sleep(130)
    print(filter.check_value_exists({'a': 1, 'b': 2}))
