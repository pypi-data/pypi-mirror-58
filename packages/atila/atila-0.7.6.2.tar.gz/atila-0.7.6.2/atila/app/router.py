...
RX_RULE = re.compile ("(/<(.+?)>)")
RE_RULE_NAME_ONLY = re.compile ("/<(?:[a-z]+?:)?([0-9a-zA-Z]+?)>")
    ...
    def add_route (self, rule, func, **options):
        ...
        if not is_alter_routing:
            if not self._started and not self._reloading and func_id in self._function_names and "argspec" not in options:
                self.log ("def {} is already defined. use another resource name, mount (ns = 'myns') or app.enable_namespace=True".format (func_id), "warn")
