    ...
    for type_packet in yml:
        if TYPES in type_packet:
            ...
            if id1 in actor_map:
                ...
                actor_map[id1] = Actor(clip_id=id1, actor_type=name)
...
def serialize_types(actor_map: Mapping[str, Actor]):
    for actor in actor_map.values():
        line = {'types': {ACTOR_ID: actor.clip_id, CSET3: {actor.actor_type.value: 1.0}}}
        yield '- {}'.format(yaml.dump(line, default_flow_style=True, width=math.inf))


def serialize_geom(actor_map: Mapping[str, Actor]):
    geom_id = 1
        ...
        for d in a.detections:
            line = {
                'geom': {
                    ...
                    BOX: f'{d.box.left} {d.box.top} {d.box.right} {d.box.bottom}',  # noqa: E501
                }
            }
            if d.keyframe:
                line['geom'][KEYFRAME] = True
            geom_id = geom_id + 1
            yield '- {}'.format(yaml.dump(line, default_flow_style=True, width=math.inf))


def serialize_activities(activity_map: Mapping[str, Activity]):
    for a in activity_map.values():
        line = {
            ACTIVITY: {
                ACTIVITY_TYPE: {a.activity_type.value: 1.0,},
                ACTIVITY_ID: a.clip_id,
                TIMESPANS: [{FRAME_TIMESPAN: [a.begin, a.end]}],
                ACTORS: [
                    {
                        ACTOR_ID: actor.clip_id,
