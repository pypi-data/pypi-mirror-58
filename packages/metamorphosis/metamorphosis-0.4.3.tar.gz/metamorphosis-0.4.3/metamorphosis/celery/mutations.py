...
class EventMutation(g.Mutation):
    ...
    def mutate(cls, root, info, **kwargs):
        ...
        if (
            ...
            rsp = celery.current_app.tasks[f'{cls.microservice.name}.consume_event_sync'].apply_async(args=(cls.microservice.name, evt_serialized))
            value = rsp.get()
            if value:
            ...
            celery.current_app.tasks[f'{cls.microservice.name}.consume_event_async'].apply_async(args=(cls.microservice.name, evt_serialized))
            return cls(data=evt)
