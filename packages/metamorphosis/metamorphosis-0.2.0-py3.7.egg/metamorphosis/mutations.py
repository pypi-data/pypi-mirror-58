...
class MutationTimings(enum.Enum):
    synchronous = 1
    asynchronous = 2
...
class EventMutation(g.Mutation):
    ...
    polling_delta = 0.1
    publish_topic = None
    allowed_timings = (MutationTimings.asynchronous, MutationTimings.synchronous)
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if cls.consequent_objecttypes:
            class MutationResponseData(g.Union):
                types = (cls.mutation_event_type, *cls.consequent_objecttypes)
            cls.data = MutationResponseData
        else:
            cls.data = cls.mutation_event_type
    ...
    def mutate(cls, root, info, **kwargs):
        ...
        if (
                ...
                ((hasattr(evt, 'timing') and evt.timing == 'sync') or
                 (not hasattr(evt, 'timing') and cls.default_timing == MutationTimings.synchronous))
        ):
            ...
            timeout = 60
            slept = 0
            while not cls.microservice.recent_results.exists(evt.id) and slept < timeout:
...
class AsyncEventMutation(EventMutation):
