...
class Tables(object):
    ...
    def get_grid_15d(self, func, params):
        ...
        x_coarse = np.logspace(params['limits']['xmin'], params['limits']['xmax'], params['calc']['nx'])
        gridtype = params['calc']['gridtype']
        if gridtype == 'log':
            y_coarse = np.logspace(params['limits']['ymin'], params['limits']['ymax'], params['calc']['ny'])
        else:
            y_coarse = np.linspace(params['limits']['ymin'], params['limits']['ymax'], params['calc']['ny'])

        diff, truth, x, y, x_fine, y_fine = self.interp_error_15d(func, gridtype, x_coarse, y_coarse)

        if params['calc']['adaptive']:
            pass
        else:
            if np.any(diff>10**(-5)):
                print('Interpolation failed to reach relative accuracy of 10^-5. '
    ...
    def get_grid_2d(self, func, params):
        """
        Determines the interpolation grid for a 2D interpolation. If adaptive is true, it refines the grid until
        the interpolation error is below the tolerance. If adaptive is false, then the input grid is returned
        together with an estimate of the interpolation error for this grid.
        :param func: function to determine the grid for
        :param params: parameters defining the limits, accuracy and method for the generation of the
        ...
        diff, truth, x, y, x_fine, y_fine = self.interp_error(func, gridtype, x_coarse, y_coarse)
        ...
        if params['calc']['adaptive']:
            while np.any(diff>tol):
                ...
                x_ind = np.sort(np.ceil(np.unique(np.hstack((x_ind-0.5, x_ind+0.5))))).astype('int')
                mask = (x_ind>0)*(x_ind<x.shape[0])
                x_ind = x_ind[mask]
                x = self.update_grid(x_ind, x, gridtype=gridtype)
                y_ind = np.sort(np.ceil(np.unique(np.hstack((y_ind-0.5, y_ind+0.5))))).astype('int')
                mask = (y_ind>0)*(y_ind<y.shape[0])
                y_ind = y_ind[mask]
                y = self.update_grid(y_ind, y, gridtype=gridtype)
                diff, truth, x, y, x_fine, y_fine = self.interp_error(func, gridtype, x, y)
    ...
    def interp_error_15d(self, func, gridtype, x, y):
        ...
        midpoints of the grid where interpolation has been performed for interpolation in 1.5D.
        ...
        for i in range(y.shape[0]):
            if i == 0:
                ...
                interp_vals = np.vstack((interp_vals, np.interp(x_fine, x, func_vals[i])))
    ...
    def interp_error(self, func, gridtype, x, y=None):
        ...
        if y is None:
            ...
            func_vals = func(x, y)
            if gridtype == 'log':
                interp_func = interpolate.RectBivariateSpline(np.log10(y), np.log10(x), func_vals)
            else:
    ...
    def update_grid(self, ind, grid, gridtype):
        ...
        if gridtype == 'log':
            grid = np.insert(grid, ind, 10**((np.log10(grid[ind-1])+np.log10(grid[ind]))/2.))
        else:
