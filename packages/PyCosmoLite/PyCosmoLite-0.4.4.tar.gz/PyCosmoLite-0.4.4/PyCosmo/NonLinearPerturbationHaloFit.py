...
class NonLinearPerturbationHaloFit(NonLinearPerturbationBase):
    ...
    """
    This class incorporates analytic fitting functions to the nonlinear matter
    power spectrum.
    """

    # TODO: Decide how to incorporate the revisions to halofit made by
    ...
    def k_nl(self, a):
        """ Compute the non-linear wave number k_nl as a function of the scale factor a

        :param a: scale factor [1]
        :return: k_nl: non-linear wave number [Mpc^-1]
        """
        logk_ran = [-3., 2.]
    ...
    def powerspec_a_k(self, a=1.0, k=0.1, diag_only=False):
        ...
        Currently both the halofit fitting function and its revision by Takahashi et al. are
        supported. The choice of the fitting function is set by pk_nonlin_type which can be set
        to either
            - 'halofit' for Smith et al., 2003, MNRAS, 341, 1311 (default)
            - 'rev_halofit' for revision by Takahashi et al., 2012, ApJ, 761, 152

        :param a: scale factor [1]
        :param k: wavenumber [Mpc^-1]
        :return: P_nl(k): nonlinear matter power spectrum P_nl(k) [Mpc^3]"""

        if self._params.pk_nonlin_type in ('halofit', 'rev_halofit'):
            return self._powerspec_nonlin_halofit_a_k(a, k, self._params.pk_nonlin_type, diag_only)
    ...
    def vir_rad(self):  # TODO: implementation of the virial radius
        r = 0.
        return r
