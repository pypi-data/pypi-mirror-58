#-------------------------------------------------------------------------------
# Purpose:     Read DINO Putfile to pandas dataframes
# Author:      Thomas de Meij
# Created:     02-03-2014. To Py3 15-08-2015 To Acquia 6-7-2019
# Licence:     Python 3.4
#-------------------------------------------------------------------------------
import os
import os.path
...
class DinoGws:
    """ container for groundwater measurement data from dinoloket csv file

    On initialisation the data is read from given path csv file:
    dn = dinogws(<filepath>)

    Data are returned with the following methods:
    series()    : return pandas timeseries in one of three units: cmmp, cmmv, cmnap
    header()    : return all available header data
    data()      : return dataframe with measured groundwater measurments in all available units
    locname()   : return nitgcode of location (series neme without filter)
    filname()   :
    srname()    :
    describe()  :
    mpref()     :
    locations() :
    merge()     : merge dinogws object with another dinogws object

    """

    def __repr__(self):
        return """ Read DINOloket csv file with groundwater measurements """
    ...
    def _readfile(self,filepath,readdata=True):
        """ Open DINO file en lees de header en eventueel de data """
        ...
        try:
            ...
            print (" : "+self.filepath)
            self.errors.append([self.filepath,"Bestand kan niet worden geopend"])
            self.flines=[]
            raise
        else:
...
if __name__ == '__main__':
    ...
    if 1: # beschikbaar maken object voor ipython + lijst bestanden

        sourcedir = ".\\testdata\\" # alles uit map testdata            
        filenames = [f for f in os.listdir(sourcedir) if os.path.isfile(os.path.join(sourcedir,f)) and f[11:13]=="_1"]

    if 0: # test functions readfile() for non-existing dinofile givenall

        print()
        headline = "Test function dn.readfile(<fakefilename>)"
        print(headline)
        print("-"*len(headline))
        print("Number of files is ",end="")
        print(len(filenames))    

        dn = dinogws("dummy.csv")

        
    if 0: # test functions readfile() for all files in testdata
          # readfile() uses functions findlines(), readheader() and readgws()
          # write headers of multiple files to csv file

        dn = dinogws()

        print()
        headline = "Test function dn.readfile() for multiples files and write headers to testdata-header.csv "
        print(headline)
        print("-"*len(headline))
        print("Number of files is ",end="")
        print(len(filenames))
        ...
        for i,file in enumerate(filenames):
            ...
            # test self.readfile()
            dn._readfile(sourcedir+file)
            
            # write headerlines to csv
            if len(dn.dfheader)>0: 
                if i==0: dn.dfheader.to_csv(".\\output\\testdata-header.csv",index=False,date_format="%d-%m-%Y")
                else:    dn.dfheader.to_csv(".\\output\\testdata-header.csv",index=False,date_format="%d-%m-%Y",header=False,mode = 'a')
            else: print(file+" has no header.")

            # print names of files with no data
            if len(dn.dfdata)==0: print(file+" has no data.")

            # print errors
            if len(dn.errors)>0:
                for line in dn.errors:
                    print("Found errors in {!s}: ".format(file))
                    print(line)

        # print execution time
        end_time = time.time()
        elapsed = "Elapsed time was %g seconds" % (end_time - start_time)
        print(elapsed)
        print("-"*len(headline))
        print("End of test function readfile()")
        print()

    if 0: # test function getseries() for all files in testdata

        start_time = time.time() 
    
        headline = "Test function readfile()"
        print(headline)
        print("-"*50)

        ##dn = dinogws()
        for i,file in enumerate(filenames):
            dn = dinogws(sourcedir+file)
 
            # create dataframe with number of values
            if len(dn.dfdata)>0:
                # test method self.getseries() 3 times
                srmp  = dn.series(units="cmmp")
                srmv  = dn.series(units="cmmv")
                srnap = dn.series(units="cmnap")

                srmplen = len(srmp[srmp.notnull()])
                srmvlen = len(srmv[srmv.notnull()])
                srnaplen = len(srnap[srnap.notnull()])
                
                if i==0: mylist = []
                mylist.append({"reeksnaam":dn.srname(),"cmmp":srmplen,"cmmv":srmvlen,"cmnap":srnaplen})

        dfnvalues = DataFrame(data=mylist,columns=["reeksnaam","cmmp","cmmv","cmnap"])
        print(dfnvalues)
    ...
    if 0: # test function mpseries()
        # '.\\testdata\\B45E0837001_1.csv'

        print("Test function mpref()")
        print("-"*50)
        print("Voor bestand B29C0191001_1.csv: ")
        dn = dinogws()
        dn._readfile(sourcedir+"B29C0191001_1.csv")
        mp = dn.mpref()
        print(mp)
        print()


        print(" Voor alle filenames:")
        for i,file in enumerate(filenames):
            dn = dinogws()
            dn._readfile(sourcedir+file)
            mp = dn.mpref()
        ...
        print("-"*50)
        print("End of test function mpref()")
        print()

    if 0: # test function decribe
        print()
        print("Test function decribe()")
        print("-"*50)

        dn = dinogws()
        testfile = ["B34D0081001_1.csv"]
        for i,file in enumerate(testfile):
            dn._readfile(sourcedir+file)
            dfdesc = dn.describe()
            print(dfdesc.T)
            
        print("-"*50)
        print("End of test function decribe()")
        print()

    if 0: # test describe met meerdere filters
        print()
        print("Test function decribe() met meerdere filters")
        print("-"*50)

        dn = dinogws()
        outfile = ".\\output\\testdesc.csv"
        if 1: # schrijf metadata filters naar csv
            for i,file in enumerate(filenames):                
                dn._readfile(sourcedir+file,readdata=True)
                if len(dn.describe())!=0:
                    # schrijf description naar een bestand
                    if i==0: 
                        outfile = ".\\output\\testdesc.csv"
                        dn.describe().to_csv(outfile,index=False,date_format="%d-%m-%Y")
                    else:    
                        dn.describe().to_csv(outfile,index=False,date_format="%d-%m-%Y",mode='a',header=False)
        df = pd.read_csv(".\\output\\testdesc.csv")

        print("Er zijn "+str(len(df))+" filters.")
        print()

    if 0: # maak bestand met locaties

        print()
        print("Test function locations() en maak bestand testloc.csv")
        print("-"*50)    

        # get dataframe with data from several filters
        df = pd.read_csv(".\\output\\testdesc.csv")

        # create locations from filter data
        dfloc = dn.locations(df=df)

        # write to location data csv
        outfile = ".\\output\\testloc.csv"
        dfloc.to_csv(outfile,index=True,date_format="%d-%m-%Y")                    

        print("Er zijn "+str(len(dfloc))+" locaties.")
        print()

    if 1: # test merge

        print("Test function dinogws.merge()")
        print("-"*50)
        print("Voor filter B12E0422_2 en B12E0423_2: ")
        dn1 = dinogws(sourcedir+"B12E0422002_1.csv")
        dn2 = dinogws(sourcedir+"B12E0423002_1.csv")
