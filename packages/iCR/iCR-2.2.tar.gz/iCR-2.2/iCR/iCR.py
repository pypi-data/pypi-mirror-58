""" 
Version 2.2 20th December 2019
...
class iCR:
  ...
  def _request(self,method,uri,data=False,**kwargs):
    ...
    try:
      if method == 'get':
        response = self.icr_session.get(request_url,headers = self.icr_session.headers,stream = stream,timeout = self.timeout)
      elif method == 'post':
        response = self.icr_session.post(request_url,data,headers = self.icr_session.headers,stream = stream,timeout = self.timeout)
      elif method == 'put':
        response = self.icr_session.put(request_url,data,headers = self.icr_session.headers,stream = stream,timeout = self.timeout)
      elif method == 'patch':
        response = self.icr_session.patch(request_url,data,headers = self.icr_session.headers,stream = stream,timeout = self.timeout)
      elif method == 'delete':
