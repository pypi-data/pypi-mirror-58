""" 
Version 2.2, 2.3 ad 2.4 20th December 2019
  - Fixed bug with upload, added stream to _request
Version 2.1 17th December 2019
  - Fixed various bugs
...
class iCR:
  ...
  def __init__(self,hostname,username,password,**kwargs):
    ...
    # not going to validate the TLS certificate of the iControl REST service
  ...
  def _request(self,method,uri,data=False,**kwargs):
    ...
    Keywords
    --------
    stream : Boolean True or False. Sets streaming on so that data does not have to be JSON
    ...
    if stream:
      self.icr_session.headers['Content-Type'] = 'application/octet-stream'
      headers = self.icr_session.headers
    else:
      headers = self.icr_session.headers
    # Set the URL
    ...
    self._debug( "Request Headers:" + str(headers) )
    self._set_auth()
    # Check whether data is valid JSON. If not, turn it into valid JSON
    if data != False and not stream:
    ...
    try:
      if method == 'get':
        response = self.icr_session.get(request_url,headers=headers,stream=stream,timeout=self.timeout)
      elif method == 'post':
        response = self.icr_session.post(request_url,data,headers=headers,stream=stream,timeout=self.timeout)
      elif method == 'put':
        response = self.icr_session.put(request_url,data,headers=headers,stream=stream,timeout=self.timeout)
      elif method == 'patch':
        response = self.icr_session.patch(request_url,data,headers=headers,stream=stream,timeout=self.timeout)
      elif method == 'delete':
  ...
  def upload(self,fp):
    ...
    Return: If successful, returns either filename or True. If unsuccessful, returns False
    """
    ...
    while True:
      ...
      response = self._request('post',request_url,file_slice,stream=True)
      if not response:
        self._debug( "Upload error:" + str(self.error) )
        return False       
      start += current_bytes
    # Set headers back to standard
    self._set_headers()
    if 'localFilePath' in self.json:
      self._debug("Upload response:" + str(self.json['localFilePath']))
      return self.json['localFilePath']
    else:
      return True    
  ...
  def download(self,fp):
    ...
    with open(fp, 'wb') as f:
      ...
      while True:              
        self.icr_session.headers['Content-Range'] = "%s-%s/%s" % (start, end, size)
        response = self.icr_session.get(request_url,stream=True,timeout = self.timeout)
        self._set_headers()
        if not response:
