...
def send_update_execution(
    ...
    """
    logger.info("[identifier:{}] Sending update execution: {} ({})".format(
        identifier, task_name, task_id))
...
def task_updated(sender=None, headers=None, body=None, **kwargs):
    ...
    logger.info("[identifier:{}] #Step-2(btask) Publishing task: {} ({})".format(
        identifier, task_name, task_id))
...
def task_started(task_id, task, args, **kwargs):
    ...
    logger.info("[identifier:{}] #Step-4(btask) Task pre run sending STARTED: {}".format(
        identifier, task_id))
