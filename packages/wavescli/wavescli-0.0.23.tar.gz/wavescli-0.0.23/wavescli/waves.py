...
logger = get_task_logger('waves.{}'.format(__name__))
config = get_config()
...
class WavesLogger(object):
    ...
    def info(self, message):
        identifier = self._task.identifier if self._task else 'no-identifier'
        self._logger.info(message, extra={'identifier': identifier})
...
def send_update_execution(
    ...
    """
    logger.info("Sending update execution: {} ({})".format(
        task_name, task_id), extra={'identifier': identifier})
...
def task_updated(sender=None, headers=None, body=None, **kwargs):
    ...
    """
    logger.info("Task published signal received by '{}' (W)".format(sender))
    task_name = headers.get('task')
    task_id = headers.get('id')

    if task_name == 'awebrunner.update_execution':
        logger.info("Task published signal skipped")
        return True
    ...
    logger.info("Publishing task: {} ({})".format(
        task_name, task_id), extra={'identifier': identifier})
...
def task_started(task_id, task, args, **kwargs):
    ...
    logger.info("Task pre run signal received: {} ".format(task_id))

    kw = kwargs.get('kwargs')
    identifier = kw.get('identifier')

    logger.info("Task pre run sending STARTED: {}".format(
        task_id), extra={'identifier': identifier})
