...
from nidhoggr.bl import BaseUserRepo, BLConfig
...
def authenticate(
    ...
    if not users.check_password(clean=req.password, hashed=user.password):
        return InvalidCredentials
...
@typed
def validate(req: auth.ValidateRequest, users: BaseUserRepo) -> Optional[YggdrasilError]:
...
@typed
def invalidate(req: auth.InvalidateRequest, users: BaseUserRepo) -> Optional[YggdrasilError]:
    user = users.get_user(access=req.accessToken)
...
@typed
def signout(req: auth.SignOutRequest, users: BaseUserRepo) -> Optional[YggdrasilError]:
