...
from tlclient.linker.constant import MsgType, CommType, FistType, HeartBeatStatus, ANY_FIST_NAME, DEFAULT_REQ_ID, SubscribeTopic
...
class Fist:
    ...
    def reg_sub(self, other_fist_name, topics=None):
        ...
        self._sub_fists.add(other_fist_name)
        return True
    ...
    def reg_req(self, other_fist_name):
        ...
        self._req_fists.add(other_fist_name)
        return True
    ...
    def on_pub_master(self, f):
        ...
        if msg_type == MsgType.CMD_SUICIDE:
            ...
            if info.comm_type == CommType.Zmq_PUB and \
                info.fist_type in self._auto_sub_fist_types and \
                    info.fist_name not in self._sub_fists:
                self.reg_sub(info.fist_name)
                ...
                info.fist_type in self._auto_req_fist_types and \
                    info.fist_type not in self._req_fists:
                self.reg_req(info.fist_name)
    ...
    def _keep_heart_beat(self, fist_to_push, sec_interval):
        ...
        while not self.is_stopped():
            self.heart_beat_msg.router_name = bytify(fist_to_push)
