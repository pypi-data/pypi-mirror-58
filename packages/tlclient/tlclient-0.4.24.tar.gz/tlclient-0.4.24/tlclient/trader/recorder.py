...
class InfluxClient(RecorderClient):
    ...
    def _proc_write_thread(self, res):
        try:
            ...
            if isinstance(e.content, dict) and e.content['error'].startswith('partial write: points beyond retention policy dropped'):
                self.logger.error('[influxdb] write_points error. (err_msg){}'.format(e.content['error']))
            else:
                self.logger.error('[influxdb] write_points error. (err_msg){}'.format(e.content))
                self.stop()
