...
def avg_pool(value, ksize, strides, padding, data_format="NHWC", name=None):
  ...
  Args:
    ...
    padding: A `string` from: `"SAME", "VALID", "LEFT"`.
...
@_dispatch.add_dispatch_list
@tf_export('conv2depu')
def conv2depu(input, filter, bias, second_input, strides, padding, use_cudnn_on_gpu=True, relu=False, bias_en=False, second_input_en=False, async=False, data_format="NHWC", dilations=[1, 1, 1, 1], name=None):
  r"""TODO: add doc.
  ...
  Args:
    ...
    second_input: A `Tensor`. Must have the same type as `input`.
    strides: A list of `ints`.
    padding: A `string` from: `"SAME", "VALID", "LEFT"`.
    use_cudnn_on_gpu: An optional `bool`. Defaults to `True`.
    relu: An optional `bool`. Defaults to `False`.
    bias_en: An optional `bool`. Defaults to `False`.
    second_input_en: An optional `bool`. Defaults to `False`.
    async: An optional `bool`. Defaults to `False`.
    data_format: An optional `string` from: `"NHWC", "NCHW"`. Defaults to `"NHWC"`.
    dilations: An optional list of `ints`. Defaults to `[1, 1, 1, 1]`.
    name: A name for the operation (optional).
  ...
  if _ctx is not None and _ctx._eager_context.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._eager_context.device_name, "Conv2DEPU",
        name, _ctx._post_execution_callbacks, input, filter, bias,
        second_input, "strides", strides, "use_cudnn_on_gpu",
        use_cudnn_on_gpu, "relu", relu, "bias_en", bias_en, "second_input_en",
        second_input_en, "async", async, "padding", padding, "data_format",
        data_format, "dilations", dilations)
      return _result
    except _core._FallbackException:
      try:
        return conv2depu_eager_fallback(
            input, filter, bias, second_input, strides=strides,
            use_cudnn_on_gpu=use_cudnn_on_gpu, relu=relu, bias_en=bias_en,
            second_input_en=second_input_en, async=async, padding=padding,
            data_format=data_format, dilations=dilations, name=name, ctx=_ctx)
      except _core._SymbolicException:
        pass  # Add nodes to the TensorFlow graph.
      except (TypeError, ValueError):
        result = _dispatch.dispatch(
              conv2depu, input=input, filter=filter, bias=bias,
                         second_input=second_input, strides=strides,
                         padding=padding, use_cudnn_on_gpu=use_cudnn_on_gpu,
                         relu=relu, bias_en=bias_en,
                         second_input_en=second_input_en, async=async,
                         data_format=data_format, dilations=dilations,
                         name=name)
        if result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
          return result
  ...
  if relu is None:
    relu = False
  relu = _execute.make_bool(relu, "relu")
  if bias_en is None:
    bias_en = False
  bias_en = _execute.make_bool(bias_en, "bias_en")
  if second_input_en is None:
    second_input_en = False
  second_input_en = _execute.make_bool(second_input_en, "second_input_en")
  if async is None:
    async = False
  async = _execute.make_bool(async, "async")
  if data_format is None:
    data_format = "NHWC"
  data_format = _execute.make_str(data_format, "data_format")
  if dilations is None:
  ...
  _inputs_flat = _op.inputs
  _attrs = ("T", _op.get_attr("T"), "strides", _op.get_attr("strides"),
            "use_cudnn_on_gpu", _op.get_attr("use_cudnn_on_gpu"), "relu",
            _op.get_attr("relu"), "bias_en", _op.get_attr("bias_en"),
            "second_input_en", _op.get_attr("second_input_en"), "async",
            _op.get_attr("async"), "padding", _op.get_attr("padding"),
            "data_format", _op.get_attr("data_format"), "dilations",
            _op.get_attr("dilations"))
  _execute.record_gradient(
      "Conv2DEPU", _inputs_flat, _attrs, _result, name)
  _result, = _result
  return _result



def conv2depu_eager_fallback(input, filter, bias, second_input, strides, padding, use_cudnn_on_gpu=True, relu=False, bias_en=False, second_input_en=False, async=False, data_format="NHWC", dilations=[1, 1, 1, 1], name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function conv2depu
  """
  _ctx = ctx if ctx else _context.context()
  if not isinstance(strides, (list, tuple)):
    raise TypeError(
  ...
  if not isinstance(dilations, (list, tuple)):
    raise TypeError(
        ...
        "'conv2depu' Op, not %r." % dilations)
  dilations = [_execute.make_int(_i, "dilations") for _i in dilations]
  _attr_T, _inputs_T = _execute.args_to_matching_eager([input, filter, bias, second_input], _ctx, _dtypes.half)
  (input, filter, bias, second_input) = _inputs_T
  _inputs_flat = [input, filter, bias, second_input]
  _attrs = ("T", _attr_T, "strides", strides, "use_cudnn_on_gpu",
  use_cudnn_on_gpu, "relu", relu, "bias_en", bias_en, "second_input_en",
  second_input_en, "async", async, "padding", padding, "data_format",
  data_format, "dilations", dilations)
  _result = _execute.execute(b"Conv2DEPU", 1, inputs=_inputs_flat,
                             attrs=_attrs, ctx=_ctx, name=name)
  _execute.record_gradient(
      "Conv2DEPU", _inputs_flat, _attrs, _result, name)
  _result, = _result
...
@_dispatch.add_dispatch_list
@tf_export('max_pool_epu')
def max_pool_epu(input, ksize, strides, padding, data_format="NHWC", async=False, name=None):
  r"""TODO: add doc.
  ...
  Args:
    ...
    strides: A list of `ints` that has length `>= 4`.
    padding: A `string` from: `"SAME", "VALID", "LEFT"`.
    data_format: An optional `string` from: `"NHWC"`. Defaults to `"NHWC"`.
    async: An optional `bool`. Defaults to `False`.
    name: A name for the operation (optional).
  ...
  if _ctx is not None and _ctx._eager_context.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._eager_context.device_name, "MaxPoolEPU",
        name, _ctx._post_execution_callbacks, input, "ksize", ksize,
        "strides", strides, "padding", padding, "data_format", data_format,
        "async", async)
      return _result
      ...
      try:
        return max_pool_epu_eager_fallback(
            input, ksize=ksize, strides=strides, padding=padding,
            data_format=data_format, async=async, name=name, ctx=_ctx)
      except _core._SymbolicException:
        pass  # Add nodes to the TensorFlow graph.
      except (TypeError, ValueError):
        result = _dispatch.dispatch(
              max_pool_epu, input=input, ksize=ksize, strides=strides,
                            padding=padding, data_format=data_format,
  ...
  if not isinstance(ksize, (list, tuple)):
    raise TypeError(
        "Expected list for 'ksize' argument to "
        "'max_pool_epu' Op, not %r." % ksize)
  ksize = [_execute.make_int(_i, "ksize") for _i in ksize]
  if not isinstance(strides, (list, tuple)):
    raise TypeError(
        "Expected list for 'strides' argument to "
        "'max_pool_epu' Op, not %r." % strides)
  strides = [_execute.make_int(_i, "strides") for _i in strides]
  padding = _execute.make_str(padding, "padding")
  if data_format is None:
    data_format = "NHWC"
  ...
  try:
    _, _, _op = _op_def_lib._apply_op_helper(
        "MaxPoolEPU", input=input, ksize=ksize, strides=strides,
                      padding=padding, data_format=data_format, async=async,
                      name=name)
  except (TypeError, ValueError):
    result = _dispatch.dispatch(
          max_pool_epu, input=input, ksize=ksize, strides=strides,
  ...
  _attrs = ("T", _op.get_attr("T"), "ksize", _op.get_attr("ksize"), "strides",
            _op.get_attr("strides"), "padding", _op.get_attr("padding"),
            "data_format", _op.get_attr("data_format"), "async",
            _op.get_attr("async"))
  _execute.record_gradient(
      "MaxPoolEPU", _inputs_flat, _attrs, _result, name)
  _result, = _result
  return _result



def max_pool_epu_eager_fallback(input, ksize, strides, padding, data_format="NHWC", async=False, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  This is for function max_pool_epu
  """
  _ctx = ctx if ctx else _context.context()
  if not isinstance(ksize, (list, tuple)):
    raise TypeError(
  ...
  if async is None:
    async = False
  async = _execute.make_bool(async, "async")
  _attr_T, (input,) = _execute.args_to_matching_eager([input], _ctx, _dtypes.half)
  _inputs_flat = [input]
  _attrs = ("T", _attr_T, "ksize", ksize, "strides", strides, "padding",
  padding, "data_format", data_format, "async", async)
  _result = _execute.execute(b"MaxPoolEPU", 1, inputs=_inputs_flat,
                             attrs=_attrs, ctx=_ctx, name=name)
  _execute.record_gradient(
      "MaxPoolEPU", _inputs_flat, _attrs, _result, name)
  _result, = _result
