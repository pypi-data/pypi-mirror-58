...
def _clean_skater_league_leader_stats(stats_list: list) -> list:
    leaders_stats = []
    for player in stats_list:
        del player['team']['logos']
        leaders_stats.append(player)
    return leaders_stats


def _skater_league_leaders(stat: str, season=None, season_type='2') -> list:
    season = _current_season() if not season else season
    # base_url = CONFIG['urls']['skater_stats']
    base_url = "https://api.nhle.com/stats/rest/en/leaders/skaters/"
    url = f"{base_url}{stat}"
    query = f"season={season} and gameType={season_type}"
    params = {
        "cayenneExp": query
    }
    req = requests.get(url, params=params)
    if req.status_code == 200:
        stats_list = req.json()['data']
        return _clean_skater_league_leader_stats(stats_list)
...
def _team_abbr(team):
    """Get the NHL API ID for a provided team"""
    teams = CONFIG['team_abbrs']
    if team.lower() not in teams.keys():
        raise JockBotNHLException(f"Unrecognized team: {team}")
    team_abbr = teams.get(team.lower())
    if not team_abbr:
        raise JockBotNHLException(f"Error retrieving abbreviation for {team}")
    return team_abbr
...
def _add_team_abbrs(game):
    home_team = game['teams']['home']['team']
    home_abbr = _team_abbr(home_team['name'])
    home_team['abbr'] = home_abbr
    away_team = game['teams']['away']['team']
    away_abbr = _team_abbr(away_team['name'])
...
def _games_on_date(date):
    ...
    if data:
        ...
        for game in data['dates'][0]['games']:
            game_link = game['link']
            game_data = _fetch_game_data(game_link)
            game['game_data'] = game_data
            games_list.append(_add_team_abbrs(game))
...
def _parse_team_games(schedule: list) -> list:
    games = []
    base_url = 'https://statsapi.web.nhl.com'
    for game in schedule:
        game_data = {}
        game_info = game['games'][0]
        game_data['teams'] = game_info['teams']
        stats_url = f"{base_url}{game_info['link']}"
        req = requests.get(stats_url)
        game_data['info'] = req.json()['gameData']
        game_data['linescore'] = req.json()['liveData']['linescore']
        game_data['boxscore'] = req.json()['liveData']['boxscore']
...
def _fetch_game_data(game_link):
    data = _api_request(game_link, base_url='https://statsapi.web.nhl.com')
...
def _fetch_league_leaders(stat, player_type, season=None, season_type='2', num_players=10, reverse=True, time_filter=0):
    ...
    url = f"{base_url}{player_type}s/{stat}?cayenneExp=season={season}%20and%20gameType={season_type}"
    # query = f"season={season} and gameType={season_type} and timeOnIce>{time_filter}"
    # query = f"season={season} and gameType={season_type}"
    # https://api.nhle.com/stats/rest/en/leaders/skaters/goals?cayenneExp=season=20192020%20and%20gameType=2
    # query = f"cayenneExp=season=20192020%20and%20gameType=2%20and%20player.positionCode%20=%20%27D%27"
    # params = {
    # "reportType": "season",
    ...
    # "sort": stat
    # }
    # leaders = _napi_request(base_url=base_url, params=params, verify=True)['data']
    leaders = _napi_request(base_url=url, verify=True)['data']
    if reverse:
        leaders.reverse()
...
def _fetch_league_leaders_teams(stat, season=None, season_type='2', reverse=False):
    ...
    """
    base_url = CONFIG['urls']['team_stat_league_leaders']
    season = _current_season() if not season else season
...
def _parse_leaders(stat, player_type, **kwargs):
    ...
    for leader in leaders_list:
        player_data = {}
        name = leader['player']['fullName']
        player_data['team'] = leader['team']['fullName']
