...
def run(model_name='mf_model', data='MovieLens1M'):
    assert model_name in ['mf_model', 'ib_model', 'ub_model']
    assert data in ['MovieLens1M', 'MovieLens100k']
    if data=='MovieLens1M':
        r_cols = ['user_id', 'movie_id', 'rating', 'unix_timestamp']
        ratings_base = pd.read_csv(path+'ml-1m/ratings.dat', sep='::', names=r_cols, encoding='latin-1')
        ratings = ratings_base.as_matrix()
    ...
    if model_name == 'mf_model':
        rs = MF(rate_train, K = 2, lam = 0.1, print_every = 2, learning_rate = 2, max_iter = 10, user_based = 0)
    elif model_name == 'ub_model': 
        rs = UserBase(rate_train, k = 30)
    else:
        rs = ItemBase(rate_train, k = 30)
