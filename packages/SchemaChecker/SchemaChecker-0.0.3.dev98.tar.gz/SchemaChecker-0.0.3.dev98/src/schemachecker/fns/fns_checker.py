...
class FnsChecker:
    ...
    @staticmethod
    def _set_error_struct(err_list: List[Tuple[str, str, str]], file: ClassVar[Dict[str, Any]]) -> None:
        """ Заполнение структуры ошибки для вывода. """
        for error in err_list:
            element_objs = [{'Путь до элемента': '',
                             'Ожидаемое значение': '',
                             'Наименование': '',
                             'Значение': ''}]

            file.verify_result['sch_asserts'].append({
                'Код результата': error[1],
                'Код ошибки': error[0],
                'Описание': error[2],
                'Объекты проверки': element_objs
            })
        ...
        for assertion in asserts:
            try:
                ...
                if not assertion_result:
                    ret_list.append((assertion['name'],
    ...
    def _mandatory_verification(self, file: ClassVar[Dict[str, Any]]) -> bool:
        ...
        if not correct_filename:
            file.verify_result['xsd_asserts'].append(
