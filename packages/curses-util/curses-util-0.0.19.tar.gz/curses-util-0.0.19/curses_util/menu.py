...
def vend(_items, _display, markable_mode, exit_keys, select_keys):
    ...
    while True:
        ...
        if chr(c) == 'g' and last_char == 'g':
            ...
            search_query = util.input('/')
            search_forward(1)
        elif c in exit_keys or chr(c) in exit_keys:
            util.cursesclean()
            return None
        elif c in select_keys or chr(c) in select_keys:
            util.cursesclean()
...
"""
    ...
    def multi_vend(self, items, display=lambda x: x):
        ...
        try:
            self._init_colors()
            return vend(items, display, True, self.exit_keys, self.select_keys)
        finally:
            util.cursesclean()
    ...
    def vend(self, items, display=lambda x: x):
        ...
        try:
            self._init_colors()
            res = vend(items, display, False, self.exit_keys, self.select_keys)
            if res == None:
                return None
            return res[0]
        finally:
