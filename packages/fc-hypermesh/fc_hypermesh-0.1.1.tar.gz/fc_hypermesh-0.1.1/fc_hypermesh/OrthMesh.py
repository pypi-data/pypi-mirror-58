""" ==============
    class OrthMesh
    ==============
    
    The aim of the class `OrthMesh` is to use functions of the `CartesianGrid` 
    module for creating regular tessallations either with p-order simplices or
    with p-order orthotopes of an d-orthope and all its m-faces (0<=m<=d).
"""

import numpy as np
from fc_tools.others import isModuleFound,LabelBaseName

class OrthMesh:
  """ 
    ==============
    class OrthMesh
    ==============
    
    The aim of the class `OrthMesh` is to use functions of the `CartesianGrid` 
    module  for creating regular tessallations with simplices or orthotopes of 
    an d-orthope and all its m-faces (0<=m<=d).
    The d-orthope to mesh is given in R^d by
      Oh=[a_1,b_1]x...x[a_d,b_d]
  
    Class attributes (main)
    -----------------------
    d     space dimension (integer).
    N     number of discretization in each space dimension or list of d 
          integers: one value by space dimension.
    order order of mesh element (default 1)          
    type  type of the mesh elements, 'simplicial'(default) or 'orthotope' 
          (string).
    box   box corresponding to the d-orthotope to mesh. Could be given by a 
          2D list or array such that 
             a_i=box[i-1][0] and b_i=box[i-1][1]
          or
             a_i=box[i-1,0] and b_i=box[i-1,1]
    Mesh  main mesh as an `EltMesh` object, ``Mesh.q`` is the vertices array 
          and ``Mesh.me`` the connectivity array.
    Faces list of list of EltMesh objects. Faces[i] contains all meshes representing
          all  (d-i)-faces of the d-orthotope Oh. The k-th (d-i)-face is given
          by the `EltMesh` object ``Faces[i][k]``: its vertices array is 
          ``Faces[i][k].q`` and its the connectivity array ``Faces[i][k].me``.
          By construction, we have
             Mesh.q[:,Faces[i][k].toGlobal] == Faces[i][k].q
          
    Class properties
    ----------------
    constructor: ``Th=OrthMesh(d,N,**kwargs)``
          
  """
  
  def __init__(self,d,N,**kwargs):
    """ Th=OrthMesh(d,N,**kwargs)
        
        Parameters
        ----------
        d     space dimension 
        N     either a 1D array/list of length `d` such that `N[i-1]` is the number 
              of discretization for `[a_i,b_i]` or either an integer if the number 
              of discretization is the same in all space directions.
        
        Optional key/value parameters
        -----------------------------
        box=[[a_1,b_1],[a_2,b_2],...,[a_d,b_d]]
            Use to set the d-orthotope. By default, the unit reference d-hypercube
            is choosed: [0,1]^d
        order=value
            order of the mesh element (default 1)
        type=value
            If value=='orthotope' then the tessallation is done with orthotopes.
            If value=='simplicial' then the tessallation is done with simplices.
        mapping=mapfun
            mapfun is a mapping function.
            
        :example:
        
        >>> Th=OrthMesh(3,[5,3,4])
        >>> Th.Mesh
        EltMesh object 
            type (str): simplicial
            type : 0
              d : 3
              m : 3
              q : (3,120)
              me : (4,360)
        >>> Th.Faces[1][0]
        EltMesh object 
            type (str): simplicial
            type : 0
              d : 3
              m : 1
              q : (3,5)
              me : (2,4)
        >>> Th.Mesh.q[:,Th.Faces[1][0].toGlobal] == Th.Faces[1][0].q
        array([[ True,  True,  True,  True,  True],
               [ True,  True,  True,  True,  True],
               [ True,  True,  True,  True,  True]], dtype=bool)
        
    """
    import fc_hypermesh.CartesianGrid as CG
    from fc_hypermesh.EltMesh import EltMesh
    from fc_hypermesh.utils import sizeof_OrthMesh
    from fc_tools.colors import selectColors
    from fc_tools.Sys import getRAM

    ctype=kwargs.get('type', 'simplex' )
    assert(ctype in ['simplex','orthotope'])
    box=kwargs.get('box',np.ones((d,1))*np.array([0,1]))
    m_min=kwargs.get('m_min',0)
    mapping=kwargs.get('mapping',None)
    order=kwargs.get('order', 1 )
    force=kwargs.get('force',False)
    verbose=kwargs.get('verbose',False)

    N=np.atleast_1d(N)
    assert len(N)==1 or len(N)==d
    if len(N)==1:
      N=N[0]*np.ones(d,dtype=int)
    
    RAM=getRAM() # in Go/GB
    RAMneeded=sizeof_OrthMesh(N,type=ctype,order=order)/10**9  # in GB
    
    if not force:
      if RAMneeded>RAM:
        raise NameError('Memory required is %.6f GB, available is %.6f GB!'%(RAMneeded,RAM))
      elif 2*RAMneeded>RAM:
        raise NameError('Memory required is %.6f GB and available is %.6f GB (too close)! Use "force=True" option to skip this test'%(RAMneeded,RAM))
    if verbose:
      print('Computed memory : %.6f GB\nAvailable memory: %.6f GB'%(RAMneeded,RAM))
      import time
      tstart=time.time()
      
    funFaces = lambda N,m: CG.genTessFaces(N,m,order=order,type=ctype)
    if (ctype=='orthotope'):
      funMesh = lambda N: CG.TessHyp(N,order)
      ntype=1
    else: # default 'simplex'
      funMesh = lambda N: CG.TessSim(N,order)
      ntype=0
    self.d=d
    self.order=order
    self.box=np.array(box,dtype=float)
    assert self.box.shape==(d,2)
    self.type=ctype
    [q,me]=funMesh(N)
    trans=lambda Q: MappingBox(Q,N,box)
    if mapping is not None:
      my_mapp=lambda Q: mapping(trans(Q))
    else:
      my_mapp=trans
    self.mapping=mapping
    self.Mesh=EltMesh(d,d,my_mapp(q),me,label=1,type=ntype,order=order)
    self.set_box()
    self.Faces=[]
    i=0
    for m in np.arange(d-1,m_min-1,-1):
      self.Faces.append(funFaces(N,m))
      nsTh=len(self.Faces[i])
      colors = selectColors(nsTh)
      for j in np.arange(nsTh):
        self.Faces[i][j].q=my_mapp(self.Faces[i][j].q)
        self.Faces[i][j].label=j+1
        self.Faces[i][j].color=colors[j]
        self.Faces[i][j].type=ntype
      i+=1
    if verbose:
      tcpu=time.time()-tstart
      RAMused=self.sizeof()/10**9 # in GB
      print('Used memory     : %.6f GB'%RAMused)
      print('Computed in     : %.3f s'%tcpu) 
      
  def __repr__(self):
    strret = ' %s object \n'%self.__class__.__name__ 
    strret +=   '        d : %d\n'%self.d 
    strret +=   '    order : %d\n'%self.order
    if isinstance(self.box,np.ndarray):
      strret += '      box : %s\n'%(str(self.box.tolist()))
    else:
      strret += '      box : %s\n'%(str(self.box))
    if self.mapping is None:
      strret +=   '  mapping : None\n'
    else:
      from fc_tools.others import func2str
      strret +=   '  mapping : %s\n'%func2str(self.mapping)
      
    strret += ' Mesh (order,type,nq,nme) : (%d,%s,%d,%d)\n'%(self.Mesh.order,self.Mesh.strtype(),self.Mesh.nq,self.Mesh.nme)
    for F in self.Faces:
      strret += ' Number of %d-faces : %d\n'%(F[0].m,len(F))
      i=0
      for f in F:
        strret += '   [%2d] (order,type,nq,nme) : (%d,%s,%d,%d)\n'%(i,f.order,f.strtype(),f.nq,f.nme)
        i+=1
    return strret         
   
  def getFacesIndex(self,m): # must be improved
    A=np.array(np.arange(self.d-1,-1,-1))
    return np.where(A==m)[0][0]
  
  
  def sizeof_faces(self,m):
    if m==self.d:
      return self.Mesh.sizeof()
    idx=self.getFacesIndex(m)
    float_size=0;int_size=0
    for Face in self.Faces[idx]:
      (fsize,isize)=Face.sizeof()
      float_size+=fsize
      int_size+=isize
    return (float_size,int_size)  
  
  def sizeof(self):
    " in Bytes"
    (float_size,int_size)=self.Mesh.sizeof()
    for Faces in self.Faces:
      for Face in Faces:
        (fsize,isize)=Face.sizeof()
        float_size+=fsize
        int_size+=isize
    return float_size+int_size
  
  def set_box(self):
    for i in range(self.d):
      self.box[i,0]=np.min(self.Mesh.q[i])
      self.box[i,1]=np.max(self.Mesh.q[i])
   
  def plotmesh(self,**kwargs):
    """ plotmesh(self,**kwargs)
    
    """
    if not isModuleFound('matplotlib'):
      print('plotmesh needs matplotlib package!')
      return
    import matplotlib.pyplot as plt
    if self.d > 3:
      print('Unable to plot in dimension %d > 3!'%self.d)
      return
    m=kwargs.pop('m',self.d)
    assert m in range(self.d+1)
    legend=kwargs.pop('legend', False);
    labels=kwargs.pop('labels', None);
    fig=plt.gcf()
    if m==self.d:
      Legend_handle,Labels=self.Mesh.plotmesh(**kwargs)
      Legend_handle=[Legend_handle]
      Labels=[Labels]
    else:
      Legend_handle=[];Labels=[]
      idx=self.getFacesIndex(m)
      F=self.Faces[idx]
      Labs=[F[i].label for i in range(len(F))]
      if labels is None:
        labels=Labs
      nF=len(F);
      for i in range(nF):
        if F[i].label in labels:
          Leg,Lab=F[i].plotmesh(**kwargs)
          Legend_handle.append(Leg)
          Labels.append(Lab)
    fig = plt.gcf()
    ax=fig.axes[0]
    if self.d==3:
      ax.set_zlim(self.box[2,0],self.box[2,1])
    ax.set_xlim(self.box[0,0],self.box[0,1])
    ax.set_xlabel('x')
    if self.d>1:
      ax.set_ylim(self.box[1,0],self.box[1,1]) 
      ax.set_ylabel('y')
    if self.d==3:
      ax.set_zlabel('z')
    #-> No background
    ax.patch.set_facecolor('None')
    fig.patch.set_visible(False) 
    #<-
    if legend:
      hl=plt.legend(Legend_handle,Labels,loc='best', ncol=int(len(Legend_handle)/10)+1)
      if hasattr(hl, 'set_draggable'):
        hl.set_draggable(True)
      elif hasattr(hl, 'draggable'):
        hl.draggable()
      
  def plotnodes(self,**kwargs):
    """ plotnodes(self,**kwargs)
    
    """
    if not isModuleFound('matplotlib'):
      print('plotNodes needs matplotlib package!')
      return
    import matplotlib.pyplot as plt
    if self.d > 3:
      print('Unable to plot in dimension %d > 3!'%self.d)
      return
    m=kwargs.pop('m',self.d)
    assert m in range(self.d+1)
    labels=kwargs.pop('labels', None);
    fig=plt.gcf()
    if m==self.d:
      self.Mesh.plotnodes(**kwargs)
    else:
      idx=self.getFacesIndex(m)
      if labels is None:
        si=np.arange(len(self.Faces[idx]))
      else:
        si=[i for i in np.arange(len(self.Faces[idx])) if self.Faces[idx][i].label in labels]
      for i in si:
        self.Faces[idx][i].plotnodes(**kwargs)
        
  def plotnodesNumber(self,**kwargs):
    """ plotnodesNumber(self,**kwargs)
    
    """
    if not isModuleFound('matplotlib'):
      print('plotNodes needs matplotlib package!')
      return
    import matplotlib.pyplot as plt
    if self.d > 3:
      print('Unable to plot in dimension %d > 3!'%self.d)
      return
    m=kwargs.pop('m',self.d)
    assert m in range(self.d+1)
    labels=kwargs.pop('labels', None);
    fig=plt.gcf()
    if m==self.d:
      self.Mesh.plotnodesNumber(**kwargs)
    else:
      idx=self.getFacesIndex(m)
      if labels is None:
        si=np.arange(len(self.Faces[idx]))
      else:
        si=[i for i in np.arange(len(self.Faces[idx])) if self.Faces[idx][i].label in labels]
      for i in si:
        self.Faces[idx][i].plotnodesNumber(**kwargs)
    
  def ploteltsNumber(self,**kwargs):
    """ ploteltsNumber(self,**kwargs)
    
    """
    if not isModuleFound('matplotlib'):
      print('ploteltsNumber needs matplotlib package!')
      return
    import matplotlib.pyplot as plt
    if self.d > 3:
      print('Unable to plot in dimension %d > 3!'%self.d)
      return
    m=kwargs.pop('m',self.d)
    assert m in range(self.d+1)
    labels=kwargs.pop('labels', None);
    fig=plt.gcf()
    if m==self.d:
      self.Mesh.ploteltsNumber(**kwargs)
    else:
      idx=self.getFacesIndex(m)
      if labels is None:
        si=np.arange(len(self.Faces[idx]))
      else:
        si=[i for i in np.arange(len(self.Faces[idx])) if self.Faces[idx][i].label in labels]
      for i in si:
        self.Faces[idx][i].ploteltsNumber(**kwargs)    
      
def MappingBox(q,N,box):
  box=np.array(box,dtype=float)
  d=len(N)
  
  for i in range(d):
    q[i]=box[i,0]+(box[i,1]-box[i,0])/(1.*N[i])*q[i] #BUG: take care with whole division
  return q
