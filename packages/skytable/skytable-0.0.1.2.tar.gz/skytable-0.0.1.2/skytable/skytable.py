...
class Skytable:
    ...
    def __repr__(self):
        ...
        # 1
        #Phrases.get_all(fields=['KeyPhraseID', 'Active', 'CompletedPageID'], formula="NOT({Active} = '')", sort=[('Priority', 'desc')])
        # =>
        # OrderedDict([('fields[]', ['KeyPhraseID', 'Active', 'CompletedPageID']), ('filterByFormula', "NOT({Active} = '')"), ('offset', None), ('sort[0][direction]', 'desc'), ('sort[0][field]', 'Priority')])
        # list(Phrases.table.find({'Active': {'$ne': True}}, {'KeyPhraseID': 1, 'Active': 1, 'CompletedPageID': 1}))
        # fields=['a','b','c','d'] -- becomes "project"=  {'a': 1, 'b': 1, 'c': 1', 'd': 1}
        # formula NOT({a} = 'A', {b} = 'B') -- becomes -- "{'$and': [{'a': {'$ne': 'A'}, {'b': {'$ne': 'B'}}]}"

        # 2
        # Phrases.get_all(fields=['KeyPhraseID', 'Active', 'CompletedPageID'], formula="AND(NOT({Active} = ''), {Priority} >= 0)", sort=[('Priority', 'desc')])
        # =>
        # OrderedDict([('fields[]', ['KeyPhraseID', 'Active', 'CompletedPageID']), ('filterByFormula', "AND(NOT({Active} = ''), {Priority} >= 0)"), ('offset', None), ('sort[0][direction]', 'desc'), ('sort[0][field]', 'Priority')])

        # 3
        # Groups.get_all(fields=['GroupURL', 'GroupName', 'Status', 'LinkedinAgentID'], formula="AND({FoundByKeyphraseID} = '%s', {Status} = '%s', NOT(OR(FIND('UK', {GroupName}), FIND('Fashion', {GroupName}), FIND('Fashion', {GroupName}))))" % ('MyPhraseName', 'New'), sort=[('Added', 'asc')], maxRecords=20)
        # =>
        # OrderedDict([('fields[]', ['GroupURL', 'GroupName', 'Status', 'LinkedinAgentID']), ('filterByFormula', "AND({FoundByKeyphraseID} = 'MyPhraseName', {Status} = 'New', NOT(OR(FIND('UK', {GroupName}), FIND('Fashion', {GroupName}), FIND('Fashion', {GroupName}))))"), ('maxRecords', 20), ('offset', None), ('sort[0][direction]', 'asc'), ('sort[0][field]', 'Added')])

        # 4
        # Agents.get_all(fields=['LinkedinAgentID', 'Active'], formula="NOT({Active} = '')", sort=[('Priority', 'desc')])
        # =>
        # OrderedDict([('fields[]', ['LinkedinAgentID', 'Active']), ('filterByFormula', "NOT({Active} = '')"), ('offset', None), ('sort[0][direction]', 'desc'), ('sort[0][field]', 'Priority')])

        # 5
        # Groups.get_all(fields=['GroupURL', 'FoundByKeyphraseID', 'GroupName', 'GroupDescription'], formula="AND({Status} = 'Joined')", sort=[('Added', 'desc')], maxRecords=25)
        # =>
        # OrderedDict([('fields[]', ['GroupURL', 'FoundByKeyphraseID', 'GroupName', 'GroupDescription']), ('filterByFormula', "AND({Status} = 'Joined')"), ('maxRecords', 25), ('offset', None), ('sort[0][direction]', 'desc'), ('sort[0][field]', 'Added')])

        # 6
        # Users.get_all(formula="AND({LinkedinUserURL} = '%s', {Status} = 'Invitation sent')" % 'http://specific_user_url')
        # =>
        # OrderedDict([('filterByFormula', "AND({LinkedinUserURL} = 'http://specific_user_url', {Status} = 'Invitation sent')"), ('offset', None)])

        # 7
        # Users.get_all(formula="AND({Status} = 'New', NOT({GroupRole} = 'Manager'))", maxRecords='NUMBER_OF_CONNECTIONS_AT_TIME')
        # =>
        # OrderedDict([('filterByFormula', "AND({Status} = 'New', NOT({GroupRole} = 'Manager'))"), ('maxRecords', 'NUMBER_OF_CONNECTIONS_AT_TIME'), ('offset', None)])

        # 8
        # Users.get_all(formula="AND({Status} = 'Connected', NOT(OR({GroupRole} = 'Owner', {GroupRole} = 'Manager')))")
        # =>
        # OrderedDict([('filterByFormula', "AND({Status} = 'Connected', NOT(OR({GroupRole} = 'Owner', {GroupRole} = 'Manager')))"), ('offset', None)])
