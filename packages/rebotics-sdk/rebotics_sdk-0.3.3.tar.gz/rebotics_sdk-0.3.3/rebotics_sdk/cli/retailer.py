...
def processing_action_realogram_report(ctx, concurrency, drop_duplicates, target):
    ...
    for processing_action in processing_actions:
        ...
        df_oos_processing_action['store_number'] = processing_action['store']['store_number']
        ...
        df_pac['user'] = processing_action['user']['username']
        df_pac['compliance_rate'] = processing_action['compliance_rates']['compliance']
    ...
    dump_path = os.path.join(target, 'reporting_{}'.format(datetime.today().strftime('%Y-%m-%d')))
    mkdir_p(dump_path)
