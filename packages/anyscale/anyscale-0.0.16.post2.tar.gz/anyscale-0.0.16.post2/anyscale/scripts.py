...
from anyscale.snapshot import (
    copy_file, create_snapshot, delete_snapshot,
...
def snapshot_download(name, target_directory):
    ...
    snapshot_info = send_json_request("snapshot_get", {
...
@session_cli.command(
    name="start_command",
    help="Start executing a command.",
    hidden=True
)
@click.option(
    "--command-id", help="ID of the command to start",
    type=int,
    default=None
)
@click.option(
    "--pid", help="PID of the associated process",
    type=int,
    default=None
)
def session_start_command(command_id, pid):
    send_json_request("session_start_command", {
        "session_command_id": command_id,
        "pid": pid
    }, post=True)
