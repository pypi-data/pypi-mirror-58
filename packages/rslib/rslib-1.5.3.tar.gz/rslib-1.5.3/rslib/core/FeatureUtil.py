...
class FeatureUtil():
    ...
    def feature_extraction(self, data, padding=False, sparse=True):
        ...
        label = [int(sample.split('@')[feature_id['label']].split(':')[0]) for sample in data]
        label_time = [int(sample.split('@')[feature_id['label']].split(':')[2]) for sample in data]

        sequence_id = [
            [[int(x.split(':')[0]) for x in
              sorted(xx.split(','), key=lambda x: x.split(':')[-1])][-self.maxlen:] if xx else [0] for xx in
             sample.split('@')[feature_id['sequence_feature']].split(';')]
            for sample in data]
        sequence_time = [
            [[int(sample.split('@')[feature_id['label']].split(':')[2]) - int(x.split(':')[1]) for x in
              sorted(xx.split(','), key=lambda x: x.split(':')[-1])][-self.maxlen:] if xx else [0] for xx in
             sample.split('@')[feature_id['sequence_feature']].split(';')]
            for sample in data]
        sequence_time_gaps = [[[0] + [abs(xx[i] - xx[i - 1]) for i in range(1, len(xx))] for xx in sample] for sample in sequence_time]

        cross_features_index = [[int(y.split(':')[0]) for y in sample.split('@')[feature_id['cross_feature']].split(',')] for sample in data]  # id+maxid*daygap
        cross_features_val = [[float(y.split(':')[1]) for y in sample.split('@')[feature_id['cross_feature']].split(',')] for sample in data]  # value
        user_features_id = [[int(y) if y else 0 for y in sample.split('@')[feature_id['user_feature']].split(',')] for sample in data]  # id+maxid*daygap
        label_week_id = [[int(data[i].split('@')[feature_id['label']].split(':')[1])] * len(sequence_id[i]) for i in range(len(data))]
        return [role_id_hash, sequence_id, sequence_time, sequence_time_gaps, cross_features_index, cross_features_val, user_features_id,
                label_week_id], np.array(label)

    def to_tfrecord(self, csvfile, filename):
        seq_num = self.config['seq_num']
        writer = tf.python_io.TFRecordWriter(filename)
        data = open(csvfile, 'r', encoding='utf8').read().split('\n')[1:-1]
        [role_id_hashs, sequence_ids, sequence_times, sequence_time_gapss, cross_features_indexs, cross_features_vals, user_features_ids,label_week_ids], labels = self.feature_extraction(data)
        ...
        for i in range(len(data)):
            ...
            sequence_id_feature = [tf.train.Feature(int64_list=tf.train.Int64List(value=sequence_id[i])) for i in range(seq_num)]
            sequence_time_feature = [tf.train.Feature(int64_list=tf.train.Int64List(value=sequence_time[i])) for i in range(seq_num)]
            sequence_time_gaps_feature = [tf.train.Feature(int64_list=tf.train.Int64List(value=sequence_time_gaps[i])) for i in range(seq_num)]
            ...
            for seq_num_i in range(seq_num):
    ...
    def read_tfrecord(self, filename, is_pred=False):
        ...
        def _parse_exmp(serial_exmp):
            ...
            sequence_id = [tf.sparse_to_dense(context_parsed['sequence_id_' + str(i)].indices, [self.maxlen],
                                              context_parsed['sequence_id_' + str(i)].values) for i in range(seq_num)]
            sequence_time = [tf.sparse_to_dense(context_parsed['sequence_time_' + str(i)].indices, [self.maxlen],
                                                context_parsed['sequence_time_' + str(i)].values) for i in range(seq_num)]
            ...
            return role_id_hash, sequence_id, sequence_time, sequence_time_gaps, cross_feature, user_feature, label_week_id, label

        def _flat_map_fn(role_id_hash, sequence_id, sequence_time, sequence_time_gaps, cross_feature, user_feature, label_week_id, label):

            return dataset_ops.Dataset.zip((
                role_id_hash.batch(batch_size=self.batchsize),
                sequence_id.padded_batch(self.batchsize, padded_shapes=([seq_num, self.maxlen])),
                sequence_time.padded_batch(self.batchsize, padded_shapes=([seq_num, self.maxlen])),
                sequence_time_gaps.padded_batch(self.batchsize, padded_shapes=([seq_num, self.maxlen])),
                ...
                label_week_id.padded_batch(self.batchsize, padded_shapes=([self.maxlen])),
                label.batch(batch_size=self.batchsize)
            ))

        def preprocess_fn(a, b, c, d, e, f, g, h):
            '''A transformation function to preprocess raw data
            into trainable input. '''
            return (a, b, c, d, e, f, g), tf.one_hot(h, self.output_unit) if self.output_unit > 1 else h
