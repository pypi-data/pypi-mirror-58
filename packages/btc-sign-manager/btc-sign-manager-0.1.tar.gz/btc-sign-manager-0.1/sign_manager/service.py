...
def get_cache_manager_class() -> Type['CacheManager']:
    ...
    error_message = 'Не найден укаазанный в "settings" класс менеджера кеширования объектов подписания. ' \
...
class CacheManager:
    ...
    def __init__(self, request_key: str = None, cache_timeout: int = 60):
        self.request_key = request_key or self.cache_key
        self.cache_timeout = cache_timeout

    def cache_object(self, object_for_sign: Type[Model]()) -> None:
        data = {
            object_for_sign.get_hash(): object_for_sign.serialize_for_cache()
        }
