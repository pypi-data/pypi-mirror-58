...
from sign_manager.service import DataProvider, get_cache_manager_class
...
class SignManagerMixin:
    ...
    __objects_to_serialization: list = []
    ...
    def get_changed_object(self, commit: bool = True) -> Any:
    ...
    def get_signed_data(self) -> tuple:
        ...
        if is_valid:
            for form in sign_formset:
                ...
                signed_data[object_hash] = DataProvider.object_signed_data_provider(form_cleaned_data)
        else:
    ...
    def get_sing_cert_data(self) -> tuple:
        ...
        if is_valid:
            sign_cert_data = DataProvider.object_sign_cert_data_provider(sign_cert_form.cleaned_data)
        else:
    ...
    def _cache_objects(self) -> None:
        ...
        for obj in self.__objects_to_serialization:
            cache_manager.cache_object(obj)
    ...
    def _get_objects_for_sign(self) -> list:
        return [sign_object.object_for_sign for sign_object in self.set_sign_initial_data()]

    def add_objects_to_serialization(self, model_objects: list) -> None:
        self.__objects_to_serialization += model_objects

    def prepare_objects(self) -> None:
        self.add_objects_to_serialization(self._get_objects_for_sign())
        self._cache_objects()
    ...
    def after_sign(self, signed_data: dict, sign_cert_data: dict) -> bool:
        ...
        data_is_valid = True
        for obj in self._get_objects_for_sign():
            obj_sign_data = signed_data.get(obj.get_hash())
