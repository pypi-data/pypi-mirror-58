...
BTCCryptoProClientManager = function (id='main_sign_manager') {
    ...
    this.element_disable_class = 'disable';
    this.element_hide_class = 'hidden';
    this.form_elements_container_selector = '';

    this.ERROR_MESSAGES = {
        'CREATE_SIGN_FAIL': 'Failed to create signature. Error: ',
        'CERTIFICATES_NOT_FOUND': 'Certificate not found: ',
    ...
    */
        ...
        return new Promise(function(resolve, reject) {
            cadesplugin.async_spawn(function* (args) {
                try {
                    ...
                    args[3](self.ERROR_MESSAGES.CREATE_SIGN_FAIL + cadesplugin.getLastError(e));
                }
    ...
    this.setCertificateDataToForm = function (data=null, test=false) {
        ...
        if (test) {
            $(self.formset_form_selector).find('input').first().val('test sign');
        }
    };
        ...
        return new Promise(function (resolve, reject) {
            cadesplugin.async_spawn(function* (args) {
                try {
                    ...
                    yield oSignedXML.propset_SignatureMethod(self.SIGN_XML_CONFIG.XmlDsigGost3410Url);
                    yield oSignedXML.propset_DigestMethod(self.SIGN_XML_CONFIG.XmlDsigGost3411Url);
    ...
    this.displayTrayError = function (object_hash, error_message, ) {
        ...
        self.tray.updateHeaderTitle('An error caught');
    };
    ...
    this.askServer = function (certificate_object) {
        return new Promise(function (main_promise_resolve, main_promise_reject) {
            cadesplugin.async_spawn(function *(args) {
                ...
                oHashedData.Algorithm = self.SIGN_BASE64_CONFIG.CADESCOM_HASH_ALGORITHM_CP_GOST_3411;
                oHashedData.DataEncoding = self.SIGN_BASE64_CONFIG.CADESCOM_BASE64_TO_BINARY;
                        ...
                        $.when(ajaxPostRequest(self.ask_url, post_data)).then(function (data) {
                            ...
                            if (promise !== undefined) {
                                promise.then(function (created_sign) {
                                    ...
                                    self.displayTrayError(object_hash,
                                        `${self.ERROR_MESSAGES.CREATE_SIGN_FAIL}: ${reason}`);
                                    reject();
                                });
                            } else {
                                resolve();
    ...
    this.blockStartSignButton = function () {
    ...
    this.getForms = function () {
        return $(self.form_elements_container_selector).find('input, select, textarea').serializeArray();
    };
        ...
        cadesplugin.then(function () {
            ...
            $(document).on('click', self.start_sign_btn_selector, function (event) {
                ...
                $.when(ajaxPostRequest(url, initial_post_data)).then(function (data) {
                    ...
                    self.tray.updateHeaderTitle('Signing...');
                    self.runSign(function (certificate_object) {
                        self.askServer(certificate_object).then(function (resolve) {
                            ...
                            self.tray.updateHeaderTitle('Sending to server...');
                            setTimeout(function () {
                                const post_data = [
                                    ...self.getForms(),
                                    ...self.COMPLETE_SIGN_HEADER
                                ...
                                $.when(ajaxPostRequest(url, post_data)).then(function (final_data) {
                                    ...
                                    self.showError(self.ERROR_MESSAGES.SEND_TO_SERVER_ERROR);
                                    self.tray.updateHeaderTitle('Error while sending to server...');
                                })
