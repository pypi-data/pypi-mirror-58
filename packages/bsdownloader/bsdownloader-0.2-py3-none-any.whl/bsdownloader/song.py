...
class Song:
    ...
    def download(self, game_path: str):
        from .http_get import raw_get
        # song folder
        ...
        # download zip
        resp = raw_get(self.song_url)
        if resp is None:
            print(f"Filed to download {self.song_name}")
            return

        # save zip
        with open(self.zip_file, mode="wb") as f:
...
def try_request(song: str) -> bytes:
    ...
    song_json = ""
    while True:
        song_json = simple_get(
            f"https://beatsaver.com/api/search/text/0?q={song}")
