...
class JobTemplateModule(BaseAnsibleTowerModule):
    module_args = {
        ...
        'inventory': {'type': 'dict', 'required': True},
    ...
    def dtofromparams(self):
        inventory_id = self.client.get_inventory(**self.params['inventory'])
        project_id = self.client.get_project(**self.params['project'])
        return {
    ...
    def getsubjectresource(self):
        ...
        params.update(self.client.asfilter('project', args['project']))
        params.update(self.client.asfilter('inventory', args['inventory']))
        return self.client.list('job_templates', params=params)
