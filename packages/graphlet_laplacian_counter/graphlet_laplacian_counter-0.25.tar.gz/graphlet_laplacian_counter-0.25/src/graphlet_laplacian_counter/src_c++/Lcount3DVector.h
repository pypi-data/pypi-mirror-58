    ...
    /*void applyOvercountCorrection(){
        ...
        for(std::vector<std::vector<std::vector<int64> > >::iterator laplacian = this->vector3D.begin(); laplacian != this->vector3D.end(); ++laplacian)
        {
            for(std::vector<std::vector<int64> > ::iterator row= laplacian->begin(); row != laplacian->end(); ++row)
            {
                ...
                {
                    if (row->at(i)%orbitOvercount[orbit]!=0)
                    {
                       int blu=5;
                    }
                    row->at(i) =  (row->at(i)/ orbitOvercount[orbit]);
                }
            }
            orbit++;
        }
...
//                }
    ...
    void getNonCorrectedRowOverOrbits(std::vector<unsigned int> orbits ,int row,std::vector<double>& correctedRow){
    ...
    std::vector<std::vector<std::vector<double> > >  getNonCorrectedGraphletLaplacians(){
