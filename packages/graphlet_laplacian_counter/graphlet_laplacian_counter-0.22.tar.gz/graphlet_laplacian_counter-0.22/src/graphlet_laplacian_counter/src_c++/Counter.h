...
public:
    ...
    Counter()  {};
    std::vector<std::vector<std::vector<int64> > > count_from_vectors(std::vector<int> src_nodes, std::vector<int> dst_nodes);
    std::vector<std::vector<std::vector<int64> > > count_from_vectors_single_node(std::vector<int> src_nodes, std::vector<int> dst_nodes, int node);

    void count(Graph& g, FILE *out, char const *outname, std::string outputPrefix, bool five_nodes);

private:

    void count_subroutine(Graph& g, Lcount3D &laplacians, bool five_nodes, std::vector<int> nodes_to_run= std::vector<int>());
