...
std::vector<std::vector<std::vector<int64> > > Counter::count_from_vectors_single_node(std::vector<int> src_nodes, std::vector<int> dst_nodes, int node){
    ...
    return lcount3DVector.getNonCorrectedGraphletLaplacians();
}



std::vector<std::vector<std::vector<int64> > > Counter::count_from_vectors(std::vector<int> src_nodes, std::vector<int> dst_nodes){
...
void Counter::count(Graph& g, FILE *out, char const *outname, std::string outputPrefix, bool five_nodes) {
...
void Counter::count_subroutine(Graph& g, Lcount3D &laplacians, bool five_nodes, std::vector<int> nodes_to_run){
...
//          }
    ...
    laplacians.applyOvercountCorrection();
#if PROGRESS_INFO
    fprintf(stderr, "\nCounting complete.\n");
#endif
...
//    }
    ...
    //writer.writeGraphletLaplacian(laplacians,outputPrefix);
    /* checker.checkRedundancieslaplacians(laplacians); */
