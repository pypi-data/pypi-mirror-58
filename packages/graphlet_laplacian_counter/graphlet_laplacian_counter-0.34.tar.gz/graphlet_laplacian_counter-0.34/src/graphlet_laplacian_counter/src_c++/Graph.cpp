...
std::vector<int> Graph::get_neighbours_k_distance(int node, int distance) {
    ...
    std::vector<bool> nodes_visited = std::vector<bool>();
    ...
    for (neighbor_it = this->node_2_edges[node].begin(); neighbor_it<this->node_2_edges[node].end(); neighbor_it++){
        ...
        if (distance>0 and !nodes_visited[neighbor]) {
            this->get_neighbours_k_distance(neighbor, distance-1, neighbors, nodes_visited);
        }
...
void Graph::get_neighbours_k_distance(int node, int distance, std::set<int>& neighbors, std::vector<bool>& nodes_visited ) {
