...
class Connection(object):
    ...
    def buildClient(self):
        ...
        """
        if not self.host:
            self.logger.error(f'[{self.id}] no host defined')
            return None
        if not isinstance(self.host, tuple):
            self.logger.error(f'[{self.id}] host entry malformed')
            return None
        ...
        try:
            ...
            self.logger.debug(f'[{self.id}] socket timeout')
            return None
        except socket.error as e:
            self.closeClientHard(client)
            self.logger.debug(f'[{self.id}] socket error: {e}')
            return None
        except Exception as e:
            self.closeClientHard(client)
            self.logger.error(f'[{self.id}] socket error: {e}')
            return None
        else:
            return client
        ...
        try:
            while True:
                ...
                try:
                    ...
                    self.logger.error(f'[{self.id}] {e}, {chunkRaw}')
                    return False, ''
                if not chunk:
                    break
                response += chunk
            ...
            self.logger.debug(f'[{self.id}] response : {response}')
    ...
    def communicate(self, commandString):
        ...
        if not self.validCommandSet(commandString):
            self.logger.error(f'[{self.id}] unknown commands: {commandString}')
        ...
        self.logger.debug(logFormat.format(self.id,
                                           commandString,
                                           getData,
                                           minBytes,
                                           numberOfChunks,
