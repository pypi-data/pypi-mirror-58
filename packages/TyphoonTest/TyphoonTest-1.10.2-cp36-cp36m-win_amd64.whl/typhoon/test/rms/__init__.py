...
def window_rms(signal, window_length=None, max_frequency=1000, match_num_of_cycles_with_samples=True):
    ...
    max_frequency : float/int or Timedelta
        If 'window_length' is not defined, sets the maximum frequency to automatically detect the 'window_length' as one period of the signal's strongest frequency. Default: 1000
    match_num_of_cycles_with_samples: bool
        If 'window_length' is not defined, tries to match the 'window_length' with an integer number of cycles of the strongest frequency. Default: True
    Returns
    -------
    rms : Series
