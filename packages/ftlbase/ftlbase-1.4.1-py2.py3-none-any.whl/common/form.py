...
from table.columns import DatetimeColumn, Column, ActionColumn, FontAwesomeLink, LinkColumn
...
class VersionTable(Table):
    data = DatetimeColumn(header=u'Data Alteração', field='revision.date_created', format='%d/%m/%Y %H:%M:%S')
    usuario = Column(header=u'Usuário', field='revision.user.username')
