...
#     """ Analisa se o usuário está logado e tem permissão para a ação correspondente """
#     if has_permission(request, clsModel, acao, permission, raise_exception):
#         return True
#     if raise_exception:
...
def commonCadastro(request, acao=ACAO_VIEW, formModel=None, goto='index', **kwargs):
    ...
    has_permission(request=request, model=clsModel, acao=acao, permission=permission)
...
def commonListaTable(request, model=None, queryset=None, goto='index', template_name="table.html", tableScript=None,
    ...
    has_permission(request=request, model=clsTable, acao=ACAO_VIEW, permission=permission)
...
def relatorio(request, *args, **kwargs):
    ...
    if configuraButtons:
        configuraButtonsNoForm(form, ACAO_REPORT, goto=home, workitem=None)
...
def versionViewCompare(request, pk=None, *args, **kwargs):
    ...
    if not has_permission(request, model=Version, acao=ACAO_VIEW, permission=permission):
...
def executeUseCase(request, pk=None, goto='', formModel=None, msg='ERROR', use_case=None, goto_error='ERROR',
                   *args, **kwargs):
    """
    ...
    :param msg: mensagem que poderá aparecer na tela com uma observação qualquer
    :param goto_error: para onde vai se houver erro na execução do use case
    """
    ...
    configuraButtonsNoForm(form, acao, goto, None)
