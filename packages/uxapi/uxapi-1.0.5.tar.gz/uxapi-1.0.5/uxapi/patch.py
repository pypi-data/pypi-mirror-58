...
class UXPatch:
    ...
    def request(self, path, api='public', method='GET',
        ...
        params = params or {}
        sp = self.get_service_provider('request')
        if sp:
            return sp(self, path, api, method, params, headers, body)
        else:
            return self._request(path, api, method, params, headers, body)

    def _request(self, path, api, method, params, headers, body):
        self.lastRestRequestTimestamp = self.milliseconds()
        request = self.sign(path, api, method, params, headers, body)
        if self.enableRateLimit:
            url = URL(request['url'])
            self.throttle(url.scheme, url.host, url.path)
        return self.fetch(request['url'], request['method'], request['headers'],
                          request['body'])

    def throttle(self, scheme, host, path):
        sp = self.get_service_provider('throttle')
        if sp:
            sp(self, scheme, host, path)
        else:
            self._throttle(scheme, host, path)

    def _throttle(self, scheme, host, path):
        super().throttle()
