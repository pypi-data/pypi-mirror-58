...
def import_auth():
 try:
  ...
  raise pxLwa('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=pxLwt)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=pxLwy)
def get_params(args,names=[],mandatory=pxLwt):
 result=()
 for name in names:
  value=pxLwl
  if name in args:
  ...
  if not value and mandatory:
   raise pxLwa('Please provide %s=... parameter'%name)
  result+=(value)
 if pxLwP(result)==1:
  return result[0]
 return result
