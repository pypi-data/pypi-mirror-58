...
def import_auth():
 try:
  ...
  raise IUNAk('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=IUNAx)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=IUNAd)
def get_params(args,names=[],mandatory=IUNAx):
 result=()
 for name in names:
  value=IUNAz
  if name in args:
  ...
  if not value and mandatory:
   raise IUNAk('Please provide %s=... parameter'%name)
  result+=(value)
 if IUNAc(result)==1:
  return result[0]
 return result
