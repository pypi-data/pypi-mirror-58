...
def import_auth():
 try:
  ...
  raise JThrM('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=JThrc)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=JThrk)
def get_params(args,names=[],mandatory=JThrc):
 result=()
 for name in names:
  value=JThrs
  if name in args:
  ...
  if not value and mandatory:
   raise JThrM('Please provide %s=... parameter'%name)
  result+=(value)
 if JThrC(result)==1:
  return result[0]
 return result
