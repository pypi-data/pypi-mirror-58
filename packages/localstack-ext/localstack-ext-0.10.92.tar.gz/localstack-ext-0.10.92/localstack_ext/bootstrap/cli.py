...
def import_auth():
 try:
  ...
  raise rFLmh('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=rFLmU)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=rFLmX)
def get_params(args,names=[],mandatory=rFLmU):
 result=()
 for name in names:
  value=rFLmY
  if name in args:
  ...
  if not value and mandatory:
   raise rFLmh('Please provide %s=... parameter'%name)
  result+=(value)
 if rFLmx(result)==1:
  return result[0]
 return result
