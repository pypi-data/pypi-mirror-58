...
def import_auth():
 try:
  ...
  raise fmKda('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=fmKdp)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=fmKds)
def get_params(args,names=[],mandatory=fmKdp):
 result=()
 for name in names:
  value=fmKdq
  if name in args:
  ...
  if not value and mandatory:
   raise fmKda('Please provide %s=... parameter'%name)
  result+=(value)
 if fmKdi(result)==1:
  return result[0]
 return result
