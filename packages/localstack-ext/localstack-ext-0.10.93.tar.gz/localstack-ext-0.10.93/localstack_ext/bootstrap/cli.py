...
def import_auth():
 try:
  ...
  raise DUeyJ('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=DUeyP)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=DUeyg)
def get_params(args,names=[],mandatory=DUeyP):
 result=()
 for name in names:
  value=DUeys
  if name in args:
  ...
  if not value and mandatory:
   raise DUeyJ('Please provide %s=... parameter'%name)
  result+=(value)
 if DUeyk(result)==1:
  return result[0]
 return result
