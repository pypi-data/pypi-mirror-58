...
def import_auth():
 try:
  ...
  raise LMzdk('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=LMzdb)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=LMzdg)
def get_params(args,names=[],mandatory=LMzdb):
 result=()
 for name in names:
  value=LMzdX
  if name in args:
  ...
  if not value and mandatory:
   raise LMzdk('Please provide %s=... parameter'%name)
  result+=(value)
 if LMzdf(result)==1:
  return result[0]
 return result
