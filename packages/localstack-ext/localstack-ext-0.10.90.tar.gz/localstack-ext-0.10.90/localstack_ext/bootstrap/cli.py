...
def import_auth():
 try:
  ...
  raise MzCyc('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=MzCyf)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=MzCym)
def get_params(args,names=[],mandatory=MzCyf):
 result=()
 for name in names:
  value=MzCyG
  if name in args:
  ...
  if not value and mandatory:
   raise MzCyc('Please provide %s=... parameter'%name)
  result+=(value)
 if MzCyB(result)==1:
  return result[0]
 return result
