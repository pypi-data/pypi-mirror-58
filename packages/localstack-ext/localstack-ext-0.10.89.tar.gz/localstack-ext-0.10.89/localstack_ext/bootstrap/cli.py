...
def import_auth():
 try:
  ...
  raise HlkyD('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=HlkyS)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=Hlkyg)
def get_params(args,names=[],mandatory=HlkyS):
 result=()
 for name in names:
  value=HlkyW
  if name in args:
  ...
  if not value and mandatory:
   raise HlkyD('Please provide %s=... parameter'%name)
  result+=(value)
 if HlkyB(result)==1:
  return result[0]
 return result
