...
def remma_epiDD_pair_cpu(y, xmat, gmat_lst, var_com, bed_file, snp_pair, out_file='remma_epiDD_pair_cpu'):
    """
    加加上位检验
    :param y: 表型
    :param xmat: 固定效应设计矩阵
    ...
    :param var_com: 方差组分
    :param bed_file: plink文件
    :param snp_pair: SNP互作对，最小值为0，最大值为num_snp-1
    :param out_file: 输出文件
    :return:
    """
    logging.info("计算V矩阵及其逆矩阵")
    ...
    cpu_t1 = time.process_time()
    logging.info("Running time: Clock time, {:.5f} sec; CPU time, {:.5f} sec.".format(clock_t1 - clock_t0, cpu_t1 - cpu_t0))
    np.savetxt(out_file, res, header='snp_0 snp_1 eff var chi p', comments='')
    return res
