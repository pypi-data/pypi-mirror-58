...
async def convert_time(windows_time):
...
async def create_players(player_info, events):
...
async def create_event(game, event, protocol, summary_stats):
    object_events = [
    ...
    if event['_event'] in object_events:
        current_event = await ObjectEvent.create(protocol, summary_stats, game, event)

    elif event['_event'] in ability_events:
        current_event = await AbilityEvent.create(summary_stats, game, event)

    elif event['_event'] == 'NNet.Game.SSelectionDeltaEvent':
        current_event = await SelectionEvent.create(game, event)

    elif event['_event'] == 'NNet.Game.SControlGroupUpdateEvent':
        current_event = await ControlGroupEvent.create(game, event)

    elif event['_event'] == 'NNet.Replay.Tracker.SUpgradeEvent':
        current_event = await UpgradeEvent.create(game, event)

    elif event['_event'] == 'NNet.Game.SCameraUpdateEvent':
        current_event = await CameraUpdateEvent.create(game, event)

    elif event['_event'] == 'NNet.Replay.Tracker.SPlayerStatsEvent':
        current_event = await PlayerStatsEvent.create(summary_stats, game, event)
