...
async def initial_summary_stats(game, metadata, detailed_info, local=False):
    summary_stats = {
        'mmr': {1: 0, 2: 0},
        'mmr_diff': {1: 0, 2: 0},
...
async def parse_replay(filename, *, local=False, detailed=False):
    try:
        ...
        players = await create_players(player_info, events)
    ...
    played_at = await convert_time(player_info['m_timeUTC'])
    ...
    summary_stats = await initial_summary_stats(current_game, metadata, detailed_info, local)
    ...
    for event in events:
        ...
        if gameloop <= game_length:
            current_event = await create_event(current_game, event, protocol, summary_stats)
            if current_event:
                # print(event)
                # print('\n')

                result = await current_event.parse_event()
    ...
    players_export = {}
    for player in players:
        summary_stats = await player.calc_pac(summary_stats, game_length)
