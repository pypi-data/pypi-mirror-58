...
class ObjectEvent(BaseEvent):
    @classmethod
    async def create(cls, protocol, summary_stats, *args):
        await super().create(*args)
        self = ObjectEvent()
        self.protocol = protocol
        self.summary_stats = summary_stats

    async def _get_or_create_game_object(self):
        event = self.event
    ...
    async def _update_obj_group_info(self, obj):
    ...
    async def parse_event(self):
        obj = await self._get_or_create_game_object()
        player = self.player
        event = self.event
        ...
        if self.type == 'NNet.Replay.Tracker.SUnitInitEvent':
            ...
            if 'Egg' in old_name or 'Cocoon' in old_name:
                await self._update_obj_group_info(obj)
