...
class SelectionEvent(BaseEvent):
    ...
    async def _add_to_selection(self, ctrl_group_num, new_obj_ids):
        if ctrl_group_num:
    ...
    async def _handle_zero_indices(self, ctrl_group_num, *, selection_type):
        """
    ...
    async def _handle_one_indices(self, ctrl_group_num, *, selection_type):
        """
    ...
    async def _create_bitmask(self, mask_x, mask_y, length):
    ...
    async def _handle_mask(self, ctrl_group_num, *, selection_type):
        """
        new:
    ...
    async def _handle_none(self, ctrl_group_num, *, selection_type):
        """
    ...
    async def _handle_new_selection(self, ctrl_group_num):
        """
        ...
        if 'ZeroIndices' in event['m_delta']['m_removeMask']:
            await self._handle_zero_indices(ctrl_group_num, selection_type='new')

        elif 'OneIndices' in event['m_delta']['m_removeMask']:
            await self._handle_one_indices(ctrl_group_num, selection_type='new')

        elif 'Mask' in event['m_delta']['m_removeMask']:
            await self._handle_mask(ctrl_group_num, selection_type='new')

        elif 'None' in event['m_delta']['m_removeMask']:
            await self._handle_none(ctrl_group_num, selection_type='new')

    async def _handle_subselection(self, ctrl_group_num):
        """
        ...
        if 'ZeroIndices' in event['m_delta']['m_removeMask']:
            await self._handle_zero_indices(ctrl_group_num, selection_type='sub')

        elif 'OneIndices' in event['m_delta']['m_removeMask']:
            await self._handle_one_indices(ctrl_group_num, selection_type='sub')

        elif 'Mask' in event['m_delta']['m_removeMask']:
            await self._handle_mask(ctrl_group_num, selection_type='sub')
            ...
            if event['m_delta']['m_addSubgroups']:
                for unit in event['m_delta']['m_addSubgroups']:
                    if unit['m_unitLink'] == 125:
                        return
                await self._handle_new_selection(ctrl_group_num)
            else:
                await self._handle_subselection(ctrl_group_num)
