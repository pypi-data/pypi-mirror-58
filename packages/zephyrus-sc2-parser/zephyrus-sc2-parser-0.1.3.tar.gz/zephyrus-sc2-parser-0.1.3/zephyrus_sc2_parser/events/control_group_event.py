...
class ControlGroupEvent(BaseEvent):
    ...
    async def _set_obj_group_info(self, ctrl_group_num):
    ...
    async def _remove_obj_group_info(self, ctrl_group_num):
        ctrl_group = self.player.control_groups[ctrl_group_num]
    ...
    async def _copy_from_selection(self, target, selection):
        for obj in selection:
            if obj not in target:
                target.append(obj)

    async def _add_to_group(self, ctrl_group_num):
    ...
    async def _remove_from_group(self, ctrl_group_num):
        """
        new:
        ...
        bitmask = await self._create_bitmask(mask_x, mask_y, length)
    ...
    async def parse_event(self):
        ...
        if event['m_controlGroupUpdate'] == 0:
            ...
            await self._copy_from_selection(control_group, player.current_selection)
            await self._set_obj_group_info(ctrl_group_num)

        if event['m_controlGroupUpdate'] == 1:
            if ctrl_group_num not in player.control_groups:
                player.control_groups[ctrl_group_num] = []
            if 'Mask' in event['m_mask']:
                await self._remove_from_group(ctrl_group_num)
            else:
                await self._add_to_group(ctrl_group_num)
            await self._set_obj_group_info(ctrl_group_num)
        ...
        if event['m_controlGroupUpdate'] == 2:
            ...
            if ctrl_group_num in player.control_groups:
                ...
                control_group = []
            await self._copy_from_selection(player.current_selection, control_group)
