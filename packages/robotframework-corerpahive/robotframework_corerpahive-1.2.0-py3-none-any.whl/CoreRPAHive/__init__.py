...
from CoreRPAHive.Util.CustomSelenium import CustomSelenium
from CoreRPAHive.ExcelParser import ParserContext
from CoreRPAHive.ExcelParser.DefaultParserStrategy import DefaultParserStrategy
from CoreRPAHive.Util.OpenpyxlHelper import OpenpyxlHelper
from CoreRPAHive.base.robotlibcore import keyword
from CoreRPAHive.ExcelParser.ExcelTestDataService import ExcelTestDataService
from CoreRPAHive.ExcelTestDataRow.TestStatus import TEST_STATUSES
from CoreRPAHive.Keywords.CoreExcelKeywords import CoreExcelKeywords
from CoreRPAHive.Config.CaptureScreenShotOption import CaptureScreenShotOption
...
class CoreRPAHive:
    """
    CoreRPAHive is a Robotic Process Automation library (RPA) for RobotFramework that allow the developer create RPA script easier and reduce complexity under robot script layer.
    ...
    def __init__(self, file=None, custom_parser=None, capture_screenshot='Always', manually_test=False, validate_data_only=False):
        """CoreRPAHive can be imported with several optional arguments.
    ...
    def start_suite(self, suite, result):
        ...
        try:
            shutil.rmtree(CoreRPAHive.REPORT_PATH)
        except:
    ...
    def end_suite(self, name, attributes):
        ...
        if self.file is not None:
            self.excelTestDataService.save_report(CoreRPAHive.REPORT_PATH+'/summary_report.xlsx')
        # Clear previous data
        self.file = None
        self.suite_source = None
        self.template_test = None
    ...
    def _capture_screenshot(self):
        try:
            ...
            try:
                os.makedirs('./' + CoreRPAHive.REPORT_PATH + '/screenshots/')
            except:
                None
            try:
                custom_selenium.capture_full_page_screenshot('./'+CoreRPAHive.REPORT_PATH+'/screenshots/'+screenshot_name)
    ...
    def merged_excel_report(self, data_type='DefaultParserStrategy'):
        ...
        reports = list(glob.iglob(os.path.join(CoreRPAHive.REPORT_PATH, '*.xlsx')))
        if len(reports) == 0:
            raise IOError('No report xlsx found under "' + CoreRPAHive.REPORT_PATH + '" folder')
        ...
        summary_file = os.path.join(CoreRPAHive.REPORT_PATH, 'summary_report.xlsx')
