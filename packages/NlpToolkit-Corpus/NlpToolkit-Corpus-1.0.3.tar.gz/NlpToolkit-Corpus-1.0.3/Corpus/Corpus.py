...
class Corpus:
    ...
    def __init__(self, fileName=None):
        ...
        if fileName is not None:
            ...
            for line in lines:
                self.addSentence(Sentence(line))
    ...
    sentenceSplitter : SentenceSplitter
        SentenceSplitter type input.
    """
    def initWithSentenceSplitter(self, fileName: str, sentenceSplitter: SentenceSplitter):
        self.sentences = []
        self.paragraphs = []
        self.wordList = CounterHashMap()
        self.fileName = fileName
    ...
    languageChecker : LanguageChecker
        LanguageChecker type input.
    """
    def initWithLanguageChecker(self, fileName: str, languageChecker: LanguageChecker):
        self.sentences = []
        self.paragraphs = []
        self.wordList = CounterHashMap()
        self.fileName = fileName
        if fileName is not None:
            ...
            for line in lines:
                sentence = Sentence()
                self.addSentence(sentence.initWithLanguageChecker(line, languageChecker))
    ...
    ----------
    s : Sentence
        Sentence type input that will be added to sentences list and its words will be added to wordList CounterHashMap.
    """
    ...
    """
    The getTrainCorpus method takes two integer inputs foldNo and foldCount for determining train data size and count 
    of fold respectively. Initially creates a new empty Corpus, then finds the sentenceCount as N. Then, starting from 
    the index 0 it loops through the index (foldNo * N) / foldCount and add each sentence of sentences list to new 
    Corpus. Later on, starting from the index ((foldNo + 1) * N) / foldCount, it loops through the index N and add each 
    ...
    fold respectively. Initially creates a new empty Corpus, then finds the sentenceCount as N. Then, starting from the 
    index (foldNo * N) / foldCount it loops through the index ((foldNo + 1) * N) / foldCount and add each sentence of 
    sentences list to new Corpus.
