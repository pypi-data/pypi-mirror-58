...
class TurkishSplitter(SentenceSplitter):
    """
    The listContains method has a String array shortcuts which holds the possible abbreviations that might end with a
    '.' but not a sentence finisher word. It also takes a String as an input and loops through the shortcuts array and
    returns true if given String has any matching item in the shortcuts array.
    ...
    def listContains(self, currentWord: str) -> bool:
    ...
    the input line with " " and SEPARATORS ({@literal ()[]{}"'״＂՛}) and increment i by one until a mismatch or end of 
    line.
    ...
    def isNextCharUpperCaseOrDigit(self, line: str, i: int) -> bool:
    ...
    def isPreviousWordUpperCase(self, line: str, i: int) -> bool:
    ...
    def isNextCharUpperCase(self, line: str, i: int) -> bool:
        while i < len(line) or line[i] == ' ':
            i = i + 1
    ...
    def isNameShortcut(self, currentWord: str) -> bool:
    ...
    """
    The repeatControl method takes a String word as an input, and a boolean exceptionMode and compress the repetitive 
    chars. With the presence of exceptionMode it directly returns the given word. Then it declares a counter i and 
    loops till the end of the given word. It compares the char at index i with the char at index (i+2) if they are 
    equal then it compares the char at index i with the char at index (i+1) and increments i by one and returns 
    concatenated result String with char at index i.
    ...
    def repeatControl(self, word: str, exceptionMode: bool) -> str:
        if exceptionMode:
            return word
        i = 0
        result = ""
    ...
    âàáäãèéêëíîòóôûúqwxÂÈÉÊËÌÒÛQWX and  0123456789. Then, it returns true if the result of contains method which checks 
    the existence of previous char and next char at apostropheLetters returns true, returns false otherwise.
    ...
    def isApostrophe(self, line: str, i: int) -> bool:
        apostropheLetters = TurkishLanguage.LETTERS + TurkishLanguage.EXTENDED_LANGUAGE_CHARACTERS + TurkishLanguage.DIGITS
        if i + 1 < len(line):
    ...
    def numberExistsBeforeAndAfter(self, line: str, i: int) -> bool:
    ...
    def isTime(self, line: str, i: int) -> bool:
        if i + 2 < len(line):
            previousChar = line[i - 1]
            nextChar = line[i + 1]
            twoNextChar = line[i + 2]
            return previousChar in TurkishLanguage.DIGITS and nextChar in TurkishLanguage.DIGITS and twoNextChar in TurkishLanguage.DIGITS
        else:
            return False

    """
    The split method takes a String line as an input. Firstly it creates a new sentence as currentSentence a new list
    ...
    . and currentWord is a shortcut or an abbreviation : assigns currentWord as currentWord+char(i) and adds currentWord 
    to currentSentence. Ex : bkz.
    ...
    If the char at index i is -,  add word to currentSentence and add sentences when the wordCount of currentSentence 
    greater than 0.
    ...
    def split(self, line: str) -> list:
        ...
        while i < len(line):
            if line[i] in SentenceSplitter.SEPARATORS:
                if line[i] == '\'' and currentWord != "" and self.isApostrophe(line, i):
                    currentWord = currentWord + line[i]
                else:
                    if currentWord != "":
                        currentSentence.addWord(Word(self.repeatControl(currentWord, webMode or emailMode)))
                    ...
                    if line[i] == '"' and bracketCount == 0 and specialQuotaCount == 0 and curlyBracketCount == 0 and \
                            roundParenthesisCount == 0 and quotaCount == 0 and self.isNextCharUpperCaseOrDigit(line,
                                                                                                               i + 1):
                ...
                if line[i] in SentenceSplitter.SENTENCE_ENDERS:
                    ...
                    if line[i] == '.' and currentWord != "" and (
                        ...
                        if line[i] == '.' and (self.listContains(currentWord) or self.isNameShortcut(currentWord)):
                            currentWord = currentWord + line[i]
                            ...
                            if roundParenthesisCount == 0 and bracketCount == 0 and curlyBracketCount == 0 and \
                                ...
                                if i + 1 < len(line) and line[i + 1] == '\'' and apostropheCount == 1 and \
                                        self.isNextCharUpperCaseOrDigit(line, i + 2):
                                    ...
                                    if i + 2 < len(line) and line[i + 1] == ' ' and line[i + 2] == '\'' and \
                                            apostropheCount == 1 and self.isNextCharUpperCaseOrDigit(line, i + 3):
                                        currentSentence.addWord(Word("'"))
                                        i += 2
                                        sentences.append(currentSentence)
                                        currentSentence = Sentence()
                                    else:
                                        if self.isNextCharUpperCaseOrDigit(line, i + 1):
                                            sentences.append(currentSentence)
                                            currentSentence = Sentence()
                            currentWord = ""
                else:
                    if line[i] == ' ':
                        ...
                        if line[i] == '-' and not webMode and roundParenthesisCount == 0 and \
                                self.isNextCharUpperCase(line, i + 1) and not self.isPreviousWordUpperCase(line, i - 1):
                            ...
                            quotaCount = 0
                            specialQuotaCount = 0
                            if currentWord != "" and currentWord.matches("\\d+"):
                                currentSentence.addWord(Word(currentWord + " -"))
                            else:
                                currentSentence.addWord(Word("-"))
                            currentWord = ""
                            ...
                            if line[i] in SentenceSplitter.PUNCTUATION_CHARACTERS or \
                                ...
                                if webMode:
                                    ...
                                    if line[i] == ',' and self.numberExistsBeforeAndAfter(line, i):
                                        currentWord = currentWord + line[i]
                                    else:
                                        if line[i] == ':' and self.isTime(line, i):
                                            currentWord = currentWord + line[i]
                                        else:
                                            if line[i] == '-' and self.numberExistsBeforeAndAfter(line, i):
                                                currentWord = currentWord + line[i]
                                                ...
                                                if currentWord != "":
                                                    currentSentence.addWord(
                                                        Word(self.repeatControl(currentWord, webMode or emailMode)))
                                                currentSentence.addWord(Word("" + line[i]))
                                                currentWord = ""
                            else:
                                if line[i] == '@':
