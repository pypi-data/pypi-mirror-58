...
def make_figure(panels=[], title='', figsize=(8, 5), write='', xmax=None, legend_outside=False):
    '''
    :param panels: panels to visualise in the plot
    :type panels: list of :py:class:`heppy.panel`
    ...
    for i, p in enumerate(panels):
        ...
        for histogram in p.bands:
            ...
            color = plot_attributes.get('color', '0.5')
            artists[histogram.name] = axes[i].fill_between(*histogram.errorband(), facecolor='none', edgecolor=color, alpha=0.5, linewidth=0, hatch='xxxxxx', label=plot_attributes['label'])
            # TODO: give bands different colours and hatches


        pointshift = 0. # points may be shifted horizonally consecutively to prevent them from overlapping
        for histogram in p.points:
            ...
            if np.count_nonzero(errorbars[0]) == 0 and np.count_nonzero(errorbars[1]) == 0:
                ...
                artists[histogram.name] = axes[i].plot(*histogram.points(pointshift), label=label, linestyle='None', marker=marker, color=color, markersize=markersize)
            else:
                artists[histogram.name] = axes[i].errorbar(*histogram.points(pointshift), yerr=errorbars, label=label, linestyle='None', marker=marker, color=color, markersize=markersize, elinewidth=1, zorder=9999)
