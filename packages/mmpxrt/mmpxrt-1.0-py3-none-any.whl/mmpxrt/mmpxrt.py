...
### Simulation parameters
    ...
    simulation['numraysB']=1e5 #Number of rays for the monocrhomatic simulation.
    simulation['numraysM']=1e5 #Number of rays for the broadband simulation.
...
## SPECTRUM OF THE SOURCE
        if (s['useBeamSpectrum']):
                minE=beamSpectrum(1,1)
                maxE=beamSpectrum(size(beamSpectrum,1),1)
                Er=0
                while True:
                    Er=random.random()*(maxE-minE)+minE
                    pEr=np.interp(Er,beamSpectrum[:,1],beamSpectrum[:,2],);
...
##################    
    ...
    ii=ii+ '  - energy resolution:  {:2.2f}'.format(EresolutionR)+ ' eV'+ nl
    ii=ii+ 'vertical spread from fwhm:  {:2.3f}'.format(p['evalu']['verticalSpreadFWHM']) +' mm '+ nl
    ii=ii+ '  - energy resolution:  {:2.2f}'.format( EresolutionF) +' eV'+ nl
    
    
    ii=ii+ 'vert. spr. narrow (fwhm) :  {:2.3f}'.format( p['evalu']['verticalSpreadFWHMNarrow']) +' mm '+ nl
    ii=ii +'  - energy resolution:  {:2.2f}'.format( EresolutionFnarrow)+ ' eV' +nl
...
def drawPSF(p,rrr,ax):
    ...
    if np.size(p['evalu']['verticalSelectPSF'])>0:
...
#draw Detector
    ...
    detY=sg['Sdetector'][1];
    detZ=sg['Sdetector'][2];
    detdx=np.sin(sg['SdetectorAngle'])*sg['windowX'];
    detdz=np.cos(sg['SdetectorAngle'])*sg['windowY'];
    windowX=sg['windowX']
    windowY=sg['windowY']
