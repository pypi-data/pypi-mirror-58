...
def weibull_dp_to_x(dp, threshold, slope, lower_asymptote, lapse_rate):
    num = scipy.stats.norm.cdf(scipy.stats.norm.ppf(lower_asymptote) + dp) - 1 - lapse_rate
    denom = -1 + lower_asymptote + lapse_rate
    x = np.log10(-np.log(num / denom)) / slope + threshold
    return x

x = weibull_dp_to_x(dp=1,
                    threshold=param_estimates_mean['threshold'],
                    lower_asymptote=param_estimates_mean['lower_asymptote'],
                    lapse_rate=param_estimates_mean['lapse_rate'],
                    slope=param_estimates_mean['slope'])
