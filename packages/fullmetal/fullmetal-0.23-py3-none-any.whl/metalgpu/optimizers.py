...
class StochasticGradientDescent():
    ...
    def update(self, w):
        ...
        if self.w_updt is None:
            self.w_updt = cp.zeros(cp.shape(w.data))
        # Use momentum if set
...
class Adam():
    ...
    def update(self, w):
        # If not initialized
        if self.m is None:
            self.m = cp.zeros(cp.shape(w.grad.data))
            self.v = cp.zeros(cp.shape(w.grad.data))
        ...
        self.w_updt =  m_hat * self.learning_rate  / (cp.sqrt(v_hat.data) + self.eps)
