...
class Conv2D(Layer):
    __slots__ =('n_filters','filter_shape','padding','stride','trainable','seed','w','b','w_opt','b_opt','type','layer_input','X_col','W_col', 'add_params')
    ...
    def set_params(self,weights,biase,optimizer=None):
        self.add_params=True

        if self.trainable == True:
            self.w = Parameter(weights)
            self.b = Parameter(biase)
        if self.trainable == False:
            self.w = Parameter(weights,False)
            self.b = Parameter(biase,False)
        # Weight optimizers
