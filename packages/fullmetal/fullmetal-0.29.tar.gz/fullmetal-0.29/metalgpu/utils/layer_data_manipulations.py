...
def determine_padding(filter_shape, output_shape="same"):
    ...
    if output_shape == "valid":
        return (0, 0), (0, 0)
    # Pad so that the output shape is the same as icput shape (given that stride=1)
...
# Method which turns the image shaped icput to column shape.
...
# Method which turns the column shaped icput to image shape.
...
def column_to_image(cols, images_shape, filter_shape, stride, output_shape='same'):
    ...
    cp.add.at(images_padded, (slice(None), k, i, j), cols)
