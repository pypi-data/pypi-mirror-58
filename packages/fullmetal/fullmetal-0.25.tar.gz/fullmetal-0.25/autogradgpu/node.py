...
def ensure_array(arrayable: Arrayable) -> cp.ndarray:
    if isinstance(arrayable, cp.ndarray):
        arrayable =  arrayable.astype(cp.float32, copy=False)
        return arrayable
    else:
        return cp.array(arrayable).astype(cp.float32, copy=False)


Nodeable = Union["Node", float, cp.ndarray]
...
class Node(object):
    ...
    def __init__(self,data: Arrayable,requires_grad: bool = False,depends_on: List[Dependency] = None, id = None, name=None) -> None:
        ...
        self.name = name
        if id is None:
            id = cp.random.randint(0, 100_000)
        self.id = id
    ...
    def zero_grad(self) -> None:
        self.type = type(self)
        self.grad = self.type(cp.zeros_like(self.data, dtype=cp.float32),requires_grad=False)
