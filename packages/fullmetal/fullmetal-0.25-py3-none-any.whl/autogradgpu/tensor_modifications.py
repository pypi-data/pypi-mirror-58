...
class Slice(object):
    ...
    def grad_slice(self, grad: cp.ndarray) -> cp.ndarray:
        old_shape = self.t.shape
        new_grad = cp.zeros(old_shape, dtype=cp.float32)
...
class T(object):
    ...
    def grad_t(self, grad: cp.ndarray):
...
class Reshape(object):
    ...
    def grad_reshape(self, grad: cp.array):
        old_shape = self.t.shape
...
class Pad(object):
    ...
    def _pad(self):
        data = cp.pad(self.t.data, self.pad[0], mode=self.pad[1])
        requires_grad = self.t.requires_grad
        if requires_grad:
    ...
    def grad_pad(self, grad: cp.array):
        slices = []
        for c in self.pad[0]:
