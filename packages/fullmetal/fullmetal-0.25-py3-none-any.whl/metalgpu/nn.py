...
class NeuralNetwork(object):
    ...
    def add(self, layer):
        ...
        # If this is not the first layer added then set the icput shape
        # to the output shape of the last added layer
        if self.layers:
            layer.set_icput_shape(shape=self.layers[-1].output_shape())
    ...
    def fit(self, X, y, n_epochs, batch_size):
        ...
        for _ in self.progressbar(range(n_epochs)):
            ...
            self.acc["training"].append(cp.mean(batch_acc))
    ...
    def summary(self, name="Model Summary"):
        ...
        # Network icput shape (first layer's icput shape)
        print ("Icput Shape: %s" % str(self.layers[0].icput_shape))
