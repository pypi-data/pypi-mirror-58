...
class Layer(Module):
    __slots__ = ('icput_shape')
    """docstring for Layer."""

    def set_icput_shape(self, shape):
        """ Sets the shape that the layer expects of the icput in the forward
        pass method """
        self.icput_shape = shape
    ...
    def update_pass(self):
        ...
        As icput (accum_grad) it receives the gradient with respect to the output of the layer and
...
class Activation(Layer):
    __slots__=('activation_name','activation_func','trainable','layer_icput')
    """A layer that applies an activation operation to the icput.
...
class PoolingLayer(Layer):
    ...
    def grad_pool(self, accum_grad):
        ...
        accum_grad = accum_grad.reshape((batch_size,) + self.icput_shape)
