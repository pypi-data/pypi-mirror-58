...
def image_resize(__path__,__factor__=1,__resized_path__=''):
    '''Convierte fichero de imagen a un tamaño diferente, ademas
       es posible cambiar el tipo de imagen de png, jpeg o gif a otro
       cuando se renombra el fichero de imagen resultante.
    '''
    try:
        #__path__ = __path__.lower()
        if os.path.exists(__path__):
            if __path__ and (__path__[-4:].lower() in IMAGE_ALLOWED_EXTENSIONS):
                ...
                if __factor__:
                    i = float(__factor__)
                    side = int(max_dim*i)
                    resized_image = im.resize((side, side), Image.ANTIALIAS)
                    if __resized_path__ and (__resized_path__[-4:].lower() in IMAGE_ALLOWED_EXTENSIONS):
                        resized_image.save(__resized_path__)
                        return __resized_path__
                    else:
                        return ''
                else:
                    return ''
...
if __name__ == "__main__":
    ...
    """
    img = qrcode.make("Datos a referenciar en un codigo qr")
