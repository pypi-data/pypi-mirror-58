        ...
        if len(_nif)>0:
            if _nif[0] in '0123456789':
                ...
                if __nif.strip().upper() == _nif:
                    ...
                    return ''.join([iso_country_2,'.',__nif])
            else:
                ...
                if _nif[0] in ALPHA_CIF_b:
                    if 7<len(_nif)<=9:
                        ...
                        if __nif.strip().upper() == _nif:
                            ...
                            LOG.debug(''.join([iso_country_2,'.',__nif]))
                            return ''.join([iso_country_2,'.',__nif])
        # identificacion fiscal ES incorrecta
        LOG.debug(''.join(['&',iso_country_2,'.',_nif]))
        return ''.join(['&',iso_country_2,'.',_nif])
...
def currencyExchangeEUR(dest=None):
    ...
    currency = urlopen(url, context=ssl.SSLContext()).read()
