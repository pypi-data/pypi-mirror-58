...
* You can also optionally include multiseries id columns (only one columns is supported currently). Multiseries id columns are used to add an ability to specify different sets of events for different series (e.g. holidays for different states in USA, holidays for different countries, etc.). 
* Value of series id might be absent for specific event in calendar file. This means that event is valid for all series in project dataset (e.g. New Year is a holiday in both USA and Mexico, see example bellow).
* If multiseries id columns are not provided all listed events will be applicable to all series in project dataset.
