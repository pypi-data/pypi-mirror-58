...
class SimpleDeasciifier(Deasciifier):
    ...
    First, it creates a str which consists of corresponding Latin versions of special Turkish characters. If given index
    is less than the length of given word and if the item of word's at given index is one of the chars of str, it loops
    given candidates list's size times and substitutes Latin characters with their corresponding Turkish versions
    and put them to newly created str modified. At the end, it adds each modified item to the candidates
    ...
    index : int     
        Integer input.
    """
    def generateCandidateList(self, candidates: list, word: str, index: int):
        s = "ıiougcsİIOUGCS"
        if index < len(word):
            if s.index(word[index]) != -1:
                size = len(candidates)
                for i in range(size):
                    if word[i] == "ı":
                        newChar = "i"
            ...
            self.generateCandidateList(candidates, word, index + 1)
    ...
    def candidateList(self, word: Word) -> list:
        ...
        self.generateCandidateList(candidates, word.getName(), 0)
        i = 0
