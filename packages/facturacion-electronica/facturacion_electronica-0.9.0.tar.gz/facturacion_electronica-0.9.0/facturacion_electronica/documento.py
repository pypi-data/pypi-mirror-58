...
llevar estrictamente las palabras Dice: y debe decir:')
    ...
    def Totales(self):
        ...
        if self.no_product:
            Totales['MntTotal'] = 0
            return Totales
        if not self.es_exportacion() and (not self.es_boleta() or not self.MntBruto):
    ...
    @property
    def Transporte(self):
        if not self.IndTraslado and not self.Aduana:
            return False
...
, en su ficha de receptor")
    ...
    def es_exportacion(self):
        if self.TipoDTE in [ 110, 111, 112 ]:
            return True
        return False
