...
class WatsonMachineLearningEngine:
    ...
    def __init__(self, credentials, is_v4=False):
        start = time.time()
        self._client = WatsonMachineLearningAPIClient(dict(credentials))
        self._is_v4 = is_v4
        if is_v4:
            self._set_or_create_space_id()
    ...
    def _reliable_create_space(self, space_name):
        ...
        logger.log_debug('TIMER: WML create space completed in {:.3f} seconds'.format(time.time() - start))
    ...
    def _reliable_set_space(self, space_id, space_name):
        ...
        logger.log_debug('TIMER: WML set space completed in {:.3f} seconds'.format(time.time() - start))

    def _set_or_create_space_id(self):
