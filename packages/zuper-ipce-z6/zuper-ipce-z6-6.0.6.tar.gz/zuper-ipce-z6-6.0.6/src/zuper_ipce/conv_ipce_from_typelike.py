...
from . import logger
from .constants import (ALL_OF, ANY_OF, ATT_PYTHON_NAME, CALLABLE_ORDERING, CALLABLE_RETURN, ID_ATT, IESO,
                        IPCE_PASS_THROUGH, JSC_ADDITIONAL_PROPERTIES, JSC_ARRAY, JSC_BOOL, JSC_DEFINITIONS,
                        JSC_DESCRIPTION, JSC_INTEGER, JSC_ITEMS, JSC_NULL, JSC_NUMBER, JSC_OBJECT, JSC_PROPERTIES,
                        JSC_PROPERTY_NAMES, JSC_REQUIRED, JSC_STRING, JSC_TITLE, JSC_TITLE_CALLABLE, JSC_TITLE_DATETIME,
                        ...
                        X_BINDINGS, X_CLASSATTS, X_CLASSVARS, X_EXTRA, X_ORDER, X_ORIG, X_PYTHON_MODULE_ATT)
...
def ipce_from_typelike_dataclass(T: TypeLike, c: IFTContext, ieso: IESO) -> TRE:
    ...
    #     logger.info(f'I did find __doc__ in {T.__name__} {id(T)}', T=T,
    #                 doc=T.__doc__)
    # else:
    #     logger.warning(f'Cannot find __doc__ in {T.__name__} {id(T)}', T=T,
    #                 doc=T.__doc__, dict=dict(T.__dict__))
    #     raise ValueError()
