...
def test_intersection1():
    ...
    # X = TypeVar("X")
    # Y = TypeVar("Y")
    D = make_Intersection((A, C))
    assert_type_roundtrip(D)


def test_complex_template1():
    X = TypeVar('X')
    Y = TypeVar('Y')
    Z = TypeVar('Z')
    ...
    D = C[bool]
    assert_type_roundtrip(D)
    assert_type_roundtrip(E)
    assert_type_roundtrip(E[C[Z]])


def test_corner2():
    X = TypeVar('X')
    @dataclass
    class ComputationSpec(Generic[X]):
        pass

    @dataclass
    class ComputationResult(Generic[X]):
        result: X

        computation_spec: ComputationSpec[X]


    assert_type_roundtrip(ComputationResult)
