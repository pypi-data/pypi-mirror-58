...
from typing import Any, cast, Dict, NewType, Optional, Set, TypeVar, Union
...
def test_ser1():
    # assert Person.__annotations__['address'] is Address
    Address_schema = ipce_from_typelike(Address)
    logger.info(id=id(Address), Address_schema=Address_schema)


    assert Address_schema['description'] == Address.__doc__

    Person_schema = ipce_from_typelike(Person)
    logger.info(Person_schema=Person_schema)
    assert_equal(Person_schema['properties']['address'], Address_schema)
...
def test_mixed_dict():
    ...
    symbols2 = {AL.__qualname__: AL}
    d = {AL(0, 0): "one", AL(1, 1): "two"}
    m = AM(d)
    assert_object_roundtrip(m, use_globals=symbols2)

    @dataclass
    class AM2:
        a: Dict[Union[int, AL], str]
