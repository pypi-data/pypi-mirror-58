...
class P2R:
    ...
    def _checkScript(self):
        ...
        for index, api in enumerate(apis):
            ...
            if _apiMethod == 'GET':
                self._code += params + \
                f"#{_apiName}#\n_{_apiReturnName} = get(url='{_apiPath}', headers={_apiHeader}).json()\n" + _apiTest
            elif _apiMethod == 'POST':
                try:
                    if isinstance(eval(_apiBody), dict):

                        self._code += params + f"#{_apiName}#\n" \
                        f"_{_apiReturnName} = post(url='{_apiPath}', json={eval(_apiBody)}, headers={_apiHeader}).json()\n" + _apiTest
                except:
                    self._code += params + f"#{_apiName}#\n" \
                    f"_{_apiReturnName} = post(url='{_apiPath}', data='{_apiBody}', headers={_apiHeader}).json()\n" + _apiTest
...
def main():
    para = ArgumentParser(description="欢迎使用小白科技之PostManScript转PythonCode【目前版本仅支持GET与POST方式】")
