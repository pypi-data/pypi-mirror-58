...
stem_version = {}
with open("stem/stem.version") as fp:
    exec(fp.read(), stem_version)
...
SUMMARY = 'Ignore this package. This is a version of Stem slightly patched to address #43 ' \
          '(https://github.com/torproject/stem/issues/43) enabling intergation with The Onion Box (www.theonionbox.com).'
...
try:
  distutils.core.setup(
    name = 'stem4tob-dry-run' if DRY_RUN else 'stem4tob',
    version = stem_version['__version__'],
    summary="Original stem - patched to operate with The Onion Box.",
    description = DRY_RUN_SUMMARY if DRY_RUN else SUMMARY,
    long_description = DESCRIPTION,
    license = stem_version['__license__'],
    author = stem_version['__author__'],
    author_email = stem_version['__contact__'],
    # maintainer='Ralph Wetzel',
    # maintainer_email='theonionbox@gmx.com',
    url = stem_version['__url__'],
    packages = ['stem', 'stem.client', 'stem.descriptor', 'stem.interpreter', 'stem.response', 'stem.util'],
    keywords = 'tor onion controller',
    scripts = ['tor-prompt'],
    package_data = {
      'stem': ['cached_fallbacks.cfg', 'cached_manual.sqlite', 'settings.cfg', 'stem.version'],
