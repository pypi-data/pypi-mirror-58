...
def picky_selection(available_downloads, logger):
	logger.log(
		"\"picky\" flag: Select which entries to download by providing their (comma-separated)"
		" indexes in the list below. Alternatively, enter 'A' to download (a)ll entries.",
...
def slizzy(track, modules, download_tracks, picky = False, fetch_limit = False):
	...
	if fetch_limit :
		logger.log(
			"\"fetch_limit\" flag: a maximum of {} {} will be fetched from each provider.".format(
	...
	if args.command in [ "dl", "lns" ]:
		for i, track in enumerate(args.tracks):
			try:
				...
				print(
					"Error: invalid track format '" + track + "'.",
					file = sys.stderr,
				)
				sys.exit(1)
