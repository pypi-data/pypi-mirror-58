...
def fetch(track, fetch_limit = False):
	...
	while isinstance(raw_entries, int): # Slider sometimes returns error codes instead of
		...
		page = requests.get(
			base_url + "/vk_auth.php",
			params = { "q" : track.query_string }
		)
...
def select(entries, track):
	entries = (
		...
		and entry.size
		and entry.bitrate in tolerance.bitrate # Filter by bitrate.
	)
