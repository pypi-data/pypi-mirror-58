...
def combine_earthstat_tifs_to_nc(tif_paths, nc_path):
    ...
    print(11, match.variables['primf'].__dict__)
    # print(22, match.variables['primf'].current_shape)
    ...
    print('lon_var[:]', lon_var[:].shape)
    print(222, match.variables['lon'][:].shape)
...
def combine_earthstat_tifs_to_nc_new(tif_paths, nc_path):
    ...
    for c, path in enumerate(tif_paths):
        ...
        print('var[:]', var[:].shape)
        print('current_array', current_array.shape)
...
def read_earthstat_nc_slice(input_nc_path, crop_name):
    ...
    # print(np.nansum(ds.ReadAsArray()[1000: 2500, 2000: 2500]))
    print(1, time.time() - start)
...
def dask_test(input_path):
    ...
    #print(f.keys()) #"""['bounds', 'c3ann', 'c3nfx', 'c3per', 'c4ann', 'c4per', 'lat', 'lat_bounds', 'lon', 'lon_bounds', 'pastr', 'primf', 'primn', 'range', 'secdf', 'secdn', 'secma', 'secmb', 'time', 'time_bnds', 'urban']"""
