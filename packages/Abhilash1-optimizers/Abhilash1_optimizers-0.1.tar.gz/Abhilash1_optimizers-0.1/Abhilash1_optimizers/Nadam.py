# -*- coding: utf-8 -*-
"""
Created on Mon Dec 30 17:33:56 2019
...
#Adamax varaiation of ADAM with L**p norm over L**2 norm
class NADAM():
    ...
    def Nadam_optimizer(data,len_data,max_itr,alpha,b_1,b_2,epsilon,noise_g,act_func,scale):
        alpha,b_1,b_2,epsilon,noise_g=NADAM.__init__(alpha,b_1,b_2,epsilon,noise_g)
        m_t,v_t,t,theta_0=NADAM.init(0,0,0,0)
        final_weight_vector=[]
        for i in range(len_data):
            ...
            for i in range(max_itr):
                ...
                v_hat=v_t/(1-b_2**t)
                m_cap=(1-b_1)*g_hat + b_1*m_hat
                theta_prev=theta_0
        
                alpha_t=(alpha*(m_cap/(math.sqrt(v_hat)+epsilon)))
    ...
    def initialize(data,max_itr):
        len_data=len(data)
        optimized_weights=NADAM.Nadam_optimizer(data,len_data,max_itr,alpha,b_1,b_2,epsilon,noise_g,act_func,scale)
...
if __name__=='__main__':
    ...
    NADAM.initialize(sample_data,100)    
