...
def _get_continuous_timeseries_with_meta(infile, drs, normalise, normalise_mean=None):
    ...
    return _do_stitching_and_normalisation(
        infile, parent_file_path, loaded, parent, normalise, normalise_mean
    )
...
def _get_parent_replacements(scmdf):
    ...
    if scmdf.get_unique_meta("mip_era", no_duplicates=True) == "CMIP5":
        ...
        replacements["parent_ensemble_member"] = replacements.pop("parent_experiment_rip")
...
def _do_stitching_and_normalisation(  # pylint:disable=too-many-locals,too-many-branches,too-many-statements
    infile, infile_parent, loaded, parent, normalise, normalise_mean
):
    cmip5 = loaded.get_unique_meta("mip_era", no_duplicates=True) == "CMIP5"
    bt_key = (
        "branch_time"
        if cmip5
        else "branch_time_in_parent"
    )
    if cmip5:
        # have to use parent file, info not in metadata
        parent_nc = netCDF4.Dataset(infile_parent)
        branch_time = netCDF4.num2date(
            loaded.metadata[bt_key],
            parent_nc.variables["time"].units,
            parent_nc.variables["time"].calendar
        )

    elif "BCC" in infile and not np.equal(loaded.metadata[bt_key], 0):
        ...
        branch_time = dt.datetime(int(loaded.metadata[bt_key]), 1, 1)
    else:
