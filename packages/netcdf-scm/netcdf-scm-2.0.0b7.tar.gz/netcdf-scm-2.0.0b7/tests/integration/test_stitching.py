...
def test_stitching_with_normalisation_two_levels_cmip5(
    tmpdir, caplog, test_marble_cmip5_crunch_output
):
    output_dir = str(tmpdir)
    ...
    normalisation = load_scmdataframe(
        glob.glob(os.path.join(test_marble_cmip5_crunch_output, "*", normalisation_path))[0]
    )

    assert parent.metadata["branch_time"] == 255135.0
    expected_norm_year_raw = 700
    ...
    _check_expected_values(res, child, range(2006, 2101), norm_shift=norm_shift)
    _check_expected_values(res, parent, range(1850, 2006), norm_shift=norm_shift)
