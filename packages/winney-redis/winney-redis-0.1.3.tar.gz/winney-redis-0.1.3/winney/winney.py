...
META_CACHE_KEY    = "WINNEY:META:{REQUEST_URL}:{PARAMS}"
CONTENT_CACHE_KEY = "WINNEY:CONTENT:{REQUEST_URL}:{PARAMS}"


class Result(object):

    def __init__(self, resp=None, url=None, method=None, data=None, json=None, cache_time=None, redis_conn=None):
    ...
    def set_cache(self, cache_time):
        ...
        params = json.dumps(self.params_data, sort_keys=True) if self.params_data else ""
        meta_key    = META_CACHE_KEY.format(REQUEST_URL=self.request_url, PARAMS=params)
        content_key = CONTENT_CACHE_KEY.format(REQUEST_URL=self.request_url, PARAMS=params)
    ...
    @classmethod
    def load_from_cache(cls, request_url, request_method, params, redis_conn):
        ...
        meta_key    = META_CACHE_KEY.format(REQUEST_URL=request_url, PARAMS=params)
        content_key = CONTENT_CACHE_KEY.format(REQUEST_URL=request_url, PARAMS=params)
...
class Winney(object):
    ...
    def _bind_func_url(self, url, method, cache_time=None):
        def req(data=None, json=None, files=None, headers=None, **kwargs):
            ...
            if cache_time:
                r = Result.load_from_cache(url2, method, data, self.redis_conn)
                if r:
                    return r
            r = self.request(method, url2, data, json, files, headers)
            return Result(r, url2, method, data, json, cache_time, self.redis_conn)
        return req
