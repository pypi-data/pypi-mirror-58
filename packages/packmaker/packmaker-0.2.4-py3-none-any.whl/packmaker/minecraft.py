...
class Library (object):
    ...
    def __init__(self, library):
        ...
        self.group, self.artifact, self.version = self.library['name'].split(':')

    def get_native_string(self):
        if 'natives' in self.library:
            return self.library['natives']['linux']
        return None

    def get_filename(self):
        nativestring = self.get_native_string()
        if nativestring:
            return '{}-{}-{}.jar'.format(self.artifact, self.version, nativestring)
        return '{}-{}.jar'.format(self.artifact, self.version)
    ...
    def matches_rule(self, rule, environment):
        if 'os' in rule:
            return rule['os']['name'] == 'linux'
        return True

    def must_extract(self):
        return 'extract' in self.library
