explorerjs = function () {
    ...
    class Explorer extends QDesktopApplication {
        ...
        init() {
            ...
            this.vm = this.init_vm({window:this.window});
        }
        ...
        init_vm(obj) {
            var {window,pan, location, root_path, loc_history}=obj;
            var vm = new Vue({
                el: `#${this.uid}`,
                delimiters: ['<%', '%>'],
                data: {
                ...
                created: function () {
                    ...
                    if (!this.pan&&!this.restoreData()) {
                        this.input("Input A url that I can host,default:http://127.0.0.1:8002/fs",(url)=>{
                            if(!url || url===''){
                                url="http://127.0.0.1:8002/fs";
                                var pan=new Pan(url);
                                this.openPath(pan);
                            }
                        })
