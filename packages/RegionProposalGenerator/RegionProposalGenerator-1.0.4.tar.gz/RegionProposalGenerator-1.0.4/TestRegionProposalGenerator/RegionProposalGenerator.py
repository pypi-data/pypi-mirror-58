...
__date__    = '2020-January-1'   
__url__     = 'https://engineering.purdue.edu/kak/distRPG/RegionProposalGenerator-1.0.4.html'
...
INTRODUCTION:
    ...
    processing.  In order to fulfill these objectives, the module provide a
    self-documenting implementation of the algorithms that should be
    relatively easy to change, rather than a highly optimized
    implementation that would be good for production work.  For high-speed
    production work, it would be best to use the implementations provided
    by the original authors of the algorithms that are incorporated in this
    module.

    At this point, the reader might ask: What is a region proposal?  To
    respond, let's say you want to detect objects in an image, but you
    neither know where exactly in the image it might be located, nor are
    you aware of the scale at which the object might have manifested
    itself.  Under these conditions, in the old days, at each scale of
    detection, you would run sliding window through the image, moving it
    from one pixel to the next, and, at position of the window, test
    whether the pixels inside the window speak to the presence of the
    object.  The main problem with this approach to object detection is
    that its computational overhead is much too high, especially when an
    image is mostly empty except perhaps for the presence of an object of
    interest somewhere.  Detecting objects in a 500x500 image at, say, 4
    different scales would involve a million windows.  Now imagine the case
    when at most one of thees windows might contain the object.  All of the
    computations in all the other windows would be wasted for the sake of
    finding that one window that may have the object in it.

    To get around this overhead, more recently research in object detection
    has taken to first creating region proposals in an image, these being
    pixel blobs that look different from the general background in the
    image, and then applying the object detection algorithm to just those
    regions.  This is the approach that was used in the first deep-learning
    based object-detection framework called R-CNN by Girshick, Donahue,
    Darrell, and Malik. Although this approach was subsequently abandoned
    in later variants starting with Fast R-CNN by using another CNN to form
    object proposals, it remains that not object detection problems allow
    for region proposals to be formed with CNNs.  That is, becoming
    familiar with the older non-learning based methods for constructing
    region proposals still has value.

    From an algorithmic standpoint, RegionProposalGenerator (RPG)
    implements elements of the Selective Search (SS) algorithm for object
    detection as proposed by Uijlings, van de Sande, Gevers, and Smeulders.
    ...
    the goal is merely to provide a platform for experimenting with the
    logic of Selective Search.  For highly efficient and optimized
    implementations of both the Selective Search algorithm and the
    graph-based image segmentation algorithm, the reader should use the
    original implementations as provided by the authors of these
    algorithms.

    The module first processes an image with the FH graph-based algorithm
    for image segmentation to divide an image into pixel blobs.  The module
    subsequently invokes elements of the SS algorithm to selectively merge
    the blobs on the basis of three properties: homogeneity of the color,
    color variance, and texture homogeneity.

    The FH algorithm is based on creating a graph based representation of
    an image that starts out by representing each pixel by a vertex.
    Initially, the edges in the graph represent the color differences
    between 8-adjacent pixels corresponding two different vertices.  The
    weight associated with an edge equals the color difference at the two
    pixels. Subsequently, for the vertex merging logic, the group of pixels
    represented by a vertex u is characterized by a property called Int(u),
    ...
    larger by using the logic of SS that allows adjacent blobs of pixels to
    merge into larger blobs provided doing so makes sense based on the
    inter-blob values for mean color levels, color variances, and texture
    values.
...
USAGE:
    ...
    this instance:
        ...
        print "\n\n\nNumber of region proposals after graph-based segmentation: ", len(segmented_graph[0])
        merged_blobs, color_map = rpg.selective_search_for_region_proposals( segmented_graph, image_name )
        print "\n\n\nNumber of region proposals after selective-search based merging: ", len(merged_blobs)
...
CONSTRUCTOR PARAMETERS: 
    ...
    Of the 9 constructor parameters listed below, the first 5 are meant for
    the FH algorithm and the last 3 for the SS algorithm.
    ...
    max_iterations: Sets an upper limit on the number of iterations of the
                    FH algorithm for image segmentation.
...
PUBLIC METHODS:
    ...
    (1)  selective_search_for_region_proposals()
         ...
         As mentioned elsewhere here, that algorithm sits on top of
         the graph based image segmentation algorithm that was
         proposed by Felzenszwalb and Huttenlocher.  The parameter
         'pixel_blobs' required by the method presented here is
         supposed to be the pixel blobs produced by the Felzenszwalb
         and Huttenlocher algorithm.

    (2)  graph_based_segmentation()

         This is an implementation of the Felzenszwalb and
         Huttenlocher algorithm for graph-based segmentation of
         images.  At the moment, it is limited to working on grayscale
         images.

    (3)  display_tensor_as_image()

         This method converts the argument tensor into a photo image
         that you can display in your terminal screen. It can convert
         tensors of three different shapes into images: (3,H,W),
         (1,H,W), and (H,W) into images, where H, for height, stands
         for the number of pixel in the vertical direction and W, for
         width, the same along the horizontal direction. When the
         first element of the shape is 3, that means that the tensor
         represents a color image in which each pixel in the (H,W)
         plane has three values for the three color channels.  On the
         other hand, when the first element is 1, that stands for a
    ...
    (4)  graying_resizing_binarizing()
         ...
         This is a demonstration of some of the more basic and
         commonly used image transformations from the
         torchvision.transformations module.  The large comments
         blocks are meant to serve as tutorial introduction to the
         syntax used for invoking these transformations.  The
         transformations shown can be used for converting a color
         image into a grayscale image, for resizing an image, for
         converting a PIL.Image into a tensor and a tensor back into
...
THE Examples DIRECTORY:
    ...
    All you have to do to reproduce the previously segmented results in those
    directories is to execute
    ...
    Python scripts:
        ...
        selective_search.py
            ...
            This is the same script that you will see in the four subdirectories
        ...
        interactive_graph_based_segmentation.py
            ...
            This interactive script can be used in two different
            modes: you can extract a portion of the image by dragging
            your mouse over the portion, or by clicking the mouse at
            the vertices of an imaginary polygon that defines the
            portion you would like to extract from the image.

        torchvision_some_basic_transformations.py    

            This is a demonstration of some of the more basic and
            commonly used image transformations from the
            torchvision.transformations module.  The transformations
            shown can be used for converting a color image into a
            grayscale image, for resizing an image, for converting a
            PIL.Image into a tensor and a tensor back into an
