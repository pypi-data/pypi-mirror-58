...
class TaskScheduler(object):
    ...
    def check_if_logging_tables_exists(self):
        ...
        new_table_query = 'CREATE TABLE IF NOT EXISTS `task_variables`(' \
                          ...
                          'temp_value VARCHAR(2500) NOT NULL,' \
                          'PRIMARY KEY (`id`),' \
                          'UNIQUE KEY `task_variables_id_uindex` (`id`),' \
                          'UNIQUE INDEX `task_variables_name_value_uindex` (`name`, `value`),' \
                          'CONSTRAINT task_variables_task_scheduler_id_fk' \
                          'FOREIGN KEY (task_id) REFERENCES beta_sc_salure.task_scheduler (id) ON DELETE CASCADE)' \
                          'ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci'
        self.mysql.raw_query(new_table_query)
    ...
    def db_variable(self, variable_name: str):
        ...
        if self.task_manual_started:
            response = self.mysql.select('task_variables', 'temp_value', f'WHERE name = \'{variable_name}\'')
        else:
            response = self.mysql.select('task_variables', 'value', f'WHERE name = \'{variable_name}\'')
        if len(response) == 0:
