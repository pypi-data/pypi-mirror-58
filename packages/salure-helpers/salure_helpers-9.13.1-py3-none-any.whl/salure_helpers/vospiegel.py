...
class VoSpiegel(object):
    ...
    def questions(self, api_key, inquiries: list):
        ...
        df = pd.DataFrame()
        for i in inquiries:
            response = requests.get(url=f'{self.url}questions', headers=header, params={'inquiry': i}).json()
    ...
    def results(self, api_key, inquiries: list):
        ...
        df = pd.DataFrame()
        for i in inquiries:
            response = requests.get(url=f'{self.url}result', headers=header, params={'inquiry': i}).json()
    ...
    def group_response(self, api_key, inquiries: list):
        ...
        df = pd.DataFrame()
        for i in inquiries:
            response = requests.get(url=f'{self.url}group-response', headers=header, params={'inquiry': i}).json()
            df_temp = pd.DataFrame(data=response['data'])
            df = pd.concat([df, df_temp])
        return df
    ...
    def labels(self, api_key, inquiries: list):
        ...
        df = pd.DataFrame()
        for i in inquiries:
            response = requests.get(url=f'{self.url}label', headers=header, params={'inquiry': i}).json()
            df_temp = pd.DataFrame(data=response['data'])
            df = pd.concat([df, df_temp])
        return df
