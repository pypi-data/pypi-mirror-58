...
class Resource:
    ...
    def get_qh_typed_words(self,used_types = ['IT', '动物', '医药', '历史人名', '地名', '成语', '法律', '财经', '食物']):
        """
        THUOCL：清华大学开放中文词库
    ...
    def terry_words(self,used_types = ['IT', '动物', '医药', '历史人名', '地名', '成语', '法律', '财经', '食物']):
        """
        定义分词
        :param used_types:
        :return: typed_words: 字典，键为类型，值为该类的词语组成的set
        ...
        """
        pwd = os.path.abspath(os.path.dirname(__file__))
        with open(pwd + "/resources/terry_words.json", "r", encoding="utf-8") as f:
            typed_words0 = json.load(f)
        typed_words = dict()
        for type0 in typed_words0:
            if type0 in used_types:
