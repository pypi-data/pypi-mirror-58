...
class Brizo:
    ...
    def execute_service(service_agreement_id, service_endpoint, account):
        ...
        signature = Keeper.get_instance().sign_hash(
            add_ethereum_prefix_and_hash_msg(service_agreement_id),
            account)
        execute_url = Brizo._create_execute_url(service_endpoint, service_agreement_id, account, signature)
    ...
    @staticmethod
    def _create_execute_url(service_endpoint, service_agreement_id, account,
                            signature=None):
        return (f'{service_endpoint}'
                f'?signature={signature}'
                f'&serviceAgreementId={service_agreement_id}'
