...
Usage:
  agcli.py [--empty] [--parsers=<names>] [--config-dir=<dir>] [--complete=<instr>] <cmd>
  agcli.py [--empty] [--parsers=<names>] [--config-dir=<dir>] [--complete=<instr>] <cmd> <sub-cmd>
  agcli.py (-h | --help)
...
Options:
  ...
  --parsers=<names>     The name of the parsers to use in order to analyze the documentation.
                        The possible values are 'explainshell' and 'docopt' [default: "docopt, explainshell, naive"].
  --config-dir=<dir>    The directory where the "agcli" package is installed. By default, this is the user directory.
  --complete=<instr>    Parse the "instr" instruction in order to fill the GUI.
"""
...
def cli():
    arguments = new_docopt.docopt(__doc__)
    ...
    if arguments["--complete"] is not None:
        parser = cli_parser.CLIParser(arguments["--complete"])
