...
timer = """
Amount of time to sleep after configuration changes
are made but before Jsnapy post tests are executed. 
This is required with performing a commit confirm.
...
skip_post_rpcs = """
When performing executing rpcs, only execute the 
post rpcs (a config change must occur).
