...
class LabJack(SingleCommDevice):
    ...
    def get_sbus_temp(self, number: int) -> float:
        ...
        return float(self.com.read_name(f"SBUS{number}_TEMP"))
    ...
    def get_ain(self, *channels: int) -> Union[float, Tuple[float, ...]]:
        ...
        ret_val = self.com.read_name(*ch_str)
        # the underlying library returns `ctypes.c_double(...).value` which is `float`
    ...
    def set_ain_resolution(self, channel: int, resolution: int) -> None:
        ...
        if resolution not in range(ain_max_resolution):
    ...
    def get_cal_current_source(self, name: Union[str, CalMicroAmpere]) -> float:
        ...
        return float(self.com.read_name(name.current_source_query))
    ...
    def get_product_id(self) -> int:
        ...
        """
        try:
            return int(self.com.read_name("PRODUCT_ID"))
    ...
    def read_resistance(self, channel: int) -> float:
        ...
        """
        return round(float(self.com.read_name(f"AIN{channel}_EF_READ_A")), 2)
