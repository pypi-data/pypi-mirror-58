...
class ISAContext:
    ...
    def __init__(self,spark,debug=False):
        ...
        self.spark.sql(
            f"""CREATE DATABASE IF NOT EXISTS `{_user_db}`
                COMMENT 'Sandbox for {os.environ.get('EMAIL',DORA_USER)}'
                LOCATION '{DORA_FS}://{DORA_BUCKET}/workspaces/{DORA_USER}/data/'""")
        self.spark.sql(f"USE `{_user_db}`")
        dynamodb = boto3.resource('dynamodb', region_name=AWS_REGION)
        self.table_status = dynamodb.Table(os.environ.get('CATALOG_TABLES'))
        self.spark.conf.set("spark.sql.shuffle.partitions", os.environ.get('DORA_CORES'))
        self.logger = logging.getLogger()
        fhandler = logging.FileHandler(filename=f"../logs/{datetime.datetime.today().strftime('%y%d%m%I%M%S')}.log", mode='a')
        formatter = logging.Formatter('%(asctime)s|%(dorasandbox)s|%(levelname)s|%(dorausername)s|%(message)s')
    ...
    def get_table_status(self,table_list):
        ...
        for table in table_list:
            ...
            self.logger.debug("%s:%s", table, table_id, extra=EXTRALOG)
            if table_meta is None:
                _dbs, _sch, _tbl = table.split('.')
                self.logger.warning("%s is not found in DORA metastore, starting import process...", table, extra=EXTRALOG)
            ...
            self.logger.debug("last update:%s, next update:%s", last_update, next_update, extra=EXTRALOG)
            ...
            if table_meta.get("execution") is not None:
                ...
                if execution['status'] == 'RUNNING':
                    ...
                    self.logger.warning("%s is now loading...", table, extra=EXTRALOG)
                    tables[table]['outdated'] = True
                    continue
            if next_update >= datetime.datetime.now():
                self.logger.info("%s is updated: %s (%s days)", table, table_meta['lastUpdate'],table_meta['cacheDays'], extra=EXTRALOG)
                tables[table]['outdated'] = False
                continue
            if next_update < datetime.datetime.now():
                self.logger.warning("%s is outdated: %s (%s days)", table, table_meta['lastUpdate'],table_meta['cacheDays'], extra=EXTRALOG)
                tables[table]['outdated'] = True
                continue
        return tables
    ...
    def load_tables(self, table_list):
        ...
        for tbl in table_list:
            ...
            self.logger.debug('load_tables:input_data: %s',input_data, extra=EXTRALOG)
    ...
    def start_execution(self, input_data):
        ...
        self.logger.info('start:%s', state_machine, extra=EXTRALOG)
        ...
        if execution['ResponseMetadata']['HTTPStatusCode'] != 200:
            self.logger.error('import:execution:%s', execution, extra=EXTRALOG)
        ...
        if describe_execution['ResponseMetadata']['HTTPStatusCode'] != 200:
            self.logger.error('import:describe_execution:%s', describe_execution, extra=EXTRALOG)
        ...
        while len(executions) > 0:
            ...
            for execution in executions:
                ...
                if execution_status.get('status') != 'RUNNING':
                    ...
                    self.logger.debug('wait_execution:execution_status:%s',execution_status, extra=EXTRALOG)
    ...
    def get_history(self, table):
        ...
        try: 
            ...
            self.logger.warning("Creating Table History for %s", table, extra=EXTRALOG)
    ...
    def get_version_of(self, table, version):
        ...
        self.logger.debug("History Table: %s", _history, extra=EXTRALOG)
        try: 
            self.spark.catalog.isCached(_table_v) 
        except AnalysisException:
            self.logger.warning("Creating cache for %s in version %s", table, version, extra=EXTRALOG)
    ...
    def sql(self, query, **kwargs):
        ...
        query = query.replace("workspace://", f"{DORA_FS}://{DORA_BUCKET}/workspaces/{DORA_USER}/notebooks/")
        query = query.replace("cloud://", f"{DORA_FS}://{DORA_BUCKET}/workspaces/{DORA_USER}/notebooks/")
        # Habilita a versão verbosa da execução
        self.verbose = kwargs.get("verbose")
        self.logger.debug('sql:parameters:%s',kwargs, extra=EXTRALOG)
        # Consulta adhoc
        if kwargs.get("connection") is not None:
            parser = sql_parser(query)
            table_name = hashlib.sha1(''.join(parser).encode('utf8')).hexdigest()
            output_path = f"{DORA_FS}://{DORA_BUCKET}/{ADHOC_DB}/{kwargs['connection']}/{table_name}"
            input_data = {"username":DORA_USER,"connection_name":kwargs["connection"],"adhoc": {"query":query ,"output": output_path}}
            self.logger.debug('adhoc:input_data:%s', input_data, extra=EXTRALOG)
            try:
                if kwargs.get("refresh"):
                    self.logger.warning('adhoc:Forced refresh data by user %s', DORA_USER, extra=EXTRALOG)
                    raise ValueError("Force Refresh")
                return self.spark.read.parquet(output_path)
            except Exception as miss_ex:
                self.logger.debug('adhoc:spark:%s', miss_ex, extra=EXTRALOG)
                execution_status = self.start_execution(input_data)
                self.logger.info('adhoc:DoraImportMachine:%s', execution_status['executionArn'], extra=EXTRALOG)
                while True:
                    sleep(2)
                    execution_status = self.steps.describe_execution(executionArn=execution_status['executionArn'])
                    if execution_status.get('status') != 'RUNNING':
                        break
                self.logger.debug('adhoc:status:%s', execution_status, extra=EXTRALOG)
                if execution_status.get('status') == 'SUCCEEDED':
                    self.logger.debug('adhoc:output:%s', execution_status.get('output'), extra=EXTRALOG)
                ...
                self.logger.debug('adhoc:failed:event:%s', failed_event, extra=EXTRALOG)
                fail_cause = failed_event['events'][0]['executionFailedEventDetails']['cause']
                self.logger.error('adhoc:error:%s', fail_cause, extra=EXTRALOG)
                try:
                ...
                self.logger.info("Execution Time: %s", (time_after_spark - timestamp).total_seconds(), extra=EXTRALOG)
        # Informar o usuário de que o comando de refresh é utilizado apenas para consultas do tipo "adhoc"
        if kwargs.get("refresh"):
            self.logger.warning('The "refresh" parameter is only used for adhoc queries.', extra=EXTRALOG)
        ...
        if len(re.findall(self.r_drop, query, re.MULTILINE | re.IGNORECASE)) > 0:
            ...
            self.logger.error(error_message, extra=EXTRALOG)
        ...
        for n, match in enumerate(re.finditer(self.r_delta, query, re.MULTILINE | re.IGNORECASE), start=1): 
            ...
            self.logger.info("table_name: %s", table_name, extra=EXTRALOG)
        ...
        self.logger.debug("sql:table_list: %s", table_list, extra=EXTRALOG)
        ...
        self.logger.debug('sql:outdated_tables: %s', outdated_tables, extra=EXTRALOG)
        ...
        self.logger.debug('sql:executions: %s', executions, extra=EXTRALOG)
        ...
        for execution in self.wait_execution(executions):
            ...
            if execution['status']=='FAILED':
                ...
                self.logger.debug('sql:import:failed:event:%s', failed_event, extra=EXTRALOG)
                fail_cause = failed_event['events'][0]['executionFailedEventDetails']['cause']
                self.logger.error('sql:import:error:%s', fail_cause, extra=EXTRALOG)
        ...
        self.logger.debug('sql:query:%s', query, extra=EXTRALOG)
        ...
        try:
            ...
            self.logger.error('sql:spark:error:%s', analysis_exception, extra=EXTRALOG)
            return self.spark.sparkContext.parallelize(Row({"error":analysis_exception})).toDF()
        finally:
            time_after_spark = datetime.datetime.now()
            self.logger.debug("Query Time: %s", (time_after_spark - time_before_spark).total_seconds(), extra=EXTRALOG)
            self.logger.info("Execution Time: %s", (time_after_spark - timestamp).total_seconds(), extra=EXTRALOG)
