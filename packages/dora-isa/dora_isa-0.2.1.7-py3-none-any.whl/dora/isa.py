...
USER_DB  = f"DORA_{DORA_USER.upper()}"
...
class ISAContext:
    ...
    def __init__(self,spark,debug=False):
        ...
        self.spark.conf.set("spark.sql.shuffle.partitions", int(os.environ.get('DORA_CORES','1').replace('\n0','')))
        self.logger = logging.getLogger()
        log_name = f"{os.environ.get('DORA_HOST','localhost')} - {datetime.datetime.today().strftime('%Y%m%d%I%M%S')}.log"
        fhandler = logging.FileHandler(filename=f"../logs/{log_name}")
    ...
    def sql(self, query, **kwargs):
        ...
        for execution in self.wait_execution(executions):
            ...
            if execution['status']=='FAILED':
                ...
                fail_cause = failed_event['events'][0]['executionFailedEventDetails']['cause']
                self.logger.error('sql:import:error:%s', fail_cause, )
        ...
        try:
            ...
            if match_view:
                ...
                for glue_keys in ['DatabaseName','CreateTime','UpdateTime','Retention','IsRegisteredWithLakeFormation','CreatedBy']:
                    if glue_keys in glue_tb: del glue_tb[glue_keys]
                ...
                presto_query = presto_query.replace("string",'varchar')
                encodedBytes = base64.b64encode(presto_query.encode("utf-8"))
                glue_tb['ViewOriginalText']=f"""/* Presto View: {str(encodedBytes, "utf-8")} */"""
            ...
            self.logger.error('sql:spark:error:%s', analysis_exception, )
