...
class Dense(tf.keras.layers.Dense):
    def __init__(self, output_shape, use_bias=True, activation=None, **kwargs ):
...
class Conv2d(tf.keras.layers.Conv2D):
    def __init__(self, kernel_size, num_filters, strides, auto_pad=True, activation=None, use_bias=False, dilation=1,
        ...
        activation = get_activation(activation)
        super(Conv2d, self).__init__(filters=num_filters, kernel_size=kernel_size, strides=strides,  padding='same'if auto_pad else 'valid', dilation_rate=dilation, activation=activation, use_bias=use_bias,data_format='channels_last',**kwargs)
        self.groups=groups
    @property
    def num_filters(self):
        return super().filters
