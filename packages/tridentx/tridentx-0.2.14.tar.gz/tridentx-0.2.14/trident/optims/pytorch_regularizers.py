...
def total_variation_norm_reg(output:torch.Tensor,reg_weight=1e-6):
    with torch.enable_grad():
        return reg_weight * (torch.sum(torch.abs(output[:, :, :, :-1] - output[:, :, :, 1:])) + torch.sum(torch.abs(output[:, :, :-1, :] - output[:, :, 1:, :])))
