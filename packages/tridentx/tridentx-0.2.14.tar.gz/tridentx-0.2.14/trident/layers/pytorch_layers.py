...
__all__ = ['Dense', 'Flatten', 'Concatenate', 'Conv1d', 'Conv2d', 'Conv3d', 'TransConv1d', 'TransConv2d', 'TransConv3d',
           ...
           'Droupout', 'AlphaDroupout', 'SelfAttention']
...
class Concatenate(Layer):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self._built == False:
            ...
            if len(shape_set) > 1:
                raise ValueError(
                    'A `Concatenate` layer requires inputs with matching shapes except for the concat axis. Got inputs shapes: %s' % (
                        shape_set))
            self._built = True
...
class _ConvNd(Layer):
    __constants__ = ['kernel_size', 'num_filters', 'strides', 'auto_pad', 'use_bias', 'dilation', 'groups',
                     'transposed']
...
class Conv2d(_ConvNd):
    def __init__(self, kernel_size, num_filters, strides=1, auto_pad=True, activation=None, use_bias=False, dilation=1,
        ...
        super(Conv2d, self).__init__(kernel_size, num_filters, strides, auto_pad, use_bias, dilation, groups, False,
                                     **kwargs)
        self.activation = get_activation(activation)
