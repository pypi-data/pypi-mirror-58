...
import numpy as np
from ..backend.common import to_list,addindent, get_time_suffix, format_time, get_terminal_size,get_session,  snake2camel,PrintException
...
class ModelBase(object):
    def __init__(self,inputs=None,output=None,input_shape=None,**kwargs):
        self.inputs= OrderedDict()

        self.targets= OrderedDict()
        self.model = None
        self.modelmeta = OrderedDict()
        self.optimizer = None
        self.lr_scheduler = None
        self._losses = OrderedDict()
        self._metrics = OrderedDict()
        self.loss_weights= OrderedDict()
        self.metrics_weights = OrderedDict()
        ...
        self.target_history = []
        self.callbacks = []
        self.training_context = {'losses': {},  # loss_wrapper
                                 'metrics': {},  # loss_wrapper
    ...
    def print_batch_progress(self,print_batch_progress_frequency):
        progress_bar(self.training_context['current_batch'], self.training_context['total_batch'],
                         'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}| epoch: {3}'.format(np.array(self.training_context['losses']['total_losses'][-1 * print_batch_progress_frequency:]).mean(),
                                                                                               ','.join(['{0}: {1:<8.3%}'.format( snake2camel(k),np.array(v[ -1 * print_batch_progress_frequency:]).mean())
                                                                                                   for k, v in self.training_context['metrics'].items()]),self.training_context['current_lr'], self.training_context['current_epoch']))



    def print_epoch_progress(self,print_epoch_progress_frequency):
        progress_bar(self.training_context['current_epoch'] , self.training_context['total_epoch'] , 'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}'.format(np.array(self.training_context['losses']['total_losses']).mean(), ','.join(['{0}: {1:<8.3%}'.format(snake2camel(k), np.array(v).mean()) for k, v in self.training_context['metrics'].items()]),self.training_context['current_lr']))


    def train_model(self,input,target,current_epoch,current_batch,total_epoch,total_batch,is_print_batch_progress=True,is_print_epoch_progress=True,log_gradients=False,log_weights=False,cumlative_grad=False):
        ...
        for k, v in self._losses.items():
            ...
            self.training_context['current_loss'] = self.training_context['current_loss'] + this_loss.mean()
...
class TrainingPlan(object):
    ...
    def add_training_item(self, training_item):
                ...
                for mbs, iter_data in enumerate(data_loader):
                    ...
                    for k,trainitem in self.training_items.items():
                        trainitem.train_model(input,target,epoch,mbs,self.num_epochs, len(data_loader.batch_sampler),
                                              self.print_progress_unit == 'batch' and mbs % self.print_progress_frequency == 0,
                                              self.print_progress_unit == 'epoch' and (epoch + 1) % self.print_progress_frequency == 0,
                                              cumlative_grad=False)
                    ...
                    if self.need_tile_image == True and self.tile_image_save_model_unit == 'batch' and mbs % self.tile_image_save_model_frequency == 0:
                        ...
                        if mbs>0 and mbs%100==0:
                            epoch_loss_history = [trainitem.epoch_loss_history for trainitem in self.training_items]
                            epoch_metric_history = [trainitem.epoch_metric_history for trainitem in self.training_items]
                    ...
                    if self.save_model_unit == 'batch' and mbs % self.save_model_frequency == 0:
                        for k, trainitem in self.training_items.items():
                            trainitem.save_model(self.save_path)
    ...
    def only_steps(self, num_steps, keep_weights_history=False, keep_gradient_history=False):
        self.execution_id = str(uuid.uuid4())[:8].__str__().replace('-', '')
        ...
        try:
                ...
                if mbs < num_steps:
                    ...
                    for k, trainitem in self.training_items.items():
                        trainitem.train_model(input, target, 0, mbs, 1,
                                              num_steps,
                                              self.print_progress_unit == 'batch' and mbs %
            ...
            loss_metric_curve(batch_loss_history, batch_metric_history, max_iteration=num_steps,
