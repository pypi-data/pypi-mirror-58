...
__all__ = ['InstanceNorm','BatchNorm','BatchNorm2d','BatchNorm3d','GroupNorm','GroupNorm2d','GroupNorm3d','LayerNorm2d','get_normalization']
...
class GroupNorm(Layer):
    def __init__(self, num_groups,affine=True, **kwargs):
        super().__init__()
        self.affine=affine
...
class InstanceNorm(Layer):
    ...
    def build(self, input_shape):
        if self._built == False or self.normalizer is None:
            self.normalizer = nn._InstanceNorm(self.input_filters, **self.norm_kwargs).to(_device)
            self._built = True
    def forward(self, *x):
        x = enforce_singleton(x)
        return self.normalizer(x)
