            ...
            try:
                ...
                for mbs, iter_data in enumerate(data_loader):
                    ...
                    for k, v in loss_fn.items():
                        ...
                        current_loss=current_loss+this_loss.mean()
                    ...
                    if self.print_progress_unit == 'batch' and mbs % self.print_progress_frequency == 0:
                        ...
                        progress_bar(mbs, len(data_loader.batch_sampler), 'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}| epoch: {3}'.format(np.array(losses['total_losses'][-1*self.print_progress_frequency:]).mean(), ','.join( ['{0}: {1:<8.3%}'.format(snake2camel(k), np.array(v[-1*self.print_progress_frequency:]).mean()) for k, v in  metrics.items()]), optimizer.param_groups[0]['lr'],epoch+1))
