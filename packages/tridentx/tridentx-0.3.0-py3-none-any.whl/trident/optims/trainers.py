...
import numpy as np
from ..backend.common import to_list,addindent, get_time_suffix, format_time, get_terminal_size,get_session,  snake2camel,PrintException,unpack_singleton,enforce_singleton
...
class ModelBase(object):
    ...
    def print_batch_progress(self, print_batch_progress_frequency):
        ...
        progress_bar(
                    self.training_context['current_batch'],
                     ...
                     'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4f}| epoch: {3}'.format(
    ...
    def print_epoch_progress(self,print_epoch_progress_frequency):
        progress_bar(self.training_context['current_epoch'] , self.training_context['total_epoch'] , 'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}'.format(self.training_context['losses']['total_losses'], ','.join(['{0}: {1:<8.3%}'.format(snake2camel(k), v) for k, v in self.training_context['metrics'].items()]),self.training_context['current_lr']))
...
class TrainingPlan(object):
    ...
    def save_model_scheduling(self, frequency: int, save_path: str, unit='batch'):
        ...
        if save_path is not None and len(save_path)>1 and not os.path.exists(save_path):
            try:
                os.makedirs(save_path)
            except Exception as e:
                PrintException()
                raise ValueError('folder:{0} is not valid path'.format(save_path))
            ...
            try:
                ...
                for mbs, iter_data in enumerate(data_loader):
                    ...
                    if self.need_tile_image == True and self.tile_image_save_model_unit == 'batch' and mbs % self.tile_image_save_model_frequency == 0:
                        ...
                        loss_history = [trainitem.batch_loss_history for trainitem in self.training_items]
                        metric_history = [trainitem.batch_metric_history for trainitem in self.training_items]
