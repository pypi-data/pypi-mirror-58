...
from .trainers import ModelMixin,progress_bar
...
class TrainingItem(object):
    ...
    def with_loss(self, loss, loss_weight=1, **kwargs):
...
class TrainingPlan(object):
    ...
    def start_now(self, ):
        ...
        training_context = trainingitem.training_context
        ...
        training_context['losses'] = {}
        training_context['losses']['total_losses'] = []
        metrics = {}
        tile_images_list = []
        print(self.__repr__)
        self.lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, verbose=False, mode='min',
                                                                       factor=0.75, patience=10, threshold=1e-4,
                                                                       cooldown=5, min_lr=1e-10, eps=1e-8)

        for callback in self.callbacks:
            callback.on_training_start(training_context)

        for epoch in range(self.num_epochs):
            training_context['current_epoch'] = epoch
            if self.device == 'cuda':
                torch.cuda.empty_cache()
            for callback in training_context['callbacks']:
                callback.on_epoch_start(training_context)
            ...
            try:
                ...
                for mbs, iter_data in enumerate(data_loader):
                    training_context['current_batch'] = mbs
                    input = None
                    target = None
                    if len(iter_data) == 1:
                        input = to_tensor(iter_data[0])
                        target = input.clone()
                    if len(iter_data) == 2:
                        input, target = to_tensor(iter_data[0]), to_tensor(iter_data[1])
                    # input, target = Variable(input).to(self.device), Variable(target).to(self.device)
                    input, target = input.to(self.device), target.to(self.device)
                    training_context['current_input'] = input
                    training_context['current_target'] = target
                    ...
                    training_context['current_model'] = model_in_train
                    training_context['current_output'] = output

                    training_context['current_loss'] = 0
                    for k, v in loss_fn.items():
                        if k not in training_context['losses']:
                            training_context['losses'][k] = []
                        this_loss = v.forward(output, target) if hasattr(v, 'forward') else v(output, target)
                        training_context['current_loss'] = training_context['current_loss'] + this_loss.mean()
                        training_context['losses'][k].append(float(to_numpy(this_loss)))

                    ##regularizer(l1,l2.....)

                    for k, v in trainingitem._output_regs.items():
                        if k + '_Loss' not in training_context['losses']:
                            training_context['losses'][k + '_Loss'] = []
                        this_loss = v(output)
                        training_context['current_loss'] = training_context['current_loss'] + this_loss.mean()
                        training_context['losses'][k + '_Loss'].append(float(to_numpy(this_loss)))

                    for k, v in trainingitem._model_regs.items():
                        if k + '_Loss' not in training_context['losses']:
                            training_context['losses'][k + '_Loss'] = []
                        this_loss = v(model_in_train)
                        training_context['current_loss'] = training_context['current_loss'] + this_loss.mean()
                        training_context['losses'][k + '_Loss'].append(float(to_numpy(this_loss)))

                    for callback in training_context['callbacks']:
                        callback.post_loss_calculation(training_context)
                    training_context['losses']['total_losses'].append(float(to_numpy(training_context['current_loss'])))

                    for k, v in metrics_fn.items():
                        if k not in metrics:
                            metrics[k] = []
                        metrics[k].append(
                            float(to_numpy(v.forward(output, target))) if hasattr(v, 'forward') else float(
                                to_numpy(v(output, target))))

                    training_context['current_metrics'] = metrics
                    # optimizer update the gradient
                    optimizer.updates(training_context['current_loss'], training_context)
                    ...
                    if self.need_tile_image == True and self.tile_image_save_model_unit == 'batch' and mbs % self.tile_image_save_model_frequency == 0:
                        ...
                        tile_rgb_images(*tile_images_list, save_path=os.path.join(self.tile_image_save_path,
                                                                                  self.self.tile_image_name_prefix),
                                        imshow=self.tile_image_imshow)
                        loss_metric_curve(trainingitem.epoch_loss_history, trainingitem.epoch_metric_history,
                    ...
                    if self.print_progress_unit == 'batch' and mbs % self.print_progress_frequency == 0:
                        # metric_key = list(metrics.keys())[0]
                        # if len(metrics[
                        #            metric_key]) > self.print_progress_frequency and mbs % self.print_progress_frequency == 0:
                            ...
                            #         np.array(metrics[metric_key][-1 * self.print_progress_frequency:]).mean())

                        progress_bar(mbs, len(data_loader.batch_sampler),
                                     'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}| epoch: {3}'.format(np.array(
                                         training_context['losses']['total_losses'][
                                         -1 * self.print_progress_frequency:]).mean(), ','.join([
                                                                                                    '{0}: {1:<8.3%}'.format(
                                                                                                        snake2camel(k),
                                                                                                        np.array(v[
                                                                                                                 -1 * self.print_progress_frequency:]).mean())
                                                                                                    for k, v in
                                                                                                    metrics.items()]),
                    ...
                    if self.save_model_unit == 'batch' and mbs % self.save_model_frequency == 0:
                    ...
                    for callback in training_context['callbacks']:
                        callback.on_batch_end(training_context)
                ...
                if epoch == 0:
                    trainingitem.batch_loss_history = training_context['losses']
                    trainingitem.batch_metric_history = metrics
                    for k, v in training_context['losses'].items():
                    ...
                    [trainingitem.batch_loss_history[k].extend(v) for k, v in training_context['losses'].items()]
                ...
                if self.print_progress_unit == 'epoch' and (epoch + 1) % self.print_progress_frequency == 0:
                    progress_bar(epoch + 1, self.num_epochs, 'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}'.format(
                        np.array(training_context['losses']['total_losses']).mean(), ','.join(
                            ['{0}: {1:<8.3%}'.format(snake2camel(k), np.array(v).mean()) for k, v in metrics.items()]),
                        optimizer.param_groups[0]['lr']))

                if self.save_model_unit == 'epoch' and (epoch + 1) % self.save_model_frequency == 0:
                    save_full_path = os.path.join(self.self.save_path,
                                                  'model_{0}_ep_{1}.pth'.format(model_in_train.__class__.__name__,
                                                                                epoch))
    ...
    def only_steps(self, num_steps, keep_weights_history=False, keep_gradient_history=False, keep_input_history=False,
        ...
        trainingitem.batch_loss_history = {}
        trainingitem.batch_loss_history['total_losses'] = []
        trainingitem.batch_metric_history = {}
        tile_images_list = []
        ...
        try:
            ...
            for mbs, iter_data in enumerate(data_loader):
                if mbs < num_steps:
                    ...
                    current_loss = 0
                    for k, v in loss_fn.items():
                        if k not in trainingitem.batch_loss_history:
                            trainingitem.batch_loss_history[k] = []
                        this_loss = v.forward(output, target) if hasattr(v, 'forward') else v(output, target)
                        current_loss = current_loss + this_loss
                        trainingitem.batch_loss_history[k].append(float(to_numpy(this_loss)))

                    ##regularizer(l1,l2.....)
                    if regularizer is not None:
                        regularizer(model_in_train, current_loss)
                    trainingitem.batch_loss_history['total_losses'].append(float(to_numpy(current_loss)))

                    for k, v in metrics_fn.items():
                        if k not in trainingitem.batch_metric_history:
                            trainingitem.batch_metric_history[k] = []
                        trainingitem.batch_metric_history[k].append(
                            float(to_numpy(v.forward(output, target))) if hasattr(v, 'forward') else float(
                                to_numpy(v(output, target))))
                    ...
                    if self.print_progress_unit == 'batch' and mbs % self.print_progress_frequency == 0:
                        progress_bar(mbs, num_steps,
                                     'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}| epoch: {3}'.format(
                                         self.training_items[0].batch_loss_history['total_losses'][-1], ','.join(
                                             ['{0}: {1:<8.3%}'.format(snake2camel(k), v[-1]) for k, v in
                                              self.training_items[0].batch_metric_history.items()]),
                                         optimizer.param_groups[0]['lr'], 1))

                    # if mbs % len(data_loader.batch_sampler) == 0:  #     break
                else:
                    break
            ...
            #         optimizer.param_groups[0]['lr']))
            #
            loss_metric_curve(trainingitem.batch_loss_history, trainingitem.batch_metric_history, max_iteration=None,
