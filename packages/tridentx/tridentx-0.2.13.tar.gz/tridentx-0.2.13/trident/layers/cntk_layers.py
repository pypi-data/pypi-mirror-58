...
def Conv2d(kernel_size, num_filters, strides=1, auto_pad=True, activation=None, use_bias=False, dilation=1,
    ...
    use_bias = use_bias if use_bias is not None else False
    init_bias = 0
    groups =groups if groups>1 else 1


    filter_rank = len(kernel_size)
    return _ConvNd(kernel_size, num_filters=num_filters,auto_pad=auto_pad,
                           strides=strides, sharing=sharing, use_bias=use_bias,
                           reduction_rank=reduction_rank, dilation=dilation, groups=groups, max_temp_mem_size_in_samples=0, input_shape=input_shape,op_name='Convolution', name = name)
