...
import numpy as np
from ..backend.common import addindent, get_time_suffix, format_time, get_terminal_size, snake2camel,PrintException,to_list
...
class TrainingPlan(object):
            ...
            try:
                ...
                for mbs, iter_data in enumerate(data_loader):
                    ...
                    if regularizer is not None:
                        regularizer(model_in_train, training_context['current_loss'])
                    ...
                    if self.print_progress_unit == 'batch' and mbs % self.print_progress_frequency == 0:
                        ...
                        if len(metrics[metric_key]) > self.print_progress_frequency  and mbs % self.print_progress_frequency  == 0:
                        ...
                        progress_bar(mbs, len(data_loader.batch_sampler), 'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}| epoch: {3}'.format(np.array(training_context['losses']['total_losses'][-1*self.print_progress_frequency:]).mean(), ','.join( ['{0}: {1:<8.3%}'.format(snake2camel(k), np.array(v[-1*self.print_progress_frequency:]).mean()) for k, v in  metrics.items()]), optimizer.param_groups[0]['lr'],epoch+1))
                ...
                if self.print_progress_unit == 'epoch' and (epoch + 1) % self.print_progress_frequency == 0:
                    progress_bar(epoch+1, self.num_epochs, 'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}'.format( np.array(training_context['losses']['total_losses']).mean(), ','.join(['{0}: {1:<8.3%}'.format(snake2camel(k), np.array(v).mean()) for k, v in metrics.items()]),  optimizer.param_groups[0]['lr']))
    ...
    def only_steps(self,num_steps,keep_weights_history=False,keep_gradient_history=False,keep_input_history=False,keep_target_history=False ):
        ...
        try:
            ...
            for mbs, iter_data in enumerate(data_loader):
                if mbs<num_steps:
                    ...
                    for k, v in metrics_fn.items():
                        ...
                        trainingitem.batch_metric_history[k].append(float(to_numpy(v.forward(output, target))) if hasattr(v, 'forward') else  float(to_numpy(v(output, target))))
                    ...
                    if self.print_progress_unit == 'batch' and mbs % self.print_progress_frequency == 0:
                        progress_bar(mbs, num_steps, 'Loss: {0:<8.3f}| {1} | learning rate: {2:<10.4e}| epoch: {3}'.format(self.training_items[0].batch_loss_history['total_losses'][-1], ','.join( ['{0}: {1:<8.3%}'.format(snake2camel(k), v[-1]) for k, v in  self.training_items[0].batch_metric_history.items()]), optimizer.param_groups[0]['lr'],1))



                    # if mbs % len(data_loader.batch_sampler) == 0:
                    #     break
                else:
                    break
            ...
            #
            loss_metric_curve(trainingitem.batch_loss_history, trainingitem.batch_metric_history, max_iteration=None, calculate_base='batch', save_path=None, imshow=True)
