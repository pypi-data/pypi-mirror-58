...
class TrainingPlan(object):
    ...
    def with_data_loader(self, data_loader, **kwargs):
        self._dataloaders[data_loader.__name__] = data_loader
        return self
    ...
    def start_now(self, ):
        def set_train(trainingitems):
            for k,trainingitem in trainingitems.items():
                trainingitem.model.train()
        def set_zero_grad(trainingitems):
            for trainingitem in trainingitems:
                trainingitem.model.zero_grad()
                trainingitem.optimizer.zero_grad()

        def get_loss(trainingitems,input,target):
            for i,(k,trainingitem) in enumerate(trainingitems.items()):
                trainingitem.model.zero_grad()
                trainingitem.optimizer.zero_grad()
                output=trainingitem.model(input)
                current_loss = 0
                ...
                if regularizer is not None:
                    regularizer(trainingitem.model, current_loss)
                ...
                for k, v in metrics_fn.items():
                    ...
                    trainingitem.batch_metric_history[k].append(float(to_numpy(v.forward(output, target))) if hasattr(v, 'forward') else float(
                        to_numpy(v(output, target))))

                current_loss.backward()
                for callback in self.callbacks:
                    callback.post_backward_calculation(self.training_context)
                trainingitem.optimizer.step()
                for callback in self.callbacks:
                    callback.pre_optimization_step(self.training_context)
                if trainingitem.constraint is not None:
                    trainingitem.constraint(trainingitem.model)
        def update_epoch_summary(trainingitems,epoch):
            for k,trainingitem in trainingitems.items():
                if epoch == 0:
        ...
        for epoch in range(self.num_epochs):
            ...
            try:
                ...
                for mbs, iter_data in enumerate(data_loader):
                    ...
                    get_loss(self.training_items, input, target)
                ...
                update_epoch_summary(self.training_items, epoch)
