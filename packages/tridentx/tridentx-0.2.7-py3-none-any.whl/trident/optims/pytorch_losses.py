...
__all__ = ['MSELoss','CrossEntropyLoss', 'make_onehot', 'MS_SSIM', 'CrossEntropyLabelSmooth', 'mixup_criterion', 'DiceLoss', 'FocalLoss', 'SoftIoULoss', 'CenterLoss', 'TripletLoss', 'LovaszSoftmax','PerceptionLoss']
...
class CenterLoss(_Loss):
    ...
    """
    def __init__(self, num_classes=751, feat_dim=2048, use_gpu=True,reduction="mean",reduced=False):
...
class StyleLoss(nn.Module):
    ...
    def forward(self,output, target):
        ...
        return F.mse_loss(G, Gt).div((img_size[1]**2 * img_size[2]**2 * img_size[3]**2))
...
class PerceptionLoss(_Loss):
    ...
    def forward(self,output, target):
        ...
        return self.styleloss(vgg_output,vgg_target).mean()

# class PerceptualLoss(_Loss):
#     def __init__(self,  ): # VGG using our perceptually-learned weights (LPIPS metric)
