...
def total_variation_norm_reg(output:torch.Tensor,loss,weight=1e-6):
    with torch.enable_grad():
        loss = loss +weight * (torch.sum(torch.abs(output[:, :, :, :-1] - output[:, :, :, 1:])) + torch.sum(torch.abs(output[:, :, :-1, :] - output[:, :, 1:, :])))
