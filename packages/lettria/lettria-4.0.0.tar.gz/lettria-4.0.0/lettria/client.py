...
class Client:
	...
	def server_request(self, text=''):
		...
		try:
			...
			# HTTPS commented due to very long DNS resolution with python's default request library
			# still investigating

			response = requests.post('http://51.254.207.74:4300/api/main', headers=headers, json={'text' : text}).json()
