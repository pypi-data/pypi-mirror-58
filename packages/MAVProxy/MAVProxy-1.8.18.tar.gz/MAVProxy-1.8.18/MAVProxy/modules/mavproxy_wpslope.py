...
class WPSlopeModule(mp_module.MPModule):
    ...
    def mavlink_packet(self, msg):
        ...
        if msg.get_type() == 'SERVO_OUTPUT_RAW':
            self.console.set_status('RearLeft', 'RearLeft %.1f' % self.ServoAngle(msg,2,20,-1), row=6)
            self.console.set_status('RearRight', 'RearRight %.1f' % self.ServoAngle(msg,4,20), row=6)
            self.console.set_status('FrontLeft', 'FrontLeft %.1f' % self.ServoAngle(msg,3,20,-1), row=6)
            self.console.set_status('FrontRight', 'FrontRight %.1f' % self.ServoAngle(msg,1,20), row=6)
