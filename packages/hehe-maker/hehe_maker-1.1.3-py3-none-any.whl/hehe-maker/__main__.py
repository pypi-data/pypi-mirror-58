...
parser.add_argument("-ins", "--insert",
                    help="Inserts the pages given at the target page, pushes the page of that number forward")
# Where to insert pages
parser.add_argument(
    "-i", "--index", help="Page number at which pages should be inserted")
parser.add_argument("-rm", "--remove", action="store_const", const=int, default=[],
                    help="Removes the pages given from the PDF specified")      # Flag to remove pages
# Pages to be removed as specified by user, nargs="*" used to set as optional and a list
parser.add_argument("rm_pages", nargs="*", metavar="R",
                    type=int, help="A page to be removed")
# Adds parameter to parser
parser.add_argument("input", help="Path to pages")
...
def remove_pages(pages_in, page_numbers):
    ...
    for n in page_numbers:
        # Our list index start at 0, but the user starts counting pages at 1
        del pages_out[n - 1]
...
def main():
    ...
    if (args.insert and not args.index) or (not args.insert and args.index):
        raise EnvironmentError(
            "You must specify both path to pages to be inserted and index of where they should be inserted; Use both -ins and -i")
