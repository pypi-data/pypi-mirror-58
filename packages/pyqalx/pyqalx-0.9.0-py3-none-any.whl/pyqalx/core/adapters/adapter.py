...
class QalxAdapter(object):
    ...
    def get(self, guid, *args, **kwargs):
        ...
        logger.debug(
            f"get {self.entity_class.entity_type} with guid {guid} with {endpoint}"
        )
    ...
    def find_one(self, **query_params):
        ...
        :param query_params: keyword arguments to represent the query e.g.
        `meta="id=case-01239876"`
        :return: an instance of `self.entity_class`
    ...
    def save(self, entity, *args, **kwargs):
        ...
        logger.debug(
            f"save {self.entity_class.entity_type} with guid {guid} with {endpoint}"
        )
    ...
    def archive(self, entity, *args, **kwargs):
        ...
        logger.debug(
            f"archive {self.entity_class.entity_type} with guid {guid} with {endpoint}"
        )
...
class QalxUnpackableAdapter(QalxAdapter):
    ...
    def _child_list_request(
        ...
        for _page in entity._chunks(kids, chunk_size=100):
            ...
            while _resp["query"]["next"] is not None:
                ...
                logger.debug(
                    f"get {self.entity_class.entity_type} with {_resp['query']['next']}"
                )
                _resp = self._process_api_request("get", _resp["query"]["next"])
                unpacked_entities += QalxListEntity(
                    self.child_entity_class, _resp
                )["data"]
    ...
    def _get_child_entities(self, entity):
        ...
        if isinstance(kids, dict):
            ...
            raise QalxError(
                f"`self.kids` key on `entity` must be dict or "
        ...
        unpacked_entities = self._child_list_request(
            entity=entity, kids=kids_guids, child_adapter=child_adapter
        )
        return unpacked_entities
    ...
    def get_kids_keys_to_save(self, kids_to_save):
        """
        Gets the keys of the kids that we want to save.
        :param kids_to_save: A dict of the kids we want to save - fully inflated
        :type kids_to_save: dict
        :return: A dict of {key:guid} of the kids we want to save
        """
        return {k: str(i["guid"]) for k, i in kids_to_save.items()}
...
class QalxValidateBlueprintAdapter(QalxAdapter):
    ...
    def _validate(self, blueprint_name=None, **kwargs):
        ...
        if blueprint_name is not None:
            ...
            jsonschema.validate(
                instance=validated_data, schema=blueprint["schema"]
            )
...
class QalxValidateUnpackableBlueprintAdapter(QalxValidateBlueprintAdapter):
    ...
    def _validate(self, blueprint_name=None, **kwargs):
        if blueprint_name is not None:
            blueprint = self._attempt_blueprint_unpack(
                blueprint_name=blueprint_name
            )
            kwargs["blueprint"] = blueprint
        return super(QalxValidateUnpackableBlueprintAdapter, self)._validate(
            blueprint_name=blueprint_name, **kwargs
        )

    def _kids_properties(self, blueprint):
        return blueprint["schema"]["properties"][self.kids][
            "properties"
        ]  # noqa
    ...
    def save(self, entity, input_file, file_name, upload=True, *args, **kwargs):
        ...
        if input_file is not None:
            if upload:
                ...
                # Entity is returned with the put_url so that the user can upload
                # the file manually
                return entity
