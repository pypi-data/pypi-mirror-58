...
def get_client_statistics(customer_name,customer_type ):
    ...
    number_request_by_week = number_request_by_week(customer_name , customer_type)
    user_progress_requests=[get_counter_of_last_four_months(customer_name, customer_type)[1],get_counter_of_last_four_months(customer_name, customer_type)[2]]
    ...
    elif customer_plan[5]=='PRO' :
        number_of_requests_allowed = ALLOWED_REQUESTS_PRO
    else:
        number_of_requests_allowed = 0

    request_total= sum(nbr_request_total_current_month)
    request_number_progress.append({"number_of_requests_used" :request_total ,"number_of_requests_allowed": number_of_requests_allowed} )
    ...
    result = {
                ...
                'number_request_by_week_of_current_month':number_request_by_week,
...
def customer_registration(customer_email, customer_type, customer_name,customer_password,first_name,last_name,phone_number,job,
               number_of_staff,plan,cms_if_exist,url_cms):
    uid = str(uuid.uuid4())
    token = str(uuid.uuid4())
    if plan == "FREE":
        expiration = 4380000
    ...
    if add_customer:
        ...
        db.add_or_update_cms_table(table_name="cms_client_table", customer_name=customer_name, customer_type=customer_type,
                                   project_name="", cms=cms_if_exist, url_shop=url_cms, token=token)
...
def contact(first_name,customer_email,subject_of_email,message_email):
    ...
    try:
        ...
        msg.attach(part2)
        server.sendmail(fromaddr, 'fatmaboussada91@gmail.com', msg.as_string())
        return True
    except Exception as e:
...
get_client_statistics('okaidi' , 'textile')