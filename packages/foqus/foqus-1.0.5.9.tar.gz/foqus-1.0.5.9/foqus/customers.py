...
def get_counter_of_last_four_months(customer, customer_type):
    ...
    last_month = (first - datetime.timedelta(days=1)).strftime("%m")
    first_day_of_last_month = (first - datetime.timedelta(days=1)).replace(day=1)
    last_last_month = (first_day_of_last_month - datetime.timedelta(days=1)).strftime("%m")
    ...
    last_four_last_months = [curr_month, last_month, last_last_month, fourth_month_ago]
    request_number = []
    for month in last_four_last_months:
        month_year=month+'-'+ str(today)[0:4]
        value = db.select_utilistation_par_mois("client_history_table", customer, customer_type, month_year)
        if value:
    ...
    last_last_month_year= last_last_month+'-'+str(datetime.date.today())[0:4]
    fourth_month_ago = fourth_month_ago+'-'+str(datetime.date.today())[0:4]
    user_progress_requests_3_month = db.select_utilistation_par_mois("client_history_table", customer, customer_type,
                                                                      last_last_month_year)
...
def number_request_by_week(customer, customer_type):
    today = datetime.date.today()
    curr_month_number = str(today)[5:7]
    test = db.get_date_counter("client_history_table", customer, customer_type, curr_month_number)
    week_list = []
    value_week_list = []
    sum_values_S01 = 0
    sum_values_S02 = 0
    ...
    sum_values_S05 = 0
    for t in test:
        res = week_of_month(t[1])
        if res == 1:
            sum_values_S01 = sum_values_S01 + t[0]
        if res == 2:
            sum_values_S02 = sum_values_S02 + t[0]
        if res == 3:
            sum_values_S03 = sum_values_S03 + t[0]
        if res == 4:
            sum_values_S04 = sum_values_S04 + t[0]
        if res == 5:
            sum_values_S05 = sum_values_S05 + t[0]

    if sum_values_S01 != 0:
        week_list.append("Semaine 01")
        value_week_list.append(sum_values_S01)
    if sum_values_S02 != 0:
        week_list.append("Semaine 02")
        value_week_list.append(sum_values_S02)
    if sum_values_S03 != 0:
        week_list.append("Semaine 03")
        value_week_list.append(sum_values_S03)
    if sum_values_S04 != 0:
        week_list.append("Semaine 04")
        value_week_list.append(sum_values_S04)
    if sum_values_S05 != 0:
        week_list.append("Semaine 05")
        value_week_list.append(sum_values_S05)

    nbr_request_by_week = []
    for j in range(0, len(week_list)):
        nbr_request_by_week.append({'week': week_list[j], 'number': value_week_list[j]})
