...
def delete_user_data(customer_name, customer_type):
    logger.info("Starting_deleting_user_data %s %s" %(customer_name, customer_type))
    from importlib import machinery
    tools = os.environ.get('TOOL_DIRECTORY')
    database = machinery.SourceFileLoader('database', tools + 'database.py').load_module()
    ...
    if db.get_customer_info_from_customer_name(customer_name):
        if db2.delete_from_users_management(customer_name, customer_type):
             logger.info("Deleting_user %s with type %s from table %s"
                         % (customer_name, customer_type, DATABASE_USERS_MANAGEMENT_TABLE_NAME))
        detection_error_projects = db2.select_from_status_project(customer_name, customer_type)[0] if db2.select_from_status_project(customer_name, customer_type) != [] else db2.select_from_status_project(customer_name, customer_type)
        for detection_table in detection_error_projects :
            logger.info('Deleting_detection_error_table %s' % detection_table)
            db2.drop_table("predict_"+customer_type+"_"+customer_name+"_"+str(detection_table.lower()))
        for table in tables_to_delete_from:
            if db2.delete_from_table(table, customer_name, customer_type):
        ...
        db2.drop_table(customer_name.replace(' ', '_'))
    ...
    if customer_name:
        ...
        if list_users and len(list_users) > 1:
            if db.get_customer_info_from_customer_name(customer_name[0]):
                if db2.delete_from_users_management_from_email(customer_email):
