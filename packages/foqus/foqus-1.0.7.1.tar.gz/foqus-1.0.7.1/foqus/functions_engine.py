...
def update_profile_customer(customer_email, customer_name, first_name, last_name,
    ...
    try:
        ...
        if old_name != customer_name:
            ...
            db.rename_table_client(old_name, customer_name)
            db.commit_db_changes()
...
def customer_registration(customer_email, customer_type, customer_name,customer_password,first_name,last_name,phone_number,job,
    ...
    add_customer = db.add_customer(customer=customer_name, customerPassword=customer_password,
                                   customerEmail=customer_email, customerJob=job, customerPhone=phone_number, token=token, aboutUs="",
                                   subjectHelp="",
                                   domaine=customer_type, firstName=first_name, lastName=last_name, stafNumber=number_of_staff, apikey=uid,
                                   expiration_duration_in_hours=expiration, type="customer")
    if add_customer:
        db.commit_db_changes()
        if token_cms:
            db.add_or_update_cms_table(table_name="cms_client_table", customer_name=customer_name, customer_type=customer_type,
        ...
        if cms_if_exist =='shopify' and plan!='FREE':
            token_cms='2a72f886d3e5c4b2a6623bff008a408f'
            if plan == 'STANDARD':
                price = 99.0
            elif plan == 'PREMIUM':
                price = 299.0
            ...
            charge_shopify_api = requests.post("https://{0}{1}".format(url_cms, endpoint), headers=headers,json=
                {
                "recurring_application_charge": {
                "name": plan,
                "price": price_dollar,
                "return_url": "https://foqus-shopify.azurewebsites.net",
                "trial_days":30,
                 "test":"false"
                }
                })
            ...
            if charge_shopify_api.status_code==201:
                ...
                transaction_id=response['recurring_application_charge']['id']
                url_confirmation=response['recurring_application_charge']['confirmation_url']
                db.update_client_payement(customer_name, customer_type, date, plan, 0, 'paypal',price*11)
                db.update_transction_id(customer_name, customer_type, str(transaction_id), plan, date, 'paypal')
                try:
                    logger.info("start sending email for confirmation shopify payment ...")
                    ...
                    msg['From'] = fromaddr
                    msg['To'] = toaddr
                    msg['Subject'] = 'Confirmez votre payement Shopify'
                    date = datetime.today().strftime("%Y-%m-%d %H:%M:%S")
                    html_file = codecs.open(PATH_EMAIL_ACTIVATE_ACCOUNT_SHOPIFY_PAYMENT, 'r')
                    html = ((str(html_file.read()).replace("#first_name", first_name)).replace("#url_confirmation",
                                                                                               url_confirmation)).replace(
                        "#plan", plan).replace("#transaction_id", str(transaction_id)).replace("#price_dollar",
                                                                                               str(
                                                                                                   price_dollar)).replace(
                        "#date",
                        date)
                ...
                try:
                    ...
                    configuration = machinery.SourceFileLoader('configuration',
                                                               tools + 'configuration.py').load_module()
                    html_file = codecs.open(configuration.PATH_EMAIL_PROBLEM_SHOPIFY_PAYMENT, 'r')
                    payement_url = DOMAIN + 'payement/'
                    html = ((str(html_file.read()).replace("#first_name", first_name)).replace("#payement_url",
                                                                                               payement_url))
                    part2 = MIMEText(html, 'html')
                    msg.attach(part2)
                    text = msg.as_string()
        ...
        send_email_of_client_inscription(customer[1], date_inscription,
...
def contact(first_name,customer_email,subject_of_email,message_email):
    ...
    try:
        ...
        msg = MIMEMultipart()
        msg['From'] = fromaddr
        msg['To'] = 'fatmaboussada91@gmail.com'
...
def forget_password(customer_email):
    try:
        ...
        mail_subject = 'Récupérez votre compte'
