...
class PostgreSQL:
    ...
    def select_all_utilistation_par_mois(self, table_name, customer_name, customer_type, month):
        ...
        try:
            self.cursor.execute("SELECT * FROM " + table_name + " WHERE customer_name = '" + customer_name +
                                "' AND  customer_type = '" + customer_type + "' AND  to_char(date , 'MM-YYYY') = '" + month + "' AND  api = '" + api + "';")
        except Exception as e:
            logger.error(
                "Table " + table_name + " doesn't exist or problem during get project from status_projects table , exception is : %s" % e)
            self.connector.rollback()
    ...
    def add_or_update_products(self, table_name="objects", reference="", urlProduit=""):
        # Pass data to fill a query placeholders and let Psycopg perform
        # the correct conversion (no more SQL injections!)
        self.cursor.execute("SELECT * FROM " + table_name + " WHERE urlProduit = '" + urlProduit + "';")
        ...
        if result is None:
            ...
            try:
                self.cursor.execute(
                    "UPDATE " + table_name + " SET reference = '" + reference + "' WHERE urlProduit = '" +
                    urlProduit + "';")
    ...
    def add_or_update_images(self, table_name="objects", reference="", urlProduit=""):
        # Pass data to fill a query placeholders and let Psycopg perform
        # the correct conversion (no more SQL injections!)
        self.cursor.execute("SELECT urlProduit FROM " + table_name + " WHERE reference =  '" + reference + "';")
        result = self.cursor.fetchone()
        if result is None:
            try:
                self.cursor.execute("INSERT INTO " + table_name + " (reference, urlProduit) VALUES ('" + reference +
