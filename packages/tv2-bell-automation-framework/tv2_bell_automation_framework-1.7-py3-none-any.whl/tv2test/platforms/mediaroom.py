...
class MediaRoomRemoteControl(RemoteControl):
    ...
    def press(self, key=None, message=None, delay=0.5, show_message=True, timestamp_name=None, ir_code=None):
        '''
        ...
        if ir_code is not None:
            print('INPUT IR CODE PRESS :::', ir_code)
            code = '%s\r\n' % ir_code
        ...
        if show_message:
            annotation_text = key if ir_code is None else 'IR code'
            self.dut.display.add_text_annotation(annotation_text, background_color=(0, 200, 0))
        ...
        if not timestamp_name:
            timestamp_name = '%s press' % (key if ir_code is None else 'IR code')
...
class MediaRoomHelper:
    ...
    def open_bell_app_launcher(self):
        ...
        def validate_bell_app_launcher():
            frame = self.dut.display.get_frame()
            template_support_icon = cv2.imread(os.path.join(os.path.dirname(__file__), 'templates', 'mediaroom', 'template_support_icon.png'))
            template_youtube_icon = cv2.imread(os.path.join(os.path.dirname(__file__), 'templates', 'mediaroom', 'template_youtube_icon.png'))
            template_netflix_icon = cv2.imread(os.path.join(os.path.dirname(__file__), 'templates', 'mediaroom', 'template_netflix_icon.png'))
            if (imgmatch.match_template(self.dut, template_support_icon, frame, crop_rect=Region(130, 940, 1900, 100)) or
                imgmatch.match_template(self.dut, template_youtube_icon, frame, crop_rect=Region(130, 940, 1900, 100)) or
                imgmatch.match_template(self.dut, template_netflix_icon, frame, crop_rect=Region(130, 940, 1900, 100))):
                return True
            return False
    ...
    def verify_vod_bell_icon(self):
        ...
        '''
        template_bell_icon = cv2.imread(os.path.join(os.path.dirname(__file__), 'templates', 'mediaroom', 'template_vod_bell_icon.png'))
    ...
    def verify_vod_on_demand(self):
        '''
        Verify whether the Bell on demand is present in the VOD store
        '''
        template_bell_icon = cv2.imread(os.path.join(os.path.dirname(__file__), 'templates', 'mediaroom', 'template_vod_on_demand.png'))
        return imgmatch.match_template(self.dut, template_bell_icon, self.dut.display.get_frame(),
                                        crop_rect=Region(53, 60, 464, 221), timestamp_name='vod_bell_on_demand_verified_at')
    ...
    def vod_launch(self, timeout_time=15.0):
        ...
        execution_start_time = time.time()
        while True:
            if self.verify_vod_on_demand():
                self.calculate_duration('vod_launch_time', 'vod_select_press', 'vod_bell_on_demand_verified_at')
                return True
            if time.time() - execution_start_time >= timeout_time:
    ...
    def search_and_detect_new_releases(self):
        ...
        for i in range(9):
            frame = self.dut.display.get_frame()
            gvision = ocr.GVisionOcr()
            texts = gvision.ocr_text(self.dut, frame, Region(504, 69, 550, 90), text='New release movies')
