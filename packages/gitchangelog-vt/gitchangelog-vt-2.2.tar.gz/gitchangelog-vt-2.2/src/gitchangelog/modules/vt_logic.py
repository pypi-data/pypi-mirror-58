...
def find_replace(general_split, jira_url):
    ...
    to_change = re.sub(r'([A-Z]+-[\d]+)', to_render, general_split)
    if re.search(r'(^[A-Za-z]{3}:)', to_change):
        del_sgi = to_change.split(" ")
        ## Deleting SGI more identifier NONE or jira for convenience.
        del(del_sgi[0]) ## Deleting SGI.
        del(del_sgi[0]) ## Deleting Identifier.
...
def commit_message(commit, jira_url):
    ...
    to_change = find_replace(general_split, jira_url)
