...
## Adding the gitchangelog directory to python path.
...
__version__ = "3.0.7"  ## replaced by autogen.sh
...
def versions_data_iter(repository, revlist=None,
    ...
    if module:
        try:
            ## Receive the module to use
            module_import = import_from("modules", module)
        except Exception:
            try:
                module_import = import_from("gitchangelog.modules", module)
            except Exception:
                print("¡Ups! You are trying to access a module that doesn't exist. "
                      "Please verify your request.")
                exit()

    else:
        ## Use vt_logic by default
    ...
    for idx, tag in enumerate(tags):
        ...
        current_version = {
            ...
            "commit": tag,
            "solicited_requests": module_import.see_request_filter(var_to_show),
        }
        ...
        for commit in commits:
            ...
            for i in list_tgs:
                ...
                if commit_tgs.find(commit.sha1) != -1:
                    tgs_split = i[1].split(",") ## When the tag is accompanied.
                    tgs = tgs_split[0]
            ...
            commit_parts = module_import.commit_message(commit, jira_url)
                ...
                for item in commit_f:
                    ...
                    if find_c:
                        sections[matched_section].append({
                            ...
                            "commit_url": module_import.get_repository_url(repository, commit),
                            "only_sgi": commit_parts["only_sgi"],
                            "second_parameter": commit_parts["jira_identifier"],
                            "commit_title": commit_parts["complement"],
                            "links_or_not": commit_parts["links_or_not"],
                            "jira_url": jira_url,
                            "tags": tgs,
                        })
...
def main():
    ...
    opts = command_cmd_lines.parse_cmd_line(usage=i(usage_msg),
    ...
    try:
        content = changelog(
            ...
            url=url, file=file, template=parameters_to_use.change_template(opts, config), module=module,
