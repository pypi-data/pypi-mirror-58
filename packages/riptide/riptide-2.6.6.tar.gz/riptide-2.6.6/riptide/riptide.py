...
def contextualize(model, transcriptome = 'none', samples = 500, silent = False, exch_weight = False,
    fraction = 0.8, minimum = None, conservative = False, objective = True, additive = False, essential = [],
    ...
    min_coefficient_dict, max_coefficient_dict, gene_hits = _assign_coefficients(transcriptome, riptide_model, minimum, gpr, defined, additive, exch_weight, essential)
    ...
    # Report on included subset of user-defined essential genes
    if len(essential) > 0:
        curr_genes = set([x.id for x in riptide_model.genes])
        included_essential = set(essential).intersection(curr_genes)
        riptide_object.included_essential = included_essential
    else:
        riptide_object.included_essential = essential
...
def _assign_coefficients(raw_transcription_dict, model, minimum, gpr, defined_coefficients, additive, exch_weight, essential):
    ...
    # If user has defined essential reactions, integrate new weights here
    if len(essential) > 0: rxn_min_coefficient_dict = _integrate_essential(model, essential, rxn_min_coefficient_dict)
...
# Assign heaviest weight during pruning to user-defined essential genes
def _integrate_essential(model, essential, coefficient_dict):
