...
class Portfolio(object):
    ...
    def apply_distortion(self, dist, axiter=None, num_plots=0):
        ...
        # M = marginal margin, barM = cumulative margin
        # Q same for capital
        df['M'] = df.gS - df.S
        df['Q'] = 1 - df.gS
        df['roe'] = df.M / (1 - df.gS)
        ...
        for line in self.line_names_ex:
            ...
            df[f'barM_{line}'] = df[f'exag_{line}'] - df[f'exa_{line}']
            # prepend 1 puts everthing off by one
            df[f'M_{line}'] = np.diff(df[f'barM_{line}'], append=0)
            # careful about where roe==0
            df[f'Q_{line}'] = df[f'M_{line}'] / df.roe
            df[f'Q_{line}'].iloc[-1] = 0
            df.loc[roe_zero, f'Q_{line}'] = np.nan
            # fix ROE indexing error?
            # df[f'barQ_{line}'] = self.cumintegral(df[f'Q_{line}'], 1)
            # shift up by one
            df[f'barQ_{line}'] = np.hstack((0, np.cumsum(df[f'Q_{line}']).iloc[:-1]))
            df[f'barroe_{line}'] = df[f'barM_{line}'] / df[f'barQ_{line}']
    ...
    def example_factory_exhibits(self, data_in, p=0, a=0):
        ...
        # !!!!!!!!!
        exhibit['M.Q'] = ex.filter(regex='^Q_', axis=0).values
        exhibit['T.Q'] = ex.filter(regex='^barQ_', axis=0).values
        exhibit['T.ROE'] = ex.filter(regex='^barroe_', axis=0).values
        exhibit['M.ROE'] = float(ex.filter(regex='^roe', axis=0).values)
        exhibit['T.M'] = ex.filter(regex='^barM_', axis=0).values
        ...
        # exhibit['T.ROE'] = roe
        exhibit['A'] = a
        exhibit = exhibit[['A', 'S', 'gS', 'M.LR', 'M.Q', 'M.ROE',   'exi/xgta', 'exi/xgtag', 'exa', 'exag',   'T.M', 'lr',   'T.Q', 'T.ROE' ]]
