...
def getParent4Ele(html,e,tag=None):
  if(e):
    start = e._start
    if not tag:
      tag = _getTag(html,html.rfind('>',0,start)+1,None,html.rfind('<',0,start))
...
def _getEnd4Tag(html,start,tag):
  if not tag:
    tag = _getTag(html,html.find('>',start)+1,None,start)
  return _getEnd(html,start,tag)
  # s = start
  # while(True):
  #   s = html.find('</'+tag, s)
  #   ss = html.find('<'+tag, s)
...
def _getStart4Tag(html,end,tag):
  if not tag: 
    tag = _getTag(html,html.rfind('>',0,end)+1,None,html.rfind('<',0,end))
  return _getStart(html,end,tag)
  # s=0
  # e=end
  # while(True):
  ...
  #     return s
  #   e=s-1
def _getStart(html,end,tag=''):
  s=0
  e=end
  endCount=0
  while(True):
    s1 = html.rfind('<'+tag,0,e)
    s2 = html.rfind('</'+tag,0,e)
    if s1<0: s1=s2
    if s2<0: s2=s1
    s = max(s1,s2)
    # s = html.rfind('<',0,e)
