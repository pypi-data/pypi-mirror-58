...
def _eda_simulate_canonical(sampling_rate=1000,
    ...
    """Simulate canonical skin conductance response (SCR)
...
def eda_simulate(duration=10, length=None, sampling_rate=1000, noise=0.01,
    ...
    start_peaks = np.linspace(0, duration, n_peaks, endpoint=False)

    for start_peak in start_peaks:
        relative_time_peak = np.abs(np.random.normal(0, 5, size=1)) + 3.0745
        scr = _eda_simulate_canonical(sampling_rate=sampling_rate,
