...
def get_error_handler_for_client_callback(kaiju, logger, topic, client_function):
    ...
    def handle_errors_from_client_code(client, userdata, msg):
        ...
        if request_topic is None:
            raise SyntaxError(
                "FATAL: Handler for topic {} failed to receive request topic.".format(
                    topic
                )
            )
        response_topic = "_response/{}".format(request_topic)
        ...
        try:
            ...
            return kaiju.publish(
                response_topic,
                {
                    "status": 500,
                    "body": {
                        "error": {"message": str(error), "trace": format_exc()},
                        "request": msg.payload,
                    },
                },
            )
...
class KaijuMqtt:
    ...
    def subscribe(self, topic, callback, options=None):
        ...
        if not self.connected_event.is_set():
            ...
            self.subscribe_topic_to_json_wrapped_functions_map[
                topic
            ] = get_json_unwrapped_callback(callback)

            self.client.message_callback_add(
                topic, self.subscribe_topic_to_json_wrapped_functions_map[topic]
            )
        ...
        def undo_subscribe():
            with self.unsub_lock:  # threads might enter here simultaneously. Stop that.
                if topic in self.subscribe_topic_to_json_wrapped_functions_map.keys():
                    ...
                    self.logger.debug("Unsubscribed from topic {}".format(topic))
    ...
    def publish(self, topic, what, options=None):
        ...
        :return:
        """
        self.logger.debug("publish on {}".format(topic))
        if options is None:
            options = {"qos": 2}
    ...
    def request(self, topic, payload=None, options=None):  # noqa: C901
        ...
        self.logger.debug("request {}".format(topic))
        if payload is None:
            payload = {}
        if options is None:
            options = {"qos": 2, "timeoutMs": 5000}
        request_id = uuid4()
        timeout_ms = options["timeoutMs"] if "timeoutMs" in options else 5000
        if "qos" not in options:
            options["qos"] = 2

        request_topic = "{}/{}".format(topic, request_id)
        ...
        self.request_topic_to_responder_map[
            request_topic
        ] = get_custom_request_responder(
            self.logger, clear_timeout, return_me, handled_event
        )
        cleanup = self.subscribe(
            "_response/{}".format(request_topic),
            self.request_topic_to_responder_map[request_topic],
            options=options,
        )

        timer = Timer((timeout_ms / 1000.0), unsub_and_fail)
        timer.start()
        self.logger.debug("publishing request on {}".format(request_topic))
    ...
    def handle(self, topic, client_function, options=None):  # noqa: C901
        ...
        self.logger.debug("handle {} messages".format(topic))
        ...
        self.handle_topic_to_function_map[
            topic
        ] = get_error_handler_for_client_callback(
            self, self.logger, topic, client_function
        )
        cleanup = self.subscribe(
            "{}/+".format(topic), self.handle_topic_to_function_map[topic], options
        )
