...
def get_error_handler_for_client_callback(kaiju, logger, topic, client_function):
    ...
    def handle_errors_from_client_code(client, userdata, msg):
        ...
        if request_topic is None:
            raise SyntaxError("FATAL: Handler for topic {} failed to receive request topic.".format(topic))
...
class KaijuMqtt:
    ...
    def handle(self, topic, client_function, options=None):  # noqa: C901
        ...
        cleanup = self.subscribe("{}/+".format(topic), self.handle_topic_to_function_map[topic], options)
        self.handler_unsubscribe_function_list.append(cleanup)

    def use_configuration(self, conf: "SslConfig"):
        if not conf.exists():
            raise ValueError("There is no configuration named {}".format(conf.dir))
        if not conf.iscomplete():
            raise ValueError("The configuration {} is missing a required file.".format(conf.dir))
