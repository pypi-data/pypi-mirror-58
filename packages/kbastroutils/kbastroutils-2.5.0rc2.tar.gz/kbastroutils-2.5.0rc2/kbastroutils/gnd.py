...
#         return xydif
    ...
    ####################
    #################### 
    def make_crclean(self,group=None,params=None,run=False,outpath=None):
        pairs = self.pairs
        meta = self.meta
        self.crclean = GrismCRClean(pairs,group,params,run,outpath,meta)
        if run:
            for i in self.crclean.meta:
                self.meta[i]['FILE'] = self.crclean.meta[i]