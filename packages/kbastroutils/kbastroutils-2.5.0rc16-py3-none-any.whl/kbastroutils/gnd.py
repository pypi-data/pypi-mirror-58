    ...
    def make_xyoff(self):
        for i in self.gid:
            ...
            if self.meta[i]['SUBARRAY']:
                ...
                xoff = make_SIP(coefxoff,xyd[0]+corner[0],xyd[1]+corner[1],startx=True)
            ...
            if method=='median':
                ...
                if not bkg:
                    ...
                    if self.meta[j]['SUBARRAY']:
                        ...
                        tmp = bkgdata[corner[0]:corner[0]+shape[0],corner[1]:corner[1]+shape[1]]
                ...
                if not flatfile:
                    ...
                    for k,kk in enumerate(y):
                        if self.meta[j]['SUBARRAY']:
                            shape = xdata.shape
                            corner = self.meta[j]['SUBARRAY_PARAMS']['CORNER']
                            a[k] = y[k].data[corner[0]:corner[0]+shape[0],corner[1]:corner[1]+shape[1]]
                        else:
                            a[k] = y[k].data
