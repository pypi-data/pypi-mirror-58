...
class Table:
    ...
    def plot(self, xname, yname, title=None, new=True, **kwargs):
        ax=Table._preplot(new)
        ...
        if not isinstance(yname, list):
            ...
            n = len(yname)
            lineObjects = ax.plot([[v]*n for v in self[:][xname]], self[:][yname], **kwargs)
            ...
            if "label" in kwargs:
                ...
                if not hasattr(ax,"savelabel"):
                    ax.savelabel = (lineObjects,kwargs["label"])

                else:

                    ax.savelabel = (ax.savelabel[0] + lineObjects,ax.savelabel[1] + kwargs["label"])

                ax.legend(ax.savelabel[0],ax.savelabel[1])
