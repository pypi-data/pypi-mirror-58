...
class hldb:
    ...
    def connect(self, needPassword=False, needUser=False ,connToBigDB=False ,connToMysql=False):
        ...
        if (connToMysql==False)&(connToBigDB==False):
            ...
            #然后从配置读取基础配置信息
            ip = myconfig.get('conf', 'ip')
            dbname = myconfig.get('conf', 'dbname')
            
            db = oracle_user + '/' + oracle_password + '@' + ip + ':1521/' + dbname
            #最后建立连接
    ...
    def ftp_update(self,remotefilepath,localfilename, needUser=False, needPassword=False):
        ...
        myconfig.read(myconfigFilePath, encoding='utf-8')
        if (needUser):
            ftpusername = input('ftp_user:')
        else:
            ftpusername = myconfig.get('conf', 'ftp_user')
            
        if (needPassword):
            ftppassword = getpass.getpass('ftp_password:')
        else:
            ftppassword = myconfig.get('conf', 'ftp_password')
        ftpip = myconfig.get('conf', 'ftpip')
