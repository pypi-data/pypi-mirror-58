...
from dataclasses import dataclass, fields, is_dataclass, replace, _PARAMS
...
class ZenericFix(metaclass=ZMeta):
    ...
    def __class_getitem__(cls0, params):
        ...
        di = DataclassInfo(name=name, bindings={}, orig=bound_att, extra=())
    ...
    def __new__(mcs, name_orig: str, bases, namespace, **kwargs):
        ...
        if "di" in namespace:
            ...
            if bases:
                ...
                clsi = DataclassInfo(
                    name="", orig=base_info.get_open(), extra=(), bindings={}
                )

                # logger.info('got clsi from subclass', base_info=base_info, clsi=clsi)

        if clsi is None:
            default = DataclassInfo(name_orig, {}, (), ())
            clsi = default
...
class GenericProxy(metaclass=FakeGenericMeta):
    ...
    def __class_getitem__(cls, params2) -> type:
        ...
        if len(types_open) != len(types2):
            ...
            raise ZValueError(msg, types=types_open, types2=types2)
...
def make_type(
    ...
    if cls.__doc__ is not None:
        if "Entity[X](T:" in cls.__doc__:
            raise ZValueError(cls=cls, bindings=bindings, symbols=symbols)
    try:
        res = make_type_(cls, bindings, symbols)
        if res.__doc__ is not None:
            if "Entity[X](T:" in res.__doc__:
                raise ZValueError(cls=cls, bindings=bindings, symbols=symbols, res=res)
        return res
...
def make_type_(
    ...
    # logger.info('make_type', str(cls), str(bindings), str(symbols))
    clsi = get_dataclass_info(cls)
    # logger.info('make_type', clsi=clsi)
    ...
    for k, v in bindings0.items():
        if k in open:
            ...
            pass
            # logger.warning(f'try to bind {k}={v} but not open ({open})')

    # logger.info(bindings0=bindings0, bindings=bindings, clsi=clsi)

    for tu in bindings:
        if tu in clsi.bindings:
            # logger.info(f'trying to bind again {tu}')
            bindings.pop(tu)
    ...
    # logger.info(new_bindings=new_bindings)
    ...
    if clsi.orig:
        ...
        name2 = name_without
    # logger.info('hence the name', name2=name2, orig=clsi.orig, new_bindings=new_bindings)
    ...
    class Fake2:
        def __getitem__(self, item):
            # logger.info(f"Fake2:getitem", item=item)
            n = name_for_type_like(item)
    ...
    clsi2 = DataclassInfo(
        name=name2, bindings=new_bindings, orig=clsi.orig, extra=extra2
    )
    set_dataclass_info(cls2, clsi2)
    #
    # if cls2.__name__ in ['PublicKey', 'Effect', 'SecurityModel'] and new_bindings:
    #     raise ZValueError('wow', cls=cls, cls2=cls2,
    #                       clsi=clsi,
    #                       clsi2=clsi2)
