...
def reorder_annotations(T, anns: Dict[str, type]) -> Dict[str, type]:
    def is_optional_ann(k: str, v: type):
        #
        # if is_ClassVar(v):
        #     return False
        return getattr(T, k, "no") != "no"

    optional = {}
    regular = {}
    for k, v in anns.items():
        if is_optional_ann(k, v):
            optional[k] = v
        else:
            regular[k] = v

    final = {}
    final.update(regular)
    final.update(optional)
    ...
    logger.info(
        debug_print(
            dict(T=T, anns=anns, optional=optional, regular=regular, final=final)
        )
    )
    final_order = list(final)
    if hasattr(T, dataclasses._FIELDS):
        the_fields = getattr(T, dataclasses._FIELDS)
        current = list(the_fields)
        ordered = sorted(current, key=final_order.index)
        logger.info(f"current {current}")
        logger.info(f"ordered {ordered}")
        final_fields = {k: the_fields[k] for k in ordered}
        setattr(T, dataclasses._FIELDS, final_fields)
        # logger.info(f'fields {the_fields}')
        # the_fields.sort(key=lambda f: final_order.index(f.name))
