...
class Model(object):
    ...
    def list(cls, conn, **kwargs):
        ...
        if resp.status != 200:
            raise BadRequestError(resp.status)
...
class DictionaryItem(Model):
    COLLECTION_PATTERN = Service.COLLECTION_PATTERN + \
        '/$service_id/dictionary/$dictionary_id/items'
    INSTANCE_PATTERN = Service.COLLECTION_PATTERN + \
        '/$service_id/dictionary/$dictionary_id/item/$item_key'
    INSTANCE_CREATE_PATTERN = Service.COLLECTION_PATTERN + \
        '/$service_id/dictionary/$dictionary_id/item'
