...
class AugSequence(Sequence):
    def __init__(self, x_set, batch_size=32, augmentations=lambda image: image):
        self.x = x_set
...
class TTABooster:
    ...
    def features_and_probs(self, x_chunck):
        layer_name = 'flatten'  # TODO: Extract the last layer name from the model
    ...
    def predict(self, x, chunk_size=100):
        ...
        for index, x_chunck in enumerate(TTABooster.chunker(x, chunk_size)):
            ...
            random_sample_probs = sample_probs[:, 0:n_classes * self.k]

            for i in range(sample_features.shape[0]):
                AC = AgglomerativeClustering(n_clusters=self.k, linkage='average', affinity='cosine').fit(
                    np.stack([sample_features[i:i + 1, x:x + n_features] for x in
                              range(0, sample_features.shape[1], n_features)],
                             axis=1).reshape(self.n_tta, n_features))
                temp = pd.DataFrame(
                    np.stack(
                        [sample_probs[i:i + 1, x:x + n_classes] for x in range(0, sample_probs.shape[1], n_classes)],
                        axis=1).reshape(self.n_tta, n_classes))
            ...
            for i in range(random_sample_probs.shape[0]):
                random_temp = pd.DataFrame(
                    np.stack([random_sample_probs[i:i + 1, x:x + n_classes] for x in
                              range(0, random_sample_probs.shape[1], n_classes)],
                             axis=1).reshape(self.k, n_classes))
