...
#
# It is responsible for watching any async_ calls used to fetch data for
...
class PreparerMultiTask(PreparerTask):
  ...
  def add_async(self, async_):
    self.async_.append(async_)
...
class Preparer(Debuggable):
  ...
  def handle_task(self, prepare_cycle, task):
    ...
    if cache_key and cache_key in self.cache:
      ...
      task.async_ = self.pool.apply_async(task.func, task.args, task.kwargs, make_cb(self, prepare_cycle, cache_key))
