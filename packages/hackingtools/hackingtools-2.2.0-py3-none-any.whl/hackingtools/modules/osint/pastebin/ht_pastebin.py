...
class StartModule():
	...
	def search_pastebin(self, keywords, run_time=0, match_total=None, crawl_total=None):
		length = 0
		time_out = False
		found_keywords = []
		paste_list = set([])
		...
		try:
			...
			while True:
				...
				Logger.printMessage("Crawled total of {n} Pastes, Keyword matches {k}".format(n=len(paste_list), k=len(found_keywords)))

				if run_time and (start_time + datetime.timedelta(seconds=run_time)) < datetime.datetime.now():
					Logger.printMessage("Reached time limit, Found {n} matches.".format(n=len(found_keywords)))
					return found_keywords
				# Exit if surpassed specified match timeout 
				if match_total and len(found_keywords) >= match_total:
					Logger.printMessage("Reached match limit, Found {n} matches.".format(n=len(found_keywords)))
					return found_keywords

				# Exit if surpassed specified crawl total timeout 
				if crawl_total and len(paste_list) >= crawl_total:
					Logger.printMessage("Reached total crawled Pastes limit, Found {n} matches.".format(n=len(found_keywords)))
					return found_keywords
	...
	def __find_keywords__(self, raw_url, found_keywords, keywords):
		...
		for keyword in keywords:
			if paste.find(keyword) != -1:
				found_keywords.append("found {k} in {r}\n".format(k=keyword, r=raw_url))
				break
