...
public:
 ...
 ContC CTextT&   append(const C& container);  // append single character or string buffer, supports all value-type STL containers 
    ...
    ContS bool   endsWithAny(const C& container, size_t from = 0, bool bCase = true, size_t* idx = nullptr) const;  //returns true if end with any of the provided strings
    ...
    ContS size_t findWordPositions(const C& list, RangeVector& pos, bool bCase, const T* sep = SeparatorsWord) const;
...
inline const T*  str(size_t from = 0) const; //return pointer to modifiable buffer (valid after C++11)
 ...
 CTextT<wchar_t> toWide();  // conversion to Unicode
  ...
  ContS CTextT&  wordsReplaceAny(const C& list, const T* with, bool bCase = true, const T* sep = SeparatorsWord);  // replace whole words matching the list
  ContS CTextT&  wordsReplaceWithChar(const C& list, const T c, bool bCase = true, const T* sep = SeparatorsWord);
...
protected:
    ...
    ContS static CharTrie BuildTrie(C& container, bool bCase = true);
