...
bool CTextT<T>::startsWith(const S& s, size_t from, bool bCase, size_t* count) const
{
    ...
    return StartsWith(str(from), (const T*)s, bCase, 0, count);
}

//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::startsWith(const std::basic_string<T>& s, size_t from, bool bCase, size_t* count) const
{
    if(isEmpty() || from >= length())
        return false;

    return StartsWith(str(from), s.c_str(), bCase, 0, count);
}
...
template <typename T>
template<typename C, typename Val, typename X>
CharTrie CTextT<T>::BuildTrie(C& list, bool bCase)
{
    CharTrie trie;
    size_t pos = 0;
    if(bCase == true)
    {
        for(auto& str : list)
        {
            trie.add((const CharTrie::byte*)(ToStr(str)), Strlen(str) * sizeof(T), pos);
            pos++;
        }
    }
    else
    {
        ...
        for(auto& str : list)
        {
            sTemp = str;
            sTemp.toLower();
            trie.add((const CharTrie::byte*)(sTemp.str()), sTemp.length()*sizeof(T), pos);
            pos++;
        }
    }
    return trie;
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::endsWith(const std::basic_string<T>& s, size_t from, bool bCase) const
{
    if(isEmpty() || EmptyOrNull(s.c_str()) || from >= length())
        return false;

    from = length() - from;  //from is calculated from the back

    return EndsWith(str(), s.c_str(), bCase, from);
}
...
template <typename T>
template<typename C, typename Val, typename X>
size_t CTextT<T>::findWordPositions(const C& list, RangeVector& pos, bool bCase, const T* sep) const
{
    if(isEmpty())
        return 0;
    ...
    {
        if(t.contain((const CharTrie::byte*)(str(start)), count * sizeof(T)))
            pos.push_back(std::make_pair(start, start + count - 1));
    }
...
size_t CTextT<T>::removeBlocks(const T* sepBegin, const T* sepEnd)
{
    ...
    RangeVector pos;
    while((nFound = findBlockPositions(sepBegin, sepEnd, pos)) > 0)
    {
        removeAt(pos);
        nTotFound += nFound;
    }
...
size_t CTextT<T>::findBlockPositions(const T* sepBegin, const T* sepEnd, std::vector< std::pair<size_t, size_t>>& pos, bool bCase) const
{
    ...
    while(*s)
    {
        ...
        pos.push_back(std::make_pair(start, posEnd));
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::nextWord(size_t& pos, CTextT<T>& word, const T* sep) const
{
    size_t start, count;
    word.clear();
...
size_t CTextT<T>::countWordFrequencies(C& container, bool bCase, const T* sep) const
{
    ...
    for(auto & v : freq)
        container.insert(container.end(), std::make_pair(v.second, v.first.str()));
...
template <typename T>
template<typename C, typename Val, typename X>
CTextT<T>& CTextT<T>::wordsReplaceAny(const C& list, const T* with, bool bCase, const T* sep)
{
    RangeVector pos;
...
//-----------------------------------------------------------------------------------------------------------
template <typename T>
bool CTextT<T>::convertToHex(const T* sep)
{
