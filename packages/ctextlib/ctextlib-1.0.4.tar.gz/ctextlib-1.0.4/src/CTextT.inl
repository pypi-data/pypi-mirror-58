...
template <typename T>
template<typename C, typename Val, typename X>
bool CTextT<T>::startsWithAny(const C& list, size_t from, bool bCase, size_t* idx, size_t* count) const
{
    size_t pos = 0;
    for(auto& s : list)
    {
...
size_t CTextT<T>::findPositions(const C& list, RangeVector& pos, bool bCase, std::vector<size_t>* indexes) const
{
    ...
    while(idx < length())
    {
        if(startsWithAny(list, idx, bCase, &idxList, &len))
        {
...
{
    RangeVector pos;
    if(findPositions(list, pos, bCase))
        removeAt(pos);
    return pos.size();
}
