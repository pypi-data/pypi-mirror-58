...
public:
 ...
 ContC CTextT&   append(const C& container);  // append single character or string buffer, supports all value-type STL containers 
    ...
    bool         convertToHex(const T* sep = SPACE);  // convert string to hex values, "Hello World" ---> "48 65 6C 6C 6F 20 57 6F 72 6C 64"
    ...
    CTextT&      equal(T c, size_t len);  // init witb len number of ch
    ...
    bool         endsWith(const std::basic_string<T>& s, size_t from = 0, bool bCase = true) const;
    ...
    ContC bool   endsWithAny(const C& container, size_t from = 0, bool bCase = true, size_t* idx = nullptr) const;  //returns true if end with any of the provided strings
    ...
    ContC size_t findPositions(const C& start, RangeVector& pos, bool bCase = true, std::vector<size_t>* indexes = nullptr) const;
    ...
    DefI bool    fromInteger(Num i, bool append = false); // convert the decimal integer to string
    DefN bool    fromHex(Num i, bool hasBase = true, bool upper = false); // create a hexadecimal string from the number
 ...
 ContN CTextT&   fromChars(const C& container); // append back values in the container (interpret as characters type T)
  ...
  DefN CTextT&   fromMatrix(const Num* buf, size_t w, size_t h, bool asHex = false, size_t wHex = 2, const T* sep = SPACE, const T* sepLine = EOL);  //compose string from a 2D numerical matrix
    ...
    ContC size_t indexOfAny(const C& container, size_t from = 0, bool bCase = true, size_t* idxList = nullptr) const; // find the offset of the first instance of the strings in the list, return string index or -1 if not found
    ...
    bool         nextWord(size_t& pos, CTextT<T>& word, const T* sep = SeparatorsWord) const;
   ...
   ContS CTextT& replaceAny(const C& what, const C& with, bool bCase = true);  // replace found strings in list 'what' with corresponding strings in list 'with'
   ...
   ContS CTextT& reverseAny(const C& container, bool bCase = true);
    ...
    bool         startsWith(const std::basic_string<T>& s, size_t from = 0, bool bCase = true, size_t* count = nullptr) const;
    ...
    ContC bool   startsWithAny(const C& container, size_t from = 0, bool bCase = true, size_t* idx = nullptr, size_t* count = nullptr) const;
...
inline const T*  str(size_t from = 0) const; //return pointer to modifiable buffer (valid after C++11)
 ...
 CTextT<wchar_t> toWide();  // conversion to Unicode
  ...
  ContC CTextT&  wordsEnclose(const T* sBegin, const T* sEnd, const C* begin = nullptr, const C* end = nullptr, const C* has = nullptr, const T* sep = SeparatorsWord); //enclose all words using provided block begin and end strings and the given condition
  ...
  ContC CTextT&  wordsRemove(const C* begin = nullptr, const C* end = nullptr, const C* has = nullptr, const T* sep = SeparatorsWord); //remove all words using provided block begin and end strings and the given condition
