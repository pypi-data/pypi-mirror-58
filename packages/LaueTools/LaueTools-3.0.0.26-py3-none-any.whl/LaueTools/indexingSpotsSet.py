...
class spotsset:
        ...
        if AssignationHKL_res is not None:
            ...
            if verbose:
                ...
                if eulers is not None:
                    print("for this three euler angles [%.1f,%.1f,%.1f]"
                        % tuple(Orientation))
                print(matrix)
    ...
    def IndexSpotsSet(self,
        ...
        while 1:
            ...
            while (((self.UBStack is not None) or (self.UBStack is not [None]))
                and self.UBStack
                ...
                MatchRate, nblinks, missRef = self.AssignHKL(UB,
                                                            grain_index,
                                                            AngleTol=AngleTol_0,
...
#                                             verbose=verbose)
    ...
    def refineUBSpotsFamily(self, grain_index, initial_matrix, getstrain=0,
                                                            use_weights=1,
                                                            nbSpotsToIndex="all",
                                                            verbose=0):
        r"""
    ...
    def refineStrainElementsSpotsFamily(
        ...
        (residues, Uxyz, newUmat, refinedTs, refinedT) = FitO.error_function_strain(results,
                                                                                fitting_parameters_keys,
                                                                                hkls,
                                                                                allparameters,
                                                                                absolutespotsindices,
...
def purgeSpotsinDict(exp_data, twicethetaChi_to_remove, dist_tolerance=0.2):
    ...
    Twicetheta, Chi, tokeep = GT.removeClosePoints_two_sets((tth, chi), twicethetaChi_to_remove,                                                                                    dist_tolerance=dist_tolerance)
...
def transform_matrix_similar(Aref, B, tol=0.001):
    ...
    """
    res, flags = ISS.comparematrices(Aref, B, tol)
...
def index_fileseries_3(fileindexrange, Index_Refine_Parameters_dict=None,
    ...
    for imageindex in list(range(firstindex, fileindexrange[1] + 1, fileindexrange[2])):
        ...
        for material_index in list(range(nb_materials)):
            ...
            # -----------------------------------------
            DataSet.IndexSpotsSet(file_to_index,
                                    key_material,
                                    emin,
                                    emax,
                                    dict_loop,
