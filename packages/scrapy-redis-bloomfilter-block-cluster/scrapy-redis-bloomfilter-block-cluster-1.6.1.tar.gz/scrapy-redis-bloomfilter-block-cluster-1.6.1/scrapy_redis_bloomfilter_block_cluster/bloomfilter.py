...
class HashMap:
    """
    使用 murmurhash3 性能更好
    返回字符串 hash 出来的 offset (整数)，范围 0 - (2^32 - 1)
    """
...
def calculation_bloom_filter_old(n, p):
    m = math.ceil(n * math.log2(math.e) * math.log2(1 / p))
    k = math.ceil(math.log1p(2) * m / n)
    mem = math.ceil(m / 8 / 1024 / 1024)  # 需要的多少 M 内存
    block_num = math.ceil(mem / 512)  # 需要多少个 Redis 512M 的内存块
    return m, k, mem, block_num
