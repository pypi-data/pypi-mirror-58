...
class LockRFPDupeFilter(RFPDupeFilter):
    def __init__(self, server, key, debug, bit, hash_number, block_num, lock_key, lock_timeout):
        super().__init__(server, key, debug, bit, hash_number, block_num)
        self.lock = self.server.lock(lock_key, lock_timeout)
    ...
    def from_settings(cls, settings):
        ...
        return cls(
            ...
            block_num=block_num, lock_key=lock_key, lock_timeout=lock_timeout
        )
