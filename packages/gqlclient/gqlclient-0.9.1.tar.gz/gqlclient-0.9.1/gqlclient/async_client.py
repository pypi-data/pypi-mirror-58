...
class AsyncGraphQLClient(GraphQLClientBase):
    ...
    async def execute_gql_call(self, query: dict) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                logger.error(f"Error connecting to graphql server: "
                             f"server={self.gql_uri}, "
                             f"detail={e}")
                raise ServerConnectionException(f"Error connecting to graphql server: "
                                                f"server={self.gql_uri}, "
                                                f"detail={e}")
            return result
    ...
    async def execute_gql_query(
        ...
        result = await self.execute_gql_call(query)
    ...
    async def execute_gql_mutation(
        ...
        result = await self.execute_gql_call(mutation)
