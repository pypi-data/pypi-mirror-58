...
class Api:
    ...
    def __build_url__(self, resource: str, path: str = None, filters=None, page: int = 1) -> str:
        ...
        return '{}{}?{}'.format(url, resource.strip('/'), parameters)
    ...
    def __read__(self, url: str) -> Dict:
        ...
        self.errors += 1
        error_message = 'GET: {} [{}] {}'.format(url, response.status_code, response.reason)
        if self.silent:
            print(error_message)
...
class Desmond(Api):
    ...
    def center_stats(self,
        ...
        if not url:
            url = self.__append_token__(self.__build_url__('stats',
                                                           filters=filters,
                                                           path='desmond/centers'))

        stats = self.__read__(url)
        ...
        if next_url:
            next_url = self.__append_token__(next_url)

            return self.center_stats(max_records=max_records, url=next_url)
    ...
    def __append_token__(self, url: str) -> str:
        c = '&'

        if '?' not in url:
            c = '?'

        return '{}{}{}={}'.format(url, c, self.__PARAM_TOKEN__, self.auth_token)
