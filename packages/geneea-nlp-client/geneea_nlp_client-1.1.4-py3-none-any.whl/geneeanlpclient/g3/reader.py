...
class _Reader:
    ...
    def _readToken(self, rawToken: JsonType, *, tokenIdx: int) -> Token:
        if self.versionMinor > 1:
            text = rawToken['text']
            off = rawToken['off']
            origText = rawToken.get('origText', text)
            origOff = rawToken.get('origOff', off)
        else:
            text = rawToken['corrText']
            off = rawToken['corrOff']
            origText = rawToken['text']
            origOff = rawToken['off']
            if origText == text:
                origText = text
    ...
    def _readSentence(self, rawSentence: JsonType) -> Sentence:
        ...
        tokens = [self._readToken(raw, tokenIdx=idx) for idx, raw in enumerate(rawTokens)]
