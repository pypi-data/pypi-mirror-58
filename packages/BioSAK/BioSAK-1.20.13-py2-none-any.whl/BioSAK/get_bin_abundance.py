...
BioSAK get_bin_abundance -sam all_bins.sam -bin all_bins -x fa

# get abundance of customized bin clusters
BioSAK get_bin_abundance -sam all_bins.sam -bin all_bins -x fa -cluster bin_cluster.txt

# get abundance of dRep produced bin clusters
BioSAK get_bin_abundance -sam all_bins.sam -bin all_bins -x fa -Cdb Cdb.csv

# How it works
The get_bin_abundance module first gets the number of reads mapped to each reference sequence in the provided sam file. 
Then get the total number/percentage of reads mapped to the sequences in each bin (cluster).
...
def get_bin_abundance(args):
    ...
    # define bin abundacne file
    if bin_cluster_file is None:
        pwd_output_file = '%s_abun.txt' % bin_folder
    else:
        cluster_file_path, cluster_file_basename, cluster_file_extension = sep_path_basename_ext(bin_cluster_file)
        pwd_output_file = '%s_%s_abun.txt' % (bin_folder, cluster_file_basename)
    ...
    print(datetime.now().strftime(time_format) + 'Abundance info exported to %s' % pwd_output_file)
