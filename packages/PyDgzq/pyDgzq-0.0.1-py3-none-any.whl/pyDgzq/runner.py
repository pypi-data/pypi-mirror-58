...
class Runner():
	...
	def __init__(self, user_name, password, tasks_obj, buf):
		self.buf = buf
		self.notes = notes()
	...
	def run(self, interval):
		...
		self.scheduler.add_job(self.query_customer_list, 'interval', seconds=65)
...
class Launcher():
	...
	def do_work(self,name,pwd):
		r = Runner(name,pwd, self.t4s, self.global_buf)
