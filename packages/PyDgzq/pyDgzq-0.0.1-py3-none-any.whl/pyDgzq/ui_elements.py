...
from pyDgzq.config import info
import json
...
class query_customer_page():
	...
	def save_info(self):
		...
		info["is_idno"] = map_[self.is_id.get()]
		info["start_date"] = self.e_date_s.get()
		info["end_date"] = self.e_date_e.get()
		info["interval"] = self.e_inter.get()
		header = '''#-*-coding:utf 8-*-\n\ninfo = '''.encode("utf-8")
		body = json.dumps(info,indent=4).encode("utf-8")
...
class setting_page():
	...
	def build_page(self):
		...
		self.e_id_text.set(info["app_id"])
		...
		self.e_aip_text.set(info["aip_key"])
		...
		self.e_s_text.set(info["secret_key"])
	...
	def save_args(self):
		...
		info["app_id"] = self.e_id.get()
		info["aip_key"] = self.e_aip.get()
		info["secret_key"] = self.e_s.get()
