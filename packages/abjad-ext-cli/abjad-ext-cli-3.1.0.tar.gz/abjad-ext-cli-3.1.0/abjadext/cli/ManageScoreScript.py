...
class ManageScoreScript(ScorePackageScript):
    ...
    def _handle_create(
        ...
        print(
            "    Created {path!s}{sep}".format(path=score_package_name, sep=os.path.sep)
        )
    ...
    def _setup_argument_parser(self, parser):
        ...
        action_group.add_argument(
            "--new", "-N", help="create a new score", metavar="TITLE"
        )
        create_group = parser.add_argument_group("--new options")
        create_group.add_argument(
            "--year", "-y", default=str(datetime.datetime.today().year), metavar="YEAR"
        )
        create_group.add_argument(
            "--composer-name", "-n", default="A Composer", metavar="NAME"
        )
        create_group.add_argument(
            "--composer-email", "-e", default="composer@email.com", metavar="EMAIL"
        )
        create_group.add_argument(
            "--composer-github", "-g", default="composer", metavar="GITHUB_USERNAME"
        )
        create_group.add_argument(
            "--composer-library", "-l", default="library", metavar="LIBRARY_NAME"
        )
        create_group.add_argument(
            "--composer-website", "-w", default="www.composer.com", metavar="WEBSITE"
        )
