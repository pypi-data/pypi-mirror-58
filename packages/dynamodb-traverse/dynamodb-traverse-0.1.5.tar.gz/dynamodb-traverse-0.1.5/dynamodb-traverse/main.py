...
class DynamoDBBase(AWSBase):
    ...
    def get_dynamodb_async(self):
        if self.local:
            return aioboto3.resource(DYNAMODB, endpoint_url=TEST_URL)
        else:
    ...
    def get_client_async(self):
        if self.local:
            return aioboto3.client(DYNAMODB, endpoint_url=TEST_URL)
        else:
    ...
    async def traverse(self, **config):
        ps = [asyncio.create_task(self.produce(i, **config.get(PRODUCER))) for i in range(config.get(PRODUCER)[THREAD_COUNT])] + [
            asyncio.create_task(self.consume(i, **config.get(CONSUMER))) for i in range(config.get(CONSUMER)[THREAD_COUNT])]
    ...
    async def consume(self, segment, **config):
        while True:
            try:
                queued_item = await asyncio.wait_for(self.queue.get(), timeout=config[TIMEOUT])
