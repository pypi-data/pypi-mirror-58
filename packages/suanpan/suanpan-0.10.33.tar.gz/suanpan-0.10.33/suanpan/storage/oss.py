...
class Storage(base.Storage):
    ...
    def copySmallFile(
        ...
        with spbar.one(total=size, config=not quiet) as pbar:
            ...
            self.autoRefreshToken(bucket.copy_object)(
                bucket.bucket_name, objectName, distName
            )
            pbar.update(size)
            return distName
    ...
    def copyLargeFile(
        ...
        with spbar.one(total=size, config=not quiet) as pbar:
            ...
            uploadId = self.autoRefreshToken(bucket.init_multipart_upload)(
                distName
            ).upload_id
            ...
            def _copy(part):
                ...
                result = self.autoRefreshToken(bucket.upload_part_copy)(
                    bucket.bucket_name,
                    objectName,
                    byteRange,
                    distName,
            ...
            self.autoRefreshToken(bucket.complete_multipart_upload)(
                distName, uploadId, parts
            )
            return distName
    ...
    def removeFile(
        ...
        bucket = self.getBucket(bucketOrBucketName)
        self.autoRefreshToken(bucket.delete_object)(fileName)
        if not quiet:
    ...
    def isFile(self, objectName, bucketOrBucketName=None):
        bucket = self.getBucket(bucketOrBucketName)
        return self.autoRefreshToken(bucket.object_exists)(objectName)

    def getStorageMd5(self, objectName, bucketOrBucketName=None):
        bucket = self.getBucket(bucketOrBucketName)
        try:
            return self.autoRefreshToken(bucket.head_object)(objectName).headers.get(
                self.CONTENT_MD5
            )
    ...
    def getStorageSize(self, objectName, bucketOrBucketName=None):
        bucket = self.getBucket(bucketOrBucketName)
        return self.autoRefreshToken(bucket.head_object)(objectName).content_length
