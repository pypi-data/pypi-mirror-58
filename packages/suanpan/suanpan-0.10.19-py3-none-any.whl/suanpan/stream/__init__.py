...
class StreamBase(
    ...
    def emptyHandler(self, context):
        raise error.Error("Method not implemented! This is an empty handler!")
    ...
    def sendSuccessMessage(self, message, data, queue=None, extra=None):
        if not all(key.startswith("out") for key in data):
            raise error.Error("Success Message data only accept keys starts with 'out'")
    ...
    def sendFailureMessage(self, message, msg, queue=None, extra=None):
        if not isinstance(msg, str):
            raise error.Error("Failure Message msg only accept string")
    ...
    def sendMissionMessage(self, message, data, queue=None, extra=None):
        if not all(key.startswith("in") for key in data):
            raise error.Error("Mission Message data only accept keys starts with 'in'")
        data = {key: value for key, value in data.items() if value is not None}
...
class Stream(StreamBase, HasTriggerHooks):
    ...
    def interval(self, value):
        if not isinstance(value, (int, float)):
            raise error.Error("Interval must be int or float")
    ...
    def start(self):
        ...
        if not runners:
            raise error.Error("Stream can not start: No Method implemented!")
