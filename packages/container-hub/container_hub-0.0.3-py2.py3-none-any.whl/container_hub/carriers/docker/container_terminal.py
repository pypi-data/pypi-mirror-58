...
def container_list() -> List[str]:
    ...
    """
    return [
        x['simulation_id'] for x in client.containers.list(
            filters={'label': "simulation_id"})]


def extract_log(sim_uid: str, result_path):
    """
    Extract (copy) Docker log and put it in results directory
    """
...
def up(
    ...
    envs = [
        f"SIM_UID={sim_uid}",
        f"SIM_REF_DATETIME={sim_ref_datetime}",
        f"RESULT_PATH={result_path.as_posix()}",
        f"SCHEDULER_ACTION={scheduler_action}",
    ]
    ...
    if hasattr(settings, 'CONTAINER_LOG_LEVEL'):
        envs += [
           f"LOG_LEVEL={settings.CONTAINER_LOG_LEVEL}",
        ]

    cmd = f"python service.py {settings.REDIS_HOST} {model_config}"
    ...
    for local_path, mount_setting in settings.DOCKER_MOUNT_POINTS.items():
        mounts.append(
            Mount(mount_setting['bind'], local_path,
                  type='bind', read_only=mount_setting['ro'])
        )
