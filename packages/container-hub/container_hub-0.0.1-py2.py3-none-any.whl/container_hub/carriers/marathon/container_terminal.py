...
def up(
    ...
    env = {
        "SIM_UID": sim_uid,
        "SIM_REF_DATETIME": sim_ref_datetime,
    ...
    if hasattr(settings, 'CONTAINER_LOG_LEVEL'):
        env.update({
            'LOG_LEVEL': settings.CONTAINER_LOG_LEVEL,
        })

    constraints = [
        MarathonConstraint("hostname", "CLUSTER", hostname)
        for hostname in settings.HOST_NAMES
    ]

    args = [
        "python",
        "service.py",
        settings.REDIS_HOST, model_config
    ]
...
def _del_app(app_name: str) -> bool:
    ...
    try:
        ...
        logger.exception(
            "Failed to delete app %s.", app_name
        )
        return False
    return True
...
def down(sim_uid: str):
    ...
    if len(apps) > 1:
        msg = f'Found more than one app that matches the name {app_name}. ' \
