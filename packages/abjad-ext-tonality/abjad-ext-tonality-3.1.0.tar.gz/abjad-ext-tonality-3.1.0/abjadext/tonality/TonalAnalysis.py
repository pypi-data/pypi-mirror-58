...
class TonalAnalysis(object):
    ...
    def _is_neighbor_note(note):
        ...
        previous_note = note._leaf(-1)
        next_note = note._leaf(1)
        ...
        preceding_interval_direction = abjad.mathtools.sign(
            preceding_interval.direction_number
        )
        following_interval = next_note.written_pitch - note.written_pitch
        following_interval_direction = abjad.mathtools.sign(
            following_interval.direction_number
        )
    ...
    def analyze_tonal_functions(self, key_signature):
        ...
        result = []
        for component in self._client:
            tonal_function = self._analyze_tonal_function(component, key_signature)
            result.append(tonal_function)
        return result
