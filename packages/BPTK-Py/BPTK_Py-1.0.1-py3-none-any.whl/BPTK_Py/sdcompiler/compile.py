...
try:
    ...
    from .plugins import StockExpressions,ExpandArrays, sortEntities,FindComplexFunctions, resolveSelf, resolveAsterisk
    standalone = False
...
def compile_xmile(src, dest, target):
    ...
    IR = resolveAsterisk(FindComplexFunctions(sortEntities(ExpandArrays(resolveSelf(StockExpressions(parse_xmile(src)))))))
