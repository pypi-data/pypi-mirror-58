    ...
    for elem in deepcopy(expression["args"]):
        ...
        if type_ == "identifier":
            ...
            if labels:
                ...
                for label in labels:
                    expression["args"] += [{"name": name_, "type": "array", "args": [{"name": label, "type": "label"}]}]

        '''
        Handle the asterisk operator: SUM(Stock[*]) becomes SUM(Stock[1], Stock[2] ... )
        '''
        if (type_ == "array"):
            if (not type(elem["args"]== list)):
                elem["args"] = [deepcopy(elem["args"])]
            ...
            if ("*" in elem["args"]):
                ...
                for arg in elem["args"]:
                    if (arg == "*"):
                        ...
                        if (labels):
                            for label in labels:
                                expression["args"] += [{"name": elem["name"],"type": "array", "args": [{"name": label, "type":"label"}]}]
                expression["args"].remove(elem)
...
def FindComplexFunctions(IR):
    ...
    :return:
    """
    complex_functions = {"trend": trendFunction, "smth1": smoothFunction, "mean": arrayFunction, "max": arrayFunction, "min": arrayFunction, "sum":arrayFunction, "prod":arrayFunction, "stddev":arrayFunction, "size":arrayFunction} # This Dict links to the functions required for building new stocks
        ...
        for name, model in IR["models"].items():
            ...
            for entity_type, entities in model["entities"].items():
                for entity in entities:
                    '''
                    The context containts the create function and the model context to manipulate
                    '''
