...
def arrayed_identifiers(expression, dimension):
    ...
    if type(expression) is list:
        return [arrayed_identifiers(x,dimension) for x in expression]

    if "args" in expression.keys():
        args = []
        for arg in expression["args"]:
            args+= [arrayed_identifiers(arg, dimension)]
...
def clone_entity(model_name, entity, idx, product=None, connects=None):
    ent = deepcopy(entity)
    try:
    ...
    if product:
        ...
        # Reparse Equation. Way easier than trying to traverse AST and find the correct part to replace / drop
        ...
        ent["equation_parsed"] = arrayed_identifiers(ent["equation_parsed"], dimension=product)
        ...
        for entity_type, entities in model["entities"].items():
            ...
            for index, entity in enumerate(deepcopy(entities)):
                ...
                if type(entity["equation_parsed"]) is list and len(entity["equation_parsed"]) > 1:
                    ...
                    _entities = [deepcopy(
                        clone_entity(model_name=name, entity=entity, idx=i, product=None, connects=IR["assignments"]))
                    ...
                    _entities = [deepcopy(clone_entity(model_name=name, entity=entity, idx=i, product=products[i],
                                                       connects=IR["assignments"])) for i in range(0, len(products))]
                ...
                if len(_entities) == 1:
                    ...
                    if type(entities[index]["equation_parsed"]) is dict:
                        if not (entities[index]["equation_parsed"]["name"] =="size" and entities[index]["equation_parsed"]["type"]=="call"):
                            entities[index]["equation_parsed"] = already_reduced
                            spread_function_arguments(
                                expression=model["entities"][entity_type][index]["equation_parsed"],
                                model_name=model["name"], IR=IR)
                    elif type(entities[index]["equation_parsed"]) is list and type(entities[index]["equation_parsed"][0]) is dict:
                        if not (entities[index]["equation_parsed"][0]["name"] =="size" and entities[index]["equation_parsed"][0]["type"]=="call"):
                            entities[index]["equation_parsed"] = already_reduced
                            spread_function_arguments(
                                expression=model["entities"][entity_type][index]["equation_parsed"],
                                model_name=model["name"], IR=IR)
                        ...
                        entities[index]["equation_parsed"] = already_reduced
                        spread_function_arguments(expression=model["entities"][entity_type][index]["equation_parsed"],
                                                  model_name=model["name"], IR=IR)
                    #if (entities[index]["name"]=="sizeconverter"):
                        #print(entities[index]["equation_parsed"])
                    entities[index]["labels"] = []
