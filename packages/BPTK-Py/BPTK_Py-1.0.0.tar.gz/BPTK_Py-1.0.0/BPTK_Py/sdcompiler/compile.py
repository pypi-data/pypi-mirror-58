...
try:
    ...
    from .plugins import StockExpressions,ExpandArrays, sortEntities,FindComplexFunctions, resolveSelf
    standalone = False
...
def compile_xmile(src, dest, target):
    ...
    IR = FindComplexFunctions(sortEntities(ExpandArrays(resolveSelf(StockExpressions(parse_xmile(src))))))
