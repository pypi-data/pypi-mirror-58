...
def plot_diagram(x_name, y_name, ax, fig, z_name = ' ', mass_min = 0.5, mass_max = 6.0, color_log = False, isochrones = [], specialIsoschrones = [], specialColor = [], highmainsequence = False, initial_z = '0.2'):
    masses = np.linspace(np.log10(0.2),np.log10(10), 60)
    masses = 10**masses
    path = get_path()
    path = path + '/Z' + initial_z
    start = folder(mass_min)
    end = np.max([folder(mass_max),60])
    ...
    for i in range(start, end+1):
        if x_name == 'built_in':
            if i >52 and highmainsequence:
                x_value, y_value, xlabel, ylabel= built_ins[y_name](path + f'/TAMS/{i}')
            else:
                x_value, y_value, xlabel, ylabel= built_ins[y_name](path + f'/ZAMS/{i}')
        else:
            if i >52 and highmainsequence:
