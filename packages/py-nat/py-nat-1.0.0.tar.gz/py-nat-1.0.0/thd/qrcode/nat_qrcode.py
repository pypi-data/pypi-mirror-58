...
class Generator:
    ...
    def setBgImgPath(self, imgPath):
        imgPath = nat_string.trim(imgPath)
        if nat_path.isFile(imgPath):
            self.__bgImgPath = nat_path.getAbsPath(imgPath)

    # 获取二维码图片的路径
    # return: string
    def getQrPath(self):
        dirPath = nat_path.getAbsPath(self.__destDirPath)
        filename = "%s.%s" %(self.__destBasename, self.__destSuffix)
        return nat_path.join(dirPath, filename)


    # 生成二维码图片
    # return bool
    def generate(self):
        if not nat_cmd.installPipCmd(self.__CMD_MYQR):
            return False

        elif not nat_path.isDir(self.__destDirPath):
            nat_console.error(u"生成二维码图片失败，目录不存在：%s" %(self.__destDirPath))
            return False

        elif "" == self.__destBasename:
            nat_console.error(u"生成二维码图片失败，文件名不合法：%s" %(self.__destBasename))
            return False

        elif "" == self.__url:
            nat_console.error(u"生成二维码图片失败，url 不合法：%s" %(self.__url))
            return False
            ...
            nat_console.warn(u"生成二维码的地址：%s" %(self.__url))

            return nat_cmd.cmd(cmdStr) and nat_path.isFile(self.getQrPath())