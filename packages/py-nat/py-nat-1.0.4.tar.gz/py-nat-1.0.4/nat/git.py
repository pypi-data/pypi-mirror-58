...
# return string
def status_retStr():
    ret = nat.cmd.execRead("git status")
...
def clone(url):
    ...
    if "" != url:
        return nat.cmd.exec("git clone %s" %(url))
    else:
        return False
...
def pull(branchName = "master", repoName = "origin", enableEdit = True):
    ...
    if "" == branchName or "" == repoName:
        ...
        if not enableEdit:
            editMode = "--no-edit"
        return nat.cmd.exec("git pull %s %s %s" %(repoName, branchName, editMode))


# 添加所有修改
# return: bool
def add():
    return nat.cmd.exec("git add .")
...
def commit(messageStr):
    nat.cmd.exec(u"git commit -m \"%s\"" %(messageStr))
...
def push(branchName = "master", repoName = "origin", withTags = False):
    ...
    if "" == branchName or "" == repoName:
        ...
        return nat.cmd.exec("git push %s %s %s" %(repoName, branchName, withTagsStr))
...
# return: [string]
def branch_list():
    ret = nat.cmd.execRead("git branch")
...
def branch_switch(branchNameStr):
    if branch_hasBranch(branchNameStr):
        ...
        if branch_current() == branchNameStr:
            ...
            nat.cmd.exec(u"git checkout %s" %(branchNameStr))
...
def branch_switchSafety(branchNameStr):
    ...
    if "" != branchNameStr and nat.cmd.exec(u"git checkout -b %s" %(branchNameStr)):
        return True
...
def tag_add(tagNameStr):
    ...
    if "" == tagNameStr:
        ...
        return nat.cmd.exec("git tag %s" %(tagNameStr))
...
def tag_remove(tagNameStr):
    ...
    if "" == tagNameStr:
        ...
        return nat.cmd.exec("git tag -d %s" %(tagNameStr))
