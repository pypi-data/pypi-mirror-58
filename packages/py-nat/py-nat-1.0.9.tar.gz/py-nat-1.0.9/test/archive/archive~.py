...
_isModeRelease = "release" == _mode
_isModeStaging = "staging" == _mode or "debug_staging" == _mode
...
if _isIOS:
	...
	# ----- AdHoc 打包
		...
		if not nat_path.cpDir(ipaDirPath, __miniDirPath):
...
if _isIOS:
	nat_path.changeCwd(_projectDir)
	nat_git.pull(targetBranchName, enableEdit = False)
	if not nat_git.push(targetBranchName, withTags = True):
