...
class _Archive(object):
    ...
    def __init__(self, projectDirPath):
        ...
        if not nat_path.isDir(projectPath):
        ...
        # 项目配置
        xcProjDirPath = nat_path.join(self.__projectDirPath, "%s.%s" %(self._projectName, self.__XC_PROJECT_SUFFIX))
        if not nat_path.isDir(xcProjDirPath):
            self._exitLog(u"初始化失败：.xccodeproj 不存在：%s" %(projectDirPath))

        self.__xcCodeProjDirPath = xcProjDirPath

        xcWorkspaceDirPath = nat_path.join(self.__projectDirPath, "%s.%s" %(self._projectName, self.__XC_WORKSPACE_SUFFIX))
        if nat_path.isDir(xcWorkspaceDirPath):
        ...
        if self._isForceEnableUpdateMd():
            ...
            if not nat_path.isFile(updateMdPath):
                nat_file.createEmptyFile(updateMdPath)

            content = nat_string.trim(self._getUpdateContent())
            if "" == content:
                self._onErrorUpdateMdEmpty()
                self._exitLog(u"初始化失败：必须填写更新内容到 %s" %(nat_path.getFilename(updateMdPath)))

            if not self._isUpdateMdContentValid():
                self._exitLog(u"初始化失败：%s 的内容格式不正确" %(nat_path.getFilename(updateMdPath)))


        # provisioning profile
        if [] == self.__getAvailProvisioningProfiles():
            self._exitLog(u"初始化失败：必须设置 bundleID 和 provisioningProfile")
    ...
    def doClean(self):
        ...
        nat_path.rmDirRecursive(exportedArchiveDirPath)

        exportedIpaDirPath = self._getAvailExportedIpaDirPath()
        nat_path.rmDirRecursive(exportedIpaDirPath)
    ...
    def __doExportIPA(self, isAdHoc):
        exportedArchivePath = self.__getAvailExportedArchivePath()
        if not nat_path.isDir(exportedArchivePath):
            self._exitLog(u"导出 IPA 失败：archive 不存在：%s" %(exportedArchivePath))

        # 创建配置选项文件
        exportedIpaDirPath = self._getAvailExportedIpaDirPath()
        exportOptsPlistPath = nat_path.join(self._getAvailExportedArchiveDirPath(), self.__EXPORT_OPTIONS_PLIST_FILENAME)
        if isAdHoc:
        ...
        if self._isEnableAutoSigningAndGenProfile():
    ...
    def doUploadToAppStore(self):
        cmd = "xcrun"
        if not nat_cmd.hasCmd(cmd):
            self._exitLog(u"xcrun 命令未安装")

        ipaFile = self.__getAvailExportedIpaPath()
        if not nat_path.isFile(ipaFile):
            self._exitLog(u"ipa 文件不存在")

        p8Path = self.__getAvailUploadAppstoreApiP8Path()
        if not nat_path.isFile(p8Path):
            self._exitLog(u"p8 文件不存在")

        apiKey = nat_string.trim(self._getUploadAppstoreApiKey())
        apiIssuer = nat_string.trim(self._getUploadAppstoreApiIssuer())
        if "" == apiIssuer:
        ...
        if not nat_cmd.cmd(cmdValidateStr):
    ...
    def _getAvailExportedDirPath(self):
        dirPath = nat_path.getAbsPath( nat_string.trim(self._getExportedArchiveDirPath()) )
        if not nat_path.isDir(dirPath) and "" != dirPath:
            nat_path.mkDirRecursive(dirPath)

        # 创建后再次检查
        if not nat_path.isDir(dirPath):
    ...
    def _getAvailExportedArchiveDirPath(self):
        ...
        dirBasename = nat_string.trim(self._getExportedArchiveDirBasename())
        if "" == dirBasename:
            dirBasename = self.__DEFAULT_EXPORTED_ARCHIVE_DIR_BASENAME

        # archive 目录
        archiveDirPath = nat_path.join(dirPath, dirBasename)
        if nat_path.mkDirRecursive(archiveDirPath):
    ...
    def __getAvailExportedArchivePath(self):
        ...
        basename = nat_string.trim(self._getExportedArchiveBasename())
        if "" == basename:
            basename = self._projectName

        return nat_path.join(dirPath, "%s.%s" %(basename, self.__ARCHIVE_SUFFIX))
    ...
    def _getAvailExportedIpaDirPath(self):
        dirPath = nat_path.getAbsPath( nat_string.trim(self._getExportedIpaDirPath()) )
        if not nat_path.isDir(dirPath) and "" != dirPath:
            nat_path.mkDirRecursive(dirPath)
        ...
        # ipa 导出目录
        exportedIpaDirPath = nat_path.join(dirPath, dirBasename)
        if nat_path.mkDir(exportedIpaDirPath):
            return exportedIpaDirPath
        else:
            self._exitLog(u"创建 ipa 的导出目录失败：%s" %(exportedIpaDirPath))

    def _getAvailExportedIpaDirBasename(self):
        dirBasename = nat_string.trim(self._getExportedIpaDirBasename())
        if "" == dirBasename:
    ...
    def __getAvailExportedIpaPath(self):
        ...
        return nat_path.join(ipaDirPath, "%s.ipa" %(self._projectName))

    # 获取导出 IPA 目录下的 manifest.plist 文件路径
    # return: string
    def _getAvailExportedIpaManifestPlistPath(self):
        return nat_path.join(self._getAvailExportedIpaDirPath(), self.__MANIFEST_PLIST_FILENAME)
    ...
    def __getAvailUploadAppstoreApiP8Path(self):
        ...
        if not nat_path.isDir(dir):
            dir = nat_path.join( nat_path.getUserHomePath(), ".private_keys" )

        filename = self.__getAvailUploadAppstoreApiP8Filename()
        if "" == filename:
            return ""
        else:
            return nat_path.join(dir, filename)



    # 输出 log 日志并退出
    def _exitLog(self, content):
        nat_console.exit(u"%s%s" %(self._ARCHIVE_ERR_PRIFIX, content))
    ...
    def removeExportedArchive(self):
        nat_path.rmDirRecursive(self.__getAvailExportedArchivePath())

    # 删除生成的 *.ipa 文件
    def removeExportedIap(self):
        nat_path.rmFile(self.__getAvailExportedIpaPath())



    # 创建导出 IPA 的选项配置文件：AdHoc
    def __createExportOptsPlistFileForAdHoc(self, exportOptsPlistPath):
        if not nat_path.isValidFilePath(exportOptsPlistPath):
            self._exitLog(u"创建 archive 的选项配置文件失败，路径不合法：%s" %(exportOptsPlistPath))

        elif not nat_plist.createEmptyFile(exportOptsPlistPath):
            self._exitLog(u"创建 archive 的选项配置文件失败：%s" %(exportOptsPlistPath))

        else:
            nat_plist.addStr(exportOptsPlistPath, "teamID", self._getTeamID())

            nat_plist.addStr(exportOptsPlistPath, "method", "ad-hoc")
            nat_plist.addStr(exportOptsPlistPath, "thinning", "<none>")
            nat_plist.addBool(exportOptsPlistPath, "stripSwiftSymbols", True)
            nat_plist.addBool(exportOptsPlistPath, "compileBitcode", True)

            self.__manageExportOptsPlistFileSigning(exportOptsPlistPath)

            dictKey = "manifest"
            downloadUrlPrefix = nat_path.join(self.__getDownloadIpaUrlDomainPath(), self._projectName)
            nat_plist.addDict(exportOptsPlistPath, dictKey)
            nat_plist.addStr(exportOptsPlistPath, "appURL", "%s.ipa" %(downloadUrlPrefix), dictKey)
            nat_plist.addStr(exportOptsPlistPath, "displayImageURL", "%s.57.png" %(downloadUrlPrefix), dictKey)
            nat_plist.addStr(exportOptsPlistPath, "fullSizeImageURL", "%s.512.png" %(downloadUrlPrefix), dictKey)
    ...
    def __createExportOptsPlistFileForAppStore(self, exportOptsPlistPath):
        if not nat_path.isValidFilePath(exportOptsPlistPath):
            ...
            nat_plist.addStr(exportOptsPlistPath, "teamID", self._getTeamID())
            nat_plist.addStr(exportOptsPlistPath, "method", "app-store")
            nat_plist.addBool(exportOptsPlistPath, "stripSwiftSymbols", True)
            nat_plist.addBool(exportOptsPlistPath, "uploadSymbols", True)
            nat_plist.addBool(exportOptsPlistPath, "uploadBitcode", True)
    ...
    def __manageExportOptsPlistFileSigning(self, exportOptsPlistPath):
        ...
        nat_plist.addStr(exportOptsPlistPath, "destination", "export")
        ...
        nat_plist.addStr(exportOptsPlistPath, "signingStyle", signingStyle)
        ...
        if bundleIdProvisionLen > 0:
            ...
            for (bundleID, provisioningProfileName) in bundleIdProvisionList:
                nat_plist.addStr(exportOptsPlistPath, bundleID, provisioningProfileName, dictKey)

        # certificate
        certificateName = self.__getAvailCertificateName()
        if "" != certificateName:
            nat_plist.addStr(exportOptsPlistPath, "signingCertificate", certificateName)


    # 获取 ipa 的下载域名前缀地址，不包含 suffix 部分
    def __getDownloadIpaUrlDomainPath(self):
        domain = nat_string.trim(self._getDownloadIpaUrlDomain())
        urlPath = nat_string.trim(self._getDownloadIpaUrlPath())
    ...
    def __getAvailTeamID(self):
        return nat_string.trim( self._getTeamID() )

    # return: string
    def __getAvailScheme(self):
        scheme = nat_string.trim( self._getScheme() )
        if "" == scheme:
    ...
    def _getAvailUpdateMdPath(self):
        basename = nat_string.trim(self._getUpdateMdBasename())
        if "" == basename:
            basename = self.__DEFAULT_UPDATE_MD_BASENAME

        return nat_path.join(self.__projectDirPath, "%s.md" %(basename))
    ...
    def __getAvailProvisioningProfiles(self):
        ...
        for (bundleId, provisioningName) in self._getProvisioningProfiles():
            bundleId = nat_string.trim(bundleId)
            provisioningName = nat_string.trim(provisioningName)
    ...
    def __getAvailCertificateName(self):
        return nat_string.trim(self._getSigningCertificate())
