...
class RFID(ADriver, APolling):
    ...
    def _read_rfid(self):
        ...
        if not error and data is not None:
            ...
            if not error and uid is not None:
                ...
                result["error"] = "anticoll: {}/{}".format(error, uid)
        elif not error and data is None:
    ...
    def _poll_device(self):
        ...
        self._logger.info("RFID._poll_device - start")
        ...
        if error is not None:
            self._logger.info("RFID._poll_device - rfid sensor produced error: {}".format(error))

        if self._is_publish_candidate(result):
