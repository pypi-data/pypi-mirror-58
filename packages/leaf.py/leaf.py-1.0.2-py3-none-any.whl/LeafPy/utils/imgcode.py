...
class picChecker():
    ...
    def __init__(self, chars=initChars(), size=(95, 27), fontsize=20, \
                 ...
                 pointBorder=(31, 30), useFormula=False):
        """ 
        初始化配置 
        """
        # 验证码配置
        ...
        # 是否使用算式
        self.useFormula = useFormula

    def createFormula(self):
        num1 = random.randint(10, 99)
        num2 = random.randint(10, 99)

        tik = random.randint(0, 1)
        if tik == 0:
            self.randStr = "%d+%d=" % (num1, num2)
            self.outvalue = num1 + num2
        else:
            if num1 > num2:
                self.randStr = "%d-%d=" % (num1, num2)
                self.outvalue = num1 - num2
            else:
                self.randStr = "%d-%d=" % (num2, num1)
                self.outvalue = num2 - num1
    ...
    def getPicString(self):
        ...
        charsToStr = stringjoin(selectedChars, '')
        return (charsToStr)
    ...
    def createChecker(self):
        ...
        if self.useFormula:
            ...
            # 将字符串加入空格
        randStr1 = stringjoin([i + " " for i in randStr], "")
        # 创建图形
    ...
    def out(self):
        ...
        o = BytesIO()
        im.save(o, format=self.outputType)
        imgdata = base64.b64encode(o.getvalue())
        if not PY2:
            imgdata = imgdata.decode("utf-8")
        img_data = "data:image/{};base64,{}".format(self.outputType, imgdata)
        return (randStr, img_data)


if __name__ == '__main__':
    c = picChecker()
    code, img = c.createChecker()
    print(code)
    o = BytesIO()
    img.save(o, format="PNG")
    img_data = base64.b64encode(o.getvalue())
    if not PY2:
        img_data = img_data.decode("utf-8")
    img_data = "data:image/png;base64,{}".format(img_data)
    print(img_data)
