...
class Compiler:
    def __init__(self, reduce=True):
        self.pyindent = 2
        self.reduce = reduce
        self.outtag = ['print>> stdout,', 'print >> stdout,', 'print ', 'print(']
    ...
    def _skipSpace(self, source):
        ...
        assert isinstance(source, list) == True

        source = list(filter(self.not_empty, source))
        # source = [x for x in source if source.strip(x) != '']
    ...
    def _rep(self, line, defkey="print ", escape=False):
        ...
        if fpos != -1:
            content = line[fpos + len(defkey):]
            if defkey == 'print(': content = content[:-1]
            if self.reduce:
                line = """%sout.write(%s, escape=%s)""" % (line[:fpos], content, escape)
            else:
                line = """%sout.write(%s, "\\n", escape=%s)""" % (line[:fpos], content, escape)
        return line
    ...
    def _wipescript(self, script):
        ...
        for line in ldat:
            ...
            if line:
                if not line[-1:] in ['-', '>', '{', '}', ';'] and not line.lower().startswith("function"):
                    line = "%s" % line
