...
from .. import conf
from ..utils.py3helpers import text_type
...
class BaseRender:
    ...
    def _init_loc(self, loc):
        if len(loc) > 0 and loc[0] == '/':
            self._loc = os.path.join(conf.settings.HERE, loc)
    ...
    def _load_template(self, name):
        ...
        if kind == 'dir':
            ...
            path = os.path.join(conf.settings.HERE, path)
...
class Render:
    def __init__(self, locs=("templates",), cache=None, base=None, render_handle=BaseRender, **kwargs):
        self._renders = []
        if isinstance(locs, text_type): locs = (locs,)
...
def frender(filepath, **kwargs):
    ...
    for tmp in templates:
        _path = os.path.join(conf.settings.HERE, tmp, filepath)
        if os.path.isfile(_path): t_files.append(_path)

    path = t_files and t_files[0]
    if not path:
        raise NameError("No template named {}".format(filepath))
