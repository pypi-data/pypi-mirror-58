...
def main(argv):
    sourceDir = os.path.join(LeafPy.__path__[0], "examples")

    if len(argv) not in [1, 2]:
        print("useage: LeafPy-admin.py <projectname> <createonpath>")
        return
    ...
    if ret:
        print("Create NewLeafPyProject: %s on %s Success!" % (prjname, prjpath))
    else:
        print("Create NewLeafPyProject: %s on %s failed." % (prjname, prjpath))
