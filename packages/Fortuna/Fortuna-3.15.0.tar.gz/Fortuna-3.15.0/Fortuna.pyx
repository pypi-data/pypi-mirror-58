...
def random_below(limit: int) -> int:
    ...
    @return: Returns a random integer in the range [0, limit),
        or (limit, 0] for negative limit. Returns zero for limit of zero.
    """
...
def random_index(limit: int) -> int:
    ...
    Returns a random integer in the range [0, number), or [number, 0) for
    negative numbers. Always returns 0 when limit is 0. This symmetry matches
    how python can index a list from the back for negative values or the front
    for positive values.
    Flat uniform distribution.
    """
...
def percent_true(truth_factor: float = 50.0) -> bool:
    """ Produces True or False based on the truth_factor,
        the probability of True as a percentage. """
...
class RandomValue:
    ...
    @return :: Callable Object. `Callable(*args, range_to=0, **kwargs) -> Value`
    ...
    def __str__(self) -> str:
        output = (
            f"RandomValue(collection, zero_cool={self.zero_cool.__name__}",
