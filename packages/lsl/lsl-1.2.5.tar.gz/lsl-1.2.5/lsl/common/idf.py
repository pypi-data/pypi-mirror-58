...
from lsl.common.mcsADP import mjdmpm2datetime, LWA_MAX_NSTD
from lsl.common.adp import freq2word, word2freq, fC
from lsl.common.stations import LWAStation, getFullStations, lwa1
...
__all__ = ['Observer', 'ProjectOffice', 'Project', 'Run', 'Scan', 'DRX', 'Solar', 'Jovian', 'parseIDF',  'getScanStartStop', 'isValid', '__version__', '__revision__', '__all__']
...
class Project(object):
    ...
    def generateSDFs(self, starting_session_id=1, run=0, verbose=False):
        ...
        start = mjdmpm2datetime(ses.scans[0].getMJD(), ses.scans[0].getMPM())
        ...
        for i,station in enumerate(ses.stations):
            ...
            session.setUCFUsername('eLWA/%s_%s_%s_%04i' % (self.id, start.strftime('%y%m%d'), start.strftime('%H%M'), ses.id))
            session.setDataReturnMethod('UCF')
            ...
            for o,obs in enumerate(ses.scans):
                obs_start = mjdmpm2datetime(obs.mjd, obs.mpm)
                if isinstance(obs, DRX):
                    ...
                    new_obs = sdf.DRX(obs.intent, obs.target, _UTC.localize(obs_start), obs.duration, 
                                      ...
                                      gain=obs.gain, MaxSNR=False, comments=comments)
...
class Run(object):
    ...
    def __init__(self, name, id, scans=None, dataReturnMethod='DRSU', comments=None, corr_channels=256, corr_inttime=1.0, corr_basis='linear', stations=getFullStations()):
        self.name = name
        self.id = int(id)
        if scans is None:
            self.scans = []
...
class Scan(object):
    ...
    def getMJD(self):
        ...
        utc = sdf.parseTime(self.start)		## TODO:  We need to get the station informaiton here somehow
    ...
    def addAltPhaseCenter(self, target_or_apc, intent=None, ra=None, dec=None, pm=None):
    ...
    def validate(self, station, verbose=False):
        ...
        if self.computeVisibility(station=station) < 1.0:
            if verbose:
                print("[%i] Error: Target is only above the horizon for %.1f%% of the scan for %s" % (os.getpid(), self.computeVisibility(station=station)*100.0, station.id))
            failures += 1
        ...
        for j,phase_center in enumerate(self.alt_phase_centers):
            ...
            pnt = self.getFixedBody()
            alt_pnt = phase_center.getFixedBody()
...
def __parseCreateScanObject(obsTemp, altTemps=[], verbose=False):
    ...
    for altTemp in altTemps:
        obsOut.addAltPhaseCenter(altTemp['target'], altTemp['intent'], altTemp['ra'], altTemp['dec'], pm=altTemp['pm'])
...
def parseIDF(filename, verbose=False):
    """
    ...
    for line in fh:
        ...
        if keyword == 'SCAN_ID':
            if obsTemp['id'] != 0:
                project.runs[0].scans.append( __parseCreateScanObject(obsTemp, altTemps=altTemps, verbose=verbose) )
                altTemp = {'id': 0, 'target': '', 'intent': '', 'ra': 0.0, 'dec': 0.0, 'pm':[0.0, 0.0]}
                altTemps = []
