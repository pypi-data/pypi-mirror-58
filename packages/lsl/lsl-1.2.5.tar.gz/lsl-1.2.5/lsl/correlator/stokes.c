...
static PyObject *PFBPSDR2(PyObject *self, PyObject *args, PyObject *kwds) {
	...
	#endif
	{
		...
		outX = (float complex *) fftwf_malloc(sizeof(float complex) * (nChan+1)*PFB_NTAP);
		outY = (float complex *) fftwf_malloc(sizeof(float complex) * (nChan+1)*PFB_NTAP);
			...
			for(j=0; j<nFFT; j++) {
				...
				for(k=0; k<2*nChan*PFB_NTAP; k++) {
				    if( secStart - 2*nChan*(PFB_NTAP-1) + k < nSamps*i ) {
					    ...
					    inX[k] = (float) *(aX + secStart - 2*nChan*(PFB_NTAP-1) + k);
					    inY[k] = (float) *(aY + secStart - 2*nChan*(PFB_NTAP-1) + k);
					}
...
Hanning window) on real-valued data to get the PSD for the four Stokes\n\
...
static PyObject *PFBPSDC2(PyObject *self, PyObject *args, PyObject *kwds) {
	...
	#endif
	{
		inX = (float complex*) fftwf_malloc(sizeof(float complex) * nChan*PFB_NTAP);
		inY = (float complex*) fftwf_malloc(sizeof(float complex) * nChan*PFB_NTAP);
		temp2 = (double *) malloc(sizeof(double)*(nChan/2+nChan%2));
			...
			for(j=0; j<nFFT; j++) {
				...
				for(k=0; k<nChan*PFB_NTAP; k++) {
				    if( secStart - nChan*(PFB_NTAP-1) + k < nSamps*i ) {
				        ...
				        inX[k] = *(aX + secStart - nChan*(PFB_NTAP-1) + k);
					    inY[k] = *(aY + secStart - nChan*(PFB_NTAP-1) + k);
					}
...
"Perform a series of polyphase filter bank transforms (4-tap plus a\n\
Hanning window) on complex-valued data to get the PSD for the four Stokes \n\
parameters: I, Q, U, and V.\n\
\n\
Input arguments are:\n\
 * signals: 2-D numpy.complex64 (stands by samples) array of data to FFT\n\
