...
def SpecMaster(signals, LFFT=64, window=noWindow, pfb=False, verbose=False, SampleRate=None, CentralFreq=0.0, ClipLevel=0):
    """
    ...
    if signals.dtype.kind == 'c':
        if pfb:
            output = _spec.PFBPSDC2(signals, LFFT=LFFT, Overlap=1, ClipLevel=ClipLevel)
        else:
            output = _spec.FPSDC3(signals, LFFT=LFFT, Overlap=1, ClipLevel=ClipLevel, window=window)
    else:
        if pfb:
            output = _spec.PFBPSDR2(signals, LFFT=LFFT, Overlap=1, ClipLevel=ClipLevel)
        else:
...
def StokesMaster(signals, antennas, LFFT=64, window=noWindow, pfb=False, verbose=False, SampleRate=None, CentralFreq=0.0, ClipLevel=0):
    """
    ...
    if signals.dtype.kind == 'c':
        if pfb:
            output = _stokes.PFBPSDC2(signals[signalsIndex1], signals[signalsIndex2], LFFT=LFFT, Overlap=1, ClipLevel=ClipLevel)
        else:
            output = _stokes.FPSDC3(signals[signalsIndex1], signals[signalsIndex2], LFFT=LFFT, Overlap=1, ClipLevel=ClipLevel, window=window)
    else:
        if pfb:
            output = _stokes.PFBPSDR2(signals[signalsIndex1], signals[signalsIndex2], LFFT=LFFT, Overlap=1, ClipLevel=ClipLevel)
        else:
...
def FXMaster(signals, antennas, LFFT=64, Overlap=1, IncludeAuto=False, verbose=False, window=noWindow, pfb=False, SampleRate=None, CentralFreq=0.0, Pol='XX', GainCorrect=False, ReturnBaselines=False, ClipLevel=0, phaseCenter='z'):
    """
    ...
    if signals.dtype.kind == 'c':
        if pfb:
            FEngine = _core.PFBEngineC2
        else:
            FEngine = _core.FEngineC3
    else:
        if pfb:
            FEngine = _core.PFBEngineR2
        else:
...
def FXStokes(signals, antennas, LFFT=64, Overlap=1, IncludeAuto=False, verbose=False, window=noWindow, pfb=False, SampleRate=None, CentralFreq=0.0,  GainCorrect=False, ReturnBaselines=False, ClipLevel=0, phaseCenter='z'):
    """
    A more advanced version of FXCorrelator for TBW and TBN data.  Given an 
    2-D array of signals (stands, time-series) and an array of stands, compute 
    the cross-correlation of the data for all baselines.  Return the frequencies 
