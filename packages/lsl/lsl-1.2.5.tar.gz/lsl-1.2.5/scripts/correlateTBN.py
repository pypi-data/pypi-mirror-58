...
def processChunk(idf, site, good, filename, intTime=5.0, LFFT=64, Overlap=1, pfb=False, pols=['xx',], ChunkSize=100):
    ...
    for s in xrange(ChunkSize):
        ...
        for pol in pols:
            print "->  %s" % pol
            blList, freq, vis = fxc.FXMaster(data, mapper, LFFT=LFFT, Overlap=Overlap, pfb=pfb, IncludeAuto=True, verbose=False, SampleRate=sampleRate, CentralFreq=centralFreq, Pol=pol, ReturnBaselines=True, GainCorrect=True)
...
def main(args):
    ...
    nFrames = int(args.avg_time*sampleRate/512)
    args.offset = idf.offset(args.offset)
    nSets = idf.getInfo('nFrames') / nFpO / nFrames
    nSets = nSets - int(args.offset*sampleRate/512) / nFrames
    ...
    print "Offset: %.3f s (%i frames)" % (args.offset, args.offset*sampleRate/512)
