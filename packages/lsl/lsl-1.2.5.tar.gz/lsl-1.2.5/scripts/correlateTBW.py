...
def processChunk(idf, site, good, filename, LFFT=64, Overlap=1, pfb=False, pols=['xx','yy']):
    ...
    for pol in pols:
        ...
        for k in xrange(nSec):
            blList, freq, vis = fxc.FXMaster(data[toKeep,k*secSize:(k+1)*secSize], mapper, LFFT=LFFT, Overlap=Overlap, pfb=pfb, IncludeAuto=True, verbose=False, SampleRate=sampleRate, CentralFreq=0.0, Pol=pol, ReturnBaselines=True, GainCorrect=True)
