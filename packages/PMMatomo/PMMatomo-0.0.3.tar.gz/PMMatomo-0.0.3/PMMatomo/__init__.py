...
class Matomo:
    ...
    def _baseUrl(self):
        period = self._period
        filter_limit = "-1" if self._filterLimit <= 0 else self._filterLimit # turn on or of the filter
        _url = self._apiUrl+"/?module=API&idSite="+str(self._siteId)+"&format=json&token_auth="+self._token+"&filter_limit="+str(filter_limit)+"&period="+period+"&date="+self._date
        return _url

    def _call_api_json(self, url) -> Optional[str]:
        result = requests.get(url)
        if result.status_code == 200:
            return result.text
        return None

    def _call_api(self, url) -> Optional[List]:
