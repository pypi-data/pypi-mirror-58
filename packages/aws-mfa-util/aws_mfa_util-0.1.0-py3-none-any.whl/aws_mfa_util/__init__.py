...
AWS_CREDENTIAL_PATH: str = str(Path.home() / '.aws/credentials')
DEBUG_PATH: str = str(Path.home() / 'Downloads/out.txt')
    ...
    try:
        ...
        if action_answers['select_action'] == 'MFA用プロファイルを新規作成':
            ...
            new_aws_mfa_credential: AwsCredential = AwsCredential(
                ...
                '',
                session_credential['aws_session_token'],
            )
            aws_credential_dict[
        ...
        if DEBUG:
            AWS_CREDENTIAL_PATH = DEBUG_PATH
