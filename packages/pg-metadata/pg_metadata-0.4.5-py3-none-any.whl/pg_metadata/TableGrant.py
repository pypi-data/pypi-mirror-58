...
class TableGrant():
    def __init__(self, row={}):
        ...
        self.FullName = '%s.%s.%s' % (self.Schema, self.Table, self.Grantee)
    ...
    def QueryAdd(self):
        return 'GRANT %s ON TABLE %s.%s TO %s;' % (', '.join(self.Actions), self.Schema, self.Table, self.Grantee)

    def QueryRemove(self):
        return 'REVOKE ALL ON TABLE %s.%s FROM %s;' % (self.Schema, self.Table, self.Grantee)
