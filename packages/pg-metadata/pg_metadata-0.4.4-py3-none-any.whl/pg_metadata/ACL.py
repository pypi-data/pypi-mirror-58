...
class ACL():
    def __init__(self, instance_type, instance_name, owner_name, acl, ddl_style=""):
        self.InstanceType = instance_type or ""
        self.InstanceType = self.InstanceType.strip().upper()
        assert len(self.InstanceType) > 0

        self.ObjectType = ("%s_GRANTS" % (self.InstanceType)).strip().upper()
    ...
    def DDL_Create(self):
        r = ""
        for grant in sorted([g.DDL_Create() for g in self.Grants.values()]):
            r += grant
            r += SEP
        return r.strip()
    ...
    def DDL_Drop(self):
        r = ""
        for grant in sorted([g.DDL_Drop() for g in self.Grants.values()]):
            r += grant
            r += SEP
        return r.strip()
