...
def gmsh_vertexidx(d):
  return np.arange(d+1)
...
def get_dims(q,me,**kwargs):
  ...
  dim,nq=q.shape
    ...
    #nq,dim=q.shape
  ndfe,nme=me.shape
  #assert ndfe==dim+1, 'Trouble with q %s and me %s shapes'%(str(q.shape), str(me.shape))
  return (dim,ndfe-1,nq,nme)

def getMeshArrays(**kwargs):
  sdim=kwargs.pop('sdim','2D')
  assert sdim in ['2D','3D','3Ds']
  d=kwargs.pop('d',2) # d-simplex
  ...
  if small:
    s='_small'
  npzfile = np.load(data_path+os.sep+'mesh%dsimp%dorder%s%s.npz'%(d,order,sdim,s))
  q=npzfile['arr_0']
  me=npzfile['arr_1']
  toGlobal=npzfile['arr_2']
  return q,me,toGlobal

def getMesh2D(d,**kwargs):
  assert d==2 or d==1 or d==0
  return getMeshArrays(sdim='2D',d=d,**kwargs)

def getMesh3D(d,**kwargs):
  assert d==3 or d==2 or d==1 or d==0
  return getMeshArrays(sdim='3D',d=d,**kwargs)

def getMesh3Ds(d,**kwargs):
  assert d==2 or d==1 or d==0
  return getMeshArrays(sdim='3Ds',d=d,**kwargs)
