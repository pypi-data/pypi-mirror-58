...
class ScrapeRequest:
    def __init__(self, term: str, count: int, domain: Optional[str] = None,
                 sleep: int = 0, proxy: Optional[str] = None,
                 language: Optional[str] = None, yandex_geo: Optional[str] = None):
        self.term = term
        self.count = count
        self.domain = domain
        self.sleep = sleep
...
class SearchResult:
    def __init__(self, rank: int, url: int, title: str, description: str,):
        self.rank = rank
        self.url = url
        self.title = title
    ...
    def __repr__(self):
        return "<SearchResult: Rank:{}, URL: {}>".format(
            self.rank,
            self.url
        )
...
class SearchScraper(metaclass=ABCMeta):
    ...
    def user_agent() -> Dict[str, str]:
        return {'User-Agent': choice(_USER_AGENTS),
                'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8'}

    async def _scrape_one(self, url: str, headers: Dict[str, str], proxy: Optional[str]) -> ScrapeResponse:
