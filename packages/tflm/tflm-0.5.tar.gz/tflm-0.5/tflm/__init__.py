...
def modelling(first, second, target, inter_portion=0.5, sqr_portion=0.5, contribution_portion=0.9, final_contribution=0.9, node_size=1.0, final=False, modeller="LightGBM"):
  ...
  first[list(first.drop(columns=cols_drop).columns)]  = first[list(first.drop(columns=cols_drop).columns)].replace({ -1 : 0.0, -1.0: 0.0}) #as long as targets not change
  second[list(second.drop(columns=cols_drop).columns)]  = second[list(second.drop(columns=cols_drop).columns)].replace({ -1 : 0.0, -1.0: 0.0}) #as long as targets not change
  ...
  def shap_frame_keras(first,second, target, node_size):
    ...
    x = first.sample(int(len(first)/1.5), random_state=22)
    y = x.pop(target).values
    val_x = first[~first.isin(x)].dropna()
    val_y = val_x.pop(target).values
    ...
    def build_model(hp):
        ...
        model.add(tf.keras.Input(shape=(len(first.columns)-1,)))
        for i in range( hp.Int('num_fc_layers', 1, 3)):
            model.add( tf.keras.layers.Dense(
                units=hp.Int('num_units_'+str(i+1), 10, 200, step=20),
                activation='relu',
    ...
    tuner = kt.RandomSearch(
        ...
        objective='val_mean_squared_error', #on validation
        max_trials=10,
        overwrite=True,
        directory='tom')
    ...
    reloaded_acc = model.evaluate(x, y)[1]
    best_acc = tuner.oracle.get_best_trials()[0].score
    
    print(best_acc)
    print(model)

    pat = int((1/np.log(input_row))*80)
    stoppy = EarlyStopping(monitor='val_mean_squared_error', patience=pat)

    model.fit(first.drop(columns=[target]), first[target], epochs=180, verbose=1, validation_split=0.2, callbacks=[stoppy])
    ...
    print("fraction" +" " + str(sampled))
    explainer  = shap.GradientExplainer(model,first.drop(columns=[target]).sample(frac=sampled,random_state=22).values)

    shap_values = explainer.shap_values(first.drop(columns=[target]).sample(frac=sampled,random_state=22).values)
  ...
  if len(main_ft) == 0:
    ...
    main_ft = shap_new_abs[shap_new_abs.cumsum().sub((shap_new_abs.sum()*sqr_portion)).le(0)]
  if len(main_ft) == 0:
    main_ft = shap_new_abs[:int(len(shap_new_abs)*0.34)]
  ...
  if len(select_ft) == 0:
    contribution_portion = 0.9
    select_ft = shap_new_abs[shap_new_abs.cumsum().sub((shap_new_abs.sum()*contribution_portion)).le(0)]
  if len(select_ft) == 0:
    select_ft = shap_new_abs[:int(len(shap_new_abs)*0.34)]
...
def runner(first_run, second_run, target, inter_portion=0.8, sqr_portion=0.8, contribution_portion=0.9, final_contribution=0.95, deflator=0.7, node_size=1.0 , runs=2, modeller="Keras"):
#def runner(first_run, second_run, target, inter_portion=0.4, sqr_portion=0.3, contribution_portion=0.9, deflator=0.6, runs=3):
  for r in range(runs):
    ...
    inter_portion = inter_portion * deflator * 1.1
