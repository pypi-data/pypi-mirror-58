...
class Matrix(object):
    ...
    def colmap(self):
        ...
        if self._colmap is None:
            self._colmap = {col:i for i,col in enumerate(self.columns)}
    ...
    def delete_columns(self, columns):
        ...
        Args:
            columns (list[str]): a list of the column names to delete.
        """
        for column_name in columns:
            index = self.colmap[column_name]
            del self.columns[index]
            for col in self.columns[index:len(self.columns)]:
                self.colmap[ self.columns[col] ] -= 1

    def rename_columns(self, **new_name_map):
        """Rename one or more columns. Expects keyword arguments in which the keyword is the current column name and the value is the new column name."""
        for oldname, newname in new_name_map.items():
            self.columns[self.colmap[oldname]] = newname
            self.colmap[newname] = self.colmap[oldname]
            del self.colmap[oldname]
    ...
    def load(self, *, automated=False):
        ...
        Raises:
            Exception when the load_policy is 'manual' and the 'automated' is True. This is provided so a user can explicitly prevent a load of the mull matrix from the source, such as when the user wishes to prevent a side effect of a full load when the source is too large for the available memory our would take too long for the intended time-to-process of the broader program."""
        if self.loaded:
            return
    ...
    def export(self, filename, topmatter=None, autosize=False):
        ...
        if re.search(r'\.csv$', filename, re.I):
            return(self.export_to_csv(filename, topmatter))
        elif re.search(r'\.xlsx$', filename, re.I):
            return(self.export_to_excel(filename, topmatter, autosize))
        elif re.search(r'\.xls$', filename, re.I):
            return(self.export_to_excel_legacy(filename, topmatter, autosize))
        else:
            raise Exception("Cannot determine export format for file " + filename)
    ...
    def export_cell(self, cell):
        ...
        if type(cell) is str:
            ...
            cell = re.sub('"', '""', cell)
            if retaint.search(cell):
                return('"' + str(cell) + '"')
        return(str(cell))

    def export_to_csv(self, filename, topmatter):
        with open(filename, 'w') as fh:
            if topmatter:
                if type(topmatter) in (list, tuple):
                    ...
                    fh.write('"' + topmatter + '"' + "\n")
            fh.write(','.join(self.export_cell(colname) for colname in self.columns) + "\n")
            for row in self._matrix:
                fh.write(','.join(self.export_cell(cell) for cell in row) + "\n")

    def export_to_excel(self, filename, topmatter, autosize):
        import openpyxl
        ...
        for icol in range(len(self.columns)):
            ...
            cell.value = self.columns[icol]
            cell.font = headerfont
            cell.border = headerborder
        irow += 1

        for rowdata in self._matrix:
            ws.append(rowdata)
        wb.save(filename)
