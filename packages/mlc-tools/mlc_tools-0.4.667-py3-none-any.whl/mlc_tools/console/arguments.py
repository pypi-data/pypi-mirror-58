...
class Arguments:
    ...
    def print_usage(self):
        ...
        print('Options:')
        options = list()
        options.append(self.build_option('mode', '<values> :-: Build mode'))
        options.append(self.build_option('format', '<values> :-: Format of serialized data'))
        options.append(self.build_option('config', '<path-to-yaml-file>:-: Path to specific config file'))
    ...
    def set_command(self, command):
        valid = [
            'init', 'clean', 'run', 'build', 'help', '--help', '-h'
        ]
