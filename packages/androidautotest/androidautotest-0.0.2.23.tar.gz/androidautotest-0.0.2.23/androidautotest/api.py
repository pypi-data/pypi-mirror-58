...
def position_to_string(position):
    ...
    logger.info(r'[matching] {"template":"%s", "screencap":"%s", "exists":"%s",  "confidence": "0.0"}' % (crop_img_path, screencap_path, r'False'))
    
    # tesseract ocr
    Command.write(r"tesseract {} {} -l eng+chi_sim".format(crop_img_path,'ocr_output'))
...
def image_to_string(template_pic):
    ...
    # position
    if type(template_pic) == list or type(template_pic) == tuple:
        return position_to_string(template_pic)
    ...
    Command.write(r"tesseract {} {} -l eng+chi_sim".format(template_path,'ocr_output'))
    with open('ocr_output.txt', mode='r',encoding='utf-8') as file:
        res = file.readline()
        file.close()
        Path.remove('ocr_output.txt')
