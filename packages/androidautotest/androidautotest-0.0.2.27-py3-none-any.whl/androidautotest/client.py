...
from .errors import PathNotExistError,CaseNotFoundError,CaseHasExistError,ADBNotFoundError
...
def install():
	...
	msg = r' To use androidautotest, please copy path "%s" into your environment variables ' % Path.path_join(module_path, 'platform-tools')
	print('*' * (len(msg)+2))
	print('*%s*' % msg)
	print('*' * (len(msg)+2))
# start asm
