...
class Account:
    ...
    def login(self):
        ...
        #  This block will check if the user is already logged in, but there is also
        #  a check_login method in this class. Could I just use lines 29-34 for that?
        ...
        try:
            ...
            if '"userInfo":{"dh":"%s"' % self.username in str(soup):
                print('Logged in..\n')
                return  # If user is already logged in, return to the calling function.
            else:
                raise LoginError
