...
This module is providing generic base classes and helper methods for to integrate
any database driver that is compatible to the Python DB API. The base classes
are providing already all attributes and methods for to handle database connections,
data selections and manipulations.

Additionally this package allows your application to switch dynamically
between databases without the need to adapt the code of your application - simply by selecting
another :ref:`configuration file <config-files>`.
...
class DbBase(SystemConnectionBase, ABC):
    ...
    def __init__(self, system: SystemBase):
        ...
        dsn = credentials.get('DSN') or credentials.get('Dsn') or credentials.get('dsn')
