...
class basic_logger():
    ...
    def format(self,change=None):
        ...
        if change == None:
            if self.counter:
                self.count += 1
                return strftime(self.raw_format).format_map({'count': self.count})
            else:
    ...
    def info(self,msg,**f_kwargs):
        msg = str(msg)
        self.count += 1
        if self.count == True:
            f_kwargs['count'] = self.count
        print(self.format(),str(msg).format_map(f_kwargs),sep=': ',flush=True)
        if self.make_file:
            self.file.write(f'INFO: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
    def debug(self,msg,**f_kwargs):
        self.count += 1
        msg = str(msg)
        if self.count == True:
            f_kwargs['count'] = self.count
        if self.Debug == False:
            return
        print('DEBUG:', self.format()+':',str(msg).format_map(f_kwargs),file=sys.stderr,flush=True)
        if self.make_file:
            self.file.write(f'DEBUG: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
    def warning(self,msg,**f_kwargs):
        self.count += 1
        msg = str(msg)
        if self.count == True:
            f_kwargs['count'] = self.count
        
        print('WARNING:', self.format()+':',str(msg).format_map(f_kwargs),file=sys.stderr,flush=True)
        if self.make_file:
            self.file.write(f'WARNING: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
    def warn(self,msg,**f_kwargs):
        
        self.warning(msg,**f_kwargs) 
    def log(self,msg,level,**f_kwargs):
        msg = str(msg)
        """log with level:
        1: info
        2: debug
        ...
        try:
            ...
            return
        if level == 1:
            self.info(msg,**f_kwargs)
        elif level == 2:
            self.debug(msg,**f_kwargs)
        elif level == 3:
            self.warning(msg,**f_kwargs)
        elif level == 4:
            self.error(msg, fatal=False,**f_kwargs)
        elif level == 5:
            self.error(msg,fatal = True, **f_kwargs)
        else:
            self.warning(f'expected an int between 1 and 5 got number {level}')
        
    def error(self,msg, fatal=False,exit_fatal=False,**f_kwargs):
        self.count += 1
        msg = str(msg)
        if self.count == True:
            f_kwargs['count'] = self.count
        if fatal == True:
            ...
            fatal1 = ''
        print(f'{fatal1}ERROR: {self.format()}: {str(msg).format_map(f_kwargs)}',file=sys.stderr,flush=True)
        if self.make_file:
            self.file.write(f'{fatal1}ERROR: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
...
class Logger(basic_logger):
    def __init__(self,name,debug=False,counter=False,costum_format=None,make_file=False):
        ...
        if self.make_file:
            self.file = open(f'{name}.log', 'w+')
    def add_logger(self,logger,name, **f_kwargs):
    ...
    def show(self,msg,insert,**f_kwargs):
        print(str(str(self.format() if insert==True else '')+':'),str(msg).format_map(f_kwargs),flush=True)
    def title(self,title,msg=None,**f_kwargs):
        msg = str(msg)
        print('-'*30,str(title),'-'*30,sep='')
        if msg!=None:
            print(self.format(),"INFO",str(msg).format_map(f_kwargs),sep=': ',flush=True)
            if self.make_file:
                self.file.write(f'INFO: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
                self.file.flush()
                os.fsync(self.file.fileno())
            
    def startup(self,msg,**f_kwargs):
        msg = str(msg)
        
        print(self.format(),"startup info",str(msg).format_map(f_kwargs),sep=': ',flush=True)
        if self.make_file:
            self.file.write(f'INFO: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
...
class file_logger(Logger):
    def __init__(self,name,debug=False,counter=False,costum_format=None,make_file=True):
        """start the logger if debug is false debug messages wont be showed you can change the debug setteng by calling basic_logger.config(debug=True or False)"""
        self.counter = counter
        self.count = 0
        count = '{count}'
...
class gui_logger(GUI_APLOCATION):
    def __init__(self,name,debug=False,costum_format=None,make_file=True,counter=False):
        """start the logger if debug is false debug messages wont be showed you can change the debug setteng by calling basic_logger.config(debug=True or False)"""
        self.Debug = debug
        thread=False
        mainloop = False
    ...
    def add_logger(self,logger,name, **f_kwargs):
        ...
        exec(f"self.{name} = logger", globals(),locals())
    def show(self,msg,insert,**f_kwargs):
        self.print(str(str(self.format() if insert==True else '')+':'),str(msg).format_map(f_kwargs),flush=True)
    def title(self,title,msg=None,**f_kwargs):
        msg = str(msg)
        self.print('-'*30,str(title),'-'*30,sep='')
        if msg!=None:
            self.print(self.format(),"INFO",str(msg).format_map(f_kwargs),sep=': ',flush=True)
            if self.make_file:
                self.file.write(f'INFO: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
                self.file.flush()
                os.fsync(self.file.fileno())
    ...
    def startup(self,msg,**f_kwargs):
        ...
        self.print(self.format(),"startup info",str(msg).format_map(f_kwargs),sep=': ',flush=True)
        if self.make_file:
            self.file.write(f'INFO: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
    ...
    def info(self,msg,**f_kwargs):
        ...
        self.print(self.format(),str(msg).format_map(f_kwargs),sep=': ',flush=True)
        if self.make_file:
            self.file.write(f'INFO: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
    def debug(self,msg,**f_kwargs):
        ...
        if self.Debug == False:
            return
        self.print('DEBUG:', self.format()+':',str(msg).format_map(f_kwargs),file=sys.stderr,flush=True)
        self.print('DEBUG:', self.format()+':',str(msg).format_map(f_kwargs),file=sys.stderr,flush=True)
        if self.make_file:
            self.file.write(f'DEBUG: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
    ...
    def warning(self,msg,**f_kwargs):
        msg = str(msg)
        self.print('WARNING:', self.format()+':',str(msg).format_map(f_kwargs),file=sys.stderr,flush=True)
        self.print('WARNING:', self.format()+':',str(msg).format_map(f_kwargs),file=sys.stderr,flush=True)
        if self.make_file:
            self.file.write(f'WARNING: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
    ...
    def error(self,msg, fatal=False,exit_fatal=False,**f_kwargs):
        ...
        if fatal == True:
            ...
            fatal1 = ''
        self.print(f'{fatal1}ERROR: {self.format()}: {str(msg).format_map(f_kwargs)}',file=sys.stderr,flush=True)
        if self.make_file:
            self.file.write(f'{fatal1}ERROR: {self.format()}: {str(msg).format_map(f_kwargs)}\n')
            self.file.flush()
            os.fsync(self.file.fileno())
...
def test():
    logger = gui_logger(__name__,counter=True)
