...
def apply_xslt(xmlFile, xsltFile):
    ...
    outstr = etree.tostring(newdom, pretty_print=True, xml_declaration = True, encoding='UTF-8')
...
def transform_xslt(sample_etree, transaction_type, from_version_str, to_version_str):
    ...
    if direction=="forwards":
        greater_version_no = "LIXI-"+transaction_type+"-"+to_version_str.replace('.','_')
        lesser_version_no = "LIXI-"+transaction_type+"-"+from_version_str.replace('.','_')
    else:
        greater_version_no = "LIXI-"+transaction_type+"-"+from_version_str.replace('.','_')
        lesser_version_no = "LIXI-"+transaction_type+"-"+to_version_str.replace('.','_')        
    
    # The complete version lists
    tranforms = []
    for filename in glob.glob(os.path.join(pkg_resources.resource_filename(__name__, 'xslts'),'*.zip')):
    ...
    tranforms_versions = [('LIXI-'+ transaction_type +'-'+ x.replace('.','_')) for x in tranforms]         
    ...
    tranforms_forwards, tranforms_backwards = count_backwards, list(reversed(count_backwards))
    
    if direction=="forwards":
        tranforms_forwards, tranforms_backwards = tranforms_backwards, tranforms_forwards
    ...
    temp_step_warnings_xml = etree.Element("{http://www.w3.org/2001/XMLSchema-instance}warnings")
    
    for first, second in zip(tranforms_list, tranforms_list[1:]):
        
        archive = zipfile.ZipFile(os.path.join(pkg_resources.resource_filename(__name__, 'xslts'), first + "-" + second + ".zip"), 'r')
        stylesheet   = io.BytesIO(archive.read(first + "-" + second + ".xsl"))
        
        archive = zipfile.ZipFile(os.path.join(pkg_resources.resource_filename(__name__, 'xslts'), first + "-" + second + "-warnings.zip"), 'r')
        ...
        temp_current_step_xml = etree.fromstring(apply_xslt(temp_prev_step_xml, stylesheet))
        ...
        temp_current_step_warnings_xml = etree.fromstring(apply_xslt(temp_prev_step_xml, warningsheet))
