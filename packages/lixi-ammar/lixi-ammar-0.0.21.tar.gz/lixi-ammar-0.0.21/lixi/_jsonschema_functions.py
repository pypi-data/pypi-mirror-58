...
def validate_json(json_message, json_schema):
    """Validates a LIXI JSON message.
    
    Args:
        json_message (dict): A python message dict.  
        json_schema (str): A python schema dict.
    ...
    Raises:
        exceptions.ValidationError: Validation errors for the xml file.
    """     
    
    ''' Validate the JSON message against the JSON schema'''
    ...
    def format_year(string_value):
        try:
            ...
            raise exceptions.ValidationError("'"+string_value+"' is not a valid year") 
    ...
    def format_fullyear(string_value):
        try:
            ...
            raise exceptions.ValidationError("'"+string_value+"' is not a valid date")
    ...
    def format_datetime(string_value):
        try:
            ...
            raise exceptions.ValidationError("'"+string_value+"' is not a valid ISO date string")
    ...
    def print_nested_error(error, old_path_list):
        ...
        if isinstance(error, exceptions.ValidationError):  
            if len(error.context) == 0:
                ...
                result_path = repr(path_list).replace("deque(['","").replace("'])","").replace("', '",".")
                out_string += ("\nERROR "+repr(counter) +": "+ error.message +"\n  PATH: "+ result_path)
                counter += 1
            else:
                if isinstance(path_list,deque) and isinstance(error.path,deque): 
                    path_list.extend(old_path_list)
    ...
    if validator.is_valid(json_message):
        ...
        if idref_is_true:
            ...
            for idref in IDREFs:
                ...
                if idref not in IDs:
                    check = True
                if check: out_string += ("\nERROR "+repr(counter) +": IDREF '"+idref+"' does not have a referenced ID, PATH: Message-wide")
                counter += 1
...
def convert_to_json_schema_string(xmlstring):
    ...
    Args: 
        xml_schema (etree):  A lxml etree LIXI schema.
    
    Result:
        json_schemaa (dict): A json LIXI schema dict.
    
    Raises:
        exceptions.ValidationErrors: Validation errors for the xml file.
    """      
...
def convert_to_json_schema(xml_schema):
    ...
    def add_annotation(element, schema):
        ...
        if element.find("./xs:annotation/xs:documentation", namespaces=ns) is not None:
            schema['description'] = element.find("./xs:annotation/xs:documentation", namespaces=ns).text
            
        if element.find("./xs:annotation/xs:appinfo/lx:path" , namespaces=ns) is not None:
            schema['path'] = element.find("./xs:annotation/xs:appinfo/lx:path" , namespaces=ns).text

        if element.find("./xs:annotation/xs:appinfo/lx:label", namespaces=ns) is not None:
            schema['label'] = element.find("./xs:annotation/xs:appinfo/lx:label", namespaces=ns).text
        
        if element.find("./xs:annotation/xs:appinfo/lx:references", namespaces=ns) is not None:
            temp_array = []
            for target in element.findall("./xs:annotation/xs:appinfo/lx:references/lx:target", namespaces=ns):
                temp_array.append(target.text)            
            
            schema['references'] = temp_array
            
        if element.find("./xs:annotation/xs:appinfo/lx:CustomDocumentation", namespaces=ns) is not None:
            schema['custom_description'] = element.find("./xs:annotation/xs:appinfo/lx:CustomDocumentation", namespaces=ns).text
            
        if element.find("./xs:annotation/xs:appinfo/lx:CustomExcerpt", namespaces=ns) is not None:
            schema['custom_excerpt'] = element.find("./xs:annotation/xs:appinfo/lx:CustomExcerpt", namespaces=ns).text             
    ...
    def add_restriction(element, schema, definitions):
        ...
        if len(element.xpath("./xs:restriction/xs:minInclusive|./xs:restriction/xs:maxInclusive|./xs:restriction/xs:minExclusive|./xs:restriction/xs:maxExclusive" , namespaces=ns)) > 0:
    ...
    def add_enumeration(element, schema):
        ...
        for enum in element.findall("./xs:restriction/xs:enumeration", ns):
            ...
            if enum.find("./xs:annotation/xs:documentation", namespaces=ns) is not None:
                temp_dict['description'] = enum.find("./xs:annotation/xs:documentation", namespaces=ns).text
                
            if enum.find("./xs:annotation/xs:appinfo/lx:path" , namespaces=ns) is not None:
                temp_dict['path'] = enum.find("./xs:annotation/xs:appinfo/lx:path" , namespaces=ns).text
    
            if enum.find("./xs:annotation/xs:documentation", namespaces=ns) is not None:
                temp_dict['label'] = enum.find("./xs:annotation/xs:appinfo/lx:label", namespaces=ns).text 
    ...
    def add_pattern(element, schema):
        ...
        schema.pop("$ref", None)
        schema['type'] = "string"
        schema['pattern'] = "^"+element.find("./xs:restriction/xs:pattern", ns).attrib['value']+"$"
    ...
    def add_range(element, schema):
        ...
        if element.find("./xs:restriction/xs:minInclusive" , namespaces=ns) is not None:
            schema['minimum'] = element.find("./xs:restriction/xs:minInclusive" , namespaces=ns).attrib['value']
            
    
        if element.find("./xs:restriction/xs:maxInclusive" , namespaces=ns) is not None:
            schema['maximum'] = element.find("./xs:restriction/xs:maxInclusive" , namespaces=ns).attrib['value']
            
    
        if element.find("./xs:restriction/xs:minExclusive" , namespaces=ns) is not None:
            schema['exclusiveMinimum'] = element.find("./xs:restriction/xs:minExclusive" , namespaces=ns).attrib['value']
            
    
        if element.find("./xs:restriction/xs:maxExclusive" , namespaces=ns) is not None:
            schema['exclusiveMaximum'] = element.find("./xs:restriction/xs:maxExclusive" , namespaces=ns).attrib['value']                    
    ...
    def add_element(element, parent_schema, properties_schema, isComplex, definitions):
        ...
        if max_occurs == "1":
            ...
            if element.attrib.get('type', "object") == "object":        
                ...
                if element.find("./xs:complexType" , namespaces=ns ) is not None or isComplex is True:
                    ...
                    add_complexType(complex_type, temp_schema, temp_schema_properties, definitions)
                        
                    if complex_type.find("./xs:choice/xs:element" , namespaces=ns) is not None:
                        temp_schema['oneOf'] = []
                        
                        # create a dummy object for every elemenet so that its name can come in
                        for sub_element in complex_type.findall("./xs:choice/xs:element", namespaces=ns):
                            ...
                            if temp_schema_properties:
                                new_temp_dict['properties'] = copy.deepcopy(temp_schema_properties)
                                
                            #Recurse
                            new_temp_dict['properties'][sub_element.attrib.get('name')] = add_element(sub_element, new_temp_dict, new_temp_dict['properties'], False, definitions)
                        ...
                        if complex_type.find("./xs:sequence/xs:element" , namespaces=ns) is not None:
                            for sub_element in complex_type.findall("./xs:sequence/xs:element", namespaces=ns):
                                #Recurse
                                temp_schema_properties[sub_element.attrib.get('name')] = add_element(sub_element, temp_schema, temp_schema_properties, False, definitions)
            else:
                ...
                if element_name == "InlineAttachment" or element_name == "Comment":
                    ...
                    temp_schema['properties']['$'] = OrderedDict()
                    xsd_to_json_type(element.attrib.get('type', "object"), temp_schema['properties']['$'], definitions)
                
                else:
                    temp_schema.pop("id", None)
                    xsd_to_json_type(element.attrib.get('type', "object"), temp_schema, definitions)
            ...
            if element.attrib.get('type', "object") == "object":        
                ...
                if element.find("./xs:complexType" , namespaces=ns) is not None:
                    complex_type = element.find("./xs:complexType" , namespaces=ns)
                    add_complexType(complex_type, new_temp_dict, new_temp_dict['properties'], definitions)                
                ...
                if element_name == "InlineAttachment" or element_name == "Comment":
                    ...
                    new_temp_dict['properties'] = OrderedDict()                    
                    new_temp_dict['properties']['$'] = OrderedDict()
                    xsd_to_json_type(element.attrib.get('type', "object"), new_temp_dict['properties']['$'], definitions)
                    
                    temp_array_schema['items'] = new_temp_dict;
                
                else:     
                    new_temp_dict = OrderedDict()
                    xsd_to_json_type(element.attrib.get('type', "object"), new_temp_dict, definitions)
                    temp_array_schema['items'] = new_temp_dict;
    ...
    definitions = OrderedDict() # we cant make these ordered as the validator would not find compile some references before they are used.
    root = xml_schema
    ...
    schema['description'] = root.xpath("./xs:annotation/xs:documentation[2]/text()", namespaces=ns)[0]
    
    # store any schema detail items
    schemadetail = root.xpath("./xs:annotation/xs:appinfo/lx:schemadetail", namespaces=ns)[0]
    for att in schemadetail.attrib:
        schema['schemadetail'][att] = schemadetail.attrib[att]
        
    # store any schema detail items
    schemacustomisation = root.xpath("./xs:annotation/xs:appinfo/li:schemacustomisation", namespaces=ns)
    if len(schemacustomisation) > 0:
        ...
        for customization_type in schemacustomisation:
            ...
            schema['schemacustomisation'][customization_type.tag.replace('{lixi.org.au/schema/appinfo_instructions}','')] = new_array       
    ...
    for complex_type in root.findall("./xs:complexType", namespaces=ns):
        definitions[complex_type.attrib.get('name')] = add_element(complex_type, schema, definitions, True, definitions) 

    # recurse and add all elemenets
    for element in root.findall("./xs:element", namespaces=ns):
        properties[element.attrib.get('name')] = add_element(element, schema, properties, False, definitions)    
...
def convert_to_xml_schema_string(jsonstring):
    ...
    Args: 
        jsonstring (str):  A json LIXI schema string.
    
    Result:
        xmlstring (str): A XML LIXI schema string.
...
def convert_to_xml_schema(json_schema):
    ...
    Args: 
        json_schema (str): A json LIXI schema dict.
    
    Result:
        xml_schema (str): A lxml etree LIXI schema.
    ...
    def create_element(parent, json_element, name=None, use=None):
        ...
        if "properties" in json_element:
            if "$" in json_element["properties"]:
                xml_type = str(json_element["properties"]["$"]["$ref"]).replace("#/definitions/","")
        ...
        if "type" in json_element:
            if json_element['type'] == 'object':
                ...
                if '$ref' in json_element['items']:
                    xml_type = str(json_element['items']["$ref"]).replace("#/definitions/","")                
    ...
    def create_complexType(parent, json_element, name=None):
        ...
        if "properties" in json_element: ##type is object
            ...
            for key, item in json_element["properties"].items():
                if key[:1] != '@' and key!='$':          
                    if 'required' in json_element :
                        use = "required" if key in json_element['required'] else "optional"
                    else:
                        use = "optional"                    
                ...
                for key, item in json_element["items"]["properties"].items():
                    if key[:1] == '@':                
                        if 'required' in json_element["items"] :
                            use = "required" if key in json_element["items"]['required'] else "optional"
                        else:
                            use = "optional"                          
                ...
                for key, item in choice_item["properties"].items():
                    if key[:1] != '@':          
                        if 'required' in choice_item :
                            use = "required" if key in choice_item['required'] else "optional"
                        else:
                            use = "optional"                    
            ...
            first_choice_item = json_element["oneOf"][0] ##if there are attributes for choice they added to all oneOf items in json hence grabbing attributes from the first only will suffice
    ...
    def create_simpleType(parent, json_element, name=None):
        ...
        elif "minimum" in json_element or "maximum" in json_element: ##simpletype is a Range
