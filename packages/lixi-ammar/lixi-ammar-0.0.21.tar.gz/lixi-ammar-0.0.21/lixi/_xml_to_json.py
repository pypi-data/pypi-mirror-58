...
def to_json(xml_etree, xsd_annotated_schema):
    ...
    def json_verify_types(json_data, xsd_element):
        ...
        for every_element in xsd_element.iter():
            if "www.w3.org" in every_element.tag:
                try: 
                    if ('attribute' in every_element.tag and every_element.attrib['type'] is not None):
                    ...
                    elif ('element' in every_element.tag and every_element.attrib['type'] is not None and every_element.attrib.get('name') is not None):
                        element_name = "$"+ every_element.attrib.get('name')
                        element_type = every_element.attrib['type']                    
                        elementsandatt_mappings[element_name] = element_type
        ...
        for del_elementname in complex_mappings:
            elementsandatt_mappings = {k: v for k, v in elementsandatt_mappings.items() if v != del_elementname}                                       
            ...
            if isinstance(v, dict):
                ...
                if k != '$':
                    ...
                    if("number" == lookup or "decimal" == lookup or "float" == lookup):
                        ...
                        if not re.match("^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$", type_cast_value):
                        ...
                        try:
                            ...
                            raise Exception("'"+type_cast_value+"' is not a valid ISO date string")
                    elif ('date' == lookup):
                        try:
                            dt = datetime.datetime.strptime(type_cast_value, '%Y-%m-%d')
                            type_cast_value = type_cast_value
                        except ValueError as e:
                            raise Exception("'"+type_cast_value+"' is not a valid date")
                    elif ('year' == lookup):
                        try:
                        ...
                        if not re.match("^[a-zA-Z0-9-_.]+$", type_cast_value):
                            raise Exception("ID '"+type_cast_value+"' contains forbidden characters")
                        elif not re.match("^[a-zA-Z_]+$", type_cast_value[:1]):
                            raise Exception("ID '"+type_cast_value+"' starts with a forbidden character")                
                        type_cast_value = type_cast_value                        
    ...
    def json_verify_occurance(json_data, xsd_element):
        ...
        unbounded_elements_names = xsd_element.xpath("//xs:element[@maxOccurs>1 or @maxOccurs='unbounded']/@name", namespaces=ns)
        ...
        for elem in schema_root.iter(tag=('{*}element','{*}complexType')):
            ...
            if 'element' in elem.tag and elem.attrib.get('name') is not None:
                if elem.attrib.get('type') is None:
                    key_name = str(elem.xpath('./xs:annotation/xs:appinfo/lx:path/text()', namespaces=ns)).strip('[]\'').replace(".", "/")
                    
                    if elem.find("./xs:complexType/xs:sequence/xs:element" , namespaces=ns) is not None:
                        sorting_elem_list [key_name] =[]
                        for child_xsd_element in elem.xpath("./xs:complexType/xs:sequence/xs:element", namespaces=ns):
                            sorting_elem_list [key_name].append(child_xsd_element.attrib.get('name'))
                    
                    elif elem.find("./xs:complexType/xs:choice/xs:element" , namespaces=ns) is not None:
                        sorting_elem_list [key_name] =[]
                        for child_xsd_element in elem.findall("./xs:complexType/xs:choice/xs:element", namespaces=ns):
                            sorting_elem_list [key_name].append(child_xsd_element.attrib.get('name'))                        
                ...
                if elem.attrib.get('name') is not None:
                    ...
                    if elem.find("./xs:sequence/xs:element" , namespaces=ns) is not None:
                        ...
                        for child_xsd_element in elem.xpath("./xs:sequence/xs:element", namespaces=ns):
                            sorting_complex_list [key_name].append(child_xsd_element.attrib.get('name'))
    ...
    def xml_sort_message(unsorted_xml, sorting_list):
        ...
        for e in unsorted_xml.xpath('//*'):
            ...
            if(elem_path_key in sorting_list):
                e [:] = sorted(e, key=lambda elem: get_elem_key(elem, elem_path_key, sorting_list))
