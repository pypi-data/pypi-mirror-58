...
from .utils import LOCAL_TIMEZONE, create_wxapp_qrcode, encode, decode
    ...
    def get_object(self, queryset=None):
        ...
        if not campaign_record.share_qrcode:
            ...
            errcode, errmsg, image = create_wxapp_qrcode(
                parameters={"page": "pages/index/index", "scene": "0_" + campaign_record.wxuser.user_id.__str__()},
                img_dir=img_dir, img_name=img_name)
            if errcode == 0:
                campaign_record.share_qrcode = 'bee_django_crm/campaign_record/share_qrcode/' + img_name
...
def campaign_record_create(request):
    ...
    record, is_create = CampaignRecord.objects.get_or_create(wxuser_id=wxuser_id, reward_id=reward_id)
    #  生成分享二维码
    if is_create == True:
        img_dir = os.path.join(settings.BASE_DIR, 'media', "bee_django_crm", "gift", "campaign_record", "share_qrcode")
        img_name = record.pk.__str__() + ".jpg"
        errcode, errmsg, image = create_wxapp_qrcode(
            parameters={"page": "pages/index/index", "scene": "0_" + record.wxuser.user_id.__str__()},
            img_dir=img_dir, img_name=img_name)
        if errcode == 0:
            record.share_qrcode = 'bee_django_crm/campaign_record/share_qrcode/' + img_name
            record.save()
...
def bargain_record_create(request):
    ...
    time_str = '2019-12-31 23:59:59'
    _dt = datetime.datetime.strptime(time_str, "%Y-%m-%d %H:%M:%S")
    _dt = LOCAL_TIMEZONE.localize(_dt)
    if timezone.now() >= _dt:
        return JsonResponse(data={
            'errCode': 4,
            'errMsg': "活动已结束"
        })
    records = BargainRecord.objects.filter(op_wxuser_id=wxuser_id, campaign_record=campaign)
    if records.exists():
        return JsonResponse(data={
            'errCode': 2,
            'errMsg': "已为ta助力过"
    ...
    records = BargainRecord.objects.filter(op_wxuser_id=wxuser_id, campaign_record__reward=reward)
    if records.count() >= 3:
        return JsonResponse(data={
            'errCode': 3,
            'errMsg': "没有助力次数了"
    ...
    if r + campaign.result >= 1000:
        r = 1000 - campaign.result
    r = "%.2f" % r
    if r <= 0:
        return JsonResponse(data={
            'errCode': 7,
