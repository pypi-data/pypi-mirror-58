...
class CampaignRecord(models.Model):
    ...
    share_qrcode = models.ImageField(upload_to='bee_django_crm/campaign_record/share_qrcode', null=True, blank=True,
                                     verbose_name='小程序二维码')
    created_at = models.DateTimeField(auto_now_add=True)
    bind_user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, blank=True, verbose_name='红包发放用户账号')
    bind_preuser = models.ForeignKey(PreUser, null=True, blank=True, verbose_name='红包发放新peruser账号')
    is_mk = models.BooleanField(default=False, verbose_name='是否是老缦客')
