        ...
        for rule in self.rules:
            ...
            if not ok:
                if self.value is None:
                    self.error("Field {} does not exist".format(self.name))
                elif type(self.value) in [str, int, bool]:
                    self.error("{}='{}' failed check {}".format(self.name, self.value, rule.name))
                else:
                    self.error("{} failed check {}".format(self.name, rule.name))
        return
        ...
        for rule in self.rules:
            ...
            if not ok:
                if self.value is None:
                    ...
                    self.error("{} failed check {}".format(self.name, str(type(self.value)), rule.name))
                return
        return
