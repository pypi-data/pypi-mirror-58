...
class CAResponseSubField(Field):
    ...
    def was_found(self, field):
        if field.name in self.found:
            old_val = self.found[field.name]
            self.error("Duplicate {} url: {} {}".format(field.name, old_val, field.value))
            return True
        self.found[field.name] = field.value
        return False
...
class MainDomainField(CAResponseSubField):
    ...
    def set(self, value):
        ...
        if self.errors:  # returns any errors
            return self.errors
        for obj in self.value:
            if not self.was_found(self.domain):
        ...
        for obj in self.value:
            ...
            for field in self.fields:
                if matches_url(field, url):
                    if not self.was_found(field):
                        self.set_field(field, url)
                    break
            else:
                self.error("`URLs` field has url {} which doesn't match expected sources/formats".format(url))
        return self.errors
