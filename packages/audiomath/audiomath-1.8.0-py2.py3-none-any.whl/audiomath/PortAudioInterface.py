...
class Player( GenericInterface.GenericPlayer ):
	...
	def __init__( self, sound, device=None, stream=None, resample=False, verbose=None, **kwargs ):
		...
		if not self.__stream.outputDevice: raise OSError( 'device does not support playback' )
		if self.sound:
...
class Recorder( GenericInterface.GenericRecorder ):
	...
	__stream = __verbose = None
	def __init__( self, seconds, device=None, stream=None, start=True, loop=False, verbose=None, filename=None, **kwargs ):
		"""
		Args:
			seconds (float, Sound):
