...
def RequireAudiomathVersion( minimumVersion ):
	"""
	...
	"""
	currentVersion = GetVersions()[ 'audiomath' ] [ 0 ]
	if [ int( x ) for x in currentVersion.split( '.' ) ] < [ int( x ) for x in minimumVersion.split( '.' ) ]:
		raise RuntimeError( 'audiomath version %s or later is required (current version is %s)' % ( minimumVersion, currentVersion ) )
