...
if False:
    error = 0.001
    stmts = file.statements.close_paths[1].statements
    candidate = filter(lambda i: is_equal_value(i[1].start[0], 40, error) or is_equal_value(i[1].end[0], 40, error),
                      [(i, stmts[i]) for i in range(0, len(stmts))])
    candidate = list(candidate)
    stmts = file.statements.close_paths[0].statements
    base = filter(lambda i: is_equal_value(i[1].start[0], 40, error) or is_equal_value(i[1].end[0], 40, error),
                       [(i, stmts[i]) for i in range(0, len(stmts))])
    base = list(base)
    stmt = candidate[0][1]
    bstmt = base[2][1]
    pts = stmt.intersections_with_halfline(bstmt.start, bstmt.end, 0.001)
    pts2 = file.statements.close_paths[1].intersections_with_halfline(bstmt.start, bstmt.end, 0.001)
...
file.write('outputs/outline_fill.gml')
...
ctx.merge(file)
ctx.dump('outputs/outline_fill_flip.gml')
