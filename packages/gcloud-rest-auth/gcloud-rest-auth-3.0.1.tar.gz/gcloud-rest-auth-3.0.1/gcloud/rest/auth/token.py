...
class Token(object):
    ...
    def _refresh_authorized_user(self,
                                       timeout     )                  :
        payload = urlencode({
        ...
        return self.session.post(url=self.token_uri, data=payload,
                                       headers=REFRESH_HEADERS,
                                       timeout=timeout)

    def _refresh_gce_metadata(self,
                                    timeout     )                  :
        return self.session.get(url=self.token_uri,
                                      headers=GCE_METADATA_HEADERS,
                                      timeout=timeout)

    def _refresh_service_account(self,
                                       timeout     )                  :
        now = int(time.time())
        assertion_payload = {
            'aud': self.token_uri,
