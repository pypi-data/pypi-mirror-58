...
class LiveList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if hasattr(settings, "COURSE_LIVE_PROVIDER_LIST"):
            ...
            provider_list = None
        context["search_form"] = UserLiveSearchForm(provider_list, {"status":status,"name": name, "provider": provider})
        scope = self.request.GET.get("scope")
        offset = self.request.GET.get("offset")
        title_str = ""
        if scope in ["day", 'week', "month"]:
...
def recover_user_live(request, user_live_id):
    if request.method == "POST":
        ...
        signals.user_live_recover.send(sender=UserLive, user_live=user_live)
        update_live_mins(user_live.user)
