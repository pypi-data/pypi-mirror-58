...
from .forms import CourseForm, SectionForm, CourseSectionForm, CourseSectionOrderForm, VideoForm, UploadImageForm, \
    ...
    CourseSectionQuestionSearchForm, UserLiveSearchForm, LiveReportSearchForm
...
class LiveReport(ListView):
    model = User
    template_name = 'bee_django_course/live/report.html'
    ...
    def get_qureyset(self):
        ...
        if not start_time or not end_time:
            messages.error(self.request, '搜索条件不完整')
            return
        return self._get_student()

    def get_context_data(self, **kwargs):
        context = super(LiveReport, self).get_context_data(**kwargs)
        context["user_class"] = self._get_class()
        context["search_form"] = LiveReportSearchForm()
        return context
