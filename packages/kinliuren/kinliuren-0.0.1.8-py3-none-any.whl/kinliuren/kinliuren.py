...
def find_sike_relations(jieqi, daygangzhi, hourgangzhi):
    ...
    if sike_list.count("上尅下") == 0 and sike_list.count("下賊上") == 0:
        ...
        for n in yy_list:
            ...
            nn_list.append(p)
        if len(set(zeikeshang_list)) == 2:
            if nn_list.count("True") == 2:
                 findtrue = ["試涉害", find_ke,  zeikeshang_list, classify, nn_list, yy_list] #結果, 尅克位置, 課式
            else:
                findtrue = ["試比用", find_ke,  zeikeshang_list, classify, nn_list, yy_list]
        elif len(set(zeikeshang_list)) > 2:
            findtrue = ["試涉害", find_ke,  zeikeshang_list, [zeikeshang_list[b] for b in duplicates(nn_list, "True")]] #結果, 尅克位置, 課式
        return  sike_list, sike, shangke_list, checkdayganzhi, checkfuyin, checkmoongeneralconflicttohour, checkfanyin, findtrue, gangzhi_yinyang(daygangzhi[0]), fan_yin
...
#print(liuren("小寒", "戊申", "己酉") )
#print(liuren("小寒", "庚戌", "丁丑"))
#print(shunkong("丁未","卯"))
