...
class ShowCodec(Dat['ShowCodec']):
    ...
    def __init__(self, id: int, seriesName: str, overview: Maybe[str], status: str, firstAired: Maybe[str]) -> None:
        self.id = id
...
class EpisodeCodec(Dat['EpisodeCodec']):
    ...
    def __init__(self, airedEpisodeNumber: int, airedSeason: int, firstAired: Maybe[str]) -> None:
...
class TvdbShow(Dat['TvdbShow']):
    ...
    def __init__(self, id: int, name: str, overview: Maybe[str], ended: bool, year: Maybe[str]) -> None:
        self.id = id
        self.name = name
        self.overview = overview
        self.ended = ended
...
class TvdbEpisode(Dat['TvdbEpisode']):
    ...
    def from_codec(data: EpisodeCodec) -> 'TvdbEpisode':
        airdate = data.firstAired.to_either("no airdate").flat_map(Try(lambda: datetime.fromisoformat(data.firstAired)))
