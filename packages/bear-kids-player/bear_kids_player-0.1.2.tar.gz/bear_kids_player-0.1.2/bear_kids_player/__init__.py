# init file pip install --index-url https://test.pypi.org/project/bear-kids-player/ --no-deps bear_kids_player-0.0.1-py3-none-any.whl
...
import os
sys.path.insert(0,bear_kids_player.__file__[:-11])
print(bear_kids_player.__file__[:-11])
path_1 = bear_kids_player.__file__[:-11]
os.chdir(path_1)
...
#
#     def __init__(self,config_load=get_config(path_1+'config.csv'),parent=None):
...
#             count_correct_fre(path = path_1+'questions.csv',index = self.child.q['index'], correct = False)
#         else:
#             count_correct_fre(path = path_1+'questions.csv',index = self.child.q['index'], correct = True)
#
...
# from PyQt5 import QtCore, QtGui, uic
# from PyQt5.QtWidgets import QApplication,QMainWindow,QDialog
# dialog1 = path_1+'Question_dialog.ui'
# Ui_DialogWindow, QtBaseClass = uic.loadUiType(dialog1)
# class childWindow(QDialog,Ui_DialogWindow):
#     def __init__(self,config_load=get_config(path_1+'config.csv')):
...
#         self.q = generate_question(path = path_1+'questions.csv',type_filter = self.config_load['type_filter'],
