...
class Server(object):
    ...
    def __init__(self, service_name,
            url=None, description=None, variables=None,
            aw_session_endpoint=False, session = None):
        self._service_name = service_name
        self.description = description
        self.variables = variables and dict((k, v.get(OpenAPIKeyWord.DEFAULT)) for (k, v) in variables.items()) or None
...
class Client(object):
    ...
    def __init__(self, name, version='v1', session=None, residency=None):
        ...
        self._models = {}
        self._defaults = session and session.get_defaults() or {}
        spec = self.load_spec_from_file("{}/activewatch/apis/{}.{}.yaml".format(
                                    os.getcwd(), name, version))
        self.load_spec(spec)
