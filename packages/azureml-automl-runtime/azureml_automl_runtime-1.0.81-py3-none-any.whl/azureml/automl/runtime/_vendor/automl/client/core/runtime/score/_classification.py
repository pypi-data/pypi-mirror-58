...
class NormMacroRecall(ClassificationMetric, ScalarMetric):
    ...
    def _norm_macro_recall_wrapper(self, y_test_bin, y_pred_proba, class_labels, test_class_labels, metric_name,
                                   sample_weight=None, logger=None):
        return _scoring_utilities.class_averaged_score(
            self._norm_macro_recall, y_test_bin, y_pred_proba,
            class_labels, test_class_labels, 'macro', metric_name,
            sample_weight=sample_weight, logger=logger)
    ...
    def compute(self):
        ...
        return self._norm_macro_recall_wrapper(
            self._y_test_bin, y_pred_proba, self._class_labels, self._test_labels,
            constants.NORM_MACRO_RECALL, sample_weight=self._sample_weight, logger=self._logger)
