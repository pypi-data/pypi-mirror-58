...
class MediaCog:
    def __init__(self, ekkobot, args):
        ...
        self._hohoho_uri = "https://www.youtube.com/watch?v=8lFjrAj-vrk"
    ...
    def commands(self):
        ...
        return [
            ...
            (EventType.TEXTMESSAGE, self.cmd_hohoho_volume, 100, None, '!hohoho-volume'),
            (EventType.TEXTMESSAGE, self.cmd_hohoho_uri, 100, None, '!hohoho-uri'),
        ]
    ...
    def cmd_hohoho(self, event):
        ...
        if self.ekko_bot.check_cmd_suitability(f'^{cmd_prefix}\s*$', event[0]['msg']):
            if self.ekko_bot.can(MediaCogPermission.HOHOHO, event):
                ...
                hohoh_mpv.vid = False
                # set volume
                hohoh_mpv.volume = self._hohoho_volume
                hohoh_mpv.playlist_append(self._hohoho_uri)
    ...
    def cmd_hohoho_uri(self, event):
        """
        Command: !hohoho-uri <value>

        Sets the hohoho uri to the given value.

        :param event: TS3Event
        """
        cmd_prefix = '!hohoho-uri'
        if self.ekko_bot.check_cmd_suitability(f'^{cmd_prefix} \d+\s*$', event[0]['msg']):
            if self.ekko_bot.can(MediaCogPermission.HOHOHO, event):
                pos, uris = self.ekko_bot.parse(MediaAliasParser.parse_queue, event, cmd_prefix, event[0]['msg'])
                uris = self._remove_url_bbcode(uris)
                if type(uris) == list:
                    uris = uris[0]
                self._hohoho_uri = uris
                self.ekko_bot.reply(f'hohoho-uri set to {self._hohoho_uri}.', event)
