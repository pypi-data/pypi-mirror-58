...
class VnsModuleTest:
    ...
    def test_vns_chainId(self, web3):
        ...
        # chain id value from gvns fixture genesis file
    ...
    def test_vns_signTypedData(self, web3, unlocked_account_dual_type, skip_if_testrpc):
        validJSONMessage = '''
            {
                "types": {
                    "EIP712Domain": [
        ...
        signature = HexBytes(web3.vns.signTypedData(
            unlocked_account_dual_type,
            json.loads(validJSONMessage)
        ))
        assert len(signature) == 32 + 32 + 1

    def test_invalid_vns_signTypedData(self,
                                       web3,
                                       unlocked_account_dual_type,
                                       skip_if_testrpc):
        skip_if_testrpc(web3)
    ...
    def test_vns_newBlockFilter(self, web3):
        ...
        # TODO: figure out why this fails in go-vnscoin
        # logs = web3.vns.getFilterLogs(filter.filter_id)
        # assert is_list_like(logs)
        # assert not logs
    ...
    def test_vns_submitHashrate(self, web3):
        # node_id from EIP 1474: https://github.com/vnscoin/EIPs/pull/1474/files
