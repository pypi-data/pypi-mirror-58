...
class GoVnscoinPersonalModuleTest:
    def test_personal_importRawKey(self, web3):
        actual = web3.gvns.personal.importRawKey(PRIVATE_KEY_HEX, PASSWORD)
        assert actual == ADDRESS

    def test_personal_listAccounts(self, web3):
        accounts = web3.gvns.personal.listAccounts()
    ...
    def test_personal_lockAccount(self, web3, unlockable_account_dual_type):
        ...
        web3.gvns.personal.lockAccount(unlockable_account_dual_type)

    def test_personal_unlockAccount_success(self,
                                            web3,
                                            unlockable_account_dual_type,
                                            unlockable_account_pw):
        result = web3.gvns.personal.unlockAccount(
            unlockable_account_dual_type,
            unlockable_account_pw
        )
    ...
    def test_personal_unlockAccount_failure(self,
        ...
        with pytest.raises(ValueError):
            web3.gvns.personal.unlockAccount(unlockable_account_dual_type, 'bad-password')

    def test_personal_newAccount(self, web3):
        new_account = web3.gvns.personal.newAccount(PASSWORD)
    ...
    def test_personal_sendTransaction(self,
                                      ...
                                      unlockable_account_dual_type,
                                      unlockable_account_pw):
        assert web3.vns.getBalance(unlockable_account_dual_type) > web3.toWei(1, 'vnser')
        txn_params = {
        ...
        txn_hash = web3.gvns.personal.sendTransaction(txn_params, unlockable_account_pw)
        assert txn_hash
    ...
    def test_personal_sign_and_ecrecover(self,
                                         ...
                                         unlockable_account_dual_type,
                                         unlockable_account_pw):
        message = 'test-web3-gvns-personal-sign'
        signature = web3.gvns.personal.sign(
            message,
            unlockable_account_dual_type,
            unlockable_account_pw
        )
        signer = web3.gvns.personal.ecRecover(message, signature)
        assert is_same_address(signer, unlockable_account_dual_type)

    @pytest.mark.xfail(reason="personal_signTypedData JSON RPC call has not been released in gvns")
    def test_personal_sign_typed_data(self,
        ...
        '''
        signature = HexBytes(web3.gvns.personal.signTypedData(
