...
@curry
def call_vns_tester(
    fn_name: str, vns_tester: "VnscoinTester", fn_args: Any, fn_kwargs: Any=None
) -> RPCResponse:
...
def without_vns_tester(
    fn: Callable[[TParams], TReturn]
) -> Callable[["VnscoinTester", TParams], TReturn]:
    # workaround for: https://github.com/pytoolz/cytoolz/issues/103
    # @functools.wraps(fn)
    def inner(vns_tester: "VnscoinTester", params: TParams) -> TReturn:
        return fn(params)
    return inner
...
def without_params(
    ...
    # workaround for: https://github.com/pytoolz/cytoolz/issues/103
    # @functools.wraps(fn)
    def inner(vns_tester: "VnscoinTester", params: Any) -> TReturn:
...
def preprocess_params(
    vns_tester: "VnscoinTester", params: Any, preprocessor_fn: Callable[..., Any]
) -> Tuple["VnscoinTester", Callable[..., Any]]:
...
def client_version(vns_tester: "VnscoinTester", params: Any) -> str:
    # TODO: account for the backend that is in use.
    from vns_tester import __version__
    return "VnscoinTester/{version}/{platform}/python{v.major}.{v.minor}.{v.micro}".format(
        version=__version__,
        v=sys.version_info,
...
def get_transaction_by_block_hash_and_index(
    vns_tester: "VnscoinTester", params: Any
) -> TxReceipt:
...
def create_log_filter(vns_tester: "VnscoinTester", params: Any) -> int:
    filter_params = params[0]
    filter_id = vns_tester.create_log_filter(**filter_params)
    return filter_id


def get_logs(vns_tester: "VnscoinTester", params: Any) -> List[LogReceipt]:
    filter_params = params[0]
...
@without_params
def create_new_account(vns_tester: "VnscoinTester") -> HexAddress:
    return vns_tester.add_account(_generate_random_private_key())


def personal_send_transaction(vns_tester: "VnscoinTester", params: Any) -> HexStr:
