...
from vnspm.uri import (
    is_supported_content_addressed_uri,
    resolve_uri_contents,
)
from vnspm.validation.manifest import (
    validate_manifest_against_schema,
    validate_raw_manifest_format,
)
from vnspm.validation.package import (
...
class ERC1319Registry(ABC):
    ...
    defines the methods specified in `ERC 1319 <https://github.com/vnscoin/EIPs/issues/1319>`__.
...
class SimpleRegistry(ERC1319Registry):
    ...
    <https://github.com/vnspm/solidity-registry>`__.
    """
...
class PM(Module):
    ...
    implementation of `ERC1319  <https://github.com/vnscoin/EIPs/issues/1319>`__, set as
    its ``registry`` attribute.
    """

    def get_package_from_manifest(self, manifest: Manifest) -> Package:
        """
        Returns a `Package <https://github.com/vnspm/py-vnspm/blob/master/vnspm/package.py>`__
    ...
    def get_package_from_uri(self, manifest_uri: URI) -> Package:
        ...
        If you want to use a specific IPFS backend, set ``VNSPM_IPFS_BACKEND_CLASS``
    ...
    def get_local_package(self, package_name: str, vnspm_dir: Path = None) -> Package:
        """
        Returns a `Package <https://github.com/vnspm/py-vnspm/blob/master/vnspm/package.py>`__
        instance built with the Manifest found at the package name in your local vnspm_dir.

        * Parameters:
            * ``package_name``: Must be the name of a package installed locally.
            * ``vnspm_dir``: Path pointing to the target vnspm directory (optional).
        """
        if not vnspm_dir:
            vnspm_dir = Path.cwd() / '_vnspm_packages'

        if not vnspm_dir.name == "_vnspm_packages" or not vnspm_dir.is_dir():
            raise PMError(f"{vnspm_dir} is not a valid vnsPM packages directory.")

        local_packages = [pkg.name for pkg in vnspm_dir.iterdir() if pkg.is_dir()]
        if package_name not in local_packages:
            raise PMError(
                f"Package: {package_name} not found in {vnspm_dir}. "
                f"Available packages include: {local_packages}."
            )

        target_manifest = json.loads(
            (vnspm_dir / package_name / "manifest.json").read_text()
        )
