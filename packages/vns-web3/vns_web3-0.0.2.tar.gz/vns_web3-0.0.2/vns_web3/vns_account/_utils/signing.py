...
def sign_transaction_dict(vns_key, transaction_dict):
    ...
    (v, r, s) = sign_transaction_hash(vns_key, transaction_hash, chain_id)
...
def hash_of_signed_transaction(txn_obj):
    ...
    See details at https://github.com/vnscoin/EIPs/blob/master/EIPS/eip-155.md
...
def to_standard_signature_bytes(vnscoin_signature_bytes):
    rs = vnscoin_signature_bytes[:-1]
    v = to_int(vnscoin_signature_bytes[-1])
...
def to_vns_v(v_raw, chain_id=None):
    if chain_id is None:
        v = v_raw + V_OFFSET
    else:
...
def sign_transaction_hash(account, transaction_hash, chain_id):
    ...
    v = to_vns_v(v_raw, chain_id)
    return (v, r, s)


def _pad_to_vns_word(bytes_val):
...
def sign_message_hash(key, msg_hash):
    ...
    (v_raw, r, s) = signature.vrs
    v = to_vns_v(v_raw)
    vns_signature_bytes = to_bytes32(r) + to_bytes32(s) + to_bytes(v)
    return (v, r, s, vns_signature_bytes)
