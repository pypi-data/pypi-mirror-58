...
from vns_keyfile import (
    create_keyfile_json,
    decode_keyfile_json,
)
from vns_keys import (
    KeyAPI,
    keys,
)
from vns_keys.exceptions import (
    ValidationError,
)
...
from vns_account._utils.signing import (
...
from vns_account._utils.transactions import (
    Transaction,
    vrs_from,
)
from vns_account.datastructures import (
    AttributeDict,
)
from vns_account.messages import (
    SignableMessage,
...
class Account(object):
    """
    The primary entry point for working with Vnscoin private keys.

    It does **not** require a connection to an Vnscoin node.
    """
    _keys = keys

    _default_kdf = os.getenv('VNS_ACCOUNT_KDF', 'scrypt')

    @combomethod
    def create(self, extra_entropy=''):
        r"""
        Creates a new private key, and returns it as a :class:`~vns_account.local.LocalAccount`.
    ...
    def decrypt(keyfile_json, password):
        """
        Decrypts a private key that was encrypted using an Vnscoin client or
    ...
    def encrypt(cls, private_key, password, kdf=None, iterations=None):
        ...
        To import this keyfile into Vnscoin clients like gvns and parity:
        encode this dictionary with :func:`json.dumps` and save it to disk where your
        client keeps key files.

        :param private_key: The raw private key
        :type private_key: hex str, bytes, int or :class:`vns_keys.datatypes.PrivateKey`
        ...
        environment variable :envvar:`VNS_ACCOUNT_KDF`. If that is not set, then
    ...
    def privateKeyToAccount(self, private_key):
        """
        .. CAUTION:: Deprecated for :meth:`~vns_account.account.Account.from_key`.
    ...
    def recover_message(self, signable_message: SignableMessage, vrs=None, signature=None):
        ...
        .. code-block:: python
            ...
            >>> from vns_account.messages import encode_defunct
    ...
    def recoverHash(self, message_hash, vrs=None, signature=None):
        ...
        .. CAUTION:: Deprecated for :meth:`~vns_account.account.Account.recover_message`.
    ...
    def recoverTransaction(self, serialized_transaction):
        """
        .. CAUTION:: Deprecated for :meth:`~vns_account.account.Account.recover_transaction`.
            This method will be removed in v0.5
        """
        warnings.warn(
    ...
    def setKeyBackend(self, backend):
        """
        .. CAUTION:: Deprecated for :meth:`~vns_account.account.Account.set_key_backend`.
            This method will be removed in v0.5
        """
        warnings.warn(
    ...
    def set_key_backend(self, backend):
        """
        Change the backend used by the underlying vns-keys library.

        *(The default is fine for most users)*

        :param backend: any backend that works in
            `vns_keys.KeyApi(backend) <https://github.com/vnscoin/vns-keys/#keyapibackendnone>`_
    ...
    def sign_message(self, signable_message: SignableMessage, private_key):
        ...
        :meth:`w3.vns.sign() <web3.vns.Vns.sign>`
        you can use :meth:`~vns_account.messages.encode_defunct`.
        ...
        :rtype: ~vns_account.datastructures.AttributeDict
        ...
        .. _EIP-191: https://eips.vnscoin.org/EIPS/eip-191
        """
        message_hash = _hash_eip191_message(signable_message)
    ...
    def signHash(self, message_hash, private_key):
        ...
        .. WARNING:: *Never* sign a hash that you didn't generate,
            ...
            send all of your account's vnser to an attacker.
            Instead, prefer :meth:`~vns_account.account.Account.sign_message`,
            which cannot accidentally sign a transaction.

        Sign the provided hash.

        .. CAUTION:: Deprecated for :meth:`~vns_account.account.Account.sign_message`.
            This method will be removed in v0.5
    ...
    def _sign_hash(self, message_hash, private_key):
        ...
        (v, r, s, vns_signature_bytes) = sign_message_hash(key, msg_hash_bytes)
        return AttributeDict({
            'messageHash': msg_hash_bytes,
            'r': r,
            's': s,
            'v': v,
            'signature': HexBytes(vns_signature_bytes),
        })

    @combomethod
    def signTransaction(self, transaction_dict, private_key):
        """
        .. CAUTION:: Deprecated for :meth:`~vns_account.account.Account.sign_transaction`.
            This method will be removed in v0.5
        """
        warnings.warn(
    ...
    def sign_transaction(self, transaction_dict, private_key):
        ...
        :meth:`w3.vns.sendRawTransaction() <web3.vns.Vns.sendRawTransaction>`.
        ...
        .. code-block:: python
            ...
            {'hash': HexBytes('0x6893a6ee8df79b0f5d64a180cd1ef35d030f3e296a5361cf04d02ce720d32ec5'),
             ...
             'v': 37}
            >>> w3.vns.sendRawTransaction(signed.rawTransaction)
        """
    ...
    def _parsePrivateKey(self, key):
        """
        Generate a :class:`vns_keys.datatypes.PrivateKey` from the provided key. If the
        key is already of type :class:`vns_keys.datatypes.PrivateKey`, return the key.

        :param key: the private key from which a :class:`vns_keys.datatypes.PrivateKey`
                    will be generated
        :type key: hex str, bytes, int or :class:`vns_keys.datatypes.PrivateKey`
        :returns: the provided key represented as a :class:`vns_keys.datatypes.PrivateKey`
        """
