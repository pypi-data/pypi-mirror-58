...
class GoVnscoinShhModuleTest():
    #
    # shh_filter
    #
    def test_shh_sync_filter(self, web3):
        sender = web3.gvns.shh.new_key_pair()
        sender_pub = web3.gvns.shh.get_public_key(sender)

        receiver = web3.gvns.shh.new_key_pair()
        receiver_pub = web3.gvns.shh.get_public_key(receiver)

        topic = '0x13370000'
        payloads = [web3.toHex(text="test message :)"), web3.toHex(text="2nd test message")]

        shh_filter_id = web3.gvns.shh.new_message_filter({
    ...
    def test_shh_sync_filter_deprecated(self, web3):
        with pytest.warns(DeprecationWarning):
            sender = web3.gvns.shh.newKeyPair()
            sender_pub = web3.gvns.shh.getPublicKey(sender)
            receiver = web3.gvns.shh.newKeyPair()
            receiver_pub = web3.gvns.shh.getPublicKey(receiver)

            topic = '0x13370000'
            payloads = [web3.toHex(text="test message :)"), web3.toHex(text="2nd test message")]

            shh_filter_id = web3.gvns.shh.newMessageFilter({
                'privateKeyID': receiver,
                'sig': sender_pub,
                'topics': [topic]
            })
    ...
    def test_shh_remove_filter_deprecated(self, web3):
        with pytest.warns(DeprecationWarning):
            ...
            shh_filter_id = web3.gvns.shh.newMessageFilter({'privateKeyID': receiver})
            shh_filter = ShhFilter(web3, shh_filter_id)
            ...
            assert web3.gvns.shh.deleteMessageFilter(shh_filter.filter_id)

            try:
                web3.gvns.shh.getMessages(shh_filter.filter_id)
                assert False
    ...
    def test_shh_remove_filter(self, web3):
        ...
        receiver_pub = web3.gvns.shh.get_public_key(receiver)
        payload = web3.toHex(text="test message :)")
        shh_filter_id = web3.gvns.shh.new_message_filter({'privateKeyID': receiver})
        shh_filter = ShhFilter(web3, shh_filter_id)
        ...
        assert web3.gvns.shh.delete_message_filter(shh_filter.filter_id)

        try:
            web3.gvns.shh.get_filter_messages(shh_filter.filter_id)
            assert False
        except BaseException:
            assert True
    ...
    def test_shh_asymmetric_key_pair(self, web3):
        ...
        key_id = web3.gvns.shh.new_key_pair()
        assert web3.gvns.shh.has_key_pair(key_id)
        assert len(web3.gvns.shh.get_public_key(key_id)) == 132
        private_key = web3.gvns.shh.get_private_key(key_id)
        assert len(private_key) == 66
        assert web3.gvns.shh.delete_key_pair(key_id)

        # Test adding a key
        assert not web3.gvns.shh.has_key_pair(key_id)
        key_id = web3.gvns.shh.add_private_key(private_key)
        assert web3.gvns.shh.has_key_pair(key_id)
        assert web3.gvns.shh.delete_key_pair(key_id)
    ...
    def test_shh_asymmetric_key_pair_deprecated(self, web3):
        # Test generating key
        with pytest.warns(DeprecationWarning):
            key_id = web3.gvns.shh.newKeyPair()
            assert web3.gvns.shh.hasKeyPair(key_id)
            assert len(web3.gvns.shh.getPublicKey(key_id)) == 132
            private_key = web3.gvns.shh.getPrivateKey(key_id)
            assert len(private_key) == 66
            assert web3.gvns.shh.deleteKeyPair(key_id)

        # Test adding a key
            assert not web3.gvns.shh.hasKeyPair(key_id)
            key_id = web3.gvns.shh.addPrivateKey(private_key)
            assert web3.gvns.shh.hasKeyPair(key_id)
            assert web3.gvns.shh.deleteKeyPair(key_id)

    def test_shh_symmetric_key_pair(self, web3):
        # Test generating key
        key_id = web3.gvns.shh.new_sym_key()
        assert web3.gvns.shh.has_sym_key(key_id)

        key = web3.gvns.shh.get_sym_key(key_id)
        assert len(key) == 66
        assert web3.gvns.shh.delete_sym_key(key_id)

        # Test adding a key
        assert not web3.gvns.shh.has_sym_key(key_id)
        key_id = web3.gvns.shh.add_sym_key(key)
        assert web3.gvns.shh.has_sym_key(key_id)
        assert web3.gvns.shh.delete_sym_key(key_id)
    ...
    def test_shh_symmetric_key_pair_deprecated(self, web3):
        with pytest.warns(DeprecationWarning):
            # Test generating key
            key_id = web3.gvns.shh.newSymKey()
            assert web3.gvns.shh.hasSymKey(key_id)

            key = web3.gvns.shh.getSymKey(key_id)
            assert len(key) == 66
            assert web3.gvns.shh.deleteSymKey(key_id)

        # Test adding a key
            assert not web3.gvns.shh.hasSymKey(key_id)
            key_id = web3.gvns.shh.addSymKey(key)
            assert web3.gvns.shh.hasSymKey(key_id)
            assert web3.gvns.shh.deleteSymKey(key_id)

    def test_shh_symmetric_key_pair_from_password_deprecated(self, web3):
        with pytest.warns(DeprecationWarning):
            key_id = web3.gvns.shh.generateSymKeyFromPassword('shh be quiet')

            assert web3.gvns.shh.hasSymKey(key_id)
            assert len(web3.gvns.shh.getSymKey(key_id)) == 66
            assert web3.gvns.shh.deleteSymKey(key_id)

    def test_shh_symmetric_key_pair_from_password(self, web3):

        key_id = web3.gvns.shh.generate_sym_key_from_password('shh be quiet')

        assert web3.gvns.shh.has_sym_key(key_id)
        assert len(web3.gvns.shh.get_sym_key(key_id)) == 66
        assert web3.gvns.shh.delete_sym_key(key_id)
    ...
    def test_shh_post(self, web3):
        receiver_pub = web3.gvns.shh.get_public_key(web3.gvns.shh.new_key_pair())
        assert web3.gvns.shh.post({
            "topic": "0x12345678",
            "powTarget": 2.5,
            "powTime": 2,
    ...
    def test_shh_post_deprecated(self, web3):
        with pytest.warns(DeprecationWarning):
            receiver_pub = web3.gvns.shh.getPublicKey(web3.gvns.shh.newKeyPair())
            assert web3.gvns.shh.post({
                "topic": "0x12345678",
                "powTarget": 2.5,
                "powTime": 2,
    ...
    def test_shh_info(self, web3):
        pre_info = web3.gvns.shh.info()
        assert pre_info["maxMessageSize"] != 2048
        assert pre_info["minPow"] != 0.6

        web3.gvns.shh.set_max_message_size(2048)
        web3.gvns.shh.set_min_pow(0.6)

        info = web3.gvns.shh.info()
    ...
    def test_shh_info_deprecated(self, web3):
        with pytest.warns(DeprecationWarning):
            ...
            web3.gvns.shh.setMaxMessageSize(1024)
            web3.gvns.shh.setMinPoW(0.5)
...
class ParityShhModuleTest():
    ...
    def test_shh_remove_filter(self, web3):
        ...
        # https://github.com/paritytech/parity-vnscoin/issues/10565
